...
const ELIMINATION_ID = "elim-roster-repair";
...
describe("elimination roster snapshot repair", () => {
  beforeEach(async () => {
  ...
  test("backfills only missing team snapshots and preserves existing snapshot edits", async () => {
    await seedCopiedTeam("alpha");
    await seedCopiedTeam("beta");
    await createRosterSnapshots(ELIMINATION_ID, ["alpha"]);
    await updateEliminationRosterSnapshot(ELIMINATION_ID, "alpha", {
      startingRotation: ["alpha-custom-sp"],
    });

    const result = await ensureEliminationRosterSnapshots(ELIMINATION_ID, ["alpha", "beta"]);
    ...
    await expect(getEliminationRosterSnapshot(ELIMINATION_ID, "alpha")).resolves.toMatchObject({
      startingRotation: ["alpha-custom-sp"],
    });
    await expect(getEliminationRosterSnapshot(ELIMINATION_ID, "beta")).resolves.toMatchObject({
      teamId: "beta",
      players: expect.arrayContaining([
        expect.objectContaining({ id: "beta-bat" }),
        expect.objectContaining({ id: "beta-arm" }),
      ]),
    });
  });

  test("rebuilds a missing snapshot before launching a GameTracker roster", async () => {
    await seedCopiedTeam("alpha");

    await expect(getEliminationRosterSnapshot(ELIMINATION_ID, "alpha")).resolves.toBeNull();
    const roster = await buildEliminationGameTrackerRoster(ELIMINATION_ID, "alpha", true);

    expect(roster.players).toEqual([
      expect.objectContaining({ playerId: "alpha-bat", name: "Dana Dunn" }),
    ]);
    expect(roster.pitchers).toEqual([
      expect.objectContaining({ playerId: "alpha-arm", name: "Riley Ray" }),
    ]);
    await expect(getEliminationRosterSnapshot(ELIMINATION_ID, "alpha")).resolves.toMatchObject({
      teamId: "alpha",
    });
...
async function seedCopiedTeam(teamId: string): Promise<void> {
  await saveEliminationTeam(ELIMINATION_ID, buildTeam(teamId));
  await saveEliminationPlayer(ELIMINATION_ID, buildPlayer(`${teamId}-bat`, teamId, "Dana", "Dunn", "SS"));
  await saveEliminationPlayer(ELIMINATION_ID, buildPlayer(`${teamId}-arm`, teamId, "Riley", "Ray", "SP"));
}
