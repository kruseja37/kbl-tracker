...
describe('franchise save-slot manifest contract', () => {
  ...
  test('backup and sync schemas cover manifest-owned shared stores and franchise roots', async () => {
    ...
    expect(SYNC_REGISTRY['kbl-tracker']).toHaveProperty('franchiseFlashpointDecay', [
      'franchiseId',
      'seasonId',
      'statsScopeId',
      'playerId',
    ...
    expect(STATIC_DATABASE_SCHEMAS['kbl-tracker'].stores.franchiseFlashpointDecay).toMatchObject({
      keyPath: ['franchiseId', 'seasonId', 'statsScopeId', 'playerId'],
      optional: true,
    });
    expect(STATIC_DATABASE_SCHEMAS['kbl-tracker'].stores.franchiseRatingsOverlays).toMatchObject({
      keyPath: 'id',
      optional: true,
    });
    expect(STATIC_DATABASE_SCHEMAS['kbl-tracker'].stores.franchiseTraitOverlays).toMatchObject({
      keyPath: 'id',
      optional: true,
    });
