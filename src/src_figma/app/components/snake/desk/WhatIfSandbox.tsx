...
export function WhatIfSandbox(props: {
  ...
  const effectivePlayerId = props.candidates.some((candidate) => candidate.id === playerId)
    ? playerId
    ...
    <div className="border-4 border-[var(--ballpark-panel-border)] p-3">
      ...
      {!props.whatIf ? (
        <div className="flex flex-wrap gap-2">
          <select className="min-h-11 border-4 border-[var(--ballpark-panel-border)] bg-[var(--ballpark-well)] px-2 text-[var(--ballpark-chalk)]" aria-label="What-if board slot" value={slotId} onChange={(event) => setSlotId(event.target.value as SnakeBoardSlotId)}>
            {Object.keys(props.boardSlots).map((slot) => <option key={slot} value={slot}>{slot}</option>)}
          </select>
          <select className="min-h-11 border-4 border-[var(--ballpark-panel-border)] bg-[var(--ballpark-well)] px-2 text-[var(--ballpark-chalk)]" aria-label="What-if player" value={effectivePlayerId} onChange={(event) => setPlayerId(event.target.value)}>
            {props.candidates.map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.name}</option>)}
          </select>
          <button className="ballpark-press-button ballpark-press-sm ballpark-press-default" disabled={!slotId || !effectivePlayerId} onClick={() => props.onStart(slotId as SnakeBoardSlotId, effectivePlayerId)}>PRICE THIS CHANGE</button>
        </div>
      ) : (
        <div>
