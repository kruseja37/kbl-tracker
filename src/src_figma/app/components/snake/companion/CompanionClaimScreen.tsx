  ...
  return (
    <main className="ballpark-page min-h-screen">
      <section className="ballpark-panel mx-auto max-w-xl">
        ...
        <h1 className="ballpark-title mt-1 text-3xl">{props.pending ? 'WAITING FOR THE MAIN DEVICE' : 'CLAIM YOUR PRIVATE DESK'}</h1>
        {accountLine}
        <CompanionHelp>
          {props.pending ? <p>YOUR DESK STAYS COVERED UNTIL THE COMMISSIONER APPROVES THIS DEVICE.</p> : null}
        ...
        <button className="ballpark-press-button ballpark-press-md ballpark-press-gold mt-4" disabled={!gmName.trim() || roomCode.length !== 4} onClick={() => void props.onClaim(gmName, roomCode)}>{props.pending ? 'SEND REQUEST AGAIN' : 'ASK TO SEE MY DESK'}</button>
