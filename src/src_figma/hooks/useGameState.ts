...
type LinkedFameEventRecord = FameEventRecord & {
  sourceEventIds?: string[];
};

type PersistedFameEventWithSource = PersistedGameState["fameEvents"][number] & {
  sourceEventIds?: string[];
};
...
export interface UseGameStateReturn {
  ...
  useGhostRunnerRef: React.MutableRefObject<boolean>;
...
function cloneLineupSnapshot(
  lineup: GameInitConfig["awayLineup"],
): GameInitConfig["awayLineup"] {
  return lineup.map((player) => ({ ...player }));
}

function cloneBenchSnapshot(
  bench: NonNullable<GameInitConfig["awayBench"]> = [],
): NonNullable<GameInitConfig["awayBench"]> {
...
function cloneGameLockLineupSnapshots<T extends GameLockLineupSnapshots | undefined>(
  snapshots: T,
): T {
...
> {
  ...
  return {
    ...
    useGhostRunner: snapshot.useGhostRunner,
...
export function useGameState(initialGameId?: string): UseGameStateReturn {
    ...
    (
      ...
      return {
        ...
        gameState: {
          ...
          useGhostRunner: useGhostRunnerRef.current,
  ...
  const [, setFameEvents] = useState<LinkedFameEventRecord[]>([]);
  const fameEventsRef = useRef<LinkedFameEventRecord[]>([]);

  const replaceFameEvents = useCallback((events: LinkedFameEventRecord[]) => {
    const linkedEvents = events.map((event) => ({ ...event }));
    fameEventsRef.current = linkedEvents;
    setFameEvents(linkedEvents);
  }, []);

  const appendFameEvent = useCallback(
    (event: FameEventRecord, sourceEventIds?: string[]) => {
      const nextEvents = [
        ...fameEventsRef.current,
        {
          ...event,
  ...
  const removeFameEventsForEventIds = useCallback((eventIds: string[]) => {
    if (eventIds.length === 0) return;
    const eventIdSet = new Set(eventIds);
    const nextEvents = fameEventsRef.current.filter(
      (event) =>
        !event.sourceEventIds?.some((eventId) => eventIdSet.has(eventId)),
    );
    if (nextEvents.length !== fameEventsRef.current.length) {
      fameEventsRef.current = nextEvents;
      setFameEvents(nextEvents);
    }
  }, []);
  ...
  const buildPersistedFameEvents = useCallback(
    (
      ...
      return fameEventsRef.current.map((fe, idx) => ({
        ...
        ...(fe.sourceEventIds?.length
          ? { sourceEventIds: fe.sourceEventIds }
          : {}),
      }));
    },
  ...
  const buildImmediateCurrentGameSnapshot = useCallback(
    (stateOverride?: GameState): PersistedGameState | null => {
      ...
      return {
        ...
        awayLineup: cloneLineupSnapshot(awayLineupRef.current),
        homeLineup: cloneLineupSnapshot(homeLineupRef.current),
        optimalLineupSnapshots: cloneGameLockLineupSnapshots(
          optimalLineupSnapshotsRef.current,
        ),
        chosenLineupSnapshots: cloneGameLockLineupSnapshots(
          chosenLineupSnapshotsRef.current,
        ),
        awayLineupState:
  ...
  const useGhostRunnerRef = useRef<boolean>(false);
  ...
  const trackInningStrikeout = useCallback(
    (pitcherId: string) => {
      if (inningPitchesRef.current.pitcherId !== pitcherId) {
        // Defensive re-baseline for legacy/restored state: never credit a K to
        // the stale pitcher whose half-inning snapshot happens to be in the ref.
        inningPitchesRef.current = {
          pitches: pitcherStats.get(pitcherId)?.pitchCount ?? 0,
          strikeouts: 0,
          pitcherId,
        };
      }
      inningPitchesRef.current.strikeouts += 1;
    },
    [pitcherStats],
  );
  ...
  const initializeGame = useCallback(
    async (config: GameInitConfig) => {
      ...
      useGhostRunnerRef.current = config.useGhostRunner ?? extraInningRunnerRef.current;
      ...
      const awayLineupSnapshot = cloneLineupSnapshot(config.awayLineup);
      const homeLineupSnapshot = cloneLineupSnapshot(config.homeLineup);
      const awayBenchSnapshot = cloneBenchSnapshot(config.awayBench);
      const homeBenchSnapshot = cloneBenchSnapshot(config.homeBench);
      ...
      chosenLineupSnapshotsRef.current = cloneGameLockLineupSnapshots(
        config.chosenLineupSnapshots,
      );
      ...
      awayUsesDhRef.current = awayLineupSnapshot.some(p => p.position === "DH");
      homeUsesDhRef.current = homeLineupSnapshot.some(p => p.position === "DH");

      // MAJ-09: Initialize full LineupState for substitution validation
      awayLineupStateRef.current = {
        lineup: awayLineupSnapshot.map((p, idx) => ({
          playerId: p.playerId,
      ...
      homeLineupStateRef.current = {
        lineup: homeLineupSnapshot.map((p, idx) => ({
          playerId: p.playerId,
          playerName: p.playerName,
          position: p.position as Position,
          battingOrder: idx + 1,
      ...
      for (const player of awayLineupSnapshot) {
        registerIdentityForSide(player.playerId, player.playerName, "away");
      }
      for (const player of homeLineupSnapshot) {
        registerIdentityForSide(player.playerId, player.playerName, "home");
      }
      for (const player of awayBenchSnapshot) {
        registerIdentityForSide(player.playerId, player.playerName, "away");
      }
      for (const player of homeBenchSnapshot) {
        registerIdentityForSide(player.playerId, player.playerName, "home");
      }
      registerIdentityForSide(
      ...
      await createGameHeader({
        ...
        startingLineups: {
          ...
          home: homeLineupSnapshot.map((player, idx) => ({
            playerId: player.playerId,
            playerName: player.playerName,
            position: player.position,
            battingOrder: idx + 1,
        ...
        benchRosters: {
          away: awayBenchSnapshot,
          home: homeBenchSnapshot,
        },
        startingPitchers: {
          away: {
            playerId: config.awayStartingPitcherId,
      ...
      for (const player of [...awayLineupSnapshot, ...homeLineupSnapshot]) {
      ...
      const leadoffBatter = awayLineupSnapshot[0];
      setGameState({
        gameId: config.gameId,
        homeScore: 0,
        awayScore: 0,
        ...
        currentCatcherId:
          homeLineupSnapshot.find((p) => p.position === "C")?.playerId || "",
        currentCatcherName:
          homeLineupSnapshot.find((p) => p.position === "C")?.playerName || "",
        awayTeamId: config.awayTeamId,
        homeTeamId: config.homeTeamId,
        awayTeamName: config.awayTeamName,
        homeTeamName: config.homeTeamName,
      ...
      try {
        ...
        if (
          ...
          const restoredExtraInningRunner =
            ...
            false;
          const restoredUseGhostRunner =
            savedSnapshot.useGhostRunner ??
            inProgressGame.useGhostRunner ??
            restoredExtraInningRunner;
          ...
          useGhostRunnerRef.current = restoredUseGhostRunner;
          ...
          replaceFameEvents(
            (savedSnapshot.fameEvents || []).map((fe) => ({
              ...
              sourceEventIds: (fe as PersistedFameEventWithSource)
                .sourceEventIds,
            })),
          );
        ...
        if (inProgressGame) {
          ...
          useGhostRunnerRef.current =
            inProgressGame.useGhostRunner ?? extraInningRunnerRef.current;
  ...
  const undoLastAction = useCallback(async (options?: { skipReload?: boolean }): Promise<boolean> => {
    ...
    try {
      const getUndoneEventIds = async () => {
        const [atBatEvents, betweenPlayEvents] = await Promise.all([
          getGameEvents(targetGameId, { includeUndone: true }),
          getBetweenPlayEvents(targetGameId, { includeUndone: true }),
        ]);
        return new Set(
          [...atBatEvents, ...betweenPlayEvents]
            .filter((event) => Boolean(event.undoneAt))
            .map((event) => event.eventId),
        );
      };

      const undoneBefore = await getUndoneEventIds();
      const undone = await undoMostRecentGameAction(targetGameId);
      if (!undone) {
        return false;
      }
      const returnedUndoneEventIds = [undone.eventId];
      ...
      if (undone.kind === "betweenPlay") {
        ...
        if (
          ...
          returnedUndoneEventIds.push(pairedAtBatUndo.eventId);
        }
      }

      const undoneAfter = await getUndoneEventIds();
      const newlyUndoneEventIds = [...undoneAfter].filter(
        (eventId) => !undoneBefore.has(eventId),
      );
      const affectedEventIds = [
        ...new Set([...returnedUndoneEventIds, ...newlyUndoneEventIds]),
      ];
      removeFameEventsForEventIds(affectedEventIds);
      ...
      await clearCurrentGame();
      const reloaded = await loadExistingGame();
      removeFameEventsForEventIds(affectedEventIds);
      return reloaded;
    } catch (err) {
  ...
  const recordOut = useCallback(
    async (
      ...
      if (outType === "K" || outType === "Kc") {
        trackInningStrikeout(gameState.currentPitcherId);
      }
  ...
  const recordError = useCallback(
    async (
      ...
      const effectiveRunnerData: RunnerAdvancement = runnerData ?? {
        fromFirst: gameState.bases.first ? "second" : undefined,
        fromSecond: gameState.bases.second ? "third" : undefined,
        fromThird: gameState.bases.third ? "home" : undefined,
      };

      // Calculate runs from the same advancement applied to bases and tracker.
      let runsScored = 0;
      if (effectiveRunnerData.fromFirst === "home") runsScored++;
      if (effectiveRunnerData.fromSecond === "home") runsScored++;
      if (effectiveRunnerData.fromThird === "home") runsScored++;
      const homeScoreAfter = gameState.isTop
        ? gameState.homeScore
        : gameState.homeScore + runsScored;
      const awayScoreAfter = gameState.isTop
      ...
      // Update game state - batter takes first, runners use the same outcomes as tracker.
      setGameState((prev) => {
        ...
        if (effectiveRunnerData) {
          ...
          if (prev.bases.first) {
            ...
            } else if (effectiveRunnerData.fromFirst === "third") {
              newThird = true;
            } else if (
              effectiveRunnerData.fromFirst === "home" ||
              effectiveRunnerData.fromFirst === "out"
          ...
          if (prev.bases.second) {
            if (effectiveRunnerData.fromSecond === "third") {
              newThird = true;
              newSecond =
                prev.bases.first && effectiveRunnerData.fromFirst === "second"; // Only occupied if R1 went there
            } else if (
              effectiveRunnerData.fromSecond === "home" ||
              effectiveRunnerData.fromSecond === "out"
            ) {
              // Second vacated
              newSecond =
                prev.bases.first && effectiveRunnerData.fromFirst === "second";
            }
          ...
          if (prev.bases.third) {
            if (
              ...
              // Third vacated
              newThird =
                prev.bases.second && effectiveRunnerData.fromSecond === "third";
            }
  ...
  const makeSubstitution = useCallback(
    async (
      ...
      if (subType === "double_switch" && franchiseIdRef.current) {
        return {
          success: false,
      ...
      try {
        ...
        return {
          success: false,
          error: "Failed to persist substitution ledger event",
        };
      }
  ...
  const switchPositions = useCallback(
    async (
      ...
      const collectSwitches = (
        ...
        for (const player of lineupStateRef.current.lineup) {
          const requestedSwitch = switches.find(
            (sw) => sw.playerId === player.playerId,
          );
          if (!requestedSwitch) continue;
          plannedSwitches.push({
            ...requestedSwitch,
      ...
      collectSwitches(awayLineupStateRef, "away");
      collectSwitches(homeLineupStateRef, "home");

      if (plannedSwitches.length === 0) {
        return { success: false, error: "No matching players found for position switch" };
      }

      try {
        await Promise.all(
          plannedSwitches.map((sw) =>
            persistBetweenPlayEvent({
              type: "position_change",
              substitution: {
                subType: "position_change",
                outPlayerId: sw.playerId,
                outPlayerName: sw.playerName,
                inPlayerId: sw.playerId,
                inPlayerName: sw.playerName,
                inPosition: sw.newPosition,
                previousPosition: sw.previousPosition,
              },
            }),
          ),
        );
        ...
        return {
          success: false,
          error: "Failed to persist position-change ledger event",
        };
      }
      ...
      if (!changedAway && !changedHome) {
        return { success: false, error: "No positions changed" };
      }
  ...
  const changePitcher = useCallback(
    (
      ...
      pendingActionRef.current = async () => {
        ...
        // Log the pitching change after the durable ledger row succeeds.
        setSubstitutionLog((prev) => [
          ...prev,
          {
            type: "pitching_change",
        ...
        // The incoming pitcher owns a new immaculate-inning window even when
        // the change happens mid-inning.
        inningPitchesRef.current = {
          pitches: pitcherStats.get(newPitcherId)?.pitchCount ?? 0,
          strikeouts: 0,
  ...
  const confirmPitchCount = useCallback(
    async (
      ...
      // Requires: 9 pitches since this pitcher's half-inning snapshot and 3 tracked strikeouts.
      if (
        pitchCountPrompt?.type === "end_inning" &&
        inningPitchesRef.current.pitcherId === pitcherId &&
        finalCount - inningPitchesRef.current.pitches === 9 &&
      ...
      const timing =
        pitchCountPrompt?.type === "pitching_change"
          ...
          : pitchCountPrompt?.type === "end_game"
            ? "end_of_game"
            : "end_of_half_inning";
      let pitchCountActionId: string | undefined;
      try {
        const pitchCountEvent = await persistBetweenPlayEvent({
        ...
        pitchCountActionId = pitchCountEvent.eventId;
      } catch (error) {
        console.error(
        ...
        return {
          ...result,
          success: false,
          error: "Failed to persist pitch-count ledger event",
        };
      }

      if (result.immaculateInning && pitchCountPrompt) {
        ...
        appendFameEvent(
          immaculateFameEvent,
          pitchCountActionId ? [pitchCountActionId] : undefined,
        );
      }
          ...
          return {
            ...
            error:
              error instanceof Error
                ? error.message
                : "Pending pitch-count action failed",
          };
        }
  ...
  const executeEndInning = useCallback(() => {
    ...
    setGameState((prev) => {
      ...
      // Re-baseline at every half flip. This alternates team pitchers safely,
      // carries cumulative counts into later and extra innings, and clears Ks.
      inningPitchesRef.current = {
        pitches: pitcherStats.get(newPitcherId)?.pitchCount ?? 0,
        strikeouts: 0,
        pitcherId: newPitcherId,
    ...
    async (opts?: EndGameOptions) => {
      ...
      try {
        ...
        if (!alreadyAggregated) {
            ...
            await Promise.race([
              (async () => {
                ...
                if (processResult.aggregation.success !== true) {
                  throw new Error(
                    processResult.aggregation.error ||
                      `Failed to aggregate completed game ${gameState.gameId}`,
                  );
                }
                endGameAggregationSucceeded = true;
              })(),
          ...
          if (!endGameAggregationSucceeded) {
            throw new Error(
              "End-game processing did not complete season aggregation. The game was archived as incomplete and schedule/playoff advancement was blocked.",
            );
          }
        } else {
        ...
        if (
          ...
          (!completedArchive ||
            completedArchive.aggregationStatus === "incomplete")
        ) {
          await archiveCompletedGame(
            persistedState,
            {
          ...
          completedArchive = await loadCompletedArchive(gameState.gameId);
        }
        ...
        if (
          ...
          throw new Error(
            "End-game archive is incomplete. Schedule/playoff advancement was blocked.",
          );
        }

        // Aggregate player stats to playoff stats store before series advancement.
        if (endGameAggregationSucceeded && playoffIdRef.current) {
          try {
            ...
            if (archiveContext.competitionType !== "elimination") {
              throw err;
            }
          }
        }
        ...
        if (endGameAggregationSucceeded && playoffSeriesIdRef.current) {
          try {
        ...
        if (!alreadyAggregated && endGameAggregationSucceeded) {
  ...
  const startGame = useCallback(() => {
    ...
    inningPitchesRef.current = {
      pitches:
        pitcherStats.get(nextGameState.currentPitcherId)?.pitchCount ?? 0,
      strikeouts: 0,
      pitcherId: nextGameState.currentPitcherId,
    };
