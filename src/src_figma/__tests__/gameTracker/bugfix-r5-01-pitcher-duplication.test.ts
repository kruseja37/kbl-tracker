...
describe('bugfix R5-01: pitcher sync does not duplicate defensive lineup rows', () => {
  test('collapses duplicate current-pitcher rows back to a single batting-order slot', () => {
    ...
    expect(reconciled.filter((entry) => entry.playerId === 'new-p')).toHaveLength(1);
