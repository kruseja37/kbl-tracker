...
**➡ NEXT = S7c** (won-bid → farm `Player.salary`, ALL real winners [human + CPU-controlled; shill excluded] — fix `franchiseInitializer.ts:754-759`; the won bid already flows to `freeze.players[].settledSalary`, write it to the visible `Player.salary` for FARM, idempotent; D-S7c-CPU open: do CPU/shill-won farm prospects excluded from `freeze.players` need the stamp?). Then S7d (supersede/delete old Gaussian, KEEP perceivedValueRange/scoutValueRange.ts — freeze dep — relocate gradeToTwentyEighty). Then RB-13b · RB-18 · B1.6.
