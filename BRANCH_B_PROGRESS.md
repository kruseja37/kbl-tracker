...
**➡ NEXT = S7b** (re-anchor `scoutRangeForProspect` `LeagueBuilderFarmAuctionDraft.tsx:103-124` → `gradeBandToPriceRange(overallGradeBand)` × chemFit on both endpoints; emit `{low,high,displayedEstimate}`; rewrite the keystone test `:262-342`; band IS the range, drop perceivedValueRange from this path). Then S7c (won-bid→farm salary, all real winners) → S7d (supersede/delete old Gaussian, keep perceivedValueRange).
