...
### S7d — recon-grounded 3-way SPLIT (Captain Explore recon 2026-06-23; too wide for one safe dispatch)

The LAST + widest Branch-B scouting change. Recon classified the ~40 `scoutedGrade` hits into the prospect-draft-origin chain (touch) vs opaque-passthrough consumers (trade/roster-analyzer — leave alone). Two HIGH-risk silent-failure hazards in S7d-3: the draft-board SORT (reorder) + `franchiseSalary.safeRoundFromScoutedGrade` (load-bearing, now only the won-bid FALLBACK post-S7c). Split:
- **S7d-1 ✅ DONE** (`b8b97e5b`) — relocate `gradeToTwentyEighty` → `gradeEngine.ts` (+ its test) + DELETE the dead `scoutPriceOpinion.ts`/test (S7b orphaned it; grep → zero). 2 importers repointed. Gate: build 0 + full suite 8084 pass / 1 fail (`wpaRuntimeBoundary`) = ZERO NEW REDS. `scoutValueRange.ts`/the Gaussian engine/oracle UNTOUCHED.
- **➡ NEXT = S7d-2** (delete the Gaussian model: `scoutProspect` + `confidenceFromAccuracy` + the `ProspectScoutingReport` interface in `prospectScoutingDraftEngine.ts`; rewrite the 2 characterized Gaussian tests in `prospectScoutingDraftEngine.test.ts` as band tests — specialist scout → narrower band, weak scout → wider band; update the 2 internal `scoutProspect` callers [:1614/:1706] to derive from the band). MEDIUM risk.
- **S7d-3 (HIGH-RISK, LAST)** — make `VisibleSafeProspectReport`/`ProspectProfile` band fields required (remove `?`); delete the Gaussian fields from `ProspectProfile` (scoutedGrade/scoutAccuracy/scoutConfidence/scoutGradeError); re-point the board SORT (`leagueBuilderStartupFarmDraft.ts:1116-1150`) to `overallGradeBand`; fix `franchiseSalary.safeRoundFromScoutedGrade`→`band.worst`; swap the scout-grade DISPLAY point→band across the prospect-draft-origin readers (`franchisePlayerProfile.ts`, `franchiseStartupProspectDraft.ts`, `LeagueBuilderDraft.tsx`, `scoutGradeDisplay`, `leagueBuilderFarmScoutingHandoff.ts`, profile-edit/continuity); **KEEP `perceivedValueRange`/`scoutValueRange.ts`** (Mode-2 freeze dep). **BV-S7d-3 NEEDED** (board order + farm-salary display + prospect-card grade display). Leave trade/roster-analyzer passthrough consumers alone.

Then RB-13b · RB-18 · B1.6.
