...
| FINDING-248 | 2026-07-20 | BUILDER VERIFIED — INDEPENDENT AUDIT PENDING | Snake Draft MLB-to-farm identity and prospect generation | New and recovered Snake drafts now carry each club's farm identity through setup, live catalog, recovery, Scout Reveal, and farm-session creation. Missing legacy identity fails closed with a generic repair control. Production farm pools use the exact Standard prospect distribution and keep true ratings hidden; focused flow 92/92, exact N=500 distribution, TypeScript, lint, build, and diff checks are green. |
