...
| FINDING-256 | 2026-07-21 | REPAIR AFTER AUDIT BLOCK — RE-AUDIT AND LIVE MIGRATION PENDING | Farm Snake host and companion recovery | First audit BLOCK, Major 5 / Minor 0. Recovery now ignores stale local Farm authority, rebuilds from frozen live truth, validates exact catalog order and Farm identity, keeps catalog-less companions polling, and keeps a visible Hotseat recovery control. |

### 2026-07-20 — FINDING-248 audit close

The hostile pre-freeze pass found one duplicate-club-set gap in the MLB-to-farm identity resolver;
the builder's own negative-feedback pass found one Auction-scope regression in the Scout Reveal
repair gate. Both were repaired with direct tests before the final freeze. The separate read-only
auditor approved exact head `914e35e9`, **Major 0 / Minor 0**. Independent proof is 92/92 focused
...
without changing public draft truth or revision, rebuilds prospects from the frozen seed, ordered
clubs, frozen Farm identities, and deterministic scout assignment, requires exact ordered ids,
builds the catalog from frozen session identity, and lets open companions reload a missing or
invalid catalog.

The first separate audit returned **BLOCK — Major 5 / Minor 0**. The repair no longer trusts a
browser-local Farm session, snapshot, or private board; catalog validation now uses exact league,
team, prospect, and Farm-identity truth; a companion can remain live and poll while the catalog is
missing; and an already-open Farm room keeps a visible recovery control after Hotseat capability
loss. The security claim is also corrected to the ruled v1 model: all devices use the owner's same
