...
| 23 | Snake Draft room | SnakeDraftRoom.tsx, SnakeCompanion.tsx, snakeAssistantBoard.ts, snakeLuxuryTax.ts, snakeLiveRoomTransport.ts, snakeLiveCatalog.ts | ⚠️ DEPLOY/JK GATE | FINDING-186-245: canonical setup/room path, roster-local tax/fit, one cloud private-board authority, Hotseat-only public writes, companion pick/trade intent, immutable exact catalog, one-step cloud correction, target/source team isolation, no-trade FARM, desktop-first companion layout, and generic-sync isolation are implemented at `fd07bba0` and independently approved at `cedf96ee`; migration 009, matching preview, and JK acceptance walk pending |
