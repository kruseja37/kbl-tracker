...
**Scope:** §18.3 — lock how 22-man and farm-prospect salaries relate; the tier-adjusted relative-to-pool rookie scale; MLB pick trading; immutable, non-tradeable FARM slots; per-draft grade distribution. Feeds the D-stack draft pieces. The original June read discussed tradeable FARM slots; that portion is historical and superseded by the 2026-07-14 ruling below.
...
> `/snake-room`. MLB pick trades now execute atomically against persisted ownership/order.
> **FARM draft-pick trades were retired by JK on 2026-07-14**; FARM salaries stay
> frozen to absolute slots without any pick-ownership exchange. Both draft phases confirm immutable manifests;
...
- **Pick-trade execution (historical finding, now superseded).** The original read found only advisory validation. Current snake code executes and persists commissioner-approved **MLB** pick trades through the snake trade/session engines, then refits private boards and re-clocks a traded live pick. FARM draft-pick trades are retired.
...
- **BUILD (historical program inventory):** (1) the relative-to-pool **rookie/draft-slot scale function** `(tier, pickValueChart, draftSlot) → price`, replacing the absolute 0.50× and the flat farm table; (2) **MLB pick-trade execution** (a pick-asset/ownership model + an executor that mutates draft-session pick order and persists, gated behind `validateTrade`); (3) a **`farmGradeMode`** parameter on prospect generation (skewing the round-keyed tables via `FARM_NERF_SCALES`), threaded from a farm-draft control, independent of the 22-man tier. The former FARM-pick-trade extension in this inventory is superseded.
...
- **DSF-2 — Tradeable asset = DRAFT PICKS (order positions) — HISTORICAL, PARTLY SUPERSEDED 2026-07-14.** The pick-ownership model and executor remain canon for the MLB snake draft. The extension to FARM draft picks is retired; FARM order and slot salaries remain immutable.
