...
## JK RULINGS — 2026-06-27 #2 (decision resolutions + parallelization)

- **Winner-honors "rarity" = per-award PRESTIGE value (#18 RESOLVED).** Assign each award a prestige-based fame value (a §16-tunable placeholder per award); multi-award winners STACK ADDITIVELY (more awards → more fame). "Nothing too complex." Build = a per-award prestige-weight table + winner fame = sum of won-award prestige values. The rarity metric is no longer open.
- **Champion / playoff MVP (#23 RESOLVED): reuse the MVP voting mechanism, scoped to the championship series (or full playoff run).** → EASY add (reuse the existing MVP voter on playoff stats) → INCLUDE in PLAYOFF-DRIVER P2/P3 (no longer "defer").
- **V8 park factors (#30): INTENDED.** Adaptive by design — DIMENSION-based out of the gate (no data baseline yet) → DATA-adaptive from 40% of season. **NEW REQUIREMENT / VERIFY (possible v1 gap):** the park DIMENSIONS (L/C/R feet + wall height) MUST load with the assigned stadium per team in the League Builder so park factors exist from game 1. Verify the stadium-dimensions → assigned-team park-factor wiring; if absent, it is a v1 build gap.
- **PARALLELIZATION (JK asked): YES — run the DRAFT UI / Stream B as a 2nd concurrent Codex lane** alongside the living-season lane (playoff driver, winner-honors, L-SIM harness). File surfaces are largely DISJOINT (draft UI = src_figma draft/setup pages + the setup spine; living season = engines + L-SIM + honors) — verify the partition first; SERIALIZE the heavy vitest/L-SIM gates. Draft-lane sequence: Tier-0 archetype converter (#1) FIRST (unblocker) → assembly merges (#26-28, bring the hub + draft screens onto trunk) → wire the screens (#11-17). Stream B plan = `STREAM_B_DRAFT_UIUX_BUILD_PLAN.md`.
