...
| **rivalry** | LSD §24 (REL-1 rivalry edge) as amended by DECISIONS_LOG 2026-06-27 RIVALRY-ENVY rulings + V1_CANON §6 record-overtake→HISTORY ruling | 2026-07-07 (canon); envy rulings 06-27 | **LOW** | PLAYER_RIVALRY_MATCHUP_ENGINE_SPEC.md (2026-06-26, DESIGN CAPTURE, not ruled/built, zero ledger trail — cited only by a Mode-1 brief and a superseded map); STADIUM_ANALYTICS_SPEC_V2.md §5.6 (park-record-overtake edge, "hop-6"); DECISIONS_LOG.md 2026-06-27 entries (newest rivalry rulings — live only in the ledger, never folded into a spec doc) | **Conflict, full:** canon §6 rules the shipped RIVALRY typing (`franchiseRelationshipOvertakeCompute.ts`) must become HISTORY (inert); the 2026-06-27 envy-edge rulings exist only in DECISIONS_LOG; the matchup-engine draft was possibly meant to house them and never updated. |
...
| **matchup history** | PLAYER_RIVALRY_MATCHUP_ENGINE_SPEC.md | 2026-06-26 | **LOW** | LSD §24.7 (REL-6 charged matchups — the only canon-cited matchup mechanic, far narrower) | **Conflict, full:** only doc naming matchup-history as a system; self-marked design-capture, never ruled, no build tickets, no canon row. Effectively an unratified vision doc — needs a JK status ruling (draft vs abandoned), not a code walk. |
...
| Requirement | Class | Spec cite | Code anchor | Light-up |
|---|---|---|---|---|
| Record-overtake edge must be inert HISTORY, not morale-bearing RIVALRY | DRIFT | V1_CANON §6:196; DECISIONS_LOG:272 | `franchiseRelationshipOvertakeCompute.ts:61,84` still types the row RIVALRY | Change 'RIVALRY'→'HISTORY' at both sites + doc-comment. **Owned** (ORPHAN_WIRING_MATRIX §13 row 155, one-line fix ticketed). |
