...
## PART 9 — PHASE 6: THE PRODUCTION SHIP GATE (appended per JK exchange 2026-07-08)
"All gates passed" = engineered-down, contained, reversible risk — never zero. The ship sequence, in order: (1) full backup export of every real-data origin (the 2026-07-07-fixed backup system); (2) saved-game migration check (already Phase-5 step 1); (3) deploy to a STAGING origin; (4) JK's acceptance walk on staging, on his real device (iPad Safari), against a COPY of real data — LLM edge-function behavior verified there (reporter features skip silently without keys); (5) production deploy with the rollback tag + backup in hand; real-data origin upgraded LAST. Escape hatches stay shippable: reserve dial k=0, per-flag soul overrides. Residual-risk classes and containments recorded in the 2026-07-08 session exchange; the principle: no UNCONTAINED risk, not "no risk."
