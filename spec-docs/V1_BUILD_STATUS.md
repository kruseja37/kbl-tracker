...
- BE **PARTIAL** — banding + 20-80 + hidden-rating gate BUILT, but bands come from the **hired-scout descriptor, not the farm archetype** (spec violation); no archetype→band mapping exists. UI **PARTIAL** — farm auction shows banded grades, but fed by the to-be-deprecated hire flow.
- Outstanding: [BE] derive 3/5/7 bands from farm archetype (author the archetype→per-area-confidence table); [BE] deprecate the scout-pool/hire-draft mechanism; [BE] enforce "wider farm bands than MLB"; [UI] move scout naming + farm-archetype pick onto the team page; [UI] add the cap-space risk-discipline headline.
