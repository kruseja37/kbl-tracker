...
**DRAFT — ~~awaiting captain ratification + the JK RIVALRY_SWEEP fork (rows at :68/:95)~~ the RIVALRY_SWEEP fork is RESOLVED (JK ruling, 2026-07-08 morning, verbal — series inference APPROVED; see `V1_CANON_2026-07-07.md` §6): this plan now awaits ONLY captain ratification; builds AFTER Mode-1 per the JK priority directive (2026-07-08 overnight: Mode-1 A-Z has absolute priority, Mode-2 builds deferred). No W-lanes dispatch from this plan until the ratification + Mode-1-completion gates clear.**
...
| RIVALRY_SWEEP :375 / SWEPT_BY_RIVAL :378 | series sweep vs rival | **MISSING input** — schedule has no series semantics (register §3.2 row 11) | derive "series" = consecutive scheduled games vs same opponent (`scheduleStorage` game numbers) + `areRivals` (`src/data/leagueStructure.ts:197`) / home-park rival | **RULED-BUILD (JK, 2026-07-08 morning, verbal — see `V1_CANON_2026-07-07.md` §6):** series inference APPROVED, no new user input required. ~~flagged JK-fork by the register; if JK declines, these 2 rows defer explicitly~~ |
...
**Row accounting:** W2a 29 + W2b 11 + W2c 12 = 52 (of which: 4 defer-with-playoffs, 1 defers-with-missing-system (WEEKLY_AWARD), ~~2 JK-fork (RIVALRY_SWEEP pair)~~ 0 JK-fork (RIVALRY_SWEEP pair RULED-BUILD 2026-07-08 morning — now dispatches like any other row), 4 neutral-by-design no-ops; 2 NEW rows added: SEND_DOWN, race.win). Everything else dispatches from data already on the GameTracker/hub pipeline — the wire-all ruling's premise verified above per row.
