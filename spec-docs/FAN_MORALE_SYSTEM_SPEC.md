...
- Partial: random-event prompts can be manually confirmed for safe scoped morale effects. Game-result base impacts now support confirmation-gated team fan morale prompts for both teams (`+1/-1` regular results, `+2/-2` shutouts). Score-only evidence remains team fan morale only after confirmation.
...
Next roadmap steps are streak detection, richer event modifiers, expected-wins baseline, formula weighting, and daily morale snapshot tracking once canonical value/designation inputs are trusted.
