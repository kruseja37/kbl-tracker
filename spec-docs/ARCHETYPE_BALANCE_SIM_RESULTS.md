...
### LOCKED (2026-06-26)
The set is now a canonical module — `src/data/historicalArchetypes.ts` (15 archetypes: id, name, exemplars, era, lore, plain "boost → sacrifice" identity, and the value-calibrated `spec` profile + `archetypeCapShift()` resolver). Validated across **all three tiers** by `historicalArchetypes.test.ts` (gates on `withinBand`): **15/15 in band at juiced (maxDev 4.7%), standard (5.7%), nerfed (8.1%)** — nerfed is the tightest tier. The workbook-baseline test is kept as provenance only (the deep-nerf workbook extremes are why we ship this historical set instead). Production wiring (per-team storage + applying the cap shift in the auction + the picker UI) is the next step, on the settled set.

### Pool strength vs pool composition (2026-06-26) — two different axes
- **Pool STRENGTH (talent level) is NOT a balance risk — weaker is safer.** The 3-tier gate in the test scales budget+caps but keeps juiced ratings (the CONSERVATIVE/hardest case → nerfed reads 8.1%). The FAITHFUL run (ratings also scaled by TIER_RATING_SCALES) is tighter: standard 5.3%, **nerfed 4.9%**; a pool at ×0.80 ratings is **3.1%**. Reason: the luxury tax is convex on rating overages, so a weaker pool → smaller overages → less tax → archetypes differentiate LESS → more balanced. The juiced 440-pool is the worst case for balance; passing there means passing on every weaker pool. (The gate stays on the conservative budget-only scaling.)
- **Pool COMPOSITION (type mix) IS the real risk** — a pool missing a player TYPE (e.g. speedsters) can't build the archetype that needs it, regardless of strength balance. Mitigation = the pool-feasibility check (NEXT build): report per-archetype support against the LOCKED pool and PROMPT the GM ("add ~N speed-first players to activate Whiteyball"); the GM decides if they care. (JK idea, endorsed.)
- **Tier = power-level lever** (lower tier narrows the MLB-vs-prospect gap → call-ups/development matter more); nerfed is fully balanced (4.9%). OPEN: confirm whether picking a tier also weakens player RATINGS or only the budget — if budget-only, "genuinely weaker teams" needs a regenerate-pool-at-tier step (the stock SML pool is juiced).
