...
**The one doc a cold-start successor reads to resume, with zero chat context.** Repo: `/private/tmp/kbl-port2`, branch `main-track` (tracks `origin/main`). Refreshed by a scribe pass at HEAD `3dfa1d38` (committer timestamp `2026-07-07T20:59:25-06:00` local / `2026-07-08 02:59 UTC` — several governing docs are named/dated `2026-07-08` because the captain labels "evening" work by UTC date; local clock is still `07-07` at time of writing. Not a real sequencing gap — noted so a successor doesn't chase a phantom day.)
...
Mode-1 (draft/setup) is the active front; Mode-2 (living season) wiring is queued behind it per JK's ruled sequencing. Eight lanes merged this session (§2), all pre-merge adversarially audited, zero JK decision forks currently open. **One lane is FAILED-AT-GATE and must NOT be merged: M1E** (diminishing positional need) — its BINDING sim gate failed at 30-team production scale and an attribution diagnostic (M1E-DIAG) is running in its worktree right now (§3, verified via live process inspection). The **SOT-roundup census is COMPLETE** (15 agents); a lander agent is writing `spec-docs/SOT_REGISTER_2026-07-08.md` — treat it as incoming if not yet on main when you read this. No open JK decision forks as of this checkpoint (last two closed: home-park-rival 2x amplifier, MatchupDramaBar retirement). Two standing human gates remain, neither blocking further build work: JK's browser feel-pass on reserve prices (Lever A, k=0.65 default) and the final v1 acceptance walk (§0 of `V1_CANON`).
...
## 3. In-flight: M1E — GATE-FAILED, DO NOT MERGE (diagnostic running, live-verified via `ps` at checkpoint time)

**M1E** (F8 diminishing positional need + F11 overstacked advisor; worktree `/private/tmp/kbl-m1e-diminish`, branch `lane/m1e-diminish`, contract `spec-docs/contracts/CONTRACT_M1E_DIMINISH.md`, design `FABLE_F8_DIMINISHING_NEED_DESIGN_2026-07-08.md`) hit its BINDING §5 sim gate and **FAILED at 30-team production scale**: 73/150 legal roster completions, spot-11 budget mean 14.3% (target ~35-45%), spread 39.1%. The builder correctly refused to commit — the full report is at the lane worktree's `spec-docs/M1E_SIM_GATE_REPORT.md` (NOT on main), whose own closing line is "No acceptance commits should be made from this state." **M1E must NOT be merged in its current state — no exceptions, regardless of who asks.**

**Attribution is genuinely open** — a methodological hole means the failure is not yet proven to be F8's fault: the 30-team production gate row was never run on PRE-change code, so there is no baseline for it. A diagnosis-only Codex lane (**M1E-DIAG**) is running in the same worktree right now: it A/B-runs the identical binding row (same seeds/env) against BASE engine code. Two outcomes: (a) BASE also fails → the finding is a pre-existing production-scale completion defect (a major Mode-1 finding in its own right — JK tests 30-team drafts) and F8 gets judged on its delta vs base; (b) BASE passes → F8 broke completion, and the diagnostic bisects the mechanism (suspects, in its contract's priority order: the 0.50 clamp floor leaking into completion machinery; position-tagged filler buckets starving projection candidates; WTP-below-reserve mass pass-outs that exhaustion cleanup can't legally fill; mustBuy/needOverride no longer firing). Successor action when M1E-DIAG finishes: read its findings in the worktree, then it's a captain/JK call — fix-and-regate F8, or split out the pre-existing defect as its own lane.

**M1E history (settled, don't re-litigate):** the lane's earlier BLOCKED state (stale `BLOCKED.md` may still sit in the worktree) was a 440-vs-660 pool-size design assumption, resolved by ruling `89931773` — the shipped 660-player pool via the production pool path is normative. That question is closed; the gate failure above is a different, later event.
...
1. **M1F** — whisper one-ceiling [F9, already ruled: one liquidity-adjusted maxBid drives verdict/room-read/budget-light everywhere; capValue becomes a separately-labeled "total capacity" line only]. Dispatchable now (M1E's slot is consumed by its diagnostic, not a build — see §3).
...
canon (V1_CANON_2026-07-07.md, JK-approved — §6 is the rulings ledger, dated, append-only)
  ...
  > execution
      ...
      SOT_REGISTER_2026-07-08.md            (Mode-2 per-system SOT census + gap lists — INCOMING at this refresh; lander agent writing it, may not be on main yet)
