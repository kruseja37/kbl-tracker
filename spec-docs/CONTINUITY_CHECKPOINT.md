...
**The one doc a cold-start successor reads to resume, with zero chat context.** Repo: `/private/tmp/kbl-port2`, branch `main-track` (tracks `origin/main`). Refreshed by a scribe pass at HEAD `3f98eb06` (2026-07-08 — NIGHT COMPLETE, walkthrough-ready, awaiting JK's morning check-in).
...
2. **Post-walkthrough small fixes:** F15 (stale legality re-check) · F17 (farm-gaps sidebar staleness) · F19 (pool-gen first-try-legal, pool-first path) · F20 (silent re-extraction on lock) — plus F14b (uncompletable-pool UI) and the VERIFY-DRAFT-SETUP-OWNERSHIP follow-up when convenient.
...
*(Landed/absorbed: P5+F6 → M1I `8eb24037`; P2 → gauntlet leg 2 PASS; P3 → M1K `e21b23cf`; P6 → M1L `11a80835`; P7 → M1M `d35f6a62`; P8 → M1N `7714b82a`; P11 → M1O `67b39e1b`; F9+P4 → M1P `500e66bf`; P10 → gauntlet leg 3 PASS; F21 → M1Q `3f98eb06` — see §2/§3.)*
