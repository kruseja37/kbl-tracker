...
**M1E-FINAL — the binding F8/F11 verdict.** Runs in the M1E worktree (`/private/tmp/kbl-m1e-diminish`, branch `lane/m1e-diminish`; contract `spec-docs/contracts/CONTRACT_M1E_DIMINISH.md`, design `FABLE_F8_DIMINISHING_NEED_DESIGN_2026-07-08.md` amended to the 8-team gate per ruling `df9acc9a`). Job, now that M1J fixed the base engine: (1) commit the preserved dirty work as two commits — the auctionSim all-or-nothing completion cleanup [M1E-FIX] and the F8/F11 feature work; (2) rebase the lane onto post-M1J main; (3) run the **organic live-driver gate BASE-vs-F8 at v1 scale — the BINDING final verdict** on whether F8 ships. Until that verdict + Opus audit: **DO-NOT-MERGE stands — no exceptions, regardless of who asks.** Process note: the FIRST M1E-FINAL dispatch hung silently (0.12s CPU in 52 minutes — killed); this is the re-dispatch, with a watchdog armed. Chain history (settled, don't re-litigate): sim gate FAILED (F8: 73/150) → M1E-DIAG proved BASE fails too (89/150, pre-existing; diagnosis `6dbcd99b` in-worktree) → live-driver run surfaced F14 → fixed as M1J (`08236fdf`) → F8 re-judges on the fixed base. Also settled: 440-vs-660 (`89931773`); 8-team re-base (`df9acc9a`). Riding elsewhere: the auctionSim-vs-live fidelity gap (post-v1/instrument wave).

**M1K — farm auction → AuctionStage fold [P3].** Worktree `/private/tmp/kbl-m1k-farmstage`, branch `lane/m1k-farmstage` off origin/main. Kills the mid-journey UI register break; the target component already exists (`/__preview/auction-stage` farm tab). Standard protocol (§5) on completion.

**M1L — post-freeze summary screen [P6].** Worktree `/private/tmp/kbl-m1l-freeze`, branch `lane/m1l-freeze` off origin/main. Freeze results are silent today; JK must SEE what froze (salaries, morale baselines) to validate it. Standard protocol (§5) on completion.

**Morale-wave build-plan design agent (Claude-side, not process-verifiable from this repo)** — preparing the Mode-2 morale-wave W-lane decomposition; its output plan needs captain ratification before W-lanes dispatch (a scope fork inside it would defer to the morning check-in per §0).
...
2. **P7** — RULES-V1-PRUNE (ruled 2026-07-07: every Rules/season-config knob wired to real behavior or removed).
3. **P8** — conference editor (ruled IN v1 2026-07-01; `conferences:[]` still hardcoded).
4. **P11** — scout-reveal screen placement (move the reveal adjacent to the farm phase; F10 proved it doesn't actually gate the MLB auction — placement-only).
5. **P2** — design-first mode validation walk (pool-first is browser-proven; design-first is not).
6. **P9** — wrong-fit penalty Option A (visible pre-bid budget debit; design in `SCOUTING_INTELLIGENCE_SPEC.md` §3.3 + recovered §13.3 copy) — last unbuilt S7 economy law. **After the M1E verdict** (shares economy surfaces).
7. **Morale W-lanes** — from the morale-wave build plan (§3), once the plan is ratified.
8. **P4** — farm-side Assistant-GM whisper (today the farm draft is advice-dead).
9. **P10** — FS-3 shill-dissolve validation (believed fixed by C3's `nonCompletingTeamIds`; prove it in the gauntlet with shills > 0).
   *(Landed/absorbed: P5+F6 → M1I `8eb24037`; P3 → M1K in flight; P6 → M1L in flight — see §2/§3.)*
