...
**The one doc a cold-start successor reads to resume, with zero chat context.** Repo: `/private/tmp/kbl-port2`, branch `main-track` (tracks `origin/main`). Refreshed by a scribe pass at HEAD `11a80835` (2026-07-08 overnight).
...
## 3. In-flight lanes (overnight roster, live-verified via `ps`/worktree inspection at checkpoint time)

**M1E-FINAL (attempt 3) — the binding F8/F11 verdict; running healthy.** Runs in the M1E worktree (`/private/tmp/kbl-m1e-diminish`, branch `lane/m1e-diminish`; contract `spec-docs/contracts/CONTRACT_M1E_DIMINISH.md`, design `FABLE_F8_DIMINISHING_NEED_DESIGN_2026-07-08.md` amended to the 8-team gate per ruling `df9acc9a`). Job, now that M1J fixed the base engine: (1) commit the preserved dirty work as two commits — the auctionSim all-or-nothing completion cleanup [M1E-FIX] and the F8/F11 feature work (**commit-first**, so a killed dispatch can't lose it); (2) rebase the lane onto post-M1J main; (3) run the **organic live-driver gate BASE-vs-F8 at v1 scale — the BINDING final verdict** on whether F8 ships. Until that verdict + Opus audit: **DO-NOT-MERGE stands — no exceptions, regardless of who asks.** Process notes: attempts 1-2 died/hung silently (attempt 2: 0.12s CPU in 52 min, killed); attempt 3 runs at medium reasoning effort with a watchdog armed. Chain history (settled, don't re-litigate): sim gate FAILED (F8: 73/150) → M1E-DIAG proved BASE fails too (89/150, pre-existing; diagnosis `6dbcd99b` in-worktree) → live-driver run surfaced F14 → fixed as M1J (`08236fdf`) → F8 re-judges on the fixed base. Also settled: 440-vs-660 (`89931773`); 8-team re-base (`df9acc9a`).

**M1M — RULES-V1-PRUNE [P7].** Worktree `/private/tmp/kbl-m1m-rulesprune`, branch `lane/m1m-rulesprune` off origin/main. Per the 2026-07-07 ruling: every Rules/season-config knob wired to real behavior or removed — no decorative knobs into a validation pass. Standard protocol (§5) on completion.

**M1N — conference editor [P8].** Worktree `/private/tmp/kbl-m1n-conference`, branch `lane/m1n-conference` off origin/main. Ruled IN v1 2026-07-01 (conference assignment at league setup, NO division editor); `conferences:[]` is still hardcoded today. Standard protocol (§5) on completion.

*(Scribe verification caveat: at booking time, M1E-FINAL's builder process was live via `ps`; the M1M/M1N worktrees existed on their branches but their builder processes were not yet observable — if either shows no progress on the next check, treat it as the known silent-death pattern and re-dispatch, don't rebuild.)*

All lane worktrees follow the standard lane pattern (§5) and the known-flake list (§6).

## 4. Queued lanes — the OVERNIGHT queue order (next dispatches as slots free; JK-gated items defer to morning per §0)

1. **M1F** — whisper one-ceiling [F9, already ruled: one liquidity-adjusted maxBid drives verdict/room-read/budget-light everywhere; capValue becomes a separately-labeled "total capacity" line only]. **Dispatch after the M1E-FINAL verdict** (shares economy surfaces).
2. **P11** — scout-reveal screen placement (move the reveal adjacent to the farm phase; F10 proved it doesn't actually gate the MLB auction — placement-only).
3. **P2** — design-first mode validation walk (pool-first is browser-proven; design-first is not).
4. **P9** — wrong-fit penalty Option A (visible pre-bid budget debit; design in `SCOUTING_INTELLIGENCE_SPEC.md` §3.3 + recovered §13.3 copy) — last unbuilt S7 economy law. **After the M1E verdict** (shares economy surfaces).
5. **P4** — farm-side Assistant-GM whisper (today the farm draft is advice-dead).
6. **P10** — FS-3 shill-dissolve validation (believed fixed by C3's `nonCompletingTeamIds`; prove it in the gauntlet with shills > 0).
7. Then **the Mode-1 GAUNTLET** (`MODE1_PUNCHLIST_2026-07-08.md` §3 — scripted end-to-end browser journeys across the {pool-first/design-first} × {6-team/**8-team**} × {shills 0/1-2} × {k=0/0.65} grid — **re-based per the 2026-07-08 8-team ruling `df9acc9a`; 30-team configs are post-v1**). **Runs BEFORE morning per the JK priority directive (§0).**
8. Then JK's **A-Z feel pass (P12)** + **F5 reskin** (ballpark-kit design wave, cosmetic, correctness-gated).

*(Landed/absorbed: P5+F6 → M1I `8eb24037`; P3 → M1K `e21b23cf`; P6 → M1L `11a80835`; P7 → M1M in flight; P8 → M1N in flight — see §2/§3.)*
**Morale W-lanes: PARKED.** The plan exists at `spec-docs/MORALE_WAVE_BUILD_PLAN_2026-07-08.md` — **DRAFT, awaiting captain ratification + the JK RIVALRY_SWEEP fork; builds AFTER Mode-1 per the JK priority directive.** No W-lanes tonight.
