...
**The one doc a cold-start successor reads to resume, with zero chat context.** Repo: `/private/tmp/kbl-port2`, branch `main-track` (tracks `origin/main`). Refreshed by a scribe pass at HEAD `24b3e6c0` (committer timestamp `2026-07-07T21:08:04-06:00` local / `2026-07-08 03:08 UTC` — several governing docs are named/dated `2026-07-08` because the captain labels "evening" work by UTC date; local clock is still `07-07` at time of writing. Not a real sequencing gap — noted so a successor doesn't chase a phantom day.)
...
Mode-1 (draft/setup) is the active front; Mode-2 (living season) wiring is queued behind it per JK's ruled sequencing. Eight lanes merged this session (§2), all pre-merge adversarially audited. **M1E remains DO-NOT-MERGE** — the M1E-DIAG A/B settled attribution: **BASE also fails the 30-team production row (89/150 legal completions) → the sim completion defect is PRE-EXISTING, F8 did not cause it** (though F8 worsens it, 73/150); a remediation lane (**M1E-FIX**) is running in the worktree right now (§3). **The SOT register LANDED (`24b3e6c0`)** — per-system canonical-doc census + spec-vs-code verification of 10 Mode-2 systems, surfacing **~45 new unowned gaps** (its §4c) that feed the pre-tuning queue, 5 new JK ruling questions (its §4b), and a banner/correction action list (its §4a — executed this pass). A second Mode-1 build lane (**M1I**, staff-carry/league-handoff) is dispatched and building (§3). JK decision forks: the 5 SOT-register §4b questions are now open (matchup-history status, aging semantics, reporter §16 mechanics retirement, REL-8 captain composite, Reliever-of-Year signal). Two standing human gates remain: JK's browser feel-pass on reserve prices (Lever A, k=0.65 default) and the final v1 acceptance walk (§0 of `V1_CANON`).
...
## 3. In-flight lanes (live-verified via `ps` at checkpoint time)

**M1E — still DO-NOT-MERGE; remediation (M1E-FIX) running.** (F8 diminishing positional need + F11 overstacked advisor; worktree `/private/tmp/kbl-m1e-diminish`, branch `lane/m1e-diminish`, contract `spec-docs/contracts/CONTRACT_M1E_DIMINISH.md`, design `FABLE_F8_DIMINISHING_NEED_DESIGN_2026-07-08.md`.) History in order: (1) the BINDING §5 sim gate FAILED at 30-team production scale — F8 code: 73/150 legal completions, spot-11 14.3%, spread 39.1%; builder correctly refused to commit. (2) The M1E-DIAG A/B ran the identical row against BASE engine code and settled attribution: **BASE FAILS TOO (89/150) — a PRE-EXISTING auctionSim completion defect at 30-team scale, not an F8 regression** (though F8 worsens completion vs base and must be re-judged as a delta after the defect is fixed). Diagnosis committed in the worktree (`6dbcd99b`, appended to `spec-docs/M1E_SIM_GATE_REPORT.md` — worktree-only, NOT on main). (3) **M1E-FIX** is now running per the diagnosis's remediation direction: fix the sim completion defect (all-or-nothing cleanup against a legal 22-body completion quote — `runAuctionSim.ts` currently stops on the first unaffordable pick, stranding partial rosters), then re-run the full gate. **The do-not-merge status is UNCHANGED until the gate passes and the lane is audited — no exceptions, regardless of who asks.** The earlier 440-vs-660 pool-size BLOCKED episode is settled (ruling `89931773`, shipped 660 pool is normative) — don't re-litigate.

**M1I — staff carry-through + league handoff [P5+F6], dispatched and building.** Worktree `/private/tmp/kbl-m1i-staffcarry`, branch `lane/m1i-staffcarry`. Scope: hired reporter/manager identity reaching the hub (generic fallback today) + the F6 handoff bug (`EndOfDraftStaffing.tsx` drops leagueId on the way to Franchise Setup — thread it, auto-select on mount). Live edits observed at checkpoint on exactly that surface (`EndOfDraftStaffing.tsx`, `FranchiseSetup.tsx`, `useFranchiseLensData.ts`, `draftRouting.ts` + new `EndOfDraftStaffing.test.tsx`), vitest running. Follows the standard protocol (§5): Opus audit → `--no-ff` merge with verdict → punch-list landing note (P5/F6 rows) → this doc's §2.

Both worktrees follow the standard lane pattern (§5) and the known-flake list (§6).
...
canon (V1_CANON_2026-07-07.md, JK-approved — §6 is the rulings ledger, dated, append-only)
  ...
  > execution
      ...
      SOT_REGISTER_2026-07-08.md            (Mode-2 per-system SOT register + spec-vs-code verification, LANDED 24b3e6c0 — look up a system's governing doc HERE before trusting any other doc's claim about it; §4a banners executed 2026-07-08, §4b = 5 open JK questions, §4c = the pre-tuning gap lists)
