...
2. **Final booking** — this doc + punch list + gauntlet report current; then assemble the **MORNING PACKET** for JK (walkthrough entry point, the F18 design fork, the F15/F17/F19 small-fix tickets, the codex-flakiness note, the parked F8 story in plain language).
3. **P2** — design-first mode validation walk = **gauntlet leg 2, if time before morning** (pool-first leg passed; design-first unproven).
4. **P9** — wrong-fit penalty Option A: **PARKED WITH F8** (shares the same economy surfaces and the same market-model tuning family; revisit when F8 un-parks on a proper harness).
5. **P10** — FS-3 shill-dissolve validation (prove in a shills 1-2 gauntlet leg).
6. Remaining gauntlet legs per the §3 grid (shills 1-2, k=0, 6-team, RUN-IT-BACK) — leg 1 (8-team pool-first shills-0 k065) = **PASS**, report at `MODE1_GAUNTLET_REPORT_2026-07-08.md`.
7. Then JK's **A-Z feel pass (P12)** + **F5 reskin** (ballpark-kit design wave, cosmetic, correctness-gated).

*(Landed/absorbed: P5+F6 → M1I `8eb24037`; P3 → M1K `e21b23cf`; P6 → M1L `11a80835`; P7 → M1M `d35f6a62`; P8 → M1N `7714b82a`; P11 → M1O `67b39e1b`; F9+P4 → M1P building — see §2/§3.)*
