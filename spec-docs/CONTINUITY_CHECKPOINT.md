...
**The one doc a cold-start successor reads to resume, with zero chat context.** Repo: `/private/tmp/kbl-port2`, branch `main-track` (tracks `origin/main`). Refreshed by a scribe pass at HEAD `67b39e1b` (2026-07-08 overnight, post-gauntlet-leg-1).
...
**AUTH-4 OVERNIGHT MODE IS ACTIVE, with the Mode-1 priority directive (§0)** — Mode-1 A-Z has absolute priority; goal = JK walkthrough-ready by morning; Mode-2 builds deferred. **HEADLINE: GAUNTLET LEGS 1 AND 2 = PASS** (`MODE1_GAUNTLET_REPORT_2026-07-08.md`). Leg 1 (8-team, pool-first, shills 0, k=0.65): the full draft A-Z ran in a real browser — ALL clubs legal in BOTH auctions, 0 console errors, 0 manual adds, staff names + freeze summary + league handoff all confirmed live end-to-end. Leg 2 (8-team, DESIGN-FIRST, shills default, k=0.65, "GauntletD8"): **design-first proven lockable+draftable — P2 CLOSED** — legal pool on the FIRST roll (zero regenerates, zero manual adds), design targets 22/22 post-lock, 30 lots drafted normally incl. live shill purchases, 0 console errors (F15 not exercised — the pool never needed regenerate; unconfirmed either way, not disproven). Six findings ticketed from the walks (F15-F20, punch-list §1c; F18 = a JK design fork for morning). **Fifteen lane merges this session (§2), all pre-merge adversarially audited** — overnight additions: **M1M** (P7 rules prune, `d35f6a62`), **M1N** (P8 conference editor, `7714b82a`), **M1O** (P11 scout-reveal placement, `67b39e1b`). **F8/F11 (lane M1E) is PARKED, verdict FAILED-ON-HARNESS-ARTIFACT** — the v1-scale final gate failed, but the gauntlet then proved real 8-team drafts complete cleanly, so the harness's pool construction is the artifact; parked on `lane/m1e-diminish` (gate report `6f6d0bbd`) pending a properly-based harness; NOT a v1 walkthrough blocker (§3). **In flight now (§3): M1P** (F9 whisper one-ceiling + P4 farm whisper) — **rerouted to a Claude-side builder** after codex startup-stall #5 tonight; audit to follow. **Codex flakiness note: 5 startup stalls tonight — prefer Claude-side builders until it recovers.** **The morale-wave build plan is PARKED as DRAFT** (`spec-docs/MORALE_WAVE_BUILD_PLAN_2026-07-08.md`, awaiting ratification + the JK RIVALRY_SWEEP fork; builds after Mode-1). **The SOT register LANDED (`24b3e6c0`)** — per-system canonical-doc census + spec-vs-code verification of 10 Mode-2 systems, surfacing **~45 new unowned gaps** (its §4c) that feed the pre-tuning queue, 5 new JK ruling questions (its §4b), and a banner/correction action list (its §4a — executed this pass). **The 5 SOT-register §4b questions were RULED same evening** (canon ledger `34a1e8d1` + aging clarification `0d2f3802`/`4835cc9a`): matchup engine IN for v1 (ratify spec, then build); reporter pre-narrator mechanics RETIRED; REL-8 captain composite BUILDS in the morale wave; Reliever-of-Year = pitching WPA ratified as shipped; in-season 5-tier age drift IS v1 (verified built-and-fed) — only multi-season arcs stay deferred. **Late-evening batch (canon §6, recorded this pass):** (1) **V1 DRAFT SCALE = 8 TEAMS (+1-2 shills)** (`df9acc9a`) — the F8 economy gate and the gauntlet grid re-base to 8-team rooms; the 30-team closer shortfall + completion cascade are POST-v1 concerns, not blockers. (2) **RECORD-OVERTAKE REVERSAL** — the 2026-07-07 overtake→HISTORY ruling is reversed; overtake edges are genuine RIVALRY (correct-as-built) and charge future matchups per REL-6; the retype ticket is CANCELLED everywhere it was queued. (3) **MATCHUP ENGINE RATIFIED-AS-AMENDED** — `MATCHUP_ENGINE_RECONCILIATION_2026-07-08.md` governs; spawn rule = MUTUAL-WPA (both players banked positive-WPA moments vs each other above a §16 threshold; one-sided domination never spawns); GameTracker recent-form suffix allowed, browser-gated (sole freeze exception). Zero open JK forks. Two standing human gates remain: JK's browser feel-pass on reserve prices (Lever A, k=0.65 default) and the final v1 acceptance walk (§0 of `V1_CANON`).
...
**M1P — whisper one-ceiling [F9] + farm whisper [P4], BUILDING via a CLAUDE-SIDE builder.** Rerouted off codex after startup-stall #5 tonight (see the flakiness note, §1). Scope: F9 as ruled (ONE liquidity-adjusted maxBid drives verdict, room-read AND budget-light; capValue only as a separately-labeled "total capacity" line) + P4 (Assistant-GM whisper presence on the farm draft, advice-dead today). Opus adversarial audit to follow the build as usual (§5) — builder ≠ auditor holds regardless of which side builds.
...
## 4. Remaining queue (overnight tail → morning)

1. **M1P** — building now (§3): F9 whisper one-ceiling + P4 farm whisper.
2. **Final booking** — this doc + punch list + gauntlet report current; then assemble the **MORNING PACKET** for JK (walkthrough entry point, the F18 design fork, the F15/F17/F19/F20 small-fix tickets, the codex-flakiness note, the parked F8 story in plain language).
3. **P9** — wrong-fit penalty Option A: **PARKED WITH F8** (shares the same economy surfaces and the same market-model tuning family; revisit when F8 un-parks on a proper harness).
4. **P10** — FS-3 shill-dissolve validation (leg 2 already proved live shill PURCHASES; the dissolve-at-completion leg still needs a full-draft shills run).
5. Remaining gauntlet legs per the §3 grid (shills 1-2 full-draft, k=0, 6-team, RUN-IT-BACK) — **leg 1 (8-team pool-first shills-0 k065) = PASS; leg 2 (8-team design-first shills-default k065, P2) = PASS**; report at `MODE1_GAUNTLET_REPORT_2026-07-08.md`.
6. Then JK's **A-Z feel pass (P12)** + **F5 reskin** (ballpark-kit design wave, cosmetic, correctness-gated).

*(Landed/absorbed: P5+F6 → M1I `8eb24037`; P2 → gauntlet leg 2 PASS; P3 → M1K `e21b23cf`; P6 → M1L `11a80835`; P7 → M1M `d35f6a62`; P8 → M1N `7714b82a`; P11 → M1O `67b39e1b`; F9+P4 → M1P building — see §2/§3.)*
