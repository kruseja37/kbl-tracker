...
| 1 | KERNEL-TRUTH-1: completion-pipeline truth kernel (A-H incl. archive-early + outcome ledger, scope rejection, civil dates) | CONTRACT_KERNEL_TRUTH_2026-07-11 | ~~**BUILDING** (codex/living-kernel)~~ → **SHIPPED PR #83** (built+audited APPROVED; OPEN, awaiting JK merge) |
| 2 | MIRROR-1: console-mirror schema + service (state machine, CAS, history reader, double-merge kill) | CONTRACT_MIRROR_1_2026-07-11 | ~~WRITTEN, HELD until deep-pass/hunt merge confirms scope~~ → **SHIPPED PR #80 — MERGED** to `main` 2026-07-11T18:20:34Z (one fix-pass: stale-boundary quarantine). Follow-on fix (apply-failed retryability) landed on the lane AFTER this merge, so it is NOT in `main` yet — it rides in via PR #86 instead |
| 3 | FIDELITY-1: L-SIM minimal bridge (synthetic games read current stored ratings + mojo×fitness; neutral + slump→recovery regimes; 7-step loop-closure proof). Acceptance language: proves CLOSURE, not final feel | ~~⏳ to write~~ → cited as CONTRACT_FIDELITY_2026-07-11 in the ship commit, but no such file was ever actually committed (verified `git log --all`) — rulings live in the commit message only | ~~QUEUED~~ → **DONE** on `integration/living-season-full` @ `0be5ea2e` (7-step closure proof PASSED, byte-identical replay, falsification 49/49); not yet on `main` |
| 4 | MIRRORUI-1: takeover UI becomes real (confirm/reject/adjust per item, oldest-first, ordinal labels) + the UI-lies batch (Wire early-return, stories/wire dedup, morale trends, pending-vs-earned labels, checkpoint ordinal) — single lane, same file surface | ~~⏳ to write~~ → cited as CONTRACT_MIRRORUI_2026-07-11 in the ship commit, but no such file was ever actually committed (verified `git log --all`) | ~~QUEUED (after 1+2 merge)~~ → **SHIPPED PR #86** (built+audited APPROVED after one service-gap STOP — apply-failed retryability; OPEN, stacked on #80 which is already merged) |
| 5 | HUNTFIX-1..n: confirmed hunt findings not covered by slices 1-4, batched by file surface | ~~ENGINE + TRACKER dispatched; KERNEL2 (result-context: 27-out CG + walk-off) queued post-KERNEL~~ | ~~BUILDING~~ → **SHIPPED**: HUNTFIX-ENGINE-1 PR #82 — **MERGED**; HUNTFIX-TRACKER-1 PR #81 — **MERGED**; KERNEL2 PR #84 — built+audited APPROVED, OPEN (stacked on #83). Plus two same-day lanes outside this row's original scope, also shipped: MODE-KILL-1 PR #87 (audited APPROVED, OPEN, stacked on #84) and FAME-CLEANUP-1 PR #85 (audited APPROVED, OPEN, needs #83 first — #81 dependency already satisfied by merge) |
| 2b | MIRROR-1b: generators stamp expectedPriorValue/proposedValue at sweep time (strict CAS tier) — AFTER KERNEL merges (same files) | ⏳ | QUEUED (unchanged — next after the flip-gate sequence below) |
| 6 | TUNE-0: tuning targets + L-SIM baseline + one-factor sensitivity ranking (deliverable = ranked knob shortlist + target table, NOT converged tuning) | ⏳ to write | ~~QUEUED~~ → **IN FLIGHT** (dispatched to Codex on the `kbl-integration` worktree) |
| 7 | SWITCH-3A: per-league activation, built default-off, migration choice for existing leagues, tuning-profile version snapshot | ⏳ to write | ~~STRETCH (cut first if day slips)~~ → **NOT STARTED** — next session; contract not yet written; remains the cut-line stretch slice |
