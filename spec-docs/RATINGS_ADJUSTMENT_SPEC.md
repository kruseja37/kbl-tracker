...
- **Magnitude:** signed continuous delta from the relative signal (age-modified) shaped by the §6A convex curve + the age-curve term + morale coupling (high morale amplifies gains / shrinks drops, per Model A), capped per checkpoint, 0–99 clamp. (See §6A for the full range model — the magnitude is the gradation, so it must be aggressive-when-warranted; ±1–2 is noise given that traits + mojo already swing the profile that much.)
...
## 6A. Range & magnitude — "earn the rare leap" (RULED)
**Philosophy (JK):** ±1–2 points "may as well be zero" — traits coming/going and mojo swings already move the profile that much, so rating movement must be **aggressive WHEN WARRANTED**. A player who is *truly elite relative to his position peers, sustained all season* can climb **~a full grade level** on a key attribute; a player only *marginally* better than his peers barely moves. Symmetric for collapse (a falling-apart player can crater into demotion/trade-bait territory). The hard part is the gate: the leap must be **earned**, but it is **possible**.

**(a) Convex reward on the relative signal — the gate.** Let `r ∈ [−1,+1]` be the player's signed standing vs his position-group cohort (age-modified; `0` = average-for-cohort, `+1` = elite, `−1` = collapsing). Per-checkpoint move:
```
if |r| < startBar:           Δ = 0                      (dead-band — most players, no move)
else:  Δ = sign(r) · baseScale · ((|r| − startBar)/(1 − startBar))^γ ,  γ > 1   (convex)
```
- **γ > 1 (convex)** is the whole point: a player just past the dead-band (marginally > peers) gets a *tiny* move; only deep-tail (truly elite / truly collapsing) performers approach the per-checkpoint cap. *Worked feel:* an average SS posting **elite-vs-SS** numbers → large, special climb; a power RF only **marginally > RF peers** → little power movement (he's not having a standout season); the median player → flat.
- **The rare leap must be SUSTAINED.** The per-checkpoint move is capped (moderate, not tiny — accumulation is allowed), so a full grade-level jump requires staying in the elite tail across *many* checkpoints. A one-checkpoint fluke is mostly absorbed by the dead-band + the slow aggregate window (§8). This is the "must earn it" mechanism.

**(b) Edge compression — both ends (§6).** Multiply the move by an edge factor: gains scale by `((99 − rating)/99)^δ`, losses by `(rating/99)^δ`. → hard to gain near 99, hard to lose near 0. Reaching an elite tier in one season needs *genuinely* elite sustained play; floor players resist bottoming out. This is why a breakout from an *undeveloped* base is a great arc (lots of room) while an *already-great* player rarely goes elite→elite+ in a single year.

**(c) Seasonal soft damping (RULED: soft, not a hard cap).** Cumulative same-direction season movement adds progressive resistance (each further point in that direction is harder to earn), with a hard backstop ≈ **one grade level**. Organic deceleration — a breakout *slows* as it climbs rather than hitting a wall. **Cadence-independent:** because the *season total* is what's bounded, running 10 checkpoints vs 5 only makes the path smoother, not bigger.

**(d) Anti-ping-pong (the "not erratic" guarantee).** Three reinforcing mechanisms: **directional hysteresis** (to *reverse* an established direction the opposing signal must clear a *higher* bar than to continue — a brief slump after a hot streak doesn't claw back gains; only a *sustained* reversal does) + the **slow aggregate-base signal** (§8 — one hot/cold checkpoint barely nudges the season aggregate) + the **asymmetric dead-band** (§6). Net: meaningful arcs, no checkpoint-to-checkpoint whipsaw.
