...
26. Merge the Fenway-lens hub (`lineups-fenway-hub`; bring forward over trunk first) — NOT-BUILT, M. **(JK-gated / Lane-A-coordinated — the user-visible hub flip; NOT done by Lane B.)** · 27. Merge draft-setup-ui's 9 screens+routes+catalog (additive, 0 conflicts) — ✅ BUILT (merge `c2158eb4`; merge-tree pre-assessed clean; build exit 0; balance engine untouched). · 28. Resolve the one collision: `src/App.tsx` route union — ✅ N/A: the draft-route side merged with ZERO conflict (my lane never touched App.tsx). The #26∩#27 union resolves when #26 lands. **NOTE: `historicalArchetypes.test.ts` fails in LARGE mixed batches (cross-file state bleed from draftPipeline/FranchiseSetup) but passes SOLO and in the 8-file consumer batch — a documented characterized red, NOT a regression.** · 29. Retire `auction-draft-ux-rehaul` after #26 (confirmed superset) — S. · 30. **`ratings-finish-c` V8 park factors — already in trunk; CORRECTION: NOT flag-dark — gated by a 40%-of-season threshold, live WAR DOES change for a non-neutral stadium past 40% games. Math sound (194 WAR tests pass). → JK ruling on intended behavior.**
