...
- **➡ NEXT = RB-9c-3 (the LAST RB-9c piece): wire the intelligence — `analyzeDraftRoster` (RB-9a) → `tiltAnalyzerFindings`/`sortByTiltedPriority` (RB-9b-2) using the board team's `farmCapIdentity` (read off `leagueData.teams`) to PRIORITIZE which gaps "scream loudest", + the D-9a-1 #4/#7 salary-check-vs-wallet-cap warning ("filling this hole pushes you over"). Build in `kbl-mode1-b`.
