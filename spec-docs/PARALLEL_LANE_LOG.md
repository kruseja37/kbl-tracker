...
- **➡ NEXT = RB-9c (now UNBLOCKED — the §9 roster-visibility board + the §3.5 tilt consumer + the farm-archetype setup picker; SUBSTANTIAL → likely sub-splits).** Consumes `tiltAnalyzerFindings`/`sortByTiltedPriority` (RB-9b-2) + `analyzeDraftRoster` (RB-9a). **Folds in JK #4/#7 (D-9a-1 REVERSED): show roster + salary + running payroll + remaining wallet on the board, analyzer salary-check wired to the real wallet cap.** Build in the `kbl-mode1-b` worktree.
