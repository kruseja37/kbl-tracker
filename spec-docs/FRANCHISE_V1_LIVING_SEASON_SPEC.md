...
| Team Captain | **Highest combined Loyalty + Charisma — NO floor** (every team always has a captain) | A morale *router*, not a development modifier; **effectiveness scales** with the Loyalty+Charisma combo (§24.9) |
...
- **Selection = no floor; effectiveness scales (JK ratified 2026-06-20 — spec corrected to match code; supersedes the earlier "Charisma ≥ 70 floor").** The Captain is the **highest combined Loyalty + Charisma** MLB player on the team, with **NO Charisma floor** — **every team *always* has a captain**, even a low-Charisma roster. (The earlier "Charisma ≥ 70" floor was spec drift; the code never enforced it.) Captain **effectiveness is not binary — it *scales* with the combined Loyalty + Charisma**: a higher combo = a more effective captain at influencing player dynamics / relationships (the §24.9 leadership-effectiveness composite operationalizes this; absolute for v1, a league-relative curve is the sim-gated extension). **Verified against code:** `computeTeamCaptains` (`src/utils/franchiseInitializer.ts:227-257`) filters to MLB-on-team players with hidden modifiers, sorts by `loyalty + charisma` desc (tiebreak `charisma`, then `id`), and takes the top — no floor; `captainPlayerId` is null only when a team has zero eligible MLB players.
