...
| Team Captain | **Highest combined Loyalty + Charisma + a small five-tier age tilt — NO floor** (every team always has a captain; JK ruling 2026-07-02 added the age tilt) | A morale *router*, not a development modifier; **effectiveness scales** with the Loyalty+Charisma combo (§24.9) |
...
- **Selection = no floor; effectiveness scales (JK ratified 2026-06-20 — spec corrected to match code; supersedes the earlier "Charisma ≥ 70 floor"). AMENDED 2026-07-02 (JK ruling 6): + a small five-tier AGE tilt.** The Captain is the **highest combined Loyalty + Charisma + age tilt** MLB player on the team, with **NO Charisma floor** — **every team *always* has a captain**, even a low-Charisma roster. The age tilt (`CAPTAIN_AGE_TILT_TIERS`) is a monotonic seniority curve — ≤22: −6 · 23-26: −2 · 27-30: 0 · 31-34: +4 · 35+: +6 — spanning 12 points against the 0-200 loyalty+charisma scale, so it breaks near-ties toward the veteran without overriding a clear leadership gap; loyalty + charisma remain the primary drivers. Captain **effectiveness is not binary — it *scales* with the combined Loyalty + Charisma**: a higher combo = a more effective captain at influencing player dynamics / relationships (the §24.9 leadership-effectiveness composite operationalizes this; absolute for v1, a league-relative curve is the sim-gated extension). **Verified against code (2026-07-02):** `computeTeamCaptains` (`src/utils/franchiseInitializer.ts`) filters to MLB-on-team players with hidden modifiers, sorts by `loyalty + charisma + captainAgeTilt(age)` desc (tiebreak `charisma`, then `id`), and takes the top — no floor; `captainPlayerId` is null only when a team has zero eligible MLB players.
...
- **Trigger (L14-Q1, ruling pass 2026-06-19):** sustained bottomed-out fan morale, measured by a **NEW dwell counter** — consecutive games at-or-below a rock-bottom fan-morale band (the existing `trendStreak` is "same-direction changes," the wrong primitive; band + duration are §16 sim-tuned). **GM-GATED:** once the dwell condition arms it, the rebrand is **OFFERED to the GM as an action, not auto-forced.**
- **RESETS on relocation — the atomic cascade, in ORDER (L14-Q4, ruling pass 2026-06-19):**
  1. **Manager auto-fired + the player morale ripple applied** — via the ONE shared L11 resolver invoked as `fireManager({reason:'rebrand', skipUserConfirm:true, suppressFanReliefBump:true})`. The fan-RELIEF bump is **suppressed** (the ~70 reset in step 6 is the authoritative fan outcome) but the per-player ripple still lands (L11-Q11).
  2. **Badges cleared + Fan Hopeful re-seeded (L14-Q2):** clear the four team/fanbase badges (Team MVP, Ace, Albatross, Fan Favorite) AND the old Fan Hopeful, then **re-seed a NEW Fan Hopeful chosen by the new fanbase** (the random-top-3-prospects mechanic). **Exempt Captain only** (it travels with the *player's* leadership). Because stats persist, the value-gated badges re-assign at the next designation checkpoint.
  3. **Stadium relocates** (user-picked — see Stadium dual-path below).
  4. **Roster fame resets team-wide (L14-Q3):** the whole roster takes the **trade-style fame reset** (Heat cools + reach floor drops toward Unknown). Rebrand is a **SECOND fame-reset valve alongside trade** — amends FAME-7 / §20.3.
  5. **Dead money cleared (L14-Q6):** a **STUBBED** `clearCarriedDeadMoney(teamId)` hook in L14; the real carried-balance ledger + wipe are built by the **economy track** (in v1 per LSD-6), a documented no-op until then.
  6. **Fan morale hard-set LAST** to `REBRAND_RESET_MORALE` (~70, a fresh content fanbase; a sim-tunable named constant, L14-Q7) so nothing overwrites it.
- **PERSISTS (because it is mid-season):** player stats, team record, player development to date. (Wiping these mid-season would feel like a save-wipe; the rebrand is a fresh start with *earned progress intact*.) *(Note: player FAME does NOT persist — it resets team-wide per step 4 above, L14-Q3.)*
- **History + new identity (L14-Q5, ruling pass 2026-06-19):** **one continuous franchise history with a relocation marker** (do NOT split into eras — less work, and the better story). The **USER enters the new team name/city**; persist a relocation record `{formerTeamName, formerStadiumName, relocatedAtSeason, relocatedAtGame}` appended to a NEW `teamHistory` array on the franchise-team metadata (no such field today) so the Almanac renders "the franchise formerly known as the X, who relocated and became the Y." Colors optional/deferred.
...
- **Trade and rebrand are the reset valves:** a trade **drops the reach floor and pulls Heat toward Unknown** — a traded star keeps *some* Heat ("reputation precedes him," not nuked) but loses the *earned floor*, so a flop in the new city can now fade to obscurity. This is the "rebuild your fame with a new fanbase" feeling, made mechanical. **A REBRAND (§14) applies this SAME reset to the ENTIRE roster** — relocation is a team-wide "new fanbase" fame reset (L14-Q3, ruling pass 2026-06-19; supersedes the original "trade is the *only* reset valve").
...
> **Ownership (L12-Q8, ruling pass 2026-06-19):** the WHOLE §20.4 status layer (draft seed, call-up, send-down, bench multiplier, league-leader) is owned by the **fame layer (L6, already built dark)** — an L6 follow-up wires it; **L12 only CONSUMES the resulting tier.** (Supersedes the earlier "league-leader→L12" split.)
...
| FAME-7 | Trade **and rebrand** are the reset valves: drops the reach floor + pulls Heat toward Unknown (star keeps some Heat, loses earned floor); every trade dilutes, never raises (headliner-raises deferred v2). **Rebrand (§14) applies the same reset team-wide** (L14-Q3, ruling pass 2026-06-19 — amends the original "trade is the only reset"). |
