...
> **➡ START HERE (Branch A): RA-2c-2 — the WIRING (live-compute, display-dark; ALL 4 forks RULED 2026-06-24, NO new forks; SMB4-asset gate APPROVED).** Wire `src/utils/franchiseCheckpointSweepCompute.ts`: (a) in `resolveCheckpointRoster` ALSO fetch `getSeasonBattingStats`/`getSeasonPitchingStats`/`getAllFieldingStats(scope.seasonId)` (index by playerId) + `buildFranchiseEffectivePositionReport({franchiseId,seasonId,statsScopeId,players})`; (b) per MLB player build a `CheckpointSignalMember` from `checkpointRatingSignal.ts` (role, ageBand any/inert, ratings=baseRatings, `poolKey=classifyRatingsPoolKey({role, effectivePosition: report.playerPositions[id]?.effectivePosition ?? primaryPosition, startsShare: report…startsShare})` [skip player if null], `categoryRates=toExpectedStatsCategoryRates({role, batting/pitching/fielding rows})`); (c) call `computeCheckpointRatingSignals(members)`; (d) change `CheckpointRosterEntry.performanceSignal:number` → `signalByRatingKey: Partial<Record<ExpectedStatsRatingKey,number>>`. **BYPASS `normalizePerformanceSignal`** (÷200000 crushes the already −1..1 signal). **DELETE `selectDevelopmentRatingKey`**; fan out the persist loop (`:246-282`) — for each ratingKey with a finite signal, call `computeCheckpointRatingDevelopment({ratingKey, baseRatingValue: entry.baseRatings[ratingKey], performanceSignal: signal, playerMorale, teamFanMorale, personality, modifiers})`, emit ONE overlay per MOVED rating. **KEEP EXACTLY:** the id formula `:267` (already keys `ratingKey` → collision-free), `confirmationStatus:'pending'` `:276`, the flag guard `:206`. Update `src/utils/tests/franchiseCheckpointSweepCompute.test.ts` (cardinality 1→N-per-moved-rating + the new signal source). **NO trackerDb bump** (reuse `franchiseRatingsOverlays`; only row CARDINALITY changes 1→N). **8 TRAPS (recon `wf_742c834c-7d9`):** bypass-normalize · delete-selectDevelopmentRatingKey · cardinality-test-update · hysteresis-OFF=pass-NO-priorRole · ageBand-inert · pitcher-arm null→skip in blend · empty/thin-cohort no-throw · id-formula-exact. Suggest split-safe: RA-2c-1 already pure+green, so RA-2c-2 is mechanical wiring. **RA-2c-1 DONE (`9ae54ef3`)**: pure `checkpointRatingSignal.ts` (classifyRatingsPoolKey + CHECKPOINT_POOL_LADDER + resolvePoolMeanMembers/resolveSpreadMembers + computeCheckpointRatingSignals; anchor-identity green); build-dark, zero-new-reds (8009/2). **RA-2b (`622cc97d`) + RA-2CQ stack (1/2a/2b/2c: `d97504dd`/`90f134f1`/`3291415c`/`0ff7e88c`) DONE.** **A2.3 RA-rookie DONE** (`738624fa`; RA-5 modifier consumer + the Branch-B auction-path `draftedAsFarmProspect` stamp are follow-ups). **A1.5d** = stadium records (still pending; own DB `kbl-franchise-stadium-records`, new flag). **⚠ A1.5b-2 (DEFERRED OPEN-DECISION): re-derive the EnrichmentPanel SVG markers from `polarToXY` so a real "tap on the fence" → r≈1.0 — a live UI change needing browser sign-off + a PRECONDITION before the carry converter is wired live (A1.5d/RA-2).** Then A-W2 tail (A2.5 §6A) + A-W1 tail (A1.3a, L12-6, L4b) + A-W3 traits. Then the gate chain (lane-merge → L-SIM final → RB-16 → D12 → D13 → flag-flip → F-141).
