...
> ## 🔵 RIGHT NOW (session end 2026-06-23, attended) — HANDING OFF TO AUTH-4 UNATTENDED RUN
...
> **➡ START HERE (the fresh AUTH-4 session): the new `A-W1.5` wave on Branch A, in HARD ORDER.** (1) **A1.5a FAME-FLUCTUATION FIX** — contract `A1.5a-FAME-FLUCTUATION-FIX` is AUTHORED + ready in `PROMPT_CONTRACTS.md`; dispatch it FIRST (fame must fluctuate up/down; honors flat-pin REGIONAL_STAR; **the diff MUST update the L-SIM upward-only soul-invariant + re-run L-SIM**). Then (2) **A1.5b carry converter**, (3) **A1.5c 4 aggregators**, (4) **A1.5d stadium records** — skeletons in PROMPT_CONTRACTS.md, Captain authors fully from `STADIUM_ANALYTICS_SPEC_V2.md` + `RATINGS_MEASUREMENT_WORKSHEET.md` (all forks pre-resolved). Then the existing **A-W2 ratings** (contact-quality data layer → RA-2b → RA-2c; RA-8; RA-rookie; A2.5 §6A) + **A-W1 tail** (A1.3a only, L12-6, L4b) + **A-W3 traits** + **Branch B** (concurrent). Then the **gate chain** (lane-merge → L-SIM final → RB-16 → D12 → D13 → flag-flip → F-141).
> **Branch topology:** Branch A = `codex/franchise-v1-next` (worktree `/Users/johnkruse/Projects/kbl-tracker`, current at `c39818b7`) · Branch B = `codex/mode1-v1-b` (`/Users/johnkruse/Projects/kbl-mode1-b`). ONE committer per branch. Dispatch: `sed -n '/START CONTRACT: <id>/,/END CONTRACT: <id>/p' …/PROMPT_CONTRACTS.md | NODE_ENV= codex exec --dangerously-bypass-approvals-and-sandbox -` (run_in_background); Opus audits the REAL diff (builder≠auditor) + independently `NODE_ENV= npx tsc -b` + FULL suite, ZERO-NEW-REDS (characterized: `wpaRuntimeBoundary` + `franchiseManualSmokeFixture` + `EliminationTeamHub.test.tsx` solo-flakes), branch-only, never push.
