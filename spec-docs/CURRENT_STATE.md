...
> **➡ START HERE (Branch A): RA-2c-2b-2 — the two REMAINING GATES (Gate 3 window-eligibility + confidence-weighting) + a deterministic roster-agnostic `createdAt` for window-active no-TV members + the §9/§3B PURGE-ON-SUPERSEDE spec rewrite. RA-2c-2b was SPLIT for clean audit; RA-2c-2b-1 ✅ DONE (`c10ec91b`, this session) = assemble the signal-bearing roster (relax all 3 guards → roster-agnostic POOL membership · new age→band mapper · `classifyRatingsPoolKey` · `computeCheckpointRatingSignals` ONCE over the full member set) + swap `performanceSignal`→`signalByRatingKey` + per-rating fan-out 1→N + delete `selectDevelopmentRatingKey`/`stableHash` + bypass `normalizePerformanceSignal`; importActual test-mock fix; gate = build 0 / affected 14/14 / full suite 2 fail (both characterized) = ZERO-NEW-REDS. RA-2c-1a + RA-2c-2a engine layer DONE. Full qualifier ruling = DECISIONS_LOG 2026-06-24 "RA-2c-2 qualifier model" + "RA-2 peer-pool, eligibility & signal model" (Gate-3 verbatim = window-active ≥1 PA/≥1 BF since last checkpoint; window-inactive STAYS a pool member, isn't moved).**
