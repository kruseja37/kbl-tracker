...
> **➡ START HERE (Branch A): RA-2c — LIVE-wire the checkpoint sweep with the REFINED model (full ruling: DECISIONS_LOG 2026-06-24 "RA-2 ratings-adjustment — peer-pool, eligibility & signal model"; display-dark behind the triple gate, SMB4-asset gate APPROVED). TWO STEPS:**
> **(1) RA-2c-1a REVISION (small, FIRST — fixes a §4:65 conflict in the committed `9ae54ef3`):** in `src/utils/checkpointRatingSignal.ts`, the MEAN must stay **position-pure** (the position-group = Rung 0); the SPREAD borrows Rung 1; and when even the position-group mean pool is below ~5-6 real peers, **SUPPRESS that category's signal** (return no signal) instead of widening the mean to all-hitters. (`resolvePoolMeanMembers` currently widens the mean below floor — the bug JK's questions caught.)
> **(2) RA-2c-2 WIRING of `src/utils/franchiseCheckpointSweepCompute.ts`:** build the **ROSTER-AGNOSTIC, WINDOW-QUALIFIED** peer pool — members for `computeCheckpointRatingSignals` = ALL players across the 32-man rosters (MLB+farm) who QUALIFY by the sample gate, **NOT MLB-roster-only** (relax the `:139` MLB filter for POOL membership). **Move eligibility = window-active** (qualified since last checkpoint → no stale re-hit). **SIGNAL = CUMULATIVE** season-to-date (RETAINED, NOT window-only — cumulative makes a bad start FOLLOW the player = anti-gaming) **+ confidence-weighting** (scale move by accumulated/full-season sample). Recent-form **tilt = §16 follow-on (defer).** Then the mechanical wiring: fetch `getSeasonBattingStats`/`getSeasonPitchingStats`/`getAllFieldingStats(scope.seasonId)` + `buildFranchiseEffectivePositionReport(...)`; build `CheckpointSignalMember`s (`classifyRatingsPoolKey({role, effectivePosition ?? primaryPosition, startsShare})`); `computeCheckpointRatingSignals`; change `CheckpointRosterEntry.performanceSignal:number`→`signalByRatingKey`; **BYPASS `normalizePerformanceSignal`** (÷200000 crushes the −1..1 signal); **DELETE `selectDevelopmentRatingKey`**; fan out the persist loop (ONE overlay per MOVED rating); **KEEP EXACTLY** the id formula `:267` + `pending` `:276` + flag guard `:206`; **NO trackerDb bump** (reuse `franchiseRatingsOverlays`, cardinality 1→N); update `franchiseCheckpointSweepCompute.test.ts`. **NEW PIECE = window-stat/eligibility compute** (window = now − last-checkpoint snapshot via extending `franchiseTrueValueSnapshots`, OR per-game aggregate of completed games — decide at build; per-game = no new persistence). **PITCHER-SYMMETRIC** (window qualifier = IP/BF, peer groups SP/RP). **PURGE-ON-SUPERSEDE: rewrite RATINGS_ADJUSTMENT_SPEC §9** (MLB-roster-only eligibility → roster-agnostic window-qualified) IN this build. **8 traps (recon `wf_742c834c-7d9`):** bypass-normalize · delete-selectDevelopmentRatingKey · cardinality-test · hysteresis-OFF · ageBand-inert · pitcher-arm null→skip · empty/thin no-throw · id-formula-exact. **RA-2c-1 DONE (`9ae54ef3`, NEEDS the 1a revision above)**: pure `checkpointRatingSignal.ts`; build-dark, 8009/2. **RA-2b (`622cc97d`) + RA-2CQ stack (1/2a/2b/2c: `d97504dd`/`90f134f1`/`3291415c`/`0ff7e88c`) DONE.** **A2.3 RA-rookie DONE** (`738624fa`; RA-5 modifier consumer + the Branch-B auction-path `draftedAsFarmProspect` stamp are follow-ups). **A1.5d** = stadium records (still pending; own DB `kbl-franchise-stadium-records`, new flag). **⚠ A1.5b-2 (DEFERRED OPEN-DECISION): re-derive the EnrichmentPanel SVG markers from `polarToXY` so a real "tap on the fence" → r≈1.0 — a live UI change needing browser sign-off + a PRECONDITION before the carry converter is wired live (A1.5d/RA-2).** Then A-W2 tail (A2.5 §6A) + A-W1 tail (A1.3a, L12-6, L4b) + A-W3 traits. Then the gate chain (lane-merge → L-SIM final → RB-16 → D12 → D13 → flag-flip → F-141).
...
> **Branch topology:** Branch A = `codex/franchise-v1-next` (worktree `/Users/johnkruse/Projects/kbl-tracker`, HEAD `3291415c`) · Branch B = `codex/mode1-v1-b` (`/Users/johnkruse/Projects/kbl-mode1-b`, HEAD `67f3fff6`, PARKED). ONE committer per branch. Dispatch: `sed -n '/CONTRACT: <id>/,/END CONTRACT: <id>/p' …/PROMPT_CONTRACTS.md | NODE_ENV= ~/.local/bin/codex exec -C <worktree> --skip-git-repo-check -s workspace-write -c model_reasoning_effort=high -o <out> -` (run_in_background); Opus audits the REAL diff (builder≠auditor) + independently `tsc -b`/build + suite, ZERO-NEW-REDS (characterized: Branch A `wpaRuntimeBoundary`[hard] + `franchiseManualSmokeFixture` + `GameTrackerLaunchState` solo-flakes; Branch B `wpaRuntimeBoundary` only), branch-only, never push.
> **⚠ BRANCH HYGIENE (resolve before the lane-merge):** the eventLog box-score-undercount CODE fix (ITPHR/GRD-as-hits, Ꝁ/D3K-as-Ks) is committed `875e4368` on its OWN branch `fix/eventlog-boxscore-undercount` — NOT yet on franchise-v1-next; needs independent gating + merge (the 3 design-doc commits on that branch are now duplicated on franchise-v1-next via cherry-pick — harmless). **+ ~28 stray `codex/*` branches carry unmerged commits** (gt-*, manager-*, wpa-*, optimal-lineup-*, etc.) — **audit them for v1-relevant work before the lane-merge** (JK fork #9).
