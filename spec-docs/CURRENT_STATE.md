...
> **➡ START HERE (Branch A): RA-2c-2b — the LIVE WIRING (display-dark behind the triple gate, SMB4-asset gate APPROVED). RA-2c-1a + RA-2c-2a ✅ DONE this session; the engine layer + qualifier model are complete. Full qualifier ruling = DECISIONS_LOG 2026-06-24 "RA-2c-2 qualifier model — the three sample gates + flat floors" + "RA-2 peer-pool, eligibility & signal model". Recon = `wf_a93cad4a-288` (verdict SOUND; corrected anchors below).**
