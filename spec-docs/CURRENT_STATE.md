...
> **➡ START HERE (Branch A): A2.4 RA-2b (pure pool aggregator).** **RA-2CQ stack COMPLETE through the SIGNAL layer** — RA-2CQ-1/2a/2b/2c DONE: pure classifier (`d97504dd`) + season count fields (`90f134f1`: `contactQualityGood/Tracked`, `weakContactInduced/Tracked`) + LIVE writer (`3291415c`, swallow-guarded) + **RA-2CQ-2c SIGNAL layer (`0ff7e88c`)**: new `contactQualityRate` category (`basis:'none'`→fixed floor 10, JK-ruled 'count early' 2026-06-24) + un-dorm `pitchingWeakContactRate` (basis 'combined' unchanged) + the RA-2a adapter `expectedStatsCategoryRates.ts` emits both rates from the season counts (sample=tracked); build-dark (grep: 0 live consumers), zero-new-reds (7999/2), no DB bump. **A2.3 RA-rookie DONE** (`738624fa`; RA-5 modifier consumer + the Branch-B auction-path `draftedAsFarmProspect` stamp are follow-ups). **A1.5d** = stadium records (still pending; own DB `kbl-franchise-stadium-records`, new flag). **⚠ A1.5b-2 (DEFERRED OPEN-DECISION): re-derive the EnrichmentPanel SVG markers from `polarToXY` so a real "tap on the fence" → r≈1.0 — a live UI change needing browser sign-off + a PRECONDITION before the carry converter is wired live (A1.5d/RA-2).** Then A-W2 tail (A2.5 §6A) + A-W1 tail (A1.3a, L12-6, L4b) + A-W3 traits. Then the gate chain (lane-merge → L-SIM final → RB-16 → D12 → D13 → flag-flip → F-141).
