...
> ## 🔵 RIGHT NOW (attended Hybrid via `/kbl-captain`, 2026-06-23) — A-W1.5: A1.5a/b + **A1.5c-1/2/3 ✅ COMMITTED** (`6b7879d7` difficulty-fielding · `8bf12bec` UBR · `e66a5399` extraBasesAllowed); **A1.5c CLOSED** (-4 deferred to RA-8). Branch B parallel: **S6 ✅** (`c545abac`). **RA-8 ✅** (`0edf060a` fields) + **A1.5c-4 ✅** (`f16cbfd3` catcher-CS rate + live writer; k=0.45 ruled) ⇒ **🎉 A1.5c FULLY CLOSED (1/2/3/4)**. → **next (Branch A) = A1.5d stadium records OR A-W2 ratings (A2.3 RA-rookie → A2.4 RA-2b) · (Branch B) = S7a + the 4 S7 forks**
> **Phase:** a long attended DESIGN session just closed; JK is starting a FRESH session to run the **AUTH-4 unattended build loop** (`/kbl-captain`; `AUTONOMOUS_RUN_PROTOCOL.md` is the standing go). **Backlog = `spec-docs/V1_BUILD_QUEUE.md`** (read first). The session produced a big SMB4-native ratings + stadium-records + fame design body (all decision-complete) and a new build wave.
> **➡ START HERE (Branch A): A1.5d stadium records OR A-W2 ratings (A2.3 RA-rookie → A2.4 RA-2b contact-quality).** **🎉 A1.5c FULLY CLOSED** — RA-8 `0edf060a` (CS/SB fields) + A1.5c-4 `f16cbfd3` (catcher-CS rate `(CS×0.95)/((CS×0.95)+(SB_allowed×0.45))` k=0.45 + the LIVE writer that populates the RA-8 fields from `BETWEEN_PLAY_EVENTS` by stamped `runnerAttribution.catcherId`, swallow-guarded). The catcher CS/SB-allowed substrate now accrues live; the rate fn + the other 3 A1.5c aggregators stay build-dark until the RA-2 ratings wiring. **A1.5d** = stadium records (gated on A1.5a+A1.5b+WPA archive; big — own DB `kbl-franchise-stadium-records`, 6 living-season hops, new `isFranchisePhase2StadiumRecordsEnabled` flag). **⚠ A1.5b-2 (DEFERRED OPEN-DECISION): re-derive the EnrichmentPanel SVG markers from `polarToXY` so a real "tap on the fence" → r≈1.0 — a live UI change needing browser sign-off + a PRECONDITION before the carry converter is wired live (A1.5d/RA-2).** Then the rest of **A-W2 ratings** (contact-quality → RA-2b → RA-2c; RA-rookie; A2.5 §6A) + **A-W1 tail** (A1.3a, L12-6, L4b) + **A-W3 traits**. Then the **gate chain** (lane-merge → L-SIM final → RB-16 → D12 → D13 → flag-flip → F-141). **⚠ A1.5b-2 (DEFERRED OPEN-DECISION): re-derive the EnrichmentPanel SVG markers from `polarToXY` so a real "tap on the fence" → r≈1.0 — a live UI change needing browser sign-off + a PRECONDITION before the carry converter is wired live (A1.5d/RA-2).** Then the rest of **A-W2 ratings** (contact-quality → RA-2b → RA-2c; RA-rookie; A2.5 §6A) + **A-W1 tail** (A1.3a, L12-6, L4b) + **A-W3 traits**. Then the **gate chain** (lane-merge → L-SIM final → RB-16 → D12 → D13 → flag-flip → F-141).
> **➡ BRANCH B (parallel, fan-out per JK): S6 ✅ (`c545abac`) — draft-board per-tool+grade bands, default-covered/long-press, no raw-ratings leak. **S7a ✅** (`d1a578ab` — pure `gradeBandToPriceRange`, midpoint range off the exported canonical `GRADE_SALARY_BOUNDS`, build-dark, zero-new-reds). **➡ NEXT = S7b** (re-anchor `scoutRangeForProspect` `LeagueBuilderFarmAuctionDraft.tsx:103-124` → `gradeBandToPriceRange(overallGradeBand)` × chemFit on both endpoints; band IS the range, DROP `perceivedValueRange` from this path; rewrite the keystone test `:262-342`) → S7c (won-bid → farm `Player.salary`, ALL real winners) → S7d (supersede/DELETE old Gaussian — **KEEP `perceivedValueRange`/`scoutValueRange.ts`**, relocate `gradeToTwentyEighty`). **ALL S7 FORKS RULED** (DECISIONS_LOG `6a16c7ea`: range=midpoint · guidance=band-is-range · salary=all-real-winners · keep-perceivedValueRange/relocate-2080). Sub-split detail in `BRANCH_B_PROGRESS.md` (`bcbb74fe`). Then RB-13b · RB-18 · B1.6. (Prior Branch-B this session: B6+S1–S5.) Detail in `BRANCH_B_PROGRESS.md` on `codex/mode1-v1-b`.
> **Branch topology:** Branch A = `codex/franchise-v1-next` (worktree `/Users/johnkruse/Projects/kbl-tracker`, HEAD `e6b43ecb`) · Branch B = `codex/mode1-v1-b` (`/Users/johnkruse/Projects/kbl-mode1-b`, HEAD `c545abac`). ONE committer per branch. Dispatch: `sed -n '/START CONTRACT: <id>/,/END CONTRACT: <id>/p' …/PROMPT_CONTRACTS.md | NODE_ENV= codex exec --dangerously-bypass-approvals-and-sandbox -` (run_in_background); Opus audits the REAL diff (builder≠auditor) + independently `NODE_ENV= npx tsc -b` + FULL suite, ZERO-NEW-REDS (characterized: `wpaRuntimeBoundary` + `franchiseManualSmokeFixture` + `EliminationTeamHub.test.tsx` solo-flakes), branch-only, never push.
