...
> **➡ NEXT (resume here — pipeline this attended session, Branch A serialized):** (1) **L13-8** — un-hold the authored contract (`PROMPT_CONTRACTS.md`), flag-gated `processCompletedGame` wiring → L13 fully built-dark ⚠ **re-verify its `processCompletedGame.ts:616-680` anchors first** (drifted since 2026-06-19: L12-3b/4/5 + RA-1 + DH-seal landed after). (2) **A1.3** trade-request wiring (cheap: `tradeRequestGeneration.ts:77` → `franchiseL10EventEngine.ts:~334`). (3) **A1.2 leg-b/leg-c — NOW RULED** (DECISIONS_LOG 2026-06-23): leg-b = §20.5 fame→player-morale tap **CHANGE-ONLY** (per-game `heatDelta`; standing-level drag deferred v1.1 — spec-annotate §20.5 at build); leg-c = §20.6 fan-morale **CHANNELS A+B** (A = fame-volume on the game's standout × the built designation tilt, per-game `createGameMoraleEvent`; B = held-Fan-Favorite steady warmth, Albatross owned by §13). Then **RA-2** (wire RA-1 live + the sim-tune curvature check) + Lane-4 tail (T/S/B13/B14) + Lane-1 tail (L12-6, L4b) + the gate chain. **G1 follow-up:** assert no fame/designation/award reader consumes a `'draft-baseline'` snapshot row.
