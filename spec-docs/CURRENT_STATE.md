...
> ## 🛑 MERGE FREEZE (2026-06-24, JK) — **finish your current ticket + commit to a clean gate-green seam, then HOLD. Do NOT start NEW work on `codex/franchise-v1-next` or `codex/mode1-v1-b`.** The **LANE-MERGE** (`codex/mode1-v1-b` → `codex/franchise-v1-next`) is being prepped — **Captain (Opus) owns it** (DECISIONS_LOG 2026-06-24), run in a **DEDICATED session on a SIDE branch with JK**. Both branches must HOLD STILL for the merge — that's the only safe way to staple the two halves together; two committers on one branch is the collision risk. **Resume all building on the COMBINED tree after the merge lands.** *If you ARE the merge session:* confirm both branches are frozen + gate-green, then proceed per the protocol (assess conflicts → side branch → full gate → JK confirm → fast-forward franchise-v1-next; branch-only, never push). *(Design-only threads — e.g. the grade letters→numbers thread — are fine; they don't commit code to either branch.)*
