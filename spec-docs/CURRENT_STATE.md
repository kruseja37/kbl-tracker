...
> ## 🔵 RIGHT NOW (AUTH-4 unattended run ACTIVE, 2026-06-23) — A-W1.5 wave: A1.5a+A1.5b ✅ COMMITTED (`0cc319a0`,`f3b48fbf`); A1.5c GROUNDED+SPLIT (4 sub-tickets) → next = dispatch A1.5c-1
...
> **➡ START HERE: dispatch A1.5c-1 (then -2, -3).** A1.5c is GROUNDED + SPLIT into 4 sub-tickets; the FULL grounding (per-aggregator source/formula/build-dark-posture + the field-leak resolution + 8 forks' documented defaults + the A1.5c-4⇔RA-8 reconciliation) is captured in the **`AUTONOMOUS_RUN_LOG.md` A1.5c entry** — author each contract from it + dispatch. **A1.5c-1** = *ByPosition difficulty-weighted fielding (cleanest; field-leak watch lives here; reuse `mapPersistedSpecialPlayType`; read STAMPED `FieldingEvent.playerId/position`; build+affected-test pure fn). **A1.5c-2** = UBR (compute-only, build-dark). **A1.5c-3** = extraBasesAllowed (derive-on-read). **A1.5c-4** = catcher-CS — **DEFERRED until RA-8/A2.2 owns the `caughtStealingAgainst`/`stolenBasesAllowed` fields.** All build-dark pure fns, file-disjoint/parallelizable. ✅ **A1.5b CARRY CONVERTER DONE** (`f3b48fbf`) — pure build-dark `src/engines/carryConverter.ts` (anisotropic inverse transform, polar→feet, air-balls-only, HR distance user-entered never computed, min(r,1.0) wall cap); field-leak gate encoded as landmark tests incl. a documented-drift test pinning the drawn-CF-fence r≈0.909 mismatch; gate tsc 0 / 9-9 / zoneCQSpray 45-45 / oracle clean. **⚠ A1.5b-2 (DEFERRED OPEN-DECISION): re-derive the EnrichmentPanel SVG markers from `polarToXY` so a real "tap on the fence" → r≈1.0 — a live UI change needing browser sign-off + a PRECONDITION before the converter is wired live (A1.5d/RA-2).** ✅ **A1.5a FAME-FLUCTUATION FIX DONE** (`0cc319a0`) — per-game reachFloor ratchet removed (fame fluctuates up/down); the honor path flat-pins `reachFloor = max(existing, REGIONAL_STAR)`; L-SIM `soul.fame-reach-monotonic` rewritten honor-aware (honored ≥ REGIONAL_STAR via locked All-Star + finalized MVP/CY; non-honored free to fall) + `reach-floor-ratchet` strengthened + falsification count stays 26 + canonical baseline regen (findings 0, sameSeedByteIdentical; FIRST regen since L13-6 → also folded in A1.2's fame→morale ripple, moraleSnapshots 47→91); full-suite zero-new-reds; oracle byte-unchanged; `applyWarLegitimacyGravity` untouched. **➡ NEXT = (2) **A1.5b carry converter**, (3) **A1.5c 4 aggregators**, (4) **A1.5d stadium records** — skeletons in PROMPT_CONTRACTS.md, Captain authors fully from `STADIUM_ANALYTICS_SPEC_V2.md` + `RATINGS_MEASUREMENT_WORKSHEET.md` (all forks pre-resolved). Then the existing **A-W2 ratings** (contact-quality data layer → RA-2b → RA-2c; RA-8; RA-rookie; A2.5 §6A) + **A-W1 tail** (A1.3a only, L12-6, L4b) + **A-W3 traits** + **Branch B** (concurrent). Then the **gate chain** (lane-merge → L-SIM final → RB-16 → D12 → D13 → flag-flip → F-141).
> **Branch topology:** Branch A = `codex/franchise-v1-next` (worktree `/Users/johnkruse/Projects/kbl-tracker`, HEAD `f3b48fbf`) · Branch B = `codex/mode1-v1-b` (`/Users/johnkruse/Projects/kbl-mode1-b`). ONE committer per branch. Dispatch: `sed -n '/START CONTRACT: <id>/,/END CONTRACT: <id>/p' …/PROMPT_CONTRACTS.md | NODE_ENV= codex exec --dangerously-bypass-approvals-and-sandbox -` (run_in_background); Opus audits the REAL diff (builder≠auditor) + independently `NODE_ENV= npx tsc -b` + FULL suite, ZERO-NEW-REDS (characterized: `wpaRuntimeBoundary` + `franchiseManualSmokeFixture` + `EliminationTeamHub.test.tsx` solo-flakes), branch-only, never push.
