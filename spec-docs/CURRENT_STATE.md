...
> **➡ START HERE (Branch A): RA-2c — LIVE-wire RA-1/RA-2a/RA-2b/CQ into the checkpoint sweep (replace `valueDelta` @ `franchiseCheckpointSweepCompute.ts:184`) + the weighted per-category→per-attribute fan-out (§16 default-equal weights); in-place/no-flag, no DB bump. ⚠ FORK-HEAVY + first NON-dark RA behavior change — ground + batch to JK the 4 deferred RA-2c forks: (1) hitter start-share NUMERATOR (batting row has none — needs fielding/roster) · (2) hitter POSITION source for pool assignment · (3) hysteresis-state PERSISTENCE (saved prior-cohort vs the no-DB-bump ruling) · (4) §4:85 fallback-ladder MEMBERSHIP composition.** **RA-2b pool aggregator DONE (`622cc97d`)**: pure `expectedStatsPoolAggregator.ts` (`aggregatePoolStats` position-pure mean + winsorized-SD borrow + per-cat count + `classifyStarterRole` 55/45 hysteresis + `isPeerPoolBelowFloor` + `RatingsPoolKey` decoupled from the TV pool); build-dark, zero-new-reds (8004/2), no DB bump. **RA-2CQ stack COMPLETE** (1/2a/2b/2c: classifier `d97504dd` + count fields `90f134f1` + writer `3291415c` + SIGNAL layer `0ff7e88c`). **A2.3 RA-rookie DONE** (`738624fa`; RA-5 modifier consumer + the Branch-B auction-path `draftedAsFarmProspect` stamp are follow-ups). **A1.5d** = stadium records (still pending; own DB `kbl-franchise-stadium-records`, new flag). **⚠ A1.5b-2 (DEFERRED OPEN-DECISION): re-derive the EnrichmentPanel SVG markers from `polarToXY` so a real "tap on the fence" → r≈1.0 — a live UI change needing browser sign-off + a PRECONDITION before the carry converter is wired live (A1.5d/RA-2).** Then A-W2 tail (A2.5 §6A) + A-W1 tail (A1.3a, L12-6, L4b) + A-W3 traits. Then the gate chain (lane-merge → L-SIM final → RB-16 → D12 → D13 → flag-flip → F-141).
