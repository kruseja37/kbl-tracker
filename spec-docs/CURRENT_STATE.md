...
> ## 🔵 RIGHT NOW (attended Hybrid via `/kbl-captain`, 2026-06-23) — A-W1.5: A1.5a/b + **A1.5c-1/2/3 ✅ COMMITTED** (`6b7879d7` difficulty-fielding · `8bf12bec` UBR · `e66a5399` extraBasesAllowed); **A1.5c CLOSED** (-4 deferred to RA-8). Branch B parallel: **S6 ✅** (`c545abac`). **RA-8 ✅ COMMITTED** (`0edf060a` — catcher CS/SB-allowed fields on `PlayerSeasonFielding`, additive build-dark, zero-new-reds). → **next = A1.5c-4 (catcher-CS rate, now unblocked) ∥ Branch B S7 (chemFit ruling captured)**
...
> **➡ START HERE: A1.5c-4 (catcher-CS rate — now UNBLOCKED by RA-8).** RA-8 ✅ (`0edf060a`) added the additive optional `caughtStealingAgainst?`+`stolenBasesAllowed?` to `PlayerSeasonFielding` + seed 0, BUILD-DARK (no writer), NO DB bump, zero-new-reds. **Grounding correction (use these paths):** `kblWpaAttribution.ts` is in `src/utils/` (NOT engines); the catcher SB/CS shares are `kblWpaAttribution.ts:1354-1378` (CS catcher 0.95 / SB catcher 0.45); the `seasonStorage.test.ts:82` "mirror" is a DECOUPLED phantom (nested-positions local interface, no prod import) — leave it. **A1.5c-4** = the catcher-CS discounted RATE `= (CS×0.95)/((CS×0.95)+(SB_allowed×0.45))`, NEVER raw CS% — AND it must define the per-game WRITER (build `caughtStealingAgainst`/`stolenBasesAllowed` from `BETWEEN_PLAY_EVENTS` grouped by stamped `runnerAttribution.catcherId`, exclude `undoneAt!=null`, bucket `catcherId===''` as unknown) since RA-8's fields are 0 until then. *(A1.5c-1/2/3 DONE earlier: difficulty-fielding `6b7879d7` · UBR `8bf12bec` · extraBasesAllowed `e66a5399`.)* Then **A1.5d stadium records** (gated on A1.5a+A1.5b+WPA archive). **⚠ A1.5b-2 (DEFERRED OPEN-DECISION): re-derive the EnrichmentPanel SVG markers from `polarToXY` so a real "tap on the fence" → r≈1.0 — a live UI change needing browser sign-off + a PRECONDITION before the carry converter is wired live (A1.5d/RA-2).** Then the rest of **A-W2 ratings** (contact-quality → RA-2b → RA-2c; RA-rookie; A2.5 §6A) + **A-W1 tail** (A1.3a, L12-6, L4b) + **A-W3 traits**. Then the **gate chain** (lane-merge → L-SIM final → RB-16 → D12 → D13 → flag-flip → F-141).
> **➡ BRANCH B (parallel, fan-out per JK): S6 ✅ (`c545abac`) — draft-board per-tool+grade bands, default-covered/long-press, no raw-ratings leak. NEXT = S7** (supersede + dead-code cleanup + the S4-pt2 price re-anchor — LAST on the scouting lane; **the parked decision is RULED: guidance = GRADE-BAND + chemFit nudge** [DECISIONS_LOG 2026-06-23] — grade-band → price range replaces true-IV `scoutRangeForProspect`, RB-1b chem-fit ≤+8% stays, won bid → stamped farm salary). Then RB-13b · RB-18 · B1.6. (Prior Branch-B this session: B6+S1–S5.) Detail in `BRANCH_B_PROGRESS.md` on `codex/mode1-v1-b`.
