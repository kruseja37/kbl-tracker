...
> **✅ DONE + VERIFIED this session (5 keystones, all zero-new-reds, no DB bumps):** B8 prospect age (`30359ae4`) · G1 draft-IV freeze → **launch-gap G1 CLOSED** (`40f876d7`) · DH franchise-seal + playoff-DH bug fix (`2550d3cf`) · **RA-1 ratings keystone** `expectedStatsEngine.ts` build-dark (`81c9fe25`) · B12 prospect archetype (`7d817965`).
> **➡ NEXT (resume here):** (1) **fame WAR-floor A1.2** — author the contract on the **CORRECTED** model: see `RA1_FAME_MODEL_PROPOSAL.md` "AUDIT CORRECTIONS (2026-06-23)" — floor is **CONTINUOUS upward-only gravity** (media darlings are a blessed archetype, §20.2/H3_KICKOFF), and the build MUST patch `applyWarLegitimacyGravity` (`fameModel.ts:161`) from bidirectional → `Heat += max(0, strength×(floor−Heat))` + a test that gravity never lowers Heat; Channel A already built (wire fame-as-volume only). (2) **L13-8** proof-test (ready, small). Then **RA-2** (wire RA-1 live + the required sim-tune curvature check) + the rest of Lane-4 (T-traits / S-scouting / B13/B14) + Lane-1 tail (trade-req, L12-6, L4b) + the gate chain. **G1 follow-up:** add an assertion that no fame/designation/award reader consumes a `'draft-baseline'` snapshot row.
