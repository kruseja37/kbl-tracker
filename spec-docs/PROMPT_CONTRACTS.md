...
## L8 — Ratings development (every-20%-of-season checkpoint sweep; first real WRITER through L2)

DSTACK L8 (line 83): `every-20%-of-games league sweep; CONSUMES the L5 §8 dampener (does not re-build it) × personality × Ambition/Resilience; per-team console change log via the L2 two-tier confirm. Team performance → ratings only (never traits).` Deps L2 ✓ / L3 ✓ / L5 ✓(L5a) / L1 ✓. SPLIT (Captain, 2026-06-18 AUTH-4):
- **L8a — the pure ratings-development engine** (THIS): the per-player, per-rating-key dev-math — raw delta from on-field performance + player morale, pushed through the L5a §8 dampener (CONSUMED), clamped to 0-99 integers, gated by an earned-magnitude shift threshold. Mirrors L7d-2/L2b purity. Standing auto-commit (pure, no surface).
- **L8b — the dark checkpoint-sweep compute + overlay writer + processCompletedGame hook**: a NEW default-OFF Phase-2 flag + `franchiseCheckpointSweepCompute.ts` mirroring `franchiseFlashpointDecayCompute.ts` (flag-gate-first dark-noop · deterministic 20%-boundary test on league-wide gameNumber/totalGames · per-checkpoint re-entry guard · mockable roster seam) that, at each 20% boundary, sweeps the MLB roster, runs L8a per player, and writes `pending` overlays via `putFranchiseRatingsOverlay` + a per-team change log. Live season path + persistence → audited HARDEST, **browser-pending**.

### DESIGN — the L8 model (AUTH-4 DEFAULTS-TAKEN; spec is SILENT on the raw delta formula — §16:272 lists dampener strength + personality multipliers as Sim-Gate-tuned; §9:158 grades EARNED+PACED drift, NOT a fixed magnitude)

- **The dampener already owns personality + Ambition/Resilience + Loyalty.** `applyFanMoraleDampener(ratingDelta, teamFanMorale, personality, {loyalty,ambition,resilience})` internally applies the §8:138-146 personality multiplier table, ambition-weights up-moves / resilience-weights down-moves (§6:115), and loyalty-amplifies (§6:110). So "× personality × Ambition/Resilience" in the DSTACK line is realized ENTIRELY inside the dampener. **L8a must NOT separately multiply the raw delta by personality/ambition/resilience/loyalty — that double-counts.** L8a's only novel inputs to the RAW (pre-dampener) delta are **on-field performance + player morale**.
- **Player morale = a strong DIRECT weight (§10:195); fan morale = NOT a direct weight (§10:196), enters ONLY via the dampener brake.** Player morale (the player's own 0-100 morale) shapes the raw delta (high morale amplifies gains / shrugs off drops; low morale inverse, self-correcting). Team FAN morale is the dampener's input, nowhere else.
- **The dampener is a BRAKE, never an accelerator (§8:132).** With-trend / zero moves pass through unchanged (`applied:false`); only counter-trend moves are softened. L8a feeds the raw signed delta in and trusts `result.dampenedDelta`. It can never flip sign or amplify → no runaway loop.
- **Shift decision is DETERMINISTIC (run rule: no Math.random/Date.now).** "Probability decides WHO, no headcount" (§10:190) is realized as an **earned-magnitude gate**: a player shifts iff `|dampenedDelta| >= shiftThreshold` AND the 0-99 clamp leaves a non-zero integer change. Bigger sustained performance → bigger earned delta → clears the gate; the count emerges from how the league played, not a fixed number. (Documented conservative default for the spec-silent probability mechanism.)
- **RATINGS ONLY, never traits (§9:153).** L8a touches a numeric rating key only. Traits = L9b.
- **Raises the CEILING, never True Value (§3:52).** L8a returns a proposed rating change; it does not touch TV/value. The overlay is the franchise-instance mutable layer (§11:205); the frozen oracle stays locked.
- **All magnitudes are SIM-TUNED PLACEHOLDERS in one `RATINGS_DEVELOPMENT_TUNING` object — shape locked, values owned by the Sim Gate.** Never hardcode a magic number inline.
- **OPEN-DECISION for JK (logged, NOT blocking):** the dampener (L5a, committed) weights a counter-trend-UP move (cold-team gain) by AMBITION such that higher ambition = MORE brake; §6:111 frames ambition as an "upside gas pedal." On a winning team up-moves are with-trend/unbraked so ambition there is moot; the odd case is a losing-team gain. L8 CONSUMES L5a as-built (L5 owns the §8 primitive; modifying it is an ownership + SMB4-asset violation) — flagged for JK's review of the §6-flavor-vs-§8-mechanics ambition direction, not changed here.

### L8a — Pure ratings-development engine (PURE; consumes the L5a §8 dampener; checkpoint-sweep WIRING deferred to L8b)

**ROUTE:** Codex 5.5 | high reasoning effort (pure deterministic engine; no store/wiring/persistence/React) → Opus 4.8 audit (auditor ≠ builder) → standing auto-commit (pure primitive, no user surface; mirrors L7d-2/L2b/L2c).

**ROLE:** You are the L-stack builder (Codex). Build the §8/§9 ratings-development DEV-MATH as a PURE engine. Mirrors L2b `ratingsOverlayMerge.ts` / L7d-2 `fanHopefulCushion.ts`: own TUNING, no store/flag/wiring/persistence/React. The ONLY non-type import is the L5a dampener (`applyFanMoraleDampener` + its result/tuning types) — CONSUME it, do NOT re-implement any dampener math. Build-dark; the checkpoint-sweep compute + overlay-writer wiring is L8b.

**GOAL:** NEW `src/engines/ratingsDevelopment.ts` exporting:
1. `RatingsDevelopmentTuning` (interface) + `RATINGS_DEVELOPMENT_TUNING` (const — §16 sim-tuned placeholders; shape locked).
2. `normalizePerformanceSignal(rawPerformance: number, tuning?: RatingsDevelopmentTuning): number` → a signed signal clamped to [-1, 1] (raw performance ÷ `performanceSignalScale`, clamped).
3. `computeRawRatingDelta(input: { performanceSignal: number; playerMorale: number }, tuning?: RatingsDevelopmentTuning): number` → the PRE-dampener signed raw rating delta = `baseDeltaScale × clamp(performanceSignal,-1,1) × moraleAlignmentMultiplier`, where the morale multiplier amplifies a with-direction gain / softens a drop for high player morale (and inverse for low), clamped to `[moraleMultiplierMin, moraleMultiplierMax]`, never sign-flipping.
4. `computeCheckpointRatingDevelopment(input: CheckpointRatingDevelopmentInput, tuning?: RatingsDevelopmentTuning): CheckpointRatingDevelopmentResult` → orchestrates: raw delta → magnitude-cap at `maxAbsDelta` (paced) → `applyFanMoraleDampener(cappedRaw, teamFanMorale, personality, modifiers)` → read `.dampenedDelta` → `proposedRating = clamp(Math.round(baseRatingValue + dampenedDelta), 0, 99)` → `appliedDelta = proposedRating - baseRatingValue` → `shouldShift = Math.abs(dampenedDelta) >= shiftThreshold && appliedDelta !== 0`.
5. `CheckpointRatingDevelopmentInput` + `CheckpointRatingDevelopmentResult` (interfaces). Plus its test file.

`CheckpointRatingDevelopmentInput` = `{ ratingKey: string; baseRatingValue: number; performanceSignal: number; playerMorale: number; teamFanMorale: number; personality: CanonicalPersonality; modifiers: Pick<HiddenModifiers,'loyalty'|'ambition'|'resilience'> }`.
`CheckpointRatingDevelopmentResult` = `{ ratingKey: string; rawDelta: number; dampenedDelta: number; appliedDelta: number; proposedRating: number; shouldShift: boolean; direction: 'up'|'down'|'none'; dampener: FanDampenerResult; reason: string }`.

`RATINGS_DEVELOPMENT_TUNING` (placeholder values — sim-tuned; DOCUMENT each as a §16 placeholder): `{ baseDeltaScale: 3, performanceSignalScale: <choose, e.g. the True-Value valueDelta that = full signal — pick a documented placeholder>, neutralMorale: 50, moraleWeightUp: 0.4, moraleWeightDown: 0.4, moraleMultiplierMin: 0.5, moraleMultiplierMax: 1.5, shiftThreshold: 0.75, maxAbsDelta: 6 }`. (`performanceSignalScale` is consumed by `normalizePerformanceSignal`; if you prefer `computeCheckpointRatingDevelopment` to accept an already-normalized `performanceSignal`, keep it in [-1,1] and let L8b call `normalizePerformanceSignal` upstream — document the contract either way.)

**SOURCE OF TRUTH:** `FRANCHISE_V1_LIVING_SEASON_SPEC.md` §8 lines 128-147 (the directional dampener — a BRAKE; the personality multiplier table; shape-locked/values-sim-tuned) + §9 lines 153-158 (RATINGS=team story, every-20% cadence, RATINGS-only-never-traits, magnitude earned+paced-not-limited) + §10 lines 188-196 (full-league sweep, probability-decides-who-no-headcount, player-morale=strong-weight, fan-morale=dampener-only) + §6 lines 110-116 (Ambition up / Resilience down / Loyalty amplifies / Charisma not own ratings) + §16 line 272 (dampener strength + personality multipliers are Sim-Gate numbers) + §3 line 52 (raises ceiling, never TV). CONSUME: L5a `applyFanMoraleDampener` (`src/engines/fanMoraleDampener.ts:43`, commit `428f7cb`). MIRROR purity: L2b `e8ec0908` / L7d-2 `aec5db99`. The model + every DEFAULT-TAKEN is in the L8 DESIGN block above.

**ALLOWED files:** NEW `src/engines/ratingsDevelopment.ts` · NEW `src/engines/__tests__/ratingsDevelopment.test.ts`. NOTHING ELSE.

**DO NOT:** re-implement ANY dampener math (consume `applyFanMoraleDampener`) · separately multiply the raw delta by personality / ambition / resilience / loyalty (the dampener already does — DOUBLE-COUNT) · use fan morale anywhere except as the dampener's `teamFanMorale` arg · touch traits / True Value / value engines / the frozen oracle / `ivEngine` / `playerDatabase` / `effectiveRatings.ts` · read/write the L2a store or any IndexedDB · wire into `processCompletedGame.ts` / add a flag / the checkpoint sweep (that is L8b) · mutate inputs · import store/IndexedDB/reporter/LLM/React or anything beyond `./fanMoraleDampener` (value) + type-only `CanonicalPersonality`/`HiddenModifiers` · use `Math.random`/`Date.now`/`new Date()` · let a delta flip sign in the dampener path · emit a rating outside 0-99 · scatter magic numbers (all in TUNING).

### TESTS (NEW `src/engines/__tests__/ratingsDevelopment.test.ts`, pure/deterministic)
`normalizePerformanceSignal`: 0→0, +scale→+1, beyond-scale clamps to +1, −scale→−1, symmetric. `computeRawRatingDelta`: zero signal→0; positive signal at neutral morale (50) → `baseDeltaScale × signal`; high morale amplifies a positive raw / low morale shrinks it; high morale shrinks a NEGATIVE raw's magnitude (shrug-off) / low morale grows it; multiplier clamped to [min,max]; never sign-flips. `computeCheckpointRatingDevelopment`: HOT team (fanMorale 80) + positive raw = with-trend → `dampenedDelta === rawCapped` (pass-through, `dampener.applied===false`); HOT team + negative raw = counter-trend-down → `|dampenedDelta| < |rawCapped|` (braked) and higher resilience → more brake; COLD team (fanMorale 20) + positive raw = counter-trend-up → braked; clamp at base 99 (+delta) → proposedRating 99, appliedDelta 0, shouldShift false; clamp at base 0 (−delta) → 0, appliedDelta 0, shouldShift false; shiftThreshold gate (just-below = no shift, just-above = shift); `maxAbsDelta` caps a huge raw; direction up/down/none correct; appliedDelta = proposedRating − base (integer); RATINGS-only (no trait field touched); determinism (same input → same output, run twice) + input-immutability + `node:fs`/Math.random/Date purity self-scan.

**VERIFICATION (prefix `NODE_ENV= `):** tsc 0 · build 0 · new test green · FULL suite — only the 2 characterized fails (`wpaRuntimeBoundary` + `franchiseManualSmokeFixture`) expected; the known non-deterministic order-flakes (`AwardsWatchlist`/`GameTrackerLaunchState`/`franchiseOffseasonGuards.component`) may appear → run solo to confirm, NOT regressions; NO OTHER new reds. Greps: `ratingsDevelopment.ts` imports ONLY `./fanMoraleDampener` (value) + type-only `CanonicalPersonality`/`HiddenModifiers`, NO store/IndexedDB/processCompletedGame/value/trait/reporter/React, NO `Math.random`/`Date.now`/`new Date()`; `fanMoraleDampener.ts`/`ivEngine.ts`/`effectiveRatings.ts` BYTE-UNCHANGED.

**STOP IF (→ SET-ASIDE):** purity can't hold · a frozen engine (`fanMoraleDampener`/`ivEngine`/`effectiveRatings`) must change to make it work · a type-only import build/circular error you can't re-home · a NEW red (≠ the 2 characterized, ≠ the known order-flakes) past 2 fix-iterations. Report BLOCKED with exact output — do NOT modify the dampener or any owned primitive to force it.

**FORMAT:** 1. Files changed (both paths + total changed-path count + passing-test count). 2. Each change w/ the §8/§9 line. 3. Verification output (+ any order-flake-solo proof). 4. "L8a complete" OR "BLOCKED: [reason]".
...
**ROUTE:** Codex 5.5 | high reasoning effort (LIVE-PATH + PERSISTENCE-WRITE class — first real WRITER through the L2a store + a new processCompletedGame dark gate; NO new store/version [reuses the L2a `franchiseRatingsOverlays` v21 store], so no migration; audited HARDEST) → Opus 4.8 audit (auditor ≠ builder) → auto-commit verified-complete, **browser-pending** (live game path + overlay writes — JK batch). Mirrors `franchiseFlashpointDecayCompute.ts` (L5b) / `franchiseFameCompute.ts` EXACTLY.

**ROLE:** You are the L-stack builder (Codex). Build L8b — the DARK, default-OFF, per-game checkpoint-sweep compute that, at each 20%-of-season boundary, sweeps the MLB roster, runs the L8a engine per player, and writes `pending` `permanent` ratings overlays through the L2a store + a per-team change log. Mirror `src/utils/franchiseFlashpointDecayCompute.ts` (`5ebb148`/L7a) structurally: flag-gate-first dark-noop · a single mockable seam · re-entry/idempotency via a deterministic id · `{status, written}` result · the outer caller wraps it in try/catch. Doubly-dark: gated by a NEW default-OFF flag AND the overlays it writes are `pending` (inert in `mergeRatingsOverlays` until confirmed by the post-D13 UI). NO new store, NO trackerDb version bump.

**GOAL (3 edits + 1 new compute + tests):**
1. **EDIT `src/utils/franchisePhase2Flags.ts`** — append a 4th flag block, cloning the flashpoint block verbatim: `export const FRANCHISE_PHASE2_CHECKPOINT_ENABLED_DEFAULT = false;` + `isFranchisePhase2CheckpointEnabled(): boolean` + `setFranchisePhase2CheckpointEnabledForTests(enabled: boolean | null): void`. Default MUST be false.
2. **NEW `src/utils/franchiseCheckpointSweepCompute.ts`** exporting:
   - `CHECKPOINT_DEV_TUNING: RatingsDevelopmentTuning = { ...RATINGS_DEVELOPMENT_TUNING, performanceSignalScale: 200000 }` — L8b OWNS the dollar→signal scale (valueDelta is signed dollars ~±$50k–$500k; the L8a default scale of 10 is for an already-small signal). §16 placeholder; comment it.
   - `isCheckpointBoundary(gameNumber: number, totalGames: number): boolean` → `Number.isInteger(gameNumber) && gameNumber > 0 && totalGames > 0 && Math.floor((gameNumber - 1) * 5 / totalGames) !== Math.floor(gameNumber * 5 / totalGames)` (fires exactly 5×/season at the 20/40/60/80/100% crossings).
   - `CheckpointRosterEntry` interface: `{ playerId: string; teamId: string; isPitcher: boolean; baseRatings: Record<string, number>; personality: CanonicalPersonality; modifiers: Pick<HiddenModifiers,'loyalty'|'ambition'|'resilience'>; playerMorale: number; teamFanMorale: number; performanceSignal: number; createdAt: string }`.
   - `resolveCheckpointRoster(scope, gameState): Promise<CheckpointRosterEntry[]>` — the data-join: `getAllFranchiseTeams(scope.franchiseId)` → `leagueId = teams[0]?.leagueIds?.[0]` (dark-noop empty if none); `getAllFranchisePlayers(scope.franchiseId)` → keep `getPlayerRosterStatusForLeague(p, leagueId) === 'MLB'`; `getFranchiseTrueValueRows({franchiseId,seasonId,statsScopeId})` → map by playerId; for each MLB player WITH a TV row, project `{ playerId, teamId: getPlayerTeamIdForLeague(p, leagueId), isPitcher: p.isPitcher, baseRatings: (p.isPitcher ? {velocity,junk,accuracy} : {power,contact,speed,fielding,arm}) read off the FLAT player fields, personality: normalizePersonality(p.personality), modifiers: { loyalty,ambition,resilience } from (p.hiddenPersonalityModifiers ?? NEUTRAL_HIDDEN_MODIFIERS), playerMorale: p.morale, teamFanMorale: (await getFranchiseMoraleSnapshot(scope,'team-fan',teamId))?.currentValue ?? 50, performanceSignal: normalizePerformanceSignal(row.valueDelta, CHECKPOINT_DEV_TUNING), createdAt: row.computedAt }`. Players with NO TV row are EXCLUDED (no measurable signal → no earned development — documented DEFAULT-TAKEN). Cache team fan morale + teamId per team (don't re-read per player).
   - `checkpointSweepSeam = { resolveCheckpointRoster }` — the single mockable indirection (mirror `flashpointSeam`).
   - `selectDevelopmentRatingKey(entry: CheckpointRosterEntry): string` — performance-typed, ONE key/shifter/checkpoint: key set = `entry.isPitcher ? ['velocity','junk','accuracy'] : ['power','contact','speed','fielding','arm']`; pick deterministically by a stable char-code hash of `playerId` mod set length (spreads keys across players, no Math.random; documented DEFAULT-TAKEN, sim-refinable — later: weakness/component targeting).
   - `persistDarkCheckpointSweepForCompletedGame(gameState: PersistedGameState, scope: CheckpointSweepScope, archiveOptions?: CompletedGameArchiveOptions): Promise<PersistDarkCheckpointSweepResult>` where `CheckpointSweepScope = FranchiseRatingsOverlayScopeInput & { franchiseId; seasonNumber: number }` (structurally satisfied by the `trueValueScope` passed at the call site). `PersistDarkCheckpointSweepResult = { status: 'dark-noop' | 'not-checkpoint' | 'written'; written: number; reason?: string }`. Body: (a) flag-gate-first → dark-noop; (b) `gameNumber = await resolveCheckpointGameNumber(gameState, archiveOptions)` (mirror `resolveFlashpointCheckpoint` but return a `number | null` — `getScheduledGame(scheduleGameId).gameNumber` when a positive integer, else null); null → dark-noop; (c) `totalGames = (await getSeasonMetadata(scope.seasonId))?.totalGames`; falsy/≤0 → dark-noop; (d) `if (!isCheckpointBoundary(gameNumber, totalGames)) return { status:'not-checkpoint', written:0 }`; (e) `roster = await checkpointSweepSeam.resolveCheckpointRoster(scope, gameState)`; empty → dark-noop; (f) per entry: `ratingKey = selectDevelopmentRatingKey(entry)`; `baseRatingValue = entry.baseRatings[ratingKey]` (skip if not finite); `dev = computeCheckpointRatingDevelopment({ ratingKey, baseRatingValue, performanceSignal: entry.performanceSignal, playerMorale: entry.playerMorale, teamFanMorale: entry.teamFanMorale, personality: entry.personality, modifiers: entry.modifiers }, CHECKPOINT_DEV_TUNING)`; if `!dev.shouldShift` skip; else build a row: `sourceEventId = \`checkpoint-${gameNumber}\``, `id = \`${franchiseId}:${seasonId}:${statsScopeId}:${playerId}:${ratingKey}:${sourceEventId}\``, `{ id, franchiseId, seasonId, statsScopeId, playerId, ratingKey, delta: dev.appliedDelta, kind: 'permanent', expiresAtGameNumber: null, confirmationStatus: 'pending', source: 'ratings-development', sourceEventId, createdAtGameNumber: gameNumber, createdAt: entry.createdAt }`; (g) `await putFranchiseRatingsOverlay(row)` for each; (h) return `{ status:'written', written: rows.length }`. The deterministic id makes a checkpoint replay idempotent (same id → put overwrites, no dup); successive checkpoints have distinct `sourceEventId` → deltas STACK across the season (per §11 permanent layer).
3. **EDIT `src/utils/processCompletedGame.ts`** — import `isFranchisePhase2CheckpointEnabled` + `persistDarkCheckpointSweepForCompletedGame`; add the gated hook IMMEDIATELY after the flashpoint gate (after line 618, before the designations `try` at 619), inside `if (trueValueScope)`:
   ```
...
**SOURCE OF TRUTH:** `FRANCHISE_V1_LIVING_SEASON_SPEC.md` §9 lines 153-158 (RATINGS=team story, every-20% cadence, ratings-only-never-traits) + §10 lines 188-196 (full-league sweep, probability-decides-who-no-headcount, player-morale strong weight, fan-morale dampener-only) + §11 lines 203-205 (two-tier confirm → `pending`; permanent persisted layer; oracle locked) + DSTACK L8 line 83. CONSUME: L8a `computeCheckpointRatingDevelopment`/`normalizePerformanceSignal`/`RATINGS_DEVELOPMENT_TUNING` (`src/engines/ratingsDevelopment.ts`) + L2a `putFranchiseRatingsOverlay`/`FranchiseRatingsOverlayRow`/`FranchiseRatingsOverlayScopeInput` (`src/utils/franchiseRatingsOverlayStorage.ts`) + L2c `summarizeOverlayChangeLog` (optional, for the change-log return). MIRROR: `src/utils/franchiseFlashpointDecayCompute.ts` (structure, seam, flag-gate, checkpoint resolver) + the `processCompletedGame.ts:612-618` flashpoint gate. JOINS: `getAllFranchiseTeams`/`getAllFranchisePlayers` (`franchisePlayerStorage.ts`), `getPlayerRosterStatusForLeague`/`getPlayerTeamIdForLeague` (`leagueBuilderStorage.ts`), `getFranchiseTrueValueRows` (`franchiseTrueValueStorage.ts`), `getFranchiseMoraleSnapshot` (`franchiseMoraleState.ts`), `getSeasonMetadata` (`seasonStorage.ts`), `getGame as getScheduledGame` (`scheduleStorage.ts`), `normalizePersonality` (`masterMoraleMatrix.ts`), `NEUTRAL_HIDDEN_MODIFIERS` (reuse `processCompletedGame.ts:91` — export it or re-declare a local const).

### DESIGN (AUTH-4 DEFAULTS-TAKEN — documented)
- **Doubly-dark:** default-OFF flag + `pending` overlays (inert in the merge until the post-D13 confirm UI). Even flag-ON, ratings do not change until a user confirms — matches §11 two-tier + "no phantom morale/dev until after D13."
- **Determinism (run rule):** NO `Math.random`/`Date.now`/`new Date()` anywhere. `createdAt` = the TV row's `computedAt` (a deterministic ISO already persisted) — never `new Date()`. Idempotency via the deterministic `id`. The boundary test is integer-only.
- **Cadence denominator = league-wide `totalGames`** (matches the league-wide `gameNumber`), NOT `gamesPerTeam`.
- **Perf signal = `valueDelta`** (signed dollars; + = outperforming baseline), normalized with `performanceSignalScale: 200000`. Players with no TV row don't develop (no measurable signal).
- **Team fan morale = `getFranchiseMoraleSnapshot(...,'team-fan',teamId)?.currentValue ?? 50`** — 50 is the proven dark-safe fallback (the morale store is empty under the default-off morale flag → neutral 50 → the dampener applies zero brake). This is correct dark behavior: with no live fan morale, development is undamped.
- **Which key:** performance-typed, one key/shifter/checkpoint, stable per-player char-code hash → spreads keys across players, deterministic, sim-refinable.
- **Overlay = `permanent`** (development raises the ceiling persistently, §11:205) + `pending` (two-tier). Deltas STACK across the 5 checkpoints (distinct `sourceEventId`); a replayed checkpoint overwrites the same id (idempotent).
- **No new store / no version bump** — reuses the L2a `franchiseRatingsOverlays` v21 store; the pin-trap test is untouched (verify it stays green).

**ALLOWED files:** EDIT `src/utils/franchisePhase2Flags.ts` · NEW `src/utils/franchiseCheckpointSweepCompute.ts` · EDIT `src/utils/processCompletedGame.ts` (the gated hook + imports; if you must export `NEUTRAL_HIDDEN_MODIFIERS` for reuse, that single export is allowed) · NEW `src/utils/tests/franchiseCheckpointSweepCompute.test.ts` (+ a focused flag test if `franchisePhase2Flags.test.ts` exists, EDIT it to add the new flag's default assertion). NOTHING ELSE — NO trackerDb/backupRestore/syncConfig/store-schema/version edits (no new store).

**DO NOT:** add a store / bump `TRACKER_DB_VERSION` / touch backup/sync/pin-trap · re-implement the L8a dev-math or the dampener (CONSUME them) · write a `confirmed` overlay (write `pending` — the confirm flow is post-D13) · build a UI/modal/confirm flow · mutate live fan/player morale or the frozen oracle / value engines / `ivEngine` / `playerDatabase` · touch traits · use `Math.random`/`Date.now`/`new Date()` · use `gamesPerTeam` as the cadence denominator (use `totalGames`) · let the hook run when the flag is OFF (flag-gate FIRST) · let a non-boundary game write anything · scatter magic numbers (the scale lives in `CHECKPOINT_DEV_TUNING`).

### TESTS (`src/utils/tests/franchiseCheckpointSweepCompute.test.ts`; mock the seam + storage; use `setFranchisePhase2CheckpointEnabledForTests`)
- `isCheckpointBoundary`: for `totalGames=10` → true exactly at gameNumber 2,4,6,8,10, false elsewhere; spot-check `totalGames=32` (true at 7,13,20,26,32 — `ceil(32k/5)`) and `162`; false for gameNumber 0/negative/non-integer or totalGames 0.
- `selectDevelopmentRatingKey`: pitcher → a key in {velocity,junk,accuracy}; hitter → a key in {power,contact,speed,fielding,arm}; deterministic (same playerId → same key); spreads across differing playerIds.
- compute flag OFF → `dark-noop`, seam NOT called, ZERO `putFranchiseRatingsOverlay` (spy).
- compute flag ON + non-boundary gameNumber → `not-checkpoint`, seam NOT called.
- compute flag ON + unresolved gameNumber (no scheduleGameId) → `dark-noop`.
- compute flag ON + boundary + mocked `checkpointSweepSeam.resolveCheckpointRoster` returning synthetic entries (some shift, some not) → writes ONE `pending` `permanent` overlay per shifter with the exact deterministic id/`sourceEventId=checkpoint-N`/`source='ratings-development'`/`delta=appliedDelta`/`createdAtGameNumber=N`/`createdAt=entry.createdAt`; non-shifters write nothing; `written` = shifter count.
- IDEMPOTENCY: run the same boundary twice → the franchiseRatingsOverlays store holds ONE row per (player,key,checkpoint) (round-trip via `getFranchiseRatingsOverlaysByScope`), no duplication.
- STACKING: two different boundaries (N1≠N2) for the same player+key → two rows (distinct sourceEventId); `resolveActiveOverlayDeltas` would sum them (note: they stay `pending` so the live merge is still zero — assert the rows persist).
- `resolveCheckpointRoster` focused test: mock `getAllFranchiseTeams`/`getAllFranchisePlayers`/`getPlayerRosterStatusForLeague`/`getFranchiseTrueValueRows`/`getFranchiseMoraleSnapshot` → returns only MLB players WITH a TV row, correct base-rating projection (pitcher vs hitter keys), `normalizePersonality` applied, modifiers fallback to NEUTRAL when absent, `teamFanMorale` falls back to 50 when no snapshot, `performanceSignal = normalizePerformanceSignal(valueDelta, CHECKPOINT_DEV_TUNING)`, `createdAt = row.computedAt`.
- purity self-scan of `franchiseCheckpointSweepCompute.ts`: no `Math.random`/`Date.now`/`new Date(`/`indexedDB.open`.

**VERIFICATION (prefix `NODE_ENV= `):** tsc 0 · build 0 · new tests green · FULL suite — only the 2 characterized fails expected (+ known order-flakes may appear → solo-confirm, NOT regressions); NO OTHER new reds. Greps: `franchiseCheckpointSweepCompute.ts` no `Math.random`/`Date.now`/`new Date(`/`indexedDB.open`; `TRACKER_DB_VERSION` still 21 (untouched); `KBL_BACKUP_VERSION` still 2; `ratingsDevelopment.ts`/`fanMoraleDampener.ts`/`franchiseRatingsOverlayStorage.ts`/`ivEngine.ts`/`playerDatabase.ts` BYTE-UNCHANGED; the new flag default is `false`; the processCompletedGame hook is flag-gated and sits after the flashpoint gate.

**STOP IF (→ SET-ASIDE, persistence/live-path safety boundary):** the overlay write isn't idempotent / a round-trip shows dup or wrong-scope rows you can't fix in 2 iterations · a new store or version bump would be needed · `processCompletedGame` existing tests go red from the hook (it must be a no-op when the flag is off) · a frozen engine/store must change · a NEW red (≠ 2 characterized, ≠ known order-flakes) past 2 fix-iterations. Report BLOCKED with exact output — do NOT force it.

**FORMAT:** 1. Files changed (EVERY path + total changed-path count + passing-test count). 2. Each change w/ the §9/§10/§11 line. 3. Verification output (+ idempotency round-trip proof + flag-default + byte-unchanged greps + any order-flake-solo). 4. "L8b complete" OR "BLOCKED: [reason]".
...
## CONTRACT — L9b-3b-ii (flag + dark trait-grant hook) — 2026-06-18 (AUTH-4 overnight)

**ROUTE: Codex 5.5 | high reasoning effort** (builder). Auditor = Opus 4.8 (independent; ≠ builder). LIVE game-path hook (flag-gated default-OFF) → audited carefully. Branch codex/franchise-v1-next.

**GOAL:** add the default-OFF `isFranchisePhase2TraitsEnabled` flag + a doubly-dark `persistDarkTraitGrantForCompletedGame` hook that, at each 20%-of-season checkpoint, loads the season's events, runs L9b-3a `computeSeasonTraitCandidates` → L9b-2 `computeTraitAcquisition` per MLB player, and writes PENDING `franchiseTraitOverlays` rows; wired after the checkpoint gate in `processCompletedGame` (inside `if (trueValueScope)`, using `trueValueScope`, matching the `[Checkpoint]` try/catch). MIRROR L8b `franchiseCheckpointSweepCompute`.

**FILE LIST:** EDIT `franchisePhase2Flags.ts` (5th flag block) · NEW `src/utils/franchiseTraitGrantCompute.ts` (the hook) · EDIT `processCompletedGame.ts` (import + the sibling gate) · NEW `src/utils/tests/franchiseTraitGrantCompute.test.ts`.

**KEY MECHANICS:** flag-gate FIRST (normal play = zero-cost no-op, no load); cadence = reuse exported `isCheckpointBoundary` (the min-sample valve makes early checkpoints dormant → trait changes emerge late-season, a populated pool — DEFAULT-TAKEN vs a season-end trigger); heldTrait `strength` = the player's current realityPercentile for that held trait from the candidates, default 0.5 (DEFAULT-TAKEN); `rosterRole:'unknown'` v1 (no bench/starter signal — DEFAULT-TAKEN); deterministic idempotent id `…:${traitName}:trait-grant-${gameNumber}`; `createdAt` from a PERSISTED value (max at-bat timestamp → ISO), NEVER Date.now/argless new Date; rows `pending`+`applied:false` (L9b-3c applies). config = `deriveAdaptiveStandardsConfig({gamesPerSeason: totalGames})`.

**HARD CONSTRAINTS (L9b-3a/b-i lessons):** no spec-doc/`*.md` edits, no git-add (Captain owns docs+staging); touch ONLY the FILE LIST; doubly-dark (flag OFF + pending-only); live game path stays safe (try/catch never blocks completion).

**Full contract text (the dispatched prompt):** `/tmp/l9b3bii_codex_prompt.md`.

**Status:** VERIFIED + COMMITTED (2026-06-18, AUTH-4 overnight). Codex 5.5 built → Opus 4.8 independently audited VERIFIED: tsc 0 / full suite 7,499 tests, 7,497 pass / 2 characterized fail, ZERO new reds. Line-by-line read confirmed: flag-gate-FIRST (zero-cost no-op on normal play), `isCheckpointBoundary` cadence, season-wide load → league roster → `computeSeasonTraitCandidates` → `computeTraitAcquisition` per player → PENDING `franchiseTraitOverlays` rows (deterministic idempotent id `…:${traitName}:trait-grant-${gameNumber}`, `applied:false`, `createdAt` from max persisted at-bat timestamp — no Date.now), `traitGrantSeam` for stubbing, live-path try/catch never blocks completion. DARK (only processCompletedGame consumes it, flag-gated). Codex hit EXACTLY the FILE LIST (no doc edits, no git-add). LIMITATION: the hook test stubs the L9b-3a→L9b-2 seam (gate/cadence/persistence/idempotency/determinism covered; the engines are tested in their own suites + the seam test) → real end-to-end is browser-pending (#22). DEFAULTS-TAKEN: held-trait strength from candidates (0.5 default) · rosterRole 'unknown' v1 · createdAt source. **⇒ L9b-3b COMPLETE (b-i `0cd75d9a` + b-ii).** trackerDb **v22**. NEXT = L9b-3c (§11 confirm + ATOMIC trait1/trait2 displacement write).

---

## CONTRACT — L9b-3c (§11 trait-confirm + ATOMIC trait1/trait2 displacement) — 2026-06-18 (AUTH-4 overnight)

**ROUTE: Codex 5.5 | high reasoning effort** (builder). Auditor = Opus 4.8 (independent; ≠ builder). The FIRST code that mutates a player's trait1/trait2 → audited carefully. Branch codex/franchise-v1-next. The LAST L9b piece.

**GOAL:** the §11 trait-confirm transform + the ATOMIC 2-slot displacement write — PURE engine `src/engines/traitOverlayConfirmation.ts` (mirror `ratingsOverlayConfirmation` [L2c] but CATEGORICAL: `applyTraitDisplacement` + `confirmTraitOverlay` + `buildTraitConfirmationRequest` + `summarizeTraitOverlayChangeLog`) + a thin impure applier `src/utils/franchiseTraitConfirmApply.ts` (`applyConfirmedTraitOverlay`: load player → displace → `saveFranchisePlayer` FLAT trait1/trait2 → mark overlay confirmed+applied; idempotent). Unlike L2c (ratings use a read-merge), traits have NO read-merge (do NOT route through `ratingsOverlayMerge`) so the WRITE is in scope. NO live caller (confirm UI = deferred post-D13 D-ticket).

**DISPLACEMENT (pure):** gain → already-held no-op · displaces-named replaces that slot · free slot fills · cap+no-displaces → applied:false. lose → remove held slot · not-held no-op. Non-canonical traitName throws (CANONICAL_TRAIT_NAMES guard). Applier idempotent via the overlay `applied` flag; player write is authoritative (cross-DB, so no single txn — documented).

**HARD CONSTRAINTS:** no spec-doc/`*.md` edits, no git-add; ONLY the 4 FILE LIST files; engine PURE (no IndexedDB/Date/random/async); NO live caller (do not wire into processCompletedGame); write FLAT trait1/trait2 (STOP if the franchise Player is nested).
...
## CONTRACT — L10-3 (flag + dark league-sweep hook) — 2026-06-18 (AUTH-4 overnight)

**ROUTE: Codex 5.5 | high reasoning effort** (builder). Auditor = Opus 4.8 (independent; ≠ builder). LIVE-PATH wiring (dark-gated) + producer→consumer SEAM → audited hard. Branch codex/franchise-v1-next. The 3rd L10 piece (per `spec-docs/L10_SCOPE_MAP.md` §3); wires L10-1 (engine `607fa015`) → L10-2 (store `a830a61f`).

**GOAL:** wire the L10 event engine to the store via a default-OFF flag + a dark league-sweep hook, mirroring the TRAITS hook (L9b-3b-ii, `src/utils/franchiseTraitGrantCompute.ts`) EXACTLY. (1) Add a 6th default-OFF Phase-2 flag block `isFranchisePhase2L10Enabled` (+ default const + override + `setFranchisePhase2L10EnabledForTests`) by cloning the TRAITS block at `franchisePhase2Flags.ts:49-58`. (2) NEW `src/utils/franchiseL10SweepCompute.ts` exporting `persistDarkL10ForCompletedGame(gameState, scope, archiveOptions)`: **flag-gate FIRST** (zero-cost no-op on normal play — no loads) → resolve the league `gameNumber` (mirror `resolveTraitGrantGameNumber` — schedule lookup, dark-noop on unresolved) → `getSeasonMetadata(scope.seasonId)?.totalGames` gate → `isCheckpointBoundary(gameNumber, totalGames)` (else `not-checkpoint`) → build `FranchiseL10Candidate[]` (players + teams) by REUSING the checkpoint roster-resolve pattern (`resolveCheckpointRoster` shape: MLB roster via `getPlayerRosterStatusForLeague` === 'MLB', per-team fan morale memoized via `getFranchiseMoraleSnapshot(scope,'team-fan',teamId)?.currentValue ?? 50`, player morale, `normalizePersonality`, `performanceSignal` via `normalizePerformanceSignal(trueValueRow.valueDelta, CHECKPOINT_DEV_TUNING)`) → call `computeFranchiseL10Events({ candidates, intensity, seedBase })` with `intensity` = franchise setting (DEFAULT `'standard'`) + `seedBase = `${franchiseId}:${seasonId}:${gameNumber}`` → write each emitted `FranchiseL10EventCandidate` as a PENDING `franchiseL10Overlays` row via `putFranchiseL10Overlay` (deterministic idempotent `id = `${franchiseId}:${seasonId}:${statsScopeId}:${targetId}:${family}:${eventType}:l10-${gameNumber}``, `applied:false`, `confirmationStatus:'pending'`, `source:'l10-random-event'`, `sourceEventId:`l10-${gameNumber}``, `createdAtGameNumber: gameNumber`, `createdAt` from the MAX persisted at-bat timestamp — mirror `resolveCreatedAtIso`, NO Date.now). Expose an `l10SweepSeam` (roster-resolve + candidate-build + `computeFranchiseL10Events`) so the hook test stubs the seam. (3) Wire a 6th gate branch `if (isFranchisePhase2L10Enabled()) { try { await persistDarkL10ForCompletedGame(gameState, trueValueScope, archiveOptions); } catch (e) { console.warn('[L10] dark random-event sweep skipped for completed game ' + gameState.gameId + ':', e); } }` inside `if (trueValueScope)` immediately AFTER the Traits gate (currently ends ~processCompletedGame.ts:636) and before the Designations try. Add the flag import + the compute import. (4) NEW test `src/utils/tests/franchiseL10SweepCompute.test.ts` mirroring `franchiseTraitGrantCompute.test.ts` (flag-off dark-noop loads nothing · flag-on non-boundary → not-checkpoint · boundary writes PENDING rows + idempotent replay · two seeded runs identical).

**DOUBLY-DARK:** flag OFF (no compute on normal play) + every written row `pending`+`applied:false` (inert until a post-D13 confirm/apply step). NO new store / trackerDb stays **v23** / KBL_BACKUP_VERSION stays **2**.

**MIRROR EXACTLY:** `franchiseTraitGrantCompute.ts` for hook shape/order/result type (`{status:'dark-noop'|'not-checkpoint'|'written', written, reason?}`), `resolveTraitGrantGameNumber`/`resolveCreatedAtIso` for game-number + timestamp resolution, `franchiseCheckpointSweepCompute.ts` `resolveCheckpointRoster` (lines 116-187) for the MLB-roster + per-team-fan-morale + performanceSignal resolve pattern. Scope type = `FranchiseL10OverlayScopeInput & { franchiseId; seasonNumber }` (the `trueValueScope` passed in already carries all four — franchiseId/seasonId/statsScopeId/seasonNumber).

**SEAM REQUIREMENT (producer→consumer):** the hook (L10-3) consumes the L10-1 engine output. The test MUST include a real seam test feeding a built `FranchiseL10Candidate[]` straight through `computeFranchiseL10Events` and asserting the emitted candidates become correctly-shaped pending overlay rows (not only stubbed). Resolve the franchiseSeasonLedgerStorage store-list PIN concern: NO store change here, so the PIN stays at 23 — do NOT touch trackerDb/backupRestore/syncConfig/the ledger PIN test.

**HARD CONSTRAINTS:** no spec-doc/`*.md` edits (Captain owns docs + staging), no git-add; ONLY the FILE LIST below; the hook must be PURE-of-wall-clock (no Date.now/Math.random); flag-gate FIRST so normal play does zero work; do NOT extend/import `franchiseRandomEventGenerator.ts` (BOUNDARY); do NOT add a store or bump any DB version; STOP and report if `trueValueScope` does not carry franchiseId/seasonId/statsScopeId/seasonNumber, or if `resolveCheckpointRoster`'s helpers are not importable.

**FILE LIST (exactly these 4):**
- EDIT `src/utils/franchisePhase2Flags.ts` (append the 6th flag block)
- NEW  `src/utils/franchiseL10SweepCompute.ts`
- EDIT `src/utils/processCompletedGame.ts` (flag import + compute import + 6th gate branch)
- NEW  `src/utils/tests/franchiseL10SweepCompute.test.ts`

**VERIFICATION (builder, report exact output):** `NODE_ENV= npx tsc --noEmit` (0 errors); `NODE_ENV= npx vitest run src/utils/tests/franchiseL10SweepCompute.test.ts` (all green); confirm the 5 prior gate branches + the new 6th are intact; grep that no Date.now/Math.random in the new compute; report every `git status` path + total changed-path count + passing-test count.

**BUILD NOTE:** The sandbox had NO Codex CLI and host node — so the Captain (Opus) BUILT the diff directly in-sandbox (a tight mirror of the trait hook), then satisfied the triangle with an INDEPENDENT decorrelated auditor (a fresh reader subagent, ≠ builder). Full `npm run build` + full suite + commit are host-gated (sandbox kills >42s processes + blocks git unlink). The dispatched-prompt file was not produced (no Codex dispatch); the contract body above IS the spec of record.

**Status:** ✅ VERIFIED + COMMITTED `8a33d9d3` (host gate passed in a fresh ATTENDED session 2026-06-18, real node v20: `NODE_ENV= npm run build` exit 0 + full suite **7,540/436, 7,538 pass / 2 characterized fail**, ZERO new reds, +5 = `franchiseL10SweepCompute.test.ts`; the 5 contracted files + the 3 session docs folded in per JK; not pushed). *Original sandbox build+audit record below:* BUILT + INDEPENDENTLY AUDITED VERIFIED, 2026-06-18 AUTH-4 overnight. tsc --noEmit exit 0 (full project) + 5 targeted tests green (incl. a real producer→consumer seam test feeding the live `computeFranchiseL10Events` — verified 3 events fire: 2 player + 1 team, so the written>0 / team-target assertions are non-vacuous). Independent audit (decorrelated reader ≠ builder): VERDICT VERIFIED, 0 major / 3 minor (M1 seam-team-path → CLOSED in-session by adding `team-dd`; M2 cosmetic single-sort divergence from mirror; M3 = sandbox probe artifacts to delete host-side). Verified: flag default OFF + flag-gate FIRST (normal play loads nothing); 6th gate branch inside `if(trueValueScope)` after Traits / before Designations, try/catch never rethrows; deterministic (no Date.now/Math.random); idempotent id `…:${family}:${eventType}:l10-${gameNumber}`; pending+applied:false; MLB-only roster + per-team fan-morale memoized + player AND team candidates; NO store/DB/backup/PIN drift (trackerDb stays v23); no franchiseRandomEventGenerator boundary violation. The diff is EXACTLY the 5 FILE LIST paths. Host gate logged in `WAITING_ON_JK.md` [ticket:L10-3]. Live path → browser-pending (#24). **NEXT after host-commit = L10-4** (stadium-change event) then L10-5 (reporter tap) per L10_SCOPE_MAP.md §3.

---

## CONTRACT — L10-4 (stadium-change resolver) — 2026-06-18 (AUTH-4, fresh attended session)

**ROUTE: Builder = a fresh in-session subagent | high reasoning effort. Auditor = Opus 4.8 Captain (independent; ≠ builder). Use high reasoning effort.** PURE engine, build-DARK (no I/O, no wiring, no live write) → low risk. Branch codex/franchise-v1-next. The 4th L10 piece (per `spec-docs/L10_SCOPE_MAP.md` §3 line 54 + §5 line 72).

**DESIGN CALL (AUTH-4 default, flagged for JK):** L10-4 is the PURE *concrete-resolution* step for a fired `stadium_change` team event, NOT a live persistence/analytics mutation. The whole L10 system is doubly-dark (overlay rows stay `applied:false` until a post-D13 apply step), so L10-4 produces the chosen park + the resulting `FranchiseTeamStadiumSnapshot` payload for a future apply step to write — it performs NO live snapshot write and NO recompute trigger (those defer to the post-D13 apply ticket, mirroring how L9b-3c's trait applier is orphaned-pending its UI). The pure resolver is the never-wasted core regardless of how the apply step lands, and is the helper L14 (rebrand) shares. Open-question #4 (stadium change on the USER's team vs AI-only) only bites at the future apply step → default `allowed/suppressed` taken upstream at L10-1; not decided here.

**GOAL:** NEW pure engine `src/engines/franchiseStadiumChangeResolver.ts` that, given a fired `stadium_change` event, deterministically picks a NEW park from the SMB pool and returns the resulting `FranchiseTeamStadiumSnapshot`. Exports:
- `pickStadiumFromPool(currentStadiumName: string | undefined, seed: string): ParkDimensions` — the SHARED deterministic pool-pick helper (reused by L14; do NOT fork). Pool = `getAllParks()` (`src/data/parkLookup.ts`). Excludes the current park (by normalized name via `getStableParkId`); if the current park is absent/undefined, the whole pool is eligible; if excluding leaves an empty pool (single-park edge), fall back to the current park. Pick index = `franchiseL10DeterministicRoll(seed)` (reuse the FNV-1a roll exported at `franchiseL10EventEngine.ts:191`) mapped to `Math.floor(roll * eligible.length)` (clamp to last index). Deterministic, total, pure.
- `resolveFranchiseStadiumChange(input: { event: FranchiseL10EventCandidate; teamName: string; currentStadiumName?: string; seedBase?: string }): FranchiseStadiumChangeResolution` — guards `event.targetKind === 'team'` && `event.eventType === 'stadium_change'` (else throw a clear Error — caller only routes stadium-change team events here). Derives the pick seed from `${seedBase ?? ''}:${event.targetId}:stadium_change:${event.seed}` (so it is reproducible from the event's own seed). Returns `{ newStadium: ParkDimensions; snapshot: FranchiseTeamStadiumSnapshot }` where `snapshot = { teamId: event.targetId, teamName, stadium: newStadium.name, stadiumId: getStableParkId(newStadium.name), hasSeedParkFactors: getDerivedParkFactorsIfAvailable(newStadium.name) !== undefined }`.
- `export interface FranchiseStadiumChangeResolution { newStadium: ParkDimensions; snapshot: FranchiseTeamStadiumSnapshot }`.

**REUSE (import, do NOT re-implement):** `getAllParks` / `getStableParkId` from `src/data/parkLookup.ts`; `getDerivedParkFactorsIfAvailable` from `src/engines/parkFactorDeriver.ts:116`; `franchiseL10DeterministicRoll` + type `FranchiseL10EventCandidate` from `src/engines/franchiseL10EventEngine.ts`; type `FranchiseTeamStadiumSnapshot` from `src/types/franchise.ts:54-60`; type `ParkDimensions` from `parkLookup.ts:5-13`.

**HARD CONSTRAINTS:** PURE — no IndexedDB/store/flag/Date.now/Math.random/async; build-DARK — NO production importer (orphaned-pending the post-D13 apply step), NO wiring into `processCompletedGame`, NO store, NO trackerDb/backup/syncConfig/PIN touch (stays **v23**); fan-morale suppression is UPSTREAM (L10-1's `teamSuppressed`) — the resolver does NOT take fan morale; do NOT extend/import `franchiseRandomEventGenerator.ts` (BOUNDARY); naming avoids `franchiseRandomEvent*`. No spec-doc/`*.md` edits, no git-add. ONLY the 2 FILE LIST files.

**FILE LIST (exactly these 2):**
- NEW `src/engines/franchiseStadiumChangeResolver.ts`
- NEW `src/engines/__tests__/franchiseStadiumChangeResolver.test.ts`

**TESTS (the new file):** determinism (same event+seed → same park, two runs identical); excludes the current park; covers the pool across varied seeds (multiple distinct parks reachable); snapshot shape correct (teamId from event.targetId, stadiumId = getStableParkId(name), hasSeedParkFactors matches `getDerivedParkFactorsIfAvailable`); current-park-absent → full pool eligible; single-eligible / current-not-in-pool edge; throws on a non-stadium / non-team event.
...
## CONTRACT — R-E-a (E2 charisma + positive-Resilience mechanisms + 3 live latent-bug fixes) — 2026-06-18 (attended)

**ROUTE: fresh in-session subagent | high reasoning effort** (builder; switched from Codex CLI for reliable prompt-passing — the prompt's backticks/`$` corrupt a shell-arg dispatch; matches the L10-4/L10-5 subagent-builder precedent). Auditor = Opus 4.8 Captain (independent; ≠ builder). Branch codex/franchise-v1-next. BUILD-DARK (whole L9b gated OFF behind `isFranchisePhase2TraitsEnabled`). First R-E ticket (the L9b trait-reality REBUILD; E1 deferred to R3 since the Noodle-Arm cut removed its near-term consumer; E3 = R-E-b next). Source of truth: ratified `TRAIT_MEASUREMENT_SPEC.md` §0.6/§0.7/§0.8 (committed `d71767aa`).

**GOAL:** in `src/engines/traitAcquisition.ts` ONLY — (1) fix 3 currently-built traits whose personality tilts are silently dead, and (2) add two DORMANT tilt mechanisms to `buildProposalBase`. (A) §0.7 live-bug fixes: add `'Cannon Arm'`+`'Durable'` to `POSITIVE_IMAGE_TRAITS`, `'Injury Prone'` to `NEGATIVE_IMAGE_TRAITS`, `IMAGE_DRIVER_SETS['Cannon Arm']=['COMPETITIVE']` (no driver for Durable/Injury Prone). (B) E2 charisma: `charismaSwing:0.30` in tuning + `CHARISMA_SENSITIVE_TRAITS={'K Neglector'}` + `charismaTilt = has ? 1 - centered(charisma,0,100)*charismaSwing : 1` (low charisma raises; mirrors resilienceTilt). (C) E2 positive-Resilience: `RESILIENCE_POSITIVE_TRAITS={'Composed','Gets Ahead'}` + `resiliencePositiveTilt = has ? 1 + centered(resilience,0,100)*resilienceSwing : 1` (high resilience raises). (D) add `charismaTilt`+`resiliencePositiveTilt` to `TraitChangeProposal.factors` + the returned factors. Both new tilts default ×1.0 so no other trait's P changes; K Neglector/Composed/Gets Ahead are NOT added to BUILDABLE_TRAITS (R1/R2) → dormant, tested by hand-built candidates.

**HARD CONSTRAINTS:** edit ONLY `traitAcquisition.ts` + `__tests__/traitAcquisition.test.ts`; do NOT touch traitCandidateBuilder/traitRealityScorer/BUILDABLE_TRAITS/any store/any `*.md`; no git-add/commit/doc-edit; reuse existing `centered`/`clamp01`; preserve all behavior except the 3 named traits' tilts.

**VERIFICATION (builder):** `NODE_ENV= npx tsc --noEmit` (0); `NODE_ENV= npx vitest run src/engines/__tests__/traitAcquisition.test.ts` (all green); report every changed path + count + passing count. (Host full build+suite gate = auditor.)

**Full contract text (dispatched prompt):** `/tmp/re_a_codex_prompt.md`.

**Status:** ✅ VERIFIED + COMMITTED (2026-06-18, attended). Fresh in-session subagent built → Opus 4.8 Captain independently audited (≠ builder) from the actual diff: VERDICT VERIFIED. Diff = EXACTLY the 2 FILE LIST files; charismaTilt (low charisma → ×1.15 raises K Neglector) + resiliencePositiveTilt (high resilience → ×1.175 raises Composed/Gets Ahead) directions correct; both default ×1.0 so no other trait's P changes (default-guard tests confirm); 3 live-bug image-set fixes (Cannon Arm/Durable/Injury Prone) correct; factors shape extended + existing exact-shape assertion updated; nothing added to BUILDABLE_TRAITS (dormant via the 2 new sets). Host gate: `NODE_ENV= npm run build` exit 0 + full suite **7,568/438, 7,566 pass / 2 characterized fail** (`wpaRuntimeBoundary` + `franchiseManualSmokeFixture`), ZERO new reds (+9 tests / +0 files = the new R-E-a cases). Committed `9eeb69d5`. NEXT = R-E-b (E3 re-evaluate-to-drop).

---

## CONTRACT — R-E-b (E3 re-evaluate-to-drop: displacement-currency unification) — 2026-06-18 (attended)

**ROUTE: fresh in-session subagent | high reasoning effort** (builder). Auditor = Opus 4.8 Captain (independent; ≠ builder). Branch codex/franchise-v1-next. BUILD-DARK. Second R-E ticket. Source of truth: `TRAIT_MEASUREMENT_SPEC.md` §0.1/§0.8 (committed `d71767aa`).

**SCOPE FINDING:** the no-personality-only rule SHRANK E3 — the builder already emits candidates for held traits with a current data signal, and lose-low already drops them, so "nothing sticky" is already functional for the data-signal case (no personality-base-P machinery needed). The one real gap is the **displacement currency**: §0.1 mandates P as the single yardstick for BOTH lose-low AND the 2-trait-cap displacement, but the displacement (`reconcileGainProposals`/`getWeakestHeld`) ranks the weakest held by the stale supplied `HeldTrait.strength`, while gains compare as the recomputed post-combiner P.

**GOAL:** in `src/engines/traitAcquisition.ts` ONLY — capture each held trait's recomputed P from the `computeTraitAcquisition` loop into a `heldProbabilityByTrait` map, pass it to `reconcileGainProposals`, and rank displacement (`getWeakestHeld` + the displacement gate ~line 381) by an `effectiveHeldStrength(held) = heldProbabilityByTrait.get(name) ?? normalizeHeldStrength(held.strength)` resolver — so a held trait is displaced by its RECOMPUTED P, falling back to the supplied strength only when it has no candidate this cycle. No change to lose-low / gain threshold / opposite-pair / buildProposalBase / the hook / the builder / BUILDABLE_TRAITS.

**TESTS:** KEY — at the 2-trait cap, the held trait with the LOWER recomputed P is displaced even when its supplied strength is HIGH (proves P, not stale strength, drives displacement); fallback — a held trait with no candidate ranks by supplied strength; regression — lose-low intact, thin-signal held trait not spuriously dropped. Existing displacement tests may legitimately change expectation (builder states which + why; no weakening).

**HARD CONSTRAINTS:** edit ONLY `traitAcquisition.ts` + `__tests__/traitAcquisition.test.ts`; no other file, no `*.md`, no git-add/commit.

**VERIFICATION (builder):** `NODE_ENV= npx tsc --noEmit` (0) + `NODE_ENV= npx vitest run src/engines/__tests__/traitAcquisition.test.ts` (all green); report changed paths + counts + any existing-test expectation changes. (Host full build+suite = auditor.)

**Full contract text (dispatched prompt):** `/tmp/re_b_codex_prompt.md`.

**Status:** ✅ VERIFIED + COMMITTED (2026-06-18, attended) → **R-E COMPLETE** (E1 deferred to R3). Fresh in-session subagent built → Opus 4.8 Captain independently audited (≠ builder) from the actual diff: VERDICT VERIFIED. Diff = EXACTLY the 2 FILE LIST files; displacement now ranks held traits by recomputed P via `effectiveHeldStrength = heldProbabilityByTrait.get(name) ?? normalizeHeldStrength(strength)`, captured right after the sufficiency check (so thin-signal held traits correctly fall back); lose-low/gain/opposite-pair/buildProposalBase/hook/builder untouched. The KEY test proves the P-over-strength flip (fails on old code); existing displacement tests unchanged (they hit the fallback path). Host gate: `NODE_ENV= npm run build` exit 0 + full suite **7,572/438, 7,570 pass / 2 characterized fail** (`wpaRuntimeBoundary` + `franchiseManualSmokeFixture`), ZERO new reds (+4 tests / +0 files). **⇒ R-E COMPLETE (R-E-a `9eeb69d5` E2+3 live bugs · R-E-b E3 displacement-currency).** NEXT = R1 (clean outcome proxies → BUILDABLE_TRAITS: strikeout family, Big/Little Hack, Slow Poke/Sprinter, Easy Target, Mind Gamer, Distractor, Base Rounder, Pick Officer, Easy Jumps, Utility, Two Way ×3, Crossed Up, Bunter).

---

## CONTRACT — R1-a (clean R1 outcome proxies: strikeout family + DP/FC/walk + steal-allowed) — 2026-06-18 (attended)

**ROUTE: fresh in-session subagent | high reasoning effort** (builder). Auditor = Opus 4.8 Captain (independent; ≠ builder). Branch codex/franchise-v1-next. BUILD-DARK. First R1 ticket — the gap-free subset (the gap traits — Big/Little Hack, Base Rounder, Distractor, Utility, Two Way, Crossed Up, Bunter — wait on JK rulings → R1-b). Source of truth: `TRAIT_MEASUREMENT_SPEC.md` §0.6.

**GOAL:** add 9 traits to `BUILDABLE_TRAITS` in `traitCandidateBuilder.ts` with new per-PA rate signals (clean proxies, no design gaps), + K Neglector's image-set membership in `traitAcquisition.ts`. Traits: **K Collector** (pitcher K-rate), **K Neglector** (pitcher 1−K-rate), **Whiffer** (batter K-rate), **Tough Out** (batter 1−K-rate), **Easy Target** (batter K-rate), **Slow Poke** (batter DP-rate), **Sprinter** (batter FC-rate), **Mind Gamer** (batter walk-rate), **Pick Officer** (pitcher 1−opposing-steal-success), **Easy Jumps** (pitcher opposing-steal-success). Strikeout detection uses the FULL K-family `{K,Kc,Ꝁ,D3K,WP_K,PB_K}` (not just K/Kc). Denominator = per PA (documented default). New `addOutcomeRateSignals` fn (mirrors addArmSignals) + a pitcher-keyed extension of `addStealSignals` (via `runnerAttribution.pitcherId`); register in `buildRawSignals`. Acq §0.7 delta: `'K Neglector'` → `NEGATIVE_IMAGE_TRAITS` + `IMAGE_DRIVER_SETS['K Neglector']=['TIMID','DROOPY']` (charisma factor already in from R-E-a). Role-bucketed pools + scorer unchanged.

**HARD CONSTRAINTS:** edit ONLY `traitCandidateBuilder.ts` + `traitAcquisition.ts` + their 2 test files; do NOT add the R1-b gap traits; do NOT touch `traitRealityScorer.ts`/scorer/peer-pool/`buildProposalBase`/any store/any `*.md`; reuse existing rate helpers; no git-add/commit.

**VERIFICATION (builder):** `NODE_ENV= npx tsc --noEmit` (0) + `NODE_ENV= npx vitest run` on the 2 test files (all green); report changed paths + counts + any existing-test changes. (Host full build+suite = auditor.)
