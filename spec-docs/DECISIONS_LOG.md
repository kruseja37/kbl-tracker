...
- The IV engine prices traits **as if every trait is Level 2.** So the scout's perceived value should nudge **DOWN** for a player whose traits will be **L1** on the joining team (team lacks the same-chemistry count to reach L2 for his traits) and **UP** when the GM is locking in **L3** (7+ same-chemistry). "Not huge, but the L1↔L3 spread is actually large." Research COMPLETE (runs wqv2dgtiz + w4pmdw4ry): quantified delta = the game's x1/x2/x4 (positive trait ~halves at L1, ~doubles→2× at L2→L3; workbook `ImportedTraits` shows ~3× L2 — a discrepancy flagged); the L2 assumption traces to the XBL `Restrict Teams to Level 2 Chemistry = TRUE` rule, absent in franchise mode. **→ FULL findings + the 11-point decision surface housed in the dedicated `spec-docs/CHEMISTRY_TRAIT_POTENCY_VALUATION_SPEC.md` (research/understanding doc, NOT ratified).** JK to work the decision surface (§9) before any build. *Open:* engine-route vs scout-route, price-vs-flag, model the call-up ripple, un-pin-hitter-raw vs keep-oracle-frozen, the workbook-vs-game L3 ramp, the adjustment magnitude (sim-tune RB-16).
