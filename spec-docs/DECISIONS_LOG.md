...
- **A — pitch-type (user tags pitch on big hits / Ks / BBs; `enrichment.pitchType`):** **Fastball = {4F, 2F, CF}; Off-speed = any OTHER pitch ({FK, CB, SL, CH, SB}).** *Elite-<pitch>* (pitcher) = Ks attributable to that specific pitch, rate vs pitcher peers (T-9). *Fastball Hitter / Off-Speed Hitter* (hitter) = batting performance on fastball- vs off-speed-tagged pitches, rate vs hitter peers. **Captain to RECOMMEND the per-pitch performance formula** (combine big-hit / K / BB outcomes into a peer-ranked score).
...
- **F — bespoke:** *Base Jogger* = **the OPPOSITE of Base Rounder — REVERSE its logic** (deep hits that only net a single / advancing fewer bases than expected on the bases). *Metal Head* = killed pitchers + nut shots **delivered by the batter** (comebacker-injury + nut-shot events); league leaders get a chance — a **protective** trait. *Wild Thing* = wild pitches that advance baserunners (the between-at-bat WP events) + dropped-third-strike-WP (the `D3K_WP` quickbar button), rate vs pitcher peers. *Workhorse* = an **ELITE** trait tied to relatively-high **innings-pitched-per-game** for either starters OR relievers (role-split rate vs peers).
