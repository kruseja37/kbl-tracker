...
### 2026-07-11 (UNIFYSETUP design): one setup screen, two format adapters — no setup-page format switcher

**JK's directive, verbatim intent:** the snake draft's setup screen should look and work exactly like the auction's — same player-profile depth, same drag-to-rank board, same tax-aware readiness feel — not a separately-maintained, less-polished page bolted on beside it. **The ruling (captain, after an adversarial design round with Codex 5.6):** unify on the auction's existing setup screen as ONE shared pipeline with two thin format adapters, not a rebuilt auction-plus-a-snake-card. Which league format a league was created as decides which adapter renders (auction-only concepts like shill count and reserve pricing show only under the auction adapter; snake-only panels — player versions, GM/hotseat seat declarations, seed order, and an archetype-honest affordability proof — show only under the snake adapter); there is deliberately NO in-page switcher to change a league's format on the setup screen itself (format changes belong in League settings, out of scope here). The auction adapter must be byte-behavior-identical to the pre-existing auction screen, proven hunk-by-hunk in the audit; the one sanctioned shared-surface change is that every player row in both formats now opens the same full profile popover (closing a gap where the snake side's version was slimmer). **Rationale:** avoids maintaining two visually and behaviorally diverging setup experiences going forward, and directly answers JK's "why does this feel bolted-on" complaint by making the polish genuinely shared rather than copied. Full record, including the design's strict ordering law (player versions must be chosen before the pool locks) and the frozen-IV law (the snake adapter always reads the locked pool's frozen prices, never live player data): `spec-docs/contracts/CONTRACT_UNIFYSETUP_2026-07-11.md`.

### 2026-07-11 (machine-load test protocol): a 3-attempt solo-pass grace for PRE-EXISTING characterized test files only, under proven heavy concurrent load

**The problem:** during the walkthrough-wave builds, a pre-existing (already-characterized, unrelated to the current lane) test file intermittently failed even when run alone — not because of a real bug, but because the build machine was running several other heavy automated build lanes at the same time. The captain proved this by running the exact same file on the same untouched code twice: once fully green, once red on two different tests — a load-contention signature, not a real regression. **The ruling:** a PRE-EXISTING characterized file (this specific test family, plus two other named flaky files already known from earlier sessions) counts as GREEN if any one of up to three solo attempts passes the whole file cleanly, with every attempt's real output recorded in the report. This grace applies ONLY to those specific, already-known-flaky, pre-existing files — it never applies to any new test written or migrated within the current build lane, which must be reliably green on its own without needing multiple tries. An identical failure on all three attempts is treated as a real, blocking problem, not covered by the grace. **Rationale:** distinguishes "the machine was busy" from "the code is actually broken" without ever lowering the bar for new work. Full record and the exact attempt ledger it produced: `spec-docs/contracts/CONTRACT_UNIFYSETUP_2026-07-11.md` (Amendment 1).

### 2026-07-10/11 (S7 farm-morale seam): the farm-snake draft must feed its own morale input, not the auction's

**The finding:** the S7 closing-gauntlet test (a real integration test driving a full 8-club snake league — both drafts, trades, staffing, franchise init — through actual storage) caught a real production defect on its very first run: every farm-snake-drafted player reached the franchise with the correct hidden rating state and frozen slot salary, but NONE of them got a draft-day morale baseline. Root cause: `franchiseInitializer.ts`'s snake branch only ever knew how to read an auction-shaped completed farm session; it had no code path at all for reading the separately-stored completed farm-SNAKE session, so `draftFreezeInputs.ts` (which builds the morale inputs) never saw those 80 players. Per the S7 contract's own law ("if the pipeline breaks, the finding is the deliverable — do not heal it mid-pipe"), the builder stopped and reported the gap with file:line evidence instead of patching around it. **Captain's ruling (Amendment 1):** grant the fix as a scoped seam, not a rewrite. `draftFreezeInputs.ts` gains one new optional input, a completed farm-snake session, keyed on the player's ABSOLUTE draft slot rather than the acquiring club or pick-execution order — so a mid-draft trade can never scramble whose frozen salary/morale attaches to whom. The franchise initializer's snake branch alone gains a fail-closed read of that session, accepting it ONLY when explicitly tagged as the completed FARM phase (anything else — including a stray non-FARM record at the same lookup key — is ignored rather than silently treated as valid). The existing auction branch is left completely untouched, byte-for-byte, so auction farm morale cannot regress. **Verification discipline:** the original repro test file was proven SHA-identical before and after the fix (nothing was changed to make it pass), and it now shows all 256 expected player-morale rows (176 MLB + 80 farm) alongside every other season-readiness check. Independent opus audit: APPROVE, re-derived the fix from scratch including a mutation attack on the absolute-slot key. Full record: `spec-docs/contracts/CONTRACT_S7_GAUNTLET_2026-07-10.md` (present on branch `codex/snake-s7-gauntlet` as of this entry; not yet merged to `main` — PR #75 is open).

### 2026-07-10 (S6 farm slot table): the frozen farm-salary geometry is a captain product ruling, not an engineering default

**The decision:** the S6 farm-snake draft needed a fixed, known-in-advance salary for every one of its 80 slots (no live bidding — it's a snake draft). The captain ruled the exact geometry directly, as a product decision: the slot table decays geometrically such that the FIRST (most valuable) slot costs exactly 3× the LAST slot's salary, and the full table's rounded dollar sum equals 75% of all clubs' farm budgets added together — leaving 25% of total league farm-budget headroom unspent by design, matching the same cushion philosophy used elsewhere in the draft economy. The table is frozen the instant the farm session is created (not recalculated mid-draft, so a trade or a slow pick can never change what a slot is worth). **Rationale:** a known, unchanging price per slot is what lets the trade-guide math (S4's exact affordability arithmetic) work for farm trades exactly the same way it works for MLB trades — there is no live-bid uncertainty to hedge against. Landed in `spec-docs/contracts/CONTRACT_S6_FARM_ROOM_2026-07-10.md` as PR #73 (merge `c3486ea2`); independently audited APPROVE.

### 2026-07-10 (S5 companion deployment model): v1 companions are the owner's own hardware, one account — guest devices are v2

**The decision:** S5's builder correctly identified that letting a FRIEND's own personal device (a phone the league owner doesn't own, not signed into the owner's account) join a draft room via just a typed-in room code is real, unbuilt security infrastructure — anonymous device claims, a room-scoped access list, and a realtime layer, none of which exist today. Rather than build that infrastructure under time pressure, the captain ruled the v1 deployment model to match how the game is actually played at a real table: every companion device (a second iPad, a spare phone) is the LEAGUE OWNER'S OWN hardware, signed into the SAME account as the main screen, physically handed to a friend for the length of the draft. Under that model, the existing account-level device sync already moves the whole session across every device with zero new plumbing; the claim/approval screen keeps its existing look and flow, but its real job is reframed honestly as TABLE-CONSENT (which seat a given device in the room is currently allowed to show), not a security boundary between strangers. True guest-device access — a friend's own phone, no shared account, needing a real server-side access list — is explicitly booked as a v2 lane, not built now. **Rationale:** ships the feature that matches today's actual use case (one owner's hardware, physically shared at a table) without silently overclaiming security it doesn't have, and without blocking the whole snake v1 program on infrastructure nobody asked for yet. Landed in `spec-docs/contracts/CONTRACT_S5_COMPANIONS_2026-07-10.md` as PR #72 (merge `26d74f70`); independently audited APPROVE.

### 2026-07-10 (TAXSWING): single-assignment swing-arm luxury tax — every arm taxed exactly once

**JK ruling, verbatim intent:** the tax engine (`luxuryTax`, `src/engines/leagueConstruction.ts`) was counting an SP/RP swing arm in BOTH the rotation and bullpen tax groups at once — his ratings could be charged twice, and even with a full four true starters rostered, a swing arm could still inflate the rotation charge though he'd never actually start. JK ruled: count every arm EXACTLY ONCE, by what the roster actually needs. **The rule built exactly to that ruling:** the rotation tax group is every pure SP, topped up — only if short of a full four-man rotation (`LEGAL_ROSTER.startingPitchers`, not hardcoded) — by the best SP/RP swing arms (descending mean VEL/JNK/ACC, ties broken by player id ascending); the bullpen group is every RP + CP + any swing arms not promoted; no arm ever appears in both groups. **Rationale:** (1) closes the double-count outright — a roster-need-based single assignment, not a stat-double-dip; (2) closes a loophole a simpler "always tax swing arms as relievers" rule would have opened (an all-swing rotation with zero pure SPs would then be taxed as if it had no starters at all, which is false); (3) the assignment lives inside the one shared settlement function, so the auction, the snake draft, both settlement bills, the affordability/legal-finish math, marginal tax, and every advisory surface all inherit it automatically — no caller-side reimplementation. **A real consequence, not just a bookkeeping fix:** a negative marginal tax is now reachable — drafting a 4th true starter can demote a swing arm out of the rotation group and LOWER a club's total bill, which the old double-count rule could never produce. **Blast radius handled honestly, not fixture-bent:** six archetype identities (Shift-Era Suppressors, Launch & Leather, Call Your Shot, Defense First, Lazer Guns, Track Stars) turned out to be quietly leaning on the old double-count bug to hit their tuned ±10% value-parity targets; JK's captain retuned all six by simulator-verified multiplier changes only — same identity, same flavor, same bands, parity restored with margin — rather than weaken the parity check itself. Full builder execution record, the Gate-5 STOP it correctly raised, the retune ruling (Amendment 1), and the independent audit are all in `spec-docs/contracts/CONTRACT_TAXSWING_2026-07-10.md`. Landed as PR #69 (merge `aa1de4a6`); a follow-up captain seam (PR #70, `b2bdc6b6`) re-synced the snake desk's tax explainer to this rule and added plain "YOUR TAX BILL GOES DOWN" copy for the new negative-marginal case.

### 2026-07-10 (S4 trade guide, dispatch-time ruling): the guide guards money and legality — pick-timing risk stays a GM judgment call, shown as fact, not probability

**JK ruling:** the S4 trade guide (dispatched this session; contract lives on its own branch, not yet merged to `main`) is scoped to guard what the GM cannot easily judge for himself — cap-legality and money consequences of a trade — and to stay OUT of predicting what he legitimately should judge himself. Concretely: the guide computes and shows hard facts a trade changes (dollars, cap headroom, legal-roster consequences, and mechanical draft-order effects such as "YOUR NEXT PICK MOVES: #9 → #14"), but it does NOT attach a probability, a confidence score, or any other prediction to whether moving up or down in pick order is good or bad for that GM's build — that judgment is left entirely to the GM. **Rationale:** the guide's authority is legality/money, where the app has ground truth; pick-timing value is opinion the app has no better claim to than the GM playing it, so dressing it up as a computed probability would overstate the app's own certainty and erode trust in the parts of the guide that ARE ground truth. This ruling governs the S4 build; see `spec-docs/contracts/` for S4's own contract once it lands on `main` (not yet confirmed as of this entry — see `V1_BUILD_STATUS.md` §0 item 2ai and `SESSION_LOG.md` 2026-07-10 later-session entry).
