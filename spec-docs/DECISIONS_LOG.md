...
- **⚠ REVERSED → RELATIVE peer-percentile, NOT absolute** (JK corrected the earlier absolute call). A rating moves on the player's percentile vs peers, because ABSOLUTE (own-vs-expected) is what *causes* league inflation (strong-hitting league → everyone beats expectations → everyone rises → rich-get-richer); RELATIVE is self-correcting (if all OFs rake, none stands out → none inflates). Re-aligns ratings WITH traits on the cohort principle.
...
- **DH removed from the position model (RULED):** not a franchise position; remove everywhere the *position* appears (consistent w/ the 2026-06-20 "no DH/UTIL" Mode-1 ruling). Verified (`wenf4w3ee`): the FROZEN `iv_oracle.json` has ZERO DH (DH→1B is value-neutral) → **no re-bless**. Code removal = a separate scoped ticket (~9 Position type defs in lockstep + `ivEngine.ts:205` keystone + the one DH player `yankeesPlayers.ts:70` + lineup/sub plumbing + ~38 test pins). **DH-the-LEAGUE-RULE** (`leagueConfig.ts usesDesignatedHitter` — do pitchers bat) is a SEPARATE concept; left untouched pending a distinct JK ruling.
