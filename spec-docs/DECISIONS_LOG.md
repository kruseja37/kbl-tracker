...
- **BENCH cohort:** cohort = position-group × ROLE (starter/bench). Verified the starter/bench split ALREADY EXISTS + is the live cohort key (`franchiseEffectivePosition.ts` RESERVE pool; a hitter starting <40% of team games = RESERVE; derived from per-game lineup replay, recomputed per checkpoint, never stored). Bench players are ALREADY scored vs bench peers, and **auto-promotion** (start-share crosses 0.40 → leaves RESERVE → measured vs starters) is existing behavior — the emergent "played into the lineup" arc needs NO code change. GREENFIELD = split the single all-bench RESERVE pool into **bench-IF / bench-OF** (2 coarse buckets — per-position bench is too thin, collapses below the 6-peer floor; contained edit to 2 files + the pool-key type). Threshold tunable.
- **PITCHERS HIT/RUN/FIELD (no arm):** pitchers' power/contact/speed/fielding ratings progress/regress on the same over-expectation model, calibrated within the PITCHER pool (the Two-Way-vs-pitcher-pool precedent extends to all pitchers now they always bat); **no arm** (all 205 pitchers have `arm=0` structurally). SP/RP split matters for pitching ratings; for hitting, SP-vs-SP + the min-sample gate handles it (RPs barely bat → their hitting freezes, correct). Substrate fully present (pitchers already accumulate batting+fielding rows, no exclusion guard; all carry batterRatings; engine is rating-key-agnostic). GREENFIELD = 4-signal decomposition + SP/RP peer-pool keying + checkpoint wiring.
