...
| Playoff confirmation | FranchiseHome can review final standings, resolve W-L ties by run differential where possible, preserve eliminated-team evidence, create brackets only from confirmed seedings, and require bracket/start confirmation before playoff play. | Same-record/same-run-differential qualifying ties block for manual seeding resolution. Season rollover, Mode 3/offseason, awards, and playoff handoff migration remain deferred. | Smoke the playoff confirmation path in real app before approval. |
