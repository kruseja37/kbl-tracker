...
Within a family the through-line tool is constant but position/grade/secondary/magnitude vary (no repeated spreads); across families the *same* grade yields very different players (the B 1B-slugger vs B CF-speedster vs B SS-wizard). *(The pitcher examples' batter ratings were floored at 20 for analyzer ISOLATION — that is NOT the generation target; see §5.7.)*

### 5.7 Pitcher non-pitching ratings — varied + real-anchored, NOT grade-coupled (RULED; audit `wdsaa71bp`)
**Pitchers bat, run, and field (§13 no-DH), so they must NOT be generated as uniform-worthless hitters.** Audit found a real gap (predictability, the thing JK flagged): the generator DOES draw pitcher batter ratings, but (a) the grade solver's uniform shift moves **all 8 attributes together**, so a pitcher's bat/run/field rise & fall **in lockstep with his stuff** (low-grade → batting floored ~20; high-grade → batting dragged up) — perfectly grade-correlated, no texture; (b) centers are ~2× too high (power/contact 35 vs real 15.8/16.3) and **arm is drawn at ~55 instead of 0**; (c) σ=7 symmetric can't reproduce the real skewed spread.
- **Real 205-pitcher anchor (computed):** power mean 15.8/σ15 (median 11, tail→77) · contact 16.3/σ16 (→99) · speed 26.5/σ21 (34/205 ≥50) · fielding 56.3/σ22 (59/205 ≥70) · **arm = 0 for ALL 205** · **3/205 are two-way bats** (power≥50 & contact≥50) — the emergent good-hitting pitcher.
- **RULED:** (1) **Decouple** — the grade-solver shift applies to **velocity/junk/accuracy ONLY** for pitchers; leave power/contact/speed/fielding/arm at their independently-drawn values (grade is driven by stuff; the bat survives as its own texture). (2) **Re-anchor** the pitcher batter draw to the real distribution with a **right-skewed** draw (low median, long tail) — power ~16/σ15, contact ~16/σ16, speed ~26/σ21, fielding ~56/σ22, **arm forced to 0** — so good-hitting / two-way pitchers emerge organically (~1.5%). (3) **Regression test** that generated pitcher batting matches the real-205 anchor (means/σ, arm=0, nonzero two-way emergence). Generator-only; no DB change. (Build task B14. Feeds the §4A pitcher hit/run/field ratings-adjustment model — same low-but-varied bat texture.)
...
| **B14** | **Pitcher non-pitching ratings — decouple + re-anchor (§5.7).** In `applyRatingShift` (`:638-654`) apply the grade shift to velocity/junk/accuracy ONLY for pitchers (leave bat/run/field/arm at base). Re-anchor the pitcher batter draw (`:605-623`) to the real-205 skewed distribution (power~16/σ15, contact~16/σ16, speed~26/σ21, fielding~56/σ22), **force arm=0**. Add a regression test vs the real anchor (means/σ, arm=0, ~1.5% two-way emergence). Stops grade-coupled / worthless-uniform pitcher bats; enables good-hitting/two-way pitchers. | `prospectScoutingDraftEngine.ts:605-623, 638-654` | **BUILD** |
