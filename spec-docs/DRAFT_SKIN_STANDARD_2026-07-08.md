...
## §5 JK forks — RESOLVED (JK 2026-07-08)
1. **The live bidding stage: ONE LANGUAGE EVERYWHERE.** JK ruled against the captain's keep-the-contrast recommendation: `AuctionStage` + WhisperPanel + both floors' inner UI CONVERT to the hard-edge standard (thick borders, hard offset shadows, zero radius) — the soft-premium `--auc-*` visual family is retired for the draft journey. Retained from the soft system: its layout/structure, its class names and test hooks (whisper-verdict, chips, testids — the reskin is treatments, not DOM renames), and any purely-functional tokens. This makes the stage conversion the largest sweep item (L, both floors + WhisperPanel + AuctionStage + auction-theme.css value rewrite onto ballpark equivalents).
2. **FranchiseSetup wizard: OUT** — separate franchise pass against `franchise-theme.css`, per captain recommendation.
...
**§7 RATIFIED CLASSIFICATION (captain, 2026-07-08 — the sweep builds from this + the full inventory in the session record):**
- GATE BEHIND HELP (relocate verbatim): DraftSetup — universe-sources explainer (:3560), pool-quality explainer (:4302 LOCKED), room-check explainer (:3373 LOCKED), both Cap Fit methodology lines (:3321/:3337 LOCKED); ArchetypePicker — both identity explainer paragraphs (:172-174; wire the parent showHelp prop, no second button); AuctionStage — the farm fog line (:649) and both scout-band legend lines (:619/:708 — ruled Help-class, no new collapse mechanism); EndOfDraftStaffing — the instruction banner (:216), and the page gains its own top-right Help button.
- SPLIT (number stays visible, lecture moves to Help): Cap Fit fused line (:3344 — {summary} stays, the static "Pool quality and salary cap are separate…" clause gates); design-first stray notice (:3512 — the {N}+names warning stays, the "a drawn pool contains only what the draw picked" clause gates).
- RULED ALWAYS (stay put): hub card subtitles (nav labels — hub gets NO Help button); all state-triggered warnings/banners (overflow rail, stale-pool, legality, settle/handoff confirmations incl. their rule clauses); CPU-turn fallback; farm need-line; empty states.
- REVERSE FIX: AuctionStage phase-label pill (:220) is ALWAYS content wrongly hidden behind Help — becomes permanently visible.
- NO SITUATIONAL/collapse mechanism this sweep — no ratified case met the bar; revisit when one does.
- Mechanics: LOCKED strings relocate with their test assertions updated to the help-open state; ArchetypePicker snapshot regenerated deliberately; sweep dispatches AFTER the BOARDFIX1 lane lands (DraftSetup file overlap).
