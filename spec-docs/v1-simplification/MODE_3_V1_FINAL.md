# MODE 3: OFFSEASON WORKSHOP — Gospel Specification — V1 BUILD DOCUMENT
...
- Simulate AI-vs-AI games (deferred entirely — not in v1)
...
| 9 | Offseason Trades | All teams | Trade window with no salary matching; user-initiated trades; waiver wire |
...
> The `offseasonScope` setting is configured during Mode 1 (League Builder §2). Three values: `'default'` (uses per-phase scope assignments from the table above), `'human-only'` (overrides all 13 phases to human-only), `'all-teams'` (overrides all 13 phases to all-teams).
...
> **v1 Scope:** Game Night Mode only. Deferred to v2: Streamlined Mode (batch processing, condensed UI, ~60% time reduction).

The Offseason Workshop uses **Game Night Mode**: full ceremony experience with animations, dramatic reveals, dice rolls, wheel spins, and card flips. Each award, retirement, and draft pick gets its moment.
...
Close the completed season: finalize standings, select postseason MVP (if playoffs occurred), distribute championship rewards, and reset all player mojo and fitness to baseline.
...
- Fame bonus: +3 fame to all championship team players
- Morale boost: +20 morale to championship team players
- Beat reporter note: championship narrative entry logged

**Screen 5 — Mojo + Fitness Reset Confirmation**
All players across all teams reset to Normal mojo AND Fit fitness state (neutral baseline for both systems). Display confirmation with count of players affected for each.
...
If no playoffs occurred, skip Screens 2-4. Flow: Final Standings → Mojo + Fitness Reset → Season Archive → Phase Complete.
...
| Championship bonuses | playerStore, fameStore | +3 fame, +20 morale |
| Mojo + fitness reset | playerStore | All players → Normal mojo, Fit fitness |
...
> **v1 Scope:** All 13 award screens with full ceremony. Trait wheel spin for award winners (60%) and top performers (30%). Team Captain moved to §15/Phase 13. Deferred to v2: 5% regular player end-of-season development trait lottery.
...
> **v1 Scope:** Full expansion draft + stadium changes from existing SMB4 stadiums. Deferred to v2: Custom stadium creation (no basis in SMB4).
...
2. **Stadium picker** — Choose from available SMB4 stadiums
...
> **v1 Scope:** Full retirement ceremony with three dice roll rounds. Retired stays retired in v1. Deferred to v2: Un-retirement via future draft classes.
...
**Three dice roll rounds per team:** Each round, remaining retirement-eligible players get a dice roll. If a player retires in round 1, they're done; survivors roll again in rounds 2 and 3. This increases overall retirement rate, especially for younger rosters where individual probabilities are low. Target remains 1-2 retirements per team per season.
...
> **v1 Scope:** Full 2-round FA with dice roll + personality system, 5-screen flow. Removed: §8.4 Free Agent Pool Signing (incompatible with 1-for-1 exchange model). Fallback rule revised: user selects exchange player if no ±30% match.
...
1. Expand to ±30%
2. If still none, user selects which player to send from the receiving team's roster

**Screen 5 — Round Summary**
All moves from completed round displayed. After Round 2, shows complete overview with team impact table showing net salary change, roster composition change, and chemistry impact.
...
> **v1 Scope:** 8-screen draft flow (Screen 1 Pre-Draft removed — retired stays retired). Traits and true ratings hidden at draft, revealed at call-up. Full scouting accuracy system with fat-tail deviation.
...
### 9.2 Screen Flow (8 screens)

**Screen 1 — Draft Class Preview**
...
> **v1 Scope:** 7-screen trade flow (user-initiated only). AI teams respond (accept/reject/counter) but do not proactively propose. Waiver wire source: players cut during offseason, not retirements. Deferred to v2: AI-initiated trade proposals (Screens 5-6).
...
**AI Trade Logic (AI-controlled teams evaluating user-initiated proposals only):**
| Factor | Weight |
|--------|--------|
| Value assessment | 30% |
| Needs fit | 25% |
...
**Screen 5 — Waiver Wire Claim**
Claim players cut during offseason phases (farm overflow resolution, release decisions from draft/trade roster management) in reverse standings order.

**Screen 6 — Waiver Wire Results**
...
- Traits revealed (hidden at draft per §9 ruling — shown for first time at call-up)
...
**Screen 9 — Team Captain Designation**
Per §4 ruling, Team Captain moved from Awards Ceremony to Phase 13. Highest combined (Loyalty + Charisma) per team on the finalized roster (per Mode 2 §17.6/C-053). Calculated AFTER all roster changes (FA, draft, trades, call-ups/send-downs) so captain is from the actual next-season roster. Captain designation handed to Mode 2 for the upcoming season.

**Screen 10 — Advance Confirmation**
...
- **Team Captain selection** (Phase 13): Loyalty + Charisma hidden modifiers determine captain (per C-053)
...
- Inactive player database integration: retired players stored for HoF eligibility and Almanac records
...
| 9 Trades | all-teams | Auto (AI responds to user-initiated proposals: accept/reject/counter) |
...
> **Note:** V2_DEFERRED_BACKLOG.md is the authoritative deferral record. This table is a quick reference within the gospel.
...
| C-053 | Team Captain: Loyalty+Charisma, no minimums | §15.2 (Screen 9) |
...
| F-132 | Team Captain: v1 scope, Loyalty+Charisma driven | §15.2 |
