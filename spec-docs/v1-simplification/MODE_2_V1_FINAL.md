...
This is the definitive V1 build specification for Mode 2, the Franchise Season. All V2 deferred material has been removed. All spec corrections from Phase A triage have been applied. This document is the complete, standalone specification that developers need to implement Mode 2 for KBL Tracker.
...
13. [Clutch Attribution & Fame Triggers](#13-clutch-attribution--fame-triggers)
...
19. [Fan Morale System](#19-fan-morale-system)
20. [Standings & Playoffs](#20-standings--playoffs)
21. [Schedule System](#21-schedule-system)
22. [Adaptive Standards Engine](#22-adaptive-standards-engine)
23. [Stadium Analytics & Park Factors](#23-stadium-analytics--park-factors)
24. [Franchise Data Flow](#26-franchise-data-flow)
25. [Decision Traceability](#28-decision-traceability)

**Appendices:**
- [V2 Deferred Features (Reference)](#appendix-a-v2-deferred-features-reference-only)
...
interface TransactionEvent {
  ...
  type: 'trade' | 'free_agent_signing' | 'release' | 'call_up'
      | 'send_down' | 'draft_pick' | 'retirement' | 'injury_list';
...
**Spec Correction (Phase A Triage):** §13.6 is relabeled from "Clutch Trigger Stacking" to "Fame Trigger Stacking" to clarify the distinction: WPA measures clutch *performance* (actual game impact), while trigger stacking measures legendary *memory* (moments fans remember). These values route to the Fame accumulator (§8.3) as Fame Boners, not clutch points.

**Rule:** HIGHEST ONLY within categories, STACK across categories.

| Category | Triggers | Fame Bonus |
|----------|----------|-----------|
| Walk-off | Walk-off single/XBH/HR | +2 to +3 |
| RBI Situation | Go-ahead, 2-out, bases loaded | +1 each (highest only) |
| Special Hit | Grand Slam | +2 |
| Count | Hit on 0-2 | +1 |

**Example:** Walk-off Grand Slam = +3 (walk-off) + 2 (grand slam) = **+5 fame points**.

**Attribution:** Batter receives positive fame bonus. Pitcher/fielder/manager receive corresponding fame modifiers based on their WPA attribution role (negative for pitcher/fielder on hit, positive for manager on good call).
...
### 14.7 Juiced State (V1 Simplified)

**Per C-092 & Phase A Triage:** Juiced is a fitness state that the user sets directly from the game interface (like any other fitness state). The v1 engine reads the Juiced state and applies downstream effects (WAR penalty -15%, fame penalties, narrative triggers) but does NOT enforce any eligibility checks or cooldowns. The user is responsible for when to set and unset Juiced.

**v1 Approach:**
- User sees Juiced as an available fitness state option
- User can set it whenever they choose
- Engine applies all downstream effects listed in §14.9 (fame penalties, WAR adjustment, narrative hooks)
- No eligibility gate, no cooldown tracking, no automatic trigger logic

**v2 Deferred:** Juiced eligibility engine (Fit requirement + 20-game cooldown + automatic triggering via random events/traits/narrative). This will be implemented in a future version when the modifier registry and narrative systems are more mature.
...
interface PlayerMojoFitness {
  ...
  gamesAtJuiced: number;                     // Almanac tracking of games at Juiced state
...
**HARD BOUNDARY RULE (V1 Correction):** Random events CAN modify: stats (STAT_BOOST/PENALTY), traits (TRAIT_GAIN/REMOVE), fame (FAME_BONUS/BONER), morale (MORALE_CHANGE), relationships (RELATIONSHIP_FORM/STRAIN), designations, and team-level effects. Random events CANNOT modify: mojo state, fitness state, or Juiced state. These are ONLY set by user observation from SMB4. Random events layer ON TOP of user-set states, never replace them.

```typescript
// Example 1: Stat boost from Hot Streak
const HOT_STREAK: Modifier = {
  id: 'MOD_HOT_STREAK',
  name: 'Hot Streak',
  category: 'MORALE_BOOST',
  scope: 'PLAYER',
  trigger: 'RANDOM_EVENT',
  probability: 0.02,  // 2% per game
  conditions: [{ type: 'MORALE', value: { min: 70 } }],
  effects: [
    { type: 'STAT_BOOST', target: 'BATTING', value: 0.15 },  // +15% to batting line
    { type: 'FAME_BONUS', target: 'PLAYER', value: 1 }      // +1 fame per achievement
  ],
  duration: 5,
  stackable: false,
  maxStacks: 1,
...
// Example 2: Trait gain from narrative event
const VETERAN_LEADERSHIP: Modifier = {
  id: 'MOD_VETERAN_LEADERSHIP',
  name: 'Veteran Leadership',
  category: 'TRAIT_GAIN',
  scope: 'PLAYER',
  trigger: 'NARRATIVE_EVENT',
  conditions: [{ type: 'SEASON_LENGTH', value: { min: 80 } }],
  effects: [{ type: 'TRAIT_GAIN', target: 'PLAYER', value: 'Leader' }],
  duration: 'PERMANENT',
  title: 'Veteran Leader',
  descriptions: ['After leading the team through tough times...'],
  narrativeTag: 'VETERAN_MILESTONE'
};
```
...
### 18.7 Milestone Data Structures
...
function calculateFanMorale(team: Team, season: Season): number {
  const teamPerformance = calculatePerformanceGap(team, season) * 0.50;
  const designatedPlayerScore = calculateDesignatedPlayerPerformance(team) * 0.20;
  ...
  const randomEventScore = calculateRandomEvents(team, season) * 0.10;
  return clamp(Math.round(teamPerformance + designatedPlayerScore + restOfRosterScore + beatReporterScore + randomEventScore), 0, 99);
}
```

**Weights (V1 Corrected):** 50% team performance (W/L vs expectations) + 20% designated player performance (MVP/Ace/Fan Favorite/Albatross performance vs salary) + 10% rest of roster (True Value delta — trading away underperforming contracts improves morale, trading away overperforming players hurts morale) + 10% beat reporter sentiment (narrative tone) + 10% random events (celebrity, drama, life events).

**Spec Correction:** The 10% "rest of roster" factor absorbs trade morale effects from deferred §19. Rather than handling trade penalties only for designated players, fan morale now responds to the entire roster's True Value delta. Trading a low True Value player improves morale; trading a star outperforming his contract hurts it.
...
**V1 Correction:** The SIMULATED status value is removed. All games are either SCHEDULED, IN_PROGRESS, COMPLETED, or SKIPPED. There is no simulation in v1.
...
### 22.3 Game Buttons (V1 UI)

**Per Phase A Triage:** All games show "SCORE" or "SKIP" buttons. There is no SIMULATE button in v1.

- **SCORE**: Opens GameTracker for manual entry (available for all games, including AI-vs-AI matchups)
- **SKIP**: Marks the game as SKIPPED, advancing to next scheduled game

The user can manually score games played between any teams or skip them. No game is ever simulated automatically. The user is the bridge between SMB4 outcomes and KBL records.
...
**Scope:** Only trades are blocked. Free agent signings (if any exist in-season), call-ups, send-downs, and IL moves remain available after the trade deadline.
...
7 spray zones: LEFT_LINE, LEFT_FIELD, LEFT_CENTER, CENTER, RIGHT_CENTER, RIGHT_FIELD, RIGHT_LINE. Each batted ball event records zone, distance, angle, outcome, and batter handedness. Spray charts support heat map visualization with filters for handedness, time range, and individual players.
...
│                                                                 │
│  TransactionEvent (trades, call-ups, IL)                        │
│       ↓                                                         │
│  ├── Roster updates                                             │
...
## Appendix A: V2 Deferred Features (Reference Only)

The following features were deferred to v2 during Phase A triage. They are not built in v1. This section documents them for future reference and planning.

### 26.1 Game Simulation & AI Game Engine (§25 Gospel)
**Status:** Deferred to v2.
- AI Game Engine probability model for simulating games between non-human teams
- SIMULATE button and GameStatus: 'SIMULATED'
- Seeded PRNG for reproducible simulation
- Variance configuration
- Unsimulate capability
- All 4 TypeScript interfaces (AIGameEngine, SimRoster, SimPlayer, SimulationOptions) deferred

**v1 Approach:** All games are manually scored (SCORE button) or skipped (SKIP button). There is no automatic simulation.

### 26.2 Trade Mechanics & Albatross Trade Discount (§19 Gospel)
**Status:** Deferred to v2.
- Albatross 15% trade discount (retained in §17.4 Designation definition but no AI trade logic applies)
- Fan Favorite amplified trade morale penalty
- Designation carryover on trade (intentionally NOT kept — designations do not carry over on trade)
- AI trade generation and evaluation logic

**v1 Approach:** Fan morale effects from trades are handled by the "rest of roster" True Value factor in §20.1 (10% of formula). Trading low True Value players improves morale; trading stars outperforming their contracts hurts morale.

### 26.3 Juiced Eligibility Engine (§14.7 Gospel)
**Status:** Deferred to v2.
- `checkJuicedEligibility()` function with Fit requirement + 20-game cooldown
- Automatic triggering via random events, traits, or narrative
- Database fields: `juicedCooldown`, `lastJuicedGame`

**v1 Approach:** Juiced is a user-set fitness state with no eligibility gates. Downstream effects (WAR -15%, fame penalties) still apply. Fields `juicedCooldown` and `lastJuicedGame` are removed from data schema; `gamesAtJuiced` is retained for almanac tracking.

### 26.4 Juiced Eligibility Criteria
**Status:** Deferred to v2.
- Fit status requirement
- 20-game cooldown between Juiced states
- Random event ("Hot Streak") trigger at 2% per game
- Trait-based triggers
- Narrative-based triggers

### 26.5 Weighted Fielder Inference (§10.2 Gospel)
**Status:** Deferred to v2.
- Secondary fielder probability tiers per direction × play type
- Tertiary fielder probability tiers
- Probabilistic fielder assignment with confidence scoring

**v1 Approach:** Deterministic primary-fielder-only inference (highest dWAR earns the play).

### 26.6 Pitch Count Estimation (§9.8 Gospel)
**Status:** Deferred to v2.
- PITCHES_PER_BATTER_ESTIMATE system (estimated from hit type + pitch sequence)
- Maddux streak detection

**v1 Approach:** Manual pitch count entry per at-bat. If skipped, pitch count remains blank on Fenway Board and Maddux streak is undetectable. Graceful degradation: both features work without pitch count data.

### 26.7 Between-Inning Enrichment Prompts (§4 Gospel)
**Status:** Deferred to v2.
- Auto-prompts for enrichment between innings
- Contextual nudges to fill spray charts, fielding sequences

**v1 Approach:** Play log entry badges serve as sole UI cue for enrichment.

### 26.8 Pitch Type Repertoire Filtering (§4 Gospel)
**Status:** Deferred to v2.
- Filter pitcher's available pitch types by past usage
- Suggest pitches based on pitcher archetype

**v1 Approach:** All 9 pitch types available for every pitcher.

### 26.9 Double Switch Atomic Operation (§7 Gospel)
**Status:** Deferred to v2.
- Single button operation combining pitching change + batting order swap

**v1 Approach:** Pitching change and batting order swap are separate operations (2 steps).

### 26.10 Storage Cost Projections (§8.5 Gospel)
**Status:** Deferred to v2.
- Projections for storage tiers across multiple seasons
- Cloud/cold storage tier cost estimates

**v1 Approach:** Hot and Warm tiers only. No cold storage or cost modeling.

### 26.11 Storage Cost Projections (§26.2 Gospel)
**Status:** Deferred to v2.
- Cold storage tier for user-initiated multi-season archive export
- Cost analysis and storage management UI

**v1 Approach:** Hot and Warm tiers only (current season + history).

### 26.12 WAR Calibration System (§11.6 Gospel)
**Status:** Deferred to v2.
- Multi-season recalibration after 2+ complete seasons
- Minimum data thresholds for recalibration
- Blend rates and confidence tracking
- Post-hoc WAR adjustments with historical replay

**v1 Approach:** No calibration (season 1 only). Fixed constants and formulas used throughout.

### 26.13 Legacy Status Tiers (§18.6 Gospel, removed)
**Status:** Deferred to v2.
- Franchise Cornerstone (2+ seasons, 5+ WAR)
- Franchise Icon (3+ seasons, 10+ WAR, 1+ award)
- Franchise Legend (5+ seasons, 18+ WAR, 2+ awards)

**v1 Approach:** Removed entirely. Dynamic top-10% career thresholds replaced with fixed floor thresholds only.

### 26.14 Season Classification (§22.4 Gospel, removed)
**Status:** Deferred to v2.
- PRIMARY: 75%+ games manually played
- MIXED: 25-74% manually played
- SIMULATED: <25% manually played

**v1 Approach:** Removed. All seasons are PRIMARY by definition (no simulation in v1).

### 26.15 Random Event Catalog (§15 Gospel Gap)
**Status:** Deferred to v2.
- Comprehensive catalog of random events (stat boosts, trait changes, life events, team events, morale shifts)
- Event probabilities and conditions
- Fame event definitions
- Relationship event definitions

**v1 Approach:** Registry architecture exists (§15.1-§15.3). Specific event catalog needs post-triage scoping.

### 26.16 Fame System Canonical Section (§8.3/§13.6 Gap)
**Status:** To be defined during v1 draft consolidation.
- Dedicated section defining: what fame IS, Fame Bonus vs Fame Boner, source combinations, thresholds, downstream consequences
- Fame accumulator (already defined in §8.3)
- Fame Trigger Stacking (moved from clutch, defined in §13.6)
- All fame sources indexed: star play categories (§10.4), fame triggers (§13.6), Juiced penalties (§14.9), milestone fame (§18), designations (§17), milestones (§18.7)

**v1 Action:** Create Fame System canonical section (post-triage) to consolidate and index all fame mechanics.

---

*MODE_2_FRANCHISE_SEASON — V1 BUILD SPEC*
*Generated: 2026-03-05*
*Source: MODE_2_FRANCHISE_SEASON_UPDATED.md (gospel) + MODE_2_V1_DRAFT.md (triage)*
