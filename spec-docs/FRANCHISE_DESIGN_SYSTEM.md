...
| **Type — two voices** | **Real chalk** (`Creamy Chalk`, bundled webfont — CAPS-ONLY, so the board renders **all-caps**, JK-approved board-wide for the scoreboard feel; falls back to `Chalkduster` → `Tox Typewriter` → Courier) for EVERY board label, score, standings number, and button — it reads as hand-chalked, not typewritten (JK ruling 2026-06-26). **Mom's Typewriter** (`moms-typewriter.ttf`) for reporter/newspaper copy ONLY (the words). *(`chalk.otf` stays rejected — childish scrawl.)* |
