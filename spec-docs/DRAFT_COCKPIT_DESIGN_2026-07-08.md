...
- Carried from the honesty pass: real farm board killing the "board's bare" boilerplate (WhisperPanel.tsx:60); lights on farm render ONLY BUDGET + SHAPE; chemistry-fit chip ships dark-first (fork 3, RESOLVED YES).
- **FOG-MATH CORRECTION (captain, 2026-07-08, after W1d audit REJECT):** the farm board ranks AND displays by the scout's **displayedEstimate** (the seeded, jittered, fog-carrying point estimate from scoutValueRange) — NEVER by the band midpoint. The scouted band is symmetric around the true opening ask, so `(low+high)/2` is algebraically the true value and using it cancels the fog exactly (this doc's earlier "range midpoint" wording was the error; the audit caught the leak before merge). Rule of general force: on any fogged surface, no derived quantity may be an exact deterministic function of a true-value anchor.
