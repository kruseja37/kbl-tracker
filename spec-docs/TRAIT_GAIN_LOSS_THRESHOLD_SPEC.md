...
- **✅ RULED (JK 2026-06-22) — the threshold engine SUPERSEDES the EOS "Trait Wheel Spin"; the award-based luck model is DEPRECATED.** The end-of-season profile change becomes simply **one more checkpoint** of the same adaptive engine: at season end, run the trait gain/loss thresholds (this spec) AND the ratings-development pass one final time → that determines the player's profile going into next season. NO separate probabilistic award-ceremony spin, award-winner-60%/top-30%/regular-5% weighting, eye-test ranking, or 15%-negative EOS rule. ⇒ `EOS_RATINGS_ADJUSTMENT_SPEC.md` §449 (the award-luck EOS model, Feb 2026) is **superseded for v1**; the negative-% inconsistency dissolves (one system). **Mode-3 (offseason) is OUT of v1 scope** — redesigned post-v1; for v1 the season-end checkpoint is the only offseason profile-change mechanism. Net cadence = 5 in-season checkpoints + 1 season-end checkpoint, all the same adaptive engine (traits + ratings).
  > **v1 IMPLEMENTATION NOTE (T-7, Captain-verified 2026-06-26):** the engine satisfies "one more checkpoint at season end" with NO separate EOS event. With `checkpointCount = 5` the 20%-grid boundaries are [20%, 40%, 60%, 80%, **100%**], and the **100% boundary IS the final game** — `isCheckpointBoundary(totalGames, totalGames, checkpointCount)` is always true (verified + test-pinned for T = 10/32/162 in `franchiseCheckpointSweepCompute.test.ts`), so the season-end trait+ratings pass runs as that final checkpoint via `processCompletedGame` → `persistDarkTraitGrantForCompletedGame`. The "5 in-season + 1 season-end" phrasing describes **5 grid checkpoints whose last (100%) IS the season-end one — not a distinct 6th event.** (OPEN-DECISION for JK only if a SEPARATE post-final-game EOS pass is ever wanted; today the final-game checkpoint serves that role, so T-7 needed no engine change.)
...
## 8C. Position-mismatch protection — protect + flag, don't prevent (RULED 2026-06-22; ⚠ catcher revised 2026-06-22 per `wp4etx4cc`)
**The bug (CONFIRMED, build-dark):** some traits' reality-signals are position-specific. The ARM signal (Cannon Arm / Noodle Arm) is **outfield-assists-only** with no position gate — and infielder throws log as `assist`/`double-play`, not `outfield_assist`. So a regularly-playing infielder holding an arm trait accrues an **artificial ~0 signal**, misread as "weak arm" → an unfair LOSS proposal that can't be re-earned there. Latent today (flag-off, overlays pending) but the compute already emits it. (Most traits are NOT affected — batting/pitching/baserunning signals are universal-by-role.)

> ⚠ **CATCHER is NOT a mismatch — earlier premise refuted (`wp4etx4cc`).** Catcher caught-stealing IS tracked: every CS/SB event carries `catcherId`+`pitcherId` and the WPA layer already splits **CS catcher 95% / pitcher 5%**, **SB-allowed pitcher 55% / catcher 45%** (`kblWpaAttribution.ts:1374-1378`). So a catcher's arm HAS a real signal — it just isn't a *stored season counting stat* yet (add `caughtStealingAgainst`/`stolenBasesAllowed` to `PlayerSeasonFielding`, field-add, no DB bump). Wire the arm signal to use catcher CS at C (not OF-assists). Catcher arm is **measurable but INELASTIC** (small moves) per JK — NOT un-regainable. The residual mismatch is now **IF arm only** (1B/2B/3B/SS — routine assists measure range/opportunity, not arm strength).

**RULED (JK) — protect strongly + make it transparent; do NOT build the IF-signal cure:**
- **Canonical `POSITION_MISMATCH_UNREGAINABLE` map** = (trait × position) combos with no valid signal (v1: the arm family — Cannon Arm — at **1B/2B/3B/SS** only; **catcher REMOVED** — measurable via CS; DH gone — position removed; derive producing-positions from each trait's signal).
- **Protect (much harder to lose, NOT impossible):** for a held trait in a mismatch combo — (a) the artificial/absent signal does NOT drive self-loss (treat it as *no-signal* → dormant, not a weak reading; fixes the unfair strip); (b) a strong **keepScore boost / much-higher displacement bar** so a competing trait only rarely bumps it. It CAN still be displaced by a genuinely far-stronger trait (the rare case JK allows) — common traits won't commonly replace it.
- **Un-regainable, accepted (IF arm only):** once lost it can't be re-earned at an IF position (no valid arm-strength signal). We deliberately do NOT build the IF-throw-strength cure in v1 — we **protect + inform**. (Catcher arm IS re-earnable via CS once the counting stat is added — it's just inelastic.)
- **Scout draft flag:** the scout flags a prospect/player carrying a mismatch-combo trait — *"drafting this player gives you a unique trait that could be lost and not regained — heads up."* Feeds draft valuation + GM awareness.
- **Roster Analyzer (team hub) awareness:** must NOT undervalue a player with an un-regainable trait — value it as a rare protected asset and surface the same flag.
- **Noodle Arm:** DELETE from `BUILDABLE_TRAITS` (`traitCandidateBuilder.ts:56`) — the spec already CUT it (`TRAIT_MEASUREMENT_SPEC §0.6:110`); it's building a dropped trait on the rejected proxy. Separate cleanup, not protection.
- **Generation NOT restricted:** a catcher with Cannon Arm is a valuable, *flagged* feature, not a bug — keep generating them (subject to the §5 scarcity weighting); the scout flags them.

**Build:** a (trait→producing-positions) map + the mismatch set (one data table); in the acquisition engine, suppress signal-driven loss + boost keepScore for mismatch combos (additive to §8B); the scout-flag + analyzer-flag consume the same map. No event-schema/trackerDb bump — the un-regainable acceptance is exactly what avoids it. This is the trait analog of the salary `POSITION_MULTIPLIERS` (precedented shape).
