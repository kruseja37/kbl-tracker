...
- **Canonical `POSITION_MISMATCH_UNREGAINABLE` map** = the (trait × position) combos where a trait can't be earned at that position (v1: the arm family — Cannon Arm — at C/1B/2B/3B/SS/DH; derive producing-positions from each trait's signal).
