...
## 8C. Position-mismatch protection — protect + flag, don't prevent (RULED 2026-06-22)
**The bug (CONFIRMED, build-dark):** some traits' reality-signals are position-specific. The ARM signal (Cannon Arm / Noodle Arm) is **outfield-assists-only** with no position gate — a catcher's caught-stealing is NOT tracked (it's credited to the pitcher), and infielder throws log as `assist`/`double-play`, not `outfield_assist`. So a regularly-playing catcher/infielder holding an arm trait accrues an **artificial ~0 signal**, which the engine misreads as "weak arm" → an unfair LOSS proposal, and the trait can never be re-earned there. Latent today (flag-off, overlays pending) but the compute already emits it. (Most traits are NOT affected — batting/pitching/baserunning signals are universal-by-role.)

**RULED (JK) — protect strongly + make it transparent; do NOT build the schema-level cure:**
- **Canonical `POSITION_MISMATCH_UNREGAINABLE` map** = the (trait × position) combos where a trait can't be earned at that position (v1: the arm family — Cannon Arm — at C/1B/2B/3B/SS; DH dropped — position removed; derive producing-positions from each trait's signal).
...
- **Un-regainable, accepted:** once lost it can't be re-earned at that position (no valid signal). We deliberately do NOT build the position-aware-signal cure (catcher caught-stealing event + per-position cohort, which would need a schema/trackerDb bump) in v1 — we **protect + inform** instead.
