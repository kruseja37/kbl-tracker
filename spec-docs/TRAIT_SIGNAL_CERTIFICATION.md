...
- **Two-Way position (CORRECTED — JK 2026-06-18):** Two Way is **pitcher-only**, and a two-way player **can only ever hold pitcher traits — there is NO gateway into the position-player (batting) trait pool.** On grant (trait evolution OR player generation), assign the pitcher a **random two-way fielding position (IF / OF / C)** so he can take the field (defensive roster only — this does NOT open any new trait pool). *(SUPERSEDES the prior "earning it makes him an everyday player, eligible thereafter for the position-player batting traits" gateway. Proxy (RULED JK 2026-06-18): earned by **elite HITTING for a pitcher** = high percentile of **wOBA per PA vs the pitcher peer pool**, valve-gated → super-rare. The "fielding at non-primary positions" proxy belongs to the distinct **Utility** trait, not Two Way.)*
