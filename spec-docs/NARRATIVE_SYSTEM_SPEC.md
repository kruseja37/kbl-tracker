...
- v1.2 - 2026-02-20 - **Critical corrections**: (1) Narrative system does NOT write to mojo, fitness, or injury states — these are SMB4-reported values entered by the user. (2) Added morale-influences-event-probability system. (3) Corrected trait Chemistry mechanics (traits from ANY Chemistry type, potency based on team count). (4) Removed REG (Random Event Generator) as standalone — events are now AI-driven and context-aware.
...
## ⚠️ CRITICAL: What the Narrative System Does NOT Control

> **Mojo**: User-reported from SMB4. The narrative system may REFERENCE mojo state in stories but NEVER writes to it.
>
> **Fitness**: User-reported from SMB4. The narrative system may REFERENCE fitness state in stories but NEVER writes to it.
>
> **Injuries**: User-reported from SMB4. KBL does NOT simulate injuries. If a player is injured in SMB4 gameplay, the user reports it. The narrative system may generate stories ABOUT injuries but does not create them.
>
> **Ratings**: The narrative system does NOT directly modify player ratings. Ratings changes happen through the EOS (End-of-Season) Ratings Adjustment system only.
>
> **What the Narrative System DOES write to**: Beat reporter articles, fan morale modifiers, narrative event history, relationship triggers (suggested, not forced).
...
| Injury | ~~Fitness state + workload analysis~~ **REMOVED** — injuries are SMB4-reported, not generated by narrative system. Beat reporter may COVER injuries reported by user. | User reports injury from SMB4 gameplay |
| Hot/Cold Streak | ~~Mojo carryover + narrative momentum~~ **READ-ONLY** — narrative may reference user-reported mojo but does NOT set it. Beat reporter covers observable hot/cold streaks. | User reports mojo state from SMB4 |
...
### 10.3 Morale-Influenced Event Probability

> **NEW v1.2**: Player and fan morale influence the probability of narrative events firing. Low morale increases negative event likelihood; high morale increases positive event likelihood.

```typescript
interface MoraleEventModifiers {
  playerMorale: number;   // 0-99
  fanMorale: number;      // 0-99
  teamPerformanceGap: number;  // Games above/below expectation
}

function getEventProbabilityModifier(
  event: NarrativeEvent, 
  modifiers: MoraleEventModifiers
): number {
  let modifier = 1.0;  // Baseline
  
  // Player morale affects individual events
  if (event.scope === 'PLAYER') {
    if (modifiers.playerMorale < 30) {
      // Low player morale: +40% chance of negative events, -30% positive
      modifier *= event.valence === 'NEGATIVE' ? 1.4 : 0.7;
    } else if (modifiers.playerMorale > 70) {
      // High player morale: +30% chance of positive events, -20% negative
      modifier *= event.valence === 'POSITIVE' ? 1.3 : 0.8;
    }
  }
  
  // Fan morale affects team-wide events
  if (event.scope === 'TEAM') {
    if (modifiers.fanMorale < 25) {
      // Fans are hostile: drama events more likely
      modifier *= event.category === 'DRAMA' ? 1.5 : 1.0;
    } else if (modifiers.fanMorale > 75) {
      // Fans are euphoric: feel-good events more likely
      modifier *= event.category === 'FEEL_GOOD' ? 1.3 : 1.0;
    }
  }
  
  // Teams underperforming expectations: more "shake things up" events
  if (modifiers.teamPerformanceGap < -5) {
    modifier *= event.category === 'TRADE_RUMOR' ? 1.5 : 1.0;
    modifier *= event.category === 'LINEUP_CHANGE' ? 1.3 : 1.0;
  }
...
### 10.4 The Event Generation Prompt
...
async function applyGeneratedEvent(
  ...
  // ═══════════════════════════════════════════════════════════════
  // INJURIES — ⚠️ DEPRECATED v1.2: Injuries are SMB4-reported
  // The narrative system does NOT generate injuries. This section
  // is preserved for reference but should NOT be implemented.
  // User reports injuries from SMB4 gameplay; beat reporter covers them.
  // ═══════════════════════════════════════════════════════════════
