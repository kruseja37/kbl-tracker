...
**Version:** 1.1.3 | **Date:** 2026-06-09 | **Status:** CANONICAL — approved by JK in spec session 2026-06-09
...
**Builder/auditor decorrelation pattern (v1.1.3):** builder and auditor are DIFFERENT model families on every implementation task — two independent error distributions; same-model self-audit tends to find its own choices plausible. Analysis/cross-source tasks (T1–T3): Fable 5 CLI builds (design-intent fidelity is the work). Implementation tasks (T4–T11): Codex 5.5 builds, Fable 5 CLI audits. **Audit gate (required before JK browser verification on T4–T11):** (1) golden tests pass, (2) NFL protocol run with documented falsification attempts, (3) section-by-section spec conformance check against this document, (4) state/persistence tasks additionally audited for migration safety and IndexedDB key-scope discipline. Reasoning-effort rule unchanged: anything touching game state, persistence, or reducers = very high / max effort; pure-function engines = high.

| # | Task | ROUTE |
|---|---|---|
| T1 | Extract FULL Salary Cap curve table (all 18 position blocks, A:N incl. sub-min I–N columns) + verify trait-table blanks from workbook → `src/data/ivCurves.ts`, `src/data/traitPricing.ts` | ROUTE: Claude Code CLI \| Fable 5 \| high (cross-source fidelity; mechanical but semantics-critical) |
...
---
*End IV_ENGINE_AND_ROSTER_INTELLIGENCE_SPEC.md v1.1.3*
