...
| D16 | (v1.1.9, supersedes v1.1.8 arm interpolation) kblIV prices arm roles by expected production with one SP-shaped pitching family and four role scalings: **SP 1.00, SP/RP 0.80, CP 0.65, RP 0.55**. CP > RP because leverage puts closers above generic relief; SP/RP < SP because swingmen carry reduced starter workload. Flexibility-as-value is explicitly deferred to the v1.1 economy batch and must not be smuggled into the curves. Acceptance: at identical ratings, ordering is SP > SP/RP > CP > RP at every pitching rating; reference probes VEL/JNK/ACC 80/70/70 and 90/85/85 keep SP at about $51.3k/$81.1k and place SP/RP in [0.76,0.84], CP in [0.60,0.70], RP in [0.50,0.60] vs SP. |
...
**Pitching role curve repricing (2026-07-04 D16):** SP is the shape baseline. SP/RP, CP, and RP use the same pitching-attribute `{min, curve1, mid, curve2}` values as SP and scale only `midSal`/`sal100`; ordering at identical ratings must be SP > SP/RP > CP > RP.

| Role | Multiplier | VEL midSal/sal100 | VEL subMin midSal/sal100 | JNK midSal/sal100 | ACC midSal/sal100 |
|---|---:|---:|---:|---:|---:|
| SP | 1.00 | 10,500 / 63,000 | 7,500 / 18,000 | 5,000 / 20,000 | 7,700 / 34,650 |
...
**Workbook reality (T1-verified, 2026-06-10; repriced 2026-07-04):** sub-min I–N curve params exist ONLY on the VEL rows of SP, SP/RP, RP, and CP. Step 1 repricing preserves the SP sub-min row `{0,1.2,30,7500,1.3,18000}` and scales it for the other arm roles: SP/RP `{0,1.2,30,6000,1.3,14400}`, CP `{0,1.2,30,4875,1.3,11700}`, RP `{0,1.2,30,4125,1.3,9900}`. No other attribute carries `subMin`; it remains optional in the type. Also T1-verified: the EXTRA block (row 117) is PITCHER-shaped (7 attrs incl. VEL/JNK/ACC, nonzero min floors) — T4 `computeIV` must expect both facts. Source of truth for all curve values: `src/data/ivCurves.ts`.

### 3.5 Trait Marginal Pricing
Each trait carries per-attribute **rating-equivalents** (workbook `Traits` sheet, values = Chemistry Level 2). Trait cost = Σ over affected attributes of `attributeCost(rating + Δ) − attributeCost(rating)` + flat fee + multiplier terms `(attrCost × mult − attrCost)`. Negative traits refund by the same mechanism. **SP/RP asymmetry (v1.1.6/A3, audit-confirmed):** NEGATIVE traits on SP/RP players price their delta-marginals on the RP curves, not the SP/RP curves (workbook anti-refund-farming guard; repriced golden test: Jon Gray Injury Prone = −$836). T4 must implement. Consequence (intended): the same trait costs more on an already-elite player (convexity), and trait values are wildly unequal — Cannon Arm ≈ +45 ARM vs Sprinter ≈ +5 SPD.
...
| DH-aware batting bonus | **RETIRED as a special case**; DH context becomes the §3.9 USAGE_INPUTS per-role derived weights (`startShare × paRatio + phFloor` for POW/CON, with SPD floors) |
...
- FLD: ALWAYS 1.00 for every pitcher (they field every inning they pitch). **CARVE-OUT (ratified 2026-06-10, V117 audit W2b; repriced 2026-07-04):** a non-two-way pitcher's FLD is MOUND fielding and prices on his PITCHER block's FLD curve — NOT on hitter curves; pricing position-player range he never uses would re-inflate the pool (~$930k phantom). Two Way trait holders price FLD on the trait position's curve. "ALL pitcher batting on hitter curves" means POW/CON/SPD.
...
2. RETIRED (2026-06-10 adjudication): the ±20% parity band was the wrong operationalization of "no no-brainers." REPLACEMENT criterion: the Fenomeno-Pastimm gap is REPORTED with its full component bridge, no forced band. Post-reprice pins: Fenomeno $124,165, Pastimm $122,198, Drake $56,490, Bradwick $58,417. If honest math lands them outside the old band, that is a finding reported to JK — weights are never adjusted to force the band.
3. OBSERVED (arm probe): Pastimm (trait-stacked elite arm) vs Drake (trait-less elite arm, VEL 92) now price on the same SP/RP role curve family; their gap isolates the multiplier-trait contribution on the honest base. Expected Pastimm > Drake; report both with breakdowns. Comparability is proven by usage math against SMB4's real-time reality, never inferred from grades (Norm A, Buzz B+): grade-vs-IV divergence is the engine's product — the strategic surface that makes team building original and replayable, and the seed for every downstream story (True Value, roster movement, player/fan morale, beat-reporter narratives) that carries from draft night through the season into the offseason.
...
| T5 ✅ COMPLETE 2026-06-11 | Salary spec integration seam: pipeline base = computeIV().kblIV in canonical dollars; Steps 1/2/trait-tiers deprecated out of live path; POSITION_MULTIPLIERS → 1.0 knobs; D15 potency-neutral salary enforced; rookie-scale hook (0.50× replaces age factor, §8.4); denomination bridge BRIDGE=300.032521 applied to all scale constants (CALIBRATE); prospect placeholders bridged; TeamHub formatters → canonical formatSalary; regression tests R1–R6 + mutation-pinned | Codex 5.5 built; Fable 5 audit: DEVIATIONS — 2 MAJOR remediated via T5-FIX/FIX-2, 4 LOW (2 fixed, F5/F6/F7 pending JK); Fable delta verify: T5-FIX DELTA VERIFIED 2026-06-11 (PROMPT_CONTRACTS T5 + T5-AUDIT + T5-FIX + T5-FIX-2 + T5-FIX-VERIFY; FINDING-134 logged for residual $M flows) |
| T6 ✅ COMPLETE 2026-06-14 | Effective Ratings Engine (`src/engines/effectiveRatings.ts`: `effectiveRatings` + `defensivePlacementRisk`) + constants-registry extension (`rosterEngineConstants.ts`, add-only) + first production reader of `traitInteractionMatrix.ts`; pure (no IV/salary/mojo/fitness/persistence touch); 12 unit tests E1–E12 | Codex 5.5 \| high built; Opus 4.8 audit: CONFORMS (Fable unavailable; auditor ≠ builder), independently re-verified (tsc 0 / build 0 / 12 focused / full suite 7,152 with only the 3 characterized fails / golden+SMB4 byte-unchanged / oracle spot-check MATCH); 1 MEDIUM finding RESOLVED → FINDING-148 (base AUX_PRICING L/R premium gap, separate ticket), 5 LOW + F1–F5 ratified (PROMPT_CONTRACTS T6 + T6-AUDIT) |
| T7 (split T7a/T7b/T7c, JK 2026-06-14) | Mode 2 Analyzer. **T7a ✅ COMPLETE 2026-06-14** = optimal lineups vs L/R + re-optimize (IV-of-effectiveRatings via NEW `src/engines/rosterAnalyzer.ts:optimizeLineup`; in-place `optimalLineup.ts` scoring swap, API stable; Codex 5.5 \| very high built, Opus audit CONFORMS, browser-pending). **T7b ✅ COMPLETE 2026-06-14** = call-up/send-down ADVISORY recs (§8.3: `recommendRosterMoves` + 4 emitters unblocked to read_only; MLB surplus = TV2 valueDelta, farm surplus = scout-visible scoutedGrade only per the no-oracle-leak ruling; advisory-only, NO execution; Codex 5.5 \| very high built, Opus audit CONFORMS, browser-pending). **T7c ✅ COMPLETE 2026-06-14** = season salary ledger (§8.4: trackerDb v15 store + LedgerEntry state machine + capCharge + ledgerCapCharge + call-up/demotion producer + rookieScale flip; salaryCalculator byte-unchanged; payroll-expectation→fan-morale + declared-budget DEFERRED; Codex 5.5 \| very high built, Opus audit CONFORMS, migration-safety proven, browser-pending). **T7 STACK COMPLETE.** | ROUTE each: Codex 5.5 \| very high → audit (Opus 4.8 while Fable unavailable; auditor ≠ builder) |
