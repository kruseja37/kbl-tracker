...
**➡ NEXT: MAIN = DT-F2 (Metal Head — needs the `enrichment.modifiers` persistence trace CONFIRMED first [unconfirmed: useGameState `mergedEnrichment` spreads `pendingEnrichment` but the panel→pendingEnrichmentRef.modifiers write is untraced]; + Workhorse — needs a new `getSeasonPitchingStats` load + the SP/RP cohort-split fork [JK lean: local Two-Way-style pool-key fold now, vs wait for T-3's general split]) → then T-3/T-6 → T-7; Lane C = PARK→WAR (V8) seed wiring (A2.8) → other ratings-finish.**
