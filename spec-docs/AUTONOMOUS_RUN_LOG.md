...
**T-9a — ✅ DONE (`a7932007`, `codex/franchise-v1-next`, branch-only, ZERO-NEW-REDS) — per-pitch-type NET-QUALITY aggregator (the dormant-trait wave template, part a).** Codex-built (gpt-5.5/xhigh, stdin-from-contract) → Opus audited the REAL diff (builder≠auditor) + ran the gate himself. NEW `addPitchTypeSignals` in `traitCandidateBuilder.ts` (one pass over `sortAtBats(atBatEvents).filter(!undone)`, mirrors `addOutcomeRateSignals`): buckets ONLY the 8 known `enrichment.pitchType` codes; emits a build-dark net-outcome signal per pitcher for each `Elite <pitch>` + per hitter `Fastball Hitter`{4F,2F,CF}/`Off-Speed Hitter`{SL,CB,CH,FK,SB}; `signalValue = Σweights/n`, `sampleSize = tagged-AB count` (the min-sample valve input → the downstream rate-basis `minSampleRate:10` keeps each trait DORMANT until ≥10 tagged ABs of that pitch). The 7-class outcome partition is EXHAUSTIVE + disjoint over the 26-member `AtBatResult` union (K-class spreads the existing `STRIKEOUT_RESULTS`, HR-class the `HOME_RUN_RESULTS` — no hardcode drift). §16 sim-tune weights: pitcher K+1 / out+0.3 / BB−1 / hit−2 / **HR−3 (heaviest)**; hitter HR+3 / big+2 / single+1 / BB+0.5 / out0 / K−1. Wired LAST in `buildRawSignals`, **INERT** (the 10 traits are NOT in `BUILDABLE_TRAITS` → `buildPeerPools`+`computeSeasonTraitCandidates` skip them). **MAKE-OR-BREAK PROVEN by test:** `tagged === plain` candidate output (zero behavior change) while the raw `Elite 4F`/`Fastball Hitter` signals fire; the pitcher 4F bucket arithmetic pinned to −7.4/10 (3K+2GO+BB+2×1B+2HR), the HR-swap = the single heaviest Δ = (−3.0−0.3)/10, and untagged/unknown('CUT')/empty/undone ABs all excluded. **Minor (audited + accepted, not blocking):** `buildRawSignals` made `export` (test-enabling, additive); `classifyPitchOutcome` THROWS on an out-of-union result (defensive fail-loud, type-unreachable — vs the file's usual permissive `.has()`; flagged for an optional skip-not-throw hardening in a later ticket). **GATE (Opus ran it himself, NOT the builder paste):** `NODE_ENV= npm run build` exit 0; focused `traitCandidateBuilder.test.ts` 114/114 (+7 new); FULL suite (this engine is in the `processCompletedGame` import chain) **2 failed / 8215 passed (519 files)** = `wpaRuntimeBoundary` (hard characterized) + `franchiseManualSmokeFixture` (order-flake, CONFIRMED solo-pass 4/4) ⇒ **ZERO NEW REDS**; `iv_oracle.json` byte-unchanged; NO trackerDb bump (v25). Committed by path (the concurrent doc-worker's gap-analysis `M` + the baton files left untouched). **T-9b — ✅ DONE (`6f3fc727`, `codex/franchise-v1-next`, branch-only, ZERO-NEW-REDS) — the 8 Elite-`<pitch>` + Fastball/Off-Speed Hitter traits are now EARNABLE in-season.** Codex-built (gpt-5.5/xhigh) → Opus audited the REAL diff (builder≠auditor) + ran the gate himself. Added the 10 to `BUILDABLE_TRAITS` (after Ace Exterminator) → the candidate builder scores + peer-percentiles them, gated by the existing rate-basis valve (`minSampleRate:10` → DORMANT until ≥10 tagged ABs of that pitch). **Make-or-break re-verified from source:** all 10 are ALREADY canonical (`TRAIT_PRICING`, each once) + role-classified (`traitRealityScorer.ts:89-97/99-114` — 8 elites pitcher-only, 2 hitter traits position-only), so `isTraitEligibleForRole` admits them for the matching role ONLY; NO traitRealityScorer change. Personality image-valence mirrors K Collector (all 10 → `POSITIVE_IMAGE_TRAITS` + `IMAGE_DRIVER_SETS` `['COMPETITIVE','EGOTISTICAL']`). **OPEN-DECISION-for-JK (logged):** the driver affinity for these 10 is a **§16 sim-tune default (Captain/AUTH-4; spec §0.7 assigns NO drivers for them)** — personality is a ≤±20% TILT never a gate (§0.8), the per-pitch reality signal is primary; tunable at RB-16. Tests: golden BUILDABLE 48→58; the T-9a `tagged===plain` INERT proof CONVERTED → the earnable proof (fires with tagged data, NONE without → build-dark-for-untagged preserved); **valve boundary PINNED (9 tagged ABs → `sufficient:false`; 10 → `sufficient:true` + finite percentile)**; role-eligibility (position never earns Elite, pitcher never earns Fastball Hitter — raw signal emits, candidate filtered); image-driver positivity + NO TOUGH/TIMID cross-axis (20 parametrized). **GATE (Opus ran it himself):** build 0; focused `traitCandidateBuilder`+`traitAcquisition` 212/212; FULL suite **2 failed / 8237 passed (519 files)** = `wpaRuntimeBoundary` (hard) + `franchiseManualSmokeFixture` (order-flake, CONFIRMED solo-pass 4/4 this session) ⇒ ZERO NEW REDS; **NO franchise/L-SIM golden shifted** (no fixture carries tagged pitch data); `iv_oracle.json` byte-unchanged; NO trackerDb bump (v25). Committed by path (gap-analysis + baton untouched).

**T-9c — ✅ DONE (`e95aba07`, `codex/franchise-v1-next`, branch-only, ZERO-NEW-REDS) — in-season max-1 Elite-pitch mutual-exclusion + the shared `ELITE_PITCH_TRAITS` const.** Codex-built (gpt-5.5/xhigh) → Opus audited the REAL diff (builder≠auditor) + ran the gate himself. NEW group-exclusion pass in `reconcileGainProposals` (`traitAcquisition.ts`, inserted right AFTER `lossNames` so a held elite being LOST frees the slot, BEFORE the cap loop — adds excluded elites to `dropped` so `survivingGains` filters them): a held Elite pitch (not being lost) **DEFENDS** its slot (all elite-pitch gains dropped); else among competing elite gains only the **highest `gainScore`** is admitted (deterministic `localeCompare` tiebreak). New `SkippedTrait.reason` member `'elite_pitch_excluded'`. Shared const: `ELITE_PITCH_TRAITS` (the 8) moved to `src/data/traitTierConfig.ts` (acyclic — it imports only ivEngine+traitPricing); `prospectScoutingDraftEngine.ts`'s `PROSPECT_ELITE_PITCH_TRAITS` is now a back-compat ALIAS (`= ELITE_PITCH_TRAITS`, same reference) — `prospectTraitsConflict` + the T-4c gen tests unchanged; the redundant `PROSPECT_ELITE_PITCH_TRAIT_NAMES` array removed (grep-confirmed no other consumer). `traitAcquisition.ts` imports the shared const; T-9a's local pitch-CODE-keyed `ELITE_PITCH_BY_CODE` left as-is (different shape). **DESIGN DEFAULT (Captain/AUTH-4 — OPEN-DECISION-for-JK):** "held elite DEFENDS" (a held Elite pitch is never displaced by a same-group GAIN) — mirrors the opposite-pair-held rule, can't strip a held trait; whether a DOMINANT new elite should DISPLACE a weak held one (§8B keepScore-vs-gainScore) is a tuning question. The loss-frees-slot path covers the common "pitch stopped being elite" case. **Tests (5 via the PUBLIC `computeTraitAcquisition`, + shared-const):** held-defends (+ non-elite gain unaffected), strongest-gain-wins (pinned via real `computeTraitWeight`), **loss-frees-slot** (held SL being lost → Elite 4F admitted), deterministic alphabetical tiebreak (equal gainScores cross-constructed), single-gain-admitted; `ELITE_PITCH_TRAITS` canonical + `.size===8` + `PROSPECT_ELITE_PITCH_TRAITS === ELITE_PITCH_TRAITS` (single source of truth). Existing opposite-pair/cap/displacement reconcile tests GREEN unchanged. **GATE (Opus ran it himself):** build 0; focused `traitAcquisition`+`prospectScoutingDraftEngine`+`traitTierConfig` 146/146; FULL suite **2 failed / 8243 passed (519 files)** = `wpaRuntimeBoundary` (hard) + `franchiseManualSmokeFixture` (order-flake, solo-pass 4/4 this session) ⇒ ZERO NEW REDS; `iv_oracle.json`/`traitCandidateBuilder.ts`/`traitPricing.ts` untouched; NO trackerDb bump (v25). Committed by path.

🎉 **T-9 COMPLETE (a `a7932007` + b `6f3fc727` + c `e95aba07`)** — the dormant-trait wave TEMPLATE: elite pitches (+ Fastball/Off-Speed Hitter) are now EARNABLE in-season. Per-pitch net-quality aggregator (HR heaviest) → BUILDABLE + K-Collector image valence + the rate min-sample valve → engine-side max-1 Elite-pitch mutual-exclusion + one shared const. All build-dark, branch-only, zero-new-reds, NO DB bump. **OPEN-DECISIONS-for-JK:** (1) the 10 traits' personality-driver affinity = K-Collector mirror (§16 default); (2) elite-pitch displacement = held-defends (vs §8B displace).

**DT-B — ✅ DONE (`2596b2c8`, `codex/franchise-v1-next`, branch-only, ZERO-NEW-REDS) — the 4 pitch-LOCATION hitter traits (High/Low/Inside/Outside Pitch) are now EARNABLE in-season.** Codex-built (gpt-5.5/xhigh, stdin-from-contract) → Opus audited the REAL diff (builder≠auditor) + ran the gate himself. NEW `addPitchLocationSignals` in `traitCandidateBuilder.ts` mirrors the HITTER half of T-9a's `addPitchTypeSignals`: 4 per-batter zone buckets keyed on `enrichment.pitchLocation` (low/high/inside/outside), **skips `'outOfZone'` + undefined** (no trait), reuses `classifyPitchOutcome` + `HITTER_PITCH_OUTCOME_WEIGHTS` UNCHANGED (shared with T-9a — Fastball/Off-Speed Hitter behavior identical), `signalValue=Σweights/n`, `sampleSize=tagged-AB count`; wired LAST in `buildRawSignals`. The 4 added to `BUILDABLE_TRAITS` + positive image valence (`COMPETITIVE`/`EGOTISTICAL`, §16 default mirror). **⚠ CORRECTION (the prior RESUME note was WRONG): the earn-side opposite-pair exclusion did NOT cover these — `OPPOSITE_PAIRS` (`traitAcquisition.ts`) lacked the pitch pairs; only the GENERATION list `prospectScoutingDraftEngine.ts:381-382` had them, so in-season a player could EARN both High+Low.** Captain caught it in the grounding sweep + verified directly (read `OPPOSITE_PAIRS`/`TRAIT_OPPOSITES` + the `reconcileGainProposals` duels). DT-B registers `['High Pitch','Low Pitch']` + `['Inside Pitch','Outside Pitch']` in `OPPOSITE_PAIRS` → feeds `TRAIT_OPPOSITES` + the existing held-offsets-gain (`:546-552`) + same-cycle duel (`:554-564`); **NO new pass** (unlike T-9c's elite-pitch group). Verified SAFE across ALL `TRAIT_OPPOSITES` consumers: reconcile gains the exclusion; generation `prospectScoutingDraftEngine.ts:729` redundant-with-existing-conflict-list = no behavior change; symmetry test (`traitAcquisition.test.ts:1420`) green. Tests (+225 lines): BUILDABLE golden 58→62; earnable (tagged ≥10/zone → that zone's candidate, untagged → none of the 4); **per-zone EXACT arithmetic pinned** (one batter across 4 zones: Low 5.5/5, High −1/1, Inside 3/2, Outside 0/3); `outOfZone`+undefined skipped; valve boundary 9→insufficient/10→sufficient+finite percentile; pitcher-role filtered (raw signal emits, candidate filtered); image valence positive + Competitive/Egotistical tilt>1 / Tough-Timid=1; the 2 earn-side exclusion tests (held Low blocks High gain `offsetting_pair_held`; competing High>Low keeps the higher gainScore). **GATE (Opus ran it himself, NOT the builder paste):** build 0; focused `traitCandidateBuilder`+`traitAcquisition`+`prospectScoutingDraftEngine` 267/267; FULL suite **2 failed / 8254 passed (519 files)** = `wpaRuntimeBoundary` (hard) + `franchiseManualSmokeFixture` (order-flake, CONFIRMED solo-pass 4/4 this session) ⇒ **ZERO NEW REDS**; `iv_oracle.json`/`traitRealityScorer.ts`/`traitPricing.ts`/`traitTierConfig.ts` untouched; NO trackerDb bump (v25). Committed by path (gap-analysis `M` + baton files left untouched). **OPEN-DECISIONS-for-JK (logged, AUTH-4 defaults taken, non-blocking):** (1) the 4 traits' personality-driver affinity = K-Collector mirror (§16 default; spec §0.7 assigns none); (2) §0.6b's "RBI incl. SF + RBI-GO" credit — the reused result-based scorer folds SF/SAC/HBP→NEUTRAL=0 (no RBI/runner context wired), a pre-existing §16 gap DT-B INHERITS (does NOT introduce); do not "fix" the shared weight table (would change Fastball/Off-Speed Hitter too). **⚠ PROCESS NOTE:** the FIRST Codex dispatch of this ticket HUNG 2h7m with zero output/zero files; the Captain wrongly reported it "normal" before checking elapsed time → JK caught it ("you're hallucinating"). Killed (nothing written), re-dispatched under a stall-detecting watchdog (kills on ~6m of no-output/no-files; 30m hard cap) → completed clean. Lesson logged to `SESSION_RULES.md` pending pen (bound-the-wait + verify-etime, never passively assume).

🎉 **DT-B COMPLETE** — pitch-LOCATION wave ticket done (1 of the 5 DT-x tickets). Pattern held: T-9a clone (hitter-only) + BUILDABLE + image valence, PLUS the earn-side opposite-pair registration the resume note had wrongly assumed already existed.

**DT-C SPLIT (grounded `wf_8bab1557-a81`):** Group C's 3 traits split into **DT-C1** (Bad Ball Hitter = pure at-bat chase-rate, clean DT-B clone, NO rating gate) + **DT-C2** (Magic Hands/Dive Wizard = web-gem + the NEW rating-gate; carries a fielder-attribution question — built next). Grounding also reconciled: §0.6b/DECISIONS_LOG (2026-06-25) WINS over the 2026-06-16 TS-8/TS-9 notes → Dive Wizard = web-gem only (NO beat_runner), Easy Target OUT of Group C.

**DT-C1 — ✅ DONE (`e33aed1e`, `codex/franchise-v1-next`, branch-only, ZERO-NEW-REDS) — Bad Ball Hitter EARNABLE (chase hit-rate).** Codex-built (gpt-5.5/xhigh, watchdog-guarded) → Opus audited the REAL diff (builder≠auditor) + ran the gate himself. NEW `addChaseSignals` mirrors `addPitchLocationSignals`: per-batter over `enrichment.chased===true` ABs, `hits-on-chase / (hits + outs-on-chase)` via the existing `classifyPitchOutcome` partition (hit={HR,SINGLE,BIGHIT}, out={K,OUT}, BB/NEUTRAL excluded from both — the anti-game outs-denominator); wired LAST in `buildRawSignals`; `'Bad Ball Hitter'` → `BUILDABLE_TRAITS` + `POSITIVE_IMAGE_TRAITS` (NO `IMAGE_DRIVER_SETS` entry — §0.7 assigns no driver → neutral tilt, OPEN-DECISION-for-JK; NO opposite pair — stands alone). Already canonical/priced/POSITION_ONLY/RARE-0.82-via-`assignTier` → NO traitRealityScorer/traitPricing/traitTierConfig touch. Tests (+162): BUILDABLE golden 62→63; EXACT-arithmetic pins (7-for-10→0.7, 2-for-10→0.2 < a 0.5 peer = the anti-game property PROVEN); NEUTRAL/walk/unchased/undone all excluded (HR+GO only → 0.5/2); valve 9→insufficient/10→sufficient+finite percentile; pitcher-role filtered; image valence positive + neutral personality tilt. **GATE (Opus ran it himself):** build 0; focused `traitCandidateBuilder`+`traitAcquisition` 233/233; FULL suite **2 failed / 8259 passed (519 files)** = `wpaRuntimeBoundary` (hard) + `franchiseManualSmokeFixture` (order-flake, solo-pass 4/4 CONFIRMED this session) ⇒ ZERO NEW REDS; `iv_oracle.json`/`traitRealityScorer`/`traitPricing`/`traitTierConfig` untouched; NO trackerDb bump (v25). Committed by path (gap-analysis `M` + baton untouched).

**JK RULINGS 2026-06-25 (attended, mid-build — DECISIONS_LOG):** (1) **rating-gate = COHORT FILTER** — the fielding<80/arm>80 threshold filters the COMPARISON COHORT (the peer pool) BEFORE percentiling, NOT post-hoc eligibility, so a sub-80 OVERPERFORMER isn't buried under the elite fielders (Captain-verified the engine does this via an EMISSION gate → `buildPeerPools` (`traitCandidateBuilder.ts:1647-1652`) pools ONLY signal-emitters → the Magic Hands percentile is sub-80 fielders vs each other); drives DT-C2. (2) **Easy Target ↔ Mind Gamer** are the opposite pair (corrects the stale 2026-06-16 TS-8 "Easy Target mirrors Bad Ball Hitter"); Bad Ball Hitter stands alone, Easy Target/Mind Gamer = a separate discipline-axis pair OUT of DT-C scope.

**➡ RESUME HERE = DT-C2 (Magic Hands + Dive Wizard — web-gem + the NEW rating-gate)** — the second half of Group C. The 2 fielder traits SHARE the web-gem signal, differ only by gate: **Magic Hands fielding<80 · Dive Wizard arm>80 · CO-HOLDABLE** (NOT an opposite pair — `['Magic Hands','Butter Fingers']` exists, leave it; do NOT pair Magic Hands↔Dive Wizard). **Build per JK's cohort-filter ruling: gate at EMISSION** so the peer pool = the eligible cohort only. TWO grounding items to settle from SOURCE before contracting: **(a) FIELDER ATTRIBUTION** — `enrichment.fieldingPlayType` is on the AtBatEvent with NO stable fielderId (only position numbers); the fielder-keyed source is `FieldingEvent.playerId` (already in `input.fieldingEvents`, consumed by `addButterFingersSignals`/`addUtilitySignals`) → web-gem = `FieldingEvent.specialPlayType ∈ {Diving,Leaping,Sliding}` (mapped from the enrichment tag) and/or `difficulty==='spectacular'` (pick one canonical predicate, documented default — GEM_PLAY_TYPES precedent `playerLineupGameLine.ts:4-28`). **(b) RATING PLUMBING** — thread `Player.fielding`/`Player.arm` (`leagueBuilderStorage.ts:270-271`, 0-99) through a NEW `TraitGrantRosterEntry` field → `resolveTraitGrantRoster` → a NEW optional `fielderRatingsByPlayer?` map on `SeasonTraitCandidateInput` (deferred-seam like `primaryPositionByPlayer`/`pitcherGradeByPlayer`; absent → dormant) → the emission gate (Magic Hands fires only if fielding<80, Dive Wizard only if arm>80; a fielder qualifying for BOTH emits both). NO new store / NO DB bump. Add Dive Wizard to `POSITIVE_IMAGE_TRAITS` (Magic Hands already fully wired). Web-gem signalValue = web-gems / fielding-chances (rate), valve-gated. FULL suite. Then DT-D (errors incl. **Noodle-Arm-re-added-as-mental-errors**) · DT-E (mojo) · DT-F (bespoke incl. **Metal-Head = PITCHER protective**) → T-3 / T-6 → T-7.
...
**DT-C2 — ✅ DONE (`44ababfb`, `codex/franchise-v1-next`, branch-only, ZERO-NEW-REDS) — Magic Hands + Dive Wizard EARNABLE (web-gem rate + the NEW cohort-filter rating gate).** Codex-built (gpt-5.5/xhigh, stdin-from-contract, watchdog-guarded — clean, no stall) → Opus audited the REAL diff (builder≠auditor) + ran the gate himself. The MOST COMPLEX wave ticket (3 source files). NEW `addWebGemSignals` in `traitCandidateBuilder.ts`: per-fielder web-gem rate keyed to `FieldingEvent.playerId` (the stable fielder id — NOT the batter-keyed `enrichment.fieldingPlayType`), `webGems / chances` where `chances += 1` per non-undone fielding event and a web-gem = `event.success && specialPlayType ∈ WEB_GEM_PLAY_TYPES`. NEW local `WEB_GEM_PLAY_TYPES = {Diving,Leaping,Sliding}` (Robbed HR / Over Shoulder EXCLUDED for v1 — OPEN-DECISION; the app's `GEM_PLAY_TYPES` includes Robbed HR; engine must NOT import the src_figma UI set). NEW optional `fielderRatingsByPlayer?: ReadonlyMap<string,{fielding;arm}>` deferred-seam on `SeasonTraitCandidateInput` (mirrors `primaryPositionByPlayer` — absent → both traits dormant, all existing callers/tests compile unchanged). **RATING GATE AT EMISSION (JK's cohort-filter ruling):** after the rate, `const r = input.fielderRatingsByPlayer?.get(playerId); if (!r) continue; if (r.fielding < 80) emit 'Magic Hands'; if (r.arm > 80) emit 'Dive Wizard';` — strict inequalities (80 excluded both ways), CO-HOLDABLE (independent ifs). Because `buildPeerPools` pools ONLY signal-emitters, the Magic Hands peer pool = ONLY fielding<80 fielders (the Dive Wizard pool = ONLY arm>80) → an elite glove NEVER enters the cohort and can't bury a sub-80 overperformer. Wired LAST in `buildRawSignals` after `addChaseSignals`. `'Magic Hands'`+`'Dive Wizard'` → `BUILDABLE_TRAITS`. `franchiseTraitGrantCompute.ts`: `fielding`/`arm` added to `TraitGrantRosterEntry`, populated from flat `player.fielding`/`player.arm` (no new import), passed at the `computeSeasonTraitCandidates(...)` call as `fielderRatingsByPlayer: new Map(...)`. `traitAcquisition.ts`: `'Dive Wizard'` → `POSITIVE_IMAGE_TRAITS` (priced positive); §0.7 assigns NO driver → NO `IMAGE_DRIVER_SETS` entry (neutral tilt, OPEN-DECISION-for-JK); Magic Hands left untouched (already fully wired incl. `['Magic Hands','Butter Fingers']`); NO Magic Hands↔Dive Wizard opposite pair (co-hold). Tests (+307 lines across 3 files): BUILDABLE golden 63→65; web-gem rate EXACT (3/10→0.3, sampleSize 10); the strict rating-gate + co-hold matrix (fielding-only/arm-only/both/fielding==80→no MH/arm==80→no DW/absent→neither/map-omitted→neither, checked on BOTH raw signals AND candidates); **the COHORT-FILTER property test (JK's key ruling) — 4 fielders, elite-glove fielding 90 produces NO Magic Hands candidate AND the sub-80 overperformer's `peerPoolSize === 3` (elite excluded from the pool) with `realityPercentile ≈ 1`** = the at-emission gate PROVEN (would fail if gated post-hoc); min-sample valve 9→insufficient/10→sufficient+finite; predicate (missed play `success:false`, non-gem `Robbed HR`, undone event all excluded → 1/3 of 3); Dive Wizard image positive + neutral tilt + no opposite; the `franchiseTraitGrantCompute` plumbing test (real franchise player fielding 64/arm 88 → roster entry; the threaded map `{fielding:64,arm:76}`/`{fielding:51,arm:90}` into the compute call, size 2). **GATE (Opus ran it himself, NOT the builder paste):** build 0; focused `traitCandidateBuilder`+`traitAcquisition`+`franchiseTraitGrantCompute` 247/247; FULL suite **2 failed / 8267 passed (519 files)** = `wpaRuntimeBoundary` (hard) + `franchiseManualSmokeFixture` (order-flake, CONFIRMED solo-pass 4/4 this session) ⇒ **ZERO NEW REDS** (+8 new passing tests); `iv_oracle.json`/`traitRealityScorer.ts`/`traitPricing.ts`/`traitTierConfig.ts` untouched; NO trackerDb bump (v25); no `src_figma` import. Committed by path (gap-analysis `M` + baton files left untouched). **OPEN-DECISIONS-for-JK (logged, AUTH-4 defaults taken, non-blocking):** (1) web-gem set excludes Robbed HR / Over Shoulder for v1 (the in-app `GEM_PLAY_TYPES` includes Robbed HR — widen later if desired); (2) Dive Wizard personality driver = none (neutral, §0.7 silent — mirror Magic Hands' TOUGH/COMPETITIVE if a driver is wanted).

🎉 **DT-C2 COMPLETE ⇒ Group C DONE (DT-C1 Bad Ball Hitter + DT-C2 Magic Hands/Dive Wizard).** The NEW rating-gate-as-cohort-filter mechanism is now established + tested for the rest of the wave to reuse.

**➡ RESUME HERE = DT-D (error traits incl. Noodle-Arm-re-added-as-mental-errors)** — next in the dormant-trait enablement wave. Then DT-E (mojo Volatile/Consistent) · DT-F (bespoke incl. Metal-Head=PITCHER protective) → T-3 (trend + SP/RP split, coordinates with DT-D on the Noodle Arm array) / T-6 (position-mismatch protection-only) → T-7 (EOS). All build-dark, live post-D13. Ground DT-D fresh from `TRAIT_MEASUREMENT_SPEC §0.6b` (error rows) + `DECISIONS_LOG 2026-06-25` before contracting.

**⚙️ PARALLEL-LANE SAFETY ANALYSIS (Captain, `wf_61cdc32d-469`, read-only, 2026-06-25):** JK asked whether a second branch can build concurrently without contamination. Mapped the modify-surface of every remaining queue workstream vs the trait-wave occupied surface (`traitCandidateBuilder.ts` + `traitAcquisition.ts` + `franchiseTraitGrantCompute.ts`). VERDICTS: **C — Ratings-engine finish (A2.5/RA-5/7/RA-9/RA-11/re-grade/V8) = SAFE (recommended)** — grep-verified zero trait imports in any C anchor file, zero DB-bump (field-additive/overlay-reuse), a "finish" lane. **B — Mode-1 scout tails (S7d-2/3) = SAFE (2nd choice)** — zero source overlap; one shared TEST file `prospectScoutingDraftEngine.test.ts` to watch. **D — living-season tails = RISKY/SPLIT** — {L4b + A1.3a} safe, but A1.3b adds a TRADE_DEMAND emitter into `processCompletedGame.ts` (same-file chokepoint, line ~1087 beside the trait-grant call) + a possible DB bump → HOLD A1.3b (deferred anyway). **E — T-3/T-6 = COLLIDES** — edits the exact two trait files every wave ticket rewrites; T-6's Noodle Arm DELETE directly opposes DT-D's Noodle Arm RE-ADD → MUST serialize on the trait branch (same lane by design). Operational: `git worktree add ../kbl-ratings-c -b codex/ratings-finish-c codex/franchise-v1-next` + APFS-clone node_modules + concurrent Codex dispatch; ONE committer per branch; **SERIALIZE heavy vitest suites across lanes** (never two concurrent full runs); keep the NO-unattended-DB-bump rule (escalate if any C sub-ticket needs a store). The single chokepoint behind every lane = `processCompletedGame.ts`; merge-order both lanes through a full-suite re-run (the partial-mock-break gotcha only shows under the full suite). **AWAITING JK's go on whether to stand up the C lane.**

---

## 2026-06-25 (attended Hybrid `/kbl-captain`) — TWO-LANE CONCURRENT BUILD: DT-D (traits) + A2.5 (ratings, NEW lane C) both DONE

JK ruled (attended) to stand up a SECOND concurrent build lane after the parallel-lane safety analysis (`wf_61cdc32d-469`) recommended Workstream C (ratings-engine finish, grep-verified zero trait-file overlap). Lane stood up: `git worktree add` → `/Users/johnkruse/Projects/kbl-ratings-c` on branch `codex/ratings-finish-c` (off the DT-C2 tip `843df3fa`), node_modules via APFS copy-on-write clone (seconds), build sanity exit 0. Then DT-D (main trait branch) + A2.5 (lane C) were grounded in parallel from source (`wf_c3f15d84-fb1`), contracted, and dispatched to TWO concurrent Codex builds (API-bound; the heavy vitest gates kept SERIALIZED — never two full suites at once).

**DT-D — ✅ DONE (`6d88b228`, `codex/franchise-v1-next`, branch-only, ZERO-NEW-REDS) — Wild Thrower + Noodle Arm EARNABLE (error-attribution signals).** Codex-built (xhigh, watchdog-clean) → Opus audited the REAL diff + ran the gate himself. NEW `addErrorSignals` (`traitCandidateBuilder.ts`) reads the **playerId-keyed `errorAttributions` stream** (the make-or-break — NOT the position-keyed `enrichment.errors[]`, which has no fielder id and would never fire) from `atBat.runnerOutcomes[].errorAttributions[]` + `betweenPlayEvent.errorAttributions[]`, partitioned by `type`: `throwing`+`fielding` → Wild Thrower, `mental` → Noodle Arm. Per-fielder rate = errors / `gamesByPlayer` (mirrors `addArmSignals`' games denominator), `sampleSize` = games, valve-gated dormant; NEGATIVE sign mirrors `addButterFingersSignals` (no inversion). **Noodle Arm RE-ADDED via the mental-error signal ONLY** — the L9b-2-cut (`c92e8619`) arm/OF-assist proxy is NOT resurrected (`addArmSignals` untouched, still Cannon Arm only). `Wild Thrower`+`Noodle Arm` → `BUILDABLE_TRAITS`; `Noodle Arm` → `NEGATIVE_IMAGE_TRAITS` (no driver, like Injury Prone); `Cannon Arm`↔`Noodle Arm` opposite pair intact (Wild Thrower already in NEGATIVE set + TIMID/DROOPY driver, untouched). Tests (+220): BUILDABLE golden 65→67; mental→Noodle Arm + throwing/fielding→Wild Thrower keyed to `fielderIds`, rate pinned (2 mental/10 games → 0.2); **the make-or-break test "emits NEITHER when only position-keyed `enrichment.errors` exist"** (proves the source); multi-fielderId credit; undone atBat+betweenPlay excluded; games<=0 → no signal; Noodle Arm negative valence + Cannon-Arm-held-blocks-Noodle-Arm-gain (`offsetting_pair_held`). **GATE (Opus ran it himself):** build 0; focused 248/248; FULL suite **2 failed / 8275 passed (519)** = `wpaRuntimeBoundary` (hard) + `franchiseManualSmokeFixture` (order-flake, CONFIRMED solo-pass 4/4 post-DT-D) ⇒ ZERO NEW REDS; `r3-round5.test.tsx` passed 7/7 in-run (the Codex-flagged "window/indexedDB not defined" = non-failing teardown noise). `iv_oracle`/scorer/pricing/tier untouched; NO DB bump (v25). Committed by path. **OPEN-DECISION-for-JK:** `addErrorSignals` pools ONLY error-makers (players with ≥1 error attribution), unlike `addArmSignals`/`addButterFingers` which pool ALL fielders → the conservative direction (harder to earn a negative trait). Cheap post-D13 tuning if JK prefers an all-fielders cohort for "league-leader" semantics.

**A2.5 — ✅ DONE (`72bfaf46`, `codex/ratings-finish-c`, branch-only, ZERO-NEW-REDS) — §6A age MODIFIER bar-shift in expected-stats. THE RATINGS-FINISH LANE OPENER.** Codex-built in the `kbl-ratings-c` worktree (xhigh, watchdog-clean) → Opus audited the REAL diff + ran the gate himself (serialized). Added `ageBandFactorByBand: Record<ExpectedStatsAgeBand, number>` to `ExpectedStatsTuning` + `EXPECTED_STATS_TUNING` (§16 defaults: prime `25-31`=1.00, tails `22-24`/`32-35`=0.96, `18-21`/`36+`=0.92, symmetric mild benefit-of-doubt). `expectedForCategory` gains a `tuning` param and multiplies the **EXPECTED bar** by `ageBandFactorByBand[input.ageBand]` BEFORE `r` is computed (the make-or-break — a §4/§6A bar-shift, NOT an additive rating delta; avoids double-counting the later §5 age-curve term RA-5 will add). `finiteOrNull` guard on the factor. Prime band 1.00 = float no-op = byte-identical for prime-age players. Consumes the already-plumbed-but-inert `ageBand` input; engine-internal ONLY (NO sweep/signal/storage touch); build-dark behind `FRANCHISE_PHASE2_CHECKPOINT_ENABLED_DEFAULT=false`; NO DB bump (v25); `FranchiseRatingsOverlayRow` shape untouched. Tests (+89): prime band byte-identical vs an all-1.0 baseline (`.toBe`); young/old LOWER expected + RAISE r (monotonic bar-shift); equal-factor tails (18-21 vs 36+) equal shift; pitcher-arm category still null `r` across age bands. **GATE (Opus ran it himself):** build 0; focused 28/28 (`expectedStatsEngine`+`checkpointRatingSignal`+`expectedStatsCategoryRates`); FULL suite **1 failed / 8271 passed (519)** = `wpaRuntimeBoundary` (hard) only ⇒ ZERO NEW REDS. `iv_oracle` untouched. **OPEN-DECISION-for-JK:** the age-band factor magnitudes are §16 sim-tune placeholders (shape is spec'd: prime strictest, tails benefit-of-doubt; numbers are tunable).

**LANE TOPOLOGY (now active — Shape A, single Captain, two worktrees):** main `/Users/johnkruse/Projects/kbl-tracker` on `codex/franchise-v1-next` = trait wave (DT-x) + integration + ALL Captain docs/ledger. `/Users/johnkruse/Projects/kbl-ratings-c` on `codex/ratings-finish-c` = ratings-engine finish (A2.5 done; next = re-grade-per-checkpoint / RA-5 / RA-9 / RA-11 / V8). ONE Codex build per worktree at a time; heavy vitest suites SERIALIZED across lanes; ONE committer per branch; contracts live in the MAIN `PROMPT_CONTRACTS.md` (piped via stdin to `codex -C <worktree>`). Both lanes eventually fold into `codex/franchise-v1-next` (merge whichever lands first, then full-suite re-run the second before its merge — the partial-mock-break gotcha only shows under the full suite).

**➡ RESUME (both lanes): MAIN = DT-E (mojo traits Volatile/Consistent)** then DT-F (bespoke incl. Metal-Head=PITCHER protective) → T-3 / T-6 → T-7. **LANE C = the next ratings-finish ticket** (ground from `V1_BUILD_QUEUE` + `RATINGS_*` specs; candidates re-grade-per-checkpoint / RA-5 age-curve / RA-9 trend / RA-11 / V8 park→WAR — pick the next build-dark, no-DB-bump, self-contained one).

---

## 2026-06-25 (attended Hybrid `/kbl-captain`) — TWO-LANE ROUND 2: DT-E (mojo traits) + RA-5 (age curve, lane C) both DONE

Second concurrent round on the two-lane topology (main trait wave + `codex/ratings-finish-c`). Both grounded-from-source in parallel (`wf_f65ca649-00e`), contracted, dispatched to 2 concurrent Codex builds, gates SERIALIZED.

**DT-E — ✅ DONE (`2a44cc2a`, `codex/franchise-v1-next`, branch-only, ZERO-NEW-REDS) — Volatile + Consistent EARNABLE (mojo-change-rate signal).** NEW `addMojoSignals` (`traitCandidateBuilder.ts`) reads `mojo_change` betweenPlayEvents filtered to `playerStateChange.stateType==='mojo'` + a real `previousValue!==newValue` transition (mirrors the EXISTING injury aggregator at `:1809`), keyed to `playerStateChange.playerId`; per-player rate = changes/`gamesByPlayer`, `sampleSize`=games. `Volatile` = rate, `Consistent` = `-rate` (inverse, rank-safe). Both → `BUILDABLE_TRAITS` (UNIVERSAL → emitted for hitter AND pitcher). **NO `traitAcquisition` change** — `['Consistent','Volatile']` opposite pair + image valence (Consistent positive / Volatile negative) already wired. The mojo time-series is already persisted (`useGameState.ts:8680-8703`) + threaded into the builder input unfiltered (`franchiseTraitGrantCompute.ts:288-298`) → build-dark, NO new seam, NO DB bump. Tests (+153): BUILDABLE golden 67→69; rate pinned (3 changes/4 games → Volatile 0.75, Consistent −0.75); ranking flips between the two traits (more changes → higher Volatile, lower Consistent); dormancy (0 changes / games=0 → no signal); no-op transition + undone + fitness/injury stateType excluded; universal-role (hitter+pitcher). **GATE (Opus ran it himself):** build 0; focused pass; FULL suite **2 failed / 8280 passed (519)** = `wpaRuntimeBoundary` (hard) + `franchiseManualSmokeFixture` (order-flake, CONFIRMED solo-pass 4/4 post-DT-E) ⇒ ZERO NEW REDS; acquisition/pricing/tier/oracle untouched; NO DB bump. **OPEN-DECISION-for-JK:** `traitPricing.ts` flags BOTH Volatile AND Consistent `polarity:'positive'` while acquisition treats Volatile as negative-image — a pre-existing inconsistency, left untouched (out of scope for a §0.6b measurement ticket; flag for a pricing-polarity cleanup).

**RA-5 — ✅ DONE (`8c3c19ea`, `codex/ratings-finish-c`, branch-only, ZERO-NEW-REDS) — §5 age-CURVE gravity term in checkpoint development.** Adds `ageCurveSlopeByBand` (per-band) + `ageSteepnessByRatingKey` to `RatingsDevelopmentTuning` (§16 defaults: prime `25-31`=0 neutral; tails ±0.35/±0.8; speed/fielding/arm 1.25/1.2/1.25 steeper than power/contact 1.0). `computeCheckpointRatingDevelopment` adds `ageGravity = slope[ageBand] × attrSteepness(ratingKey)` to the signed MOVE BEFORE the existing cap/dampener/clamp — **DISTINCT from A2.5's expected-bar shift** (RA-5 = MOVE path / after r; A2.5 = SIGNAL path) → no double-count (§5 "why both"). `ageBand` threaded from the sweep (`franchiseCheckpointSweepCompute.ts` — added to `CheckpointRosterEntry`, derived via the existing `ageToExpectedStatsBand(player.age)`, de-duplicated with `member.ageBand`, passed into the dev call). Type-only import from `expectedStatsEngine` (no logic double-count; `expectedStatsEngine.ts` UNTOUCHED). Prime/undefined band = 0 = byte-identical; L10 path (`franchiseL10SweepCompute`) passes no ageBand → stays neutral. Build-dark behind the Phase-2 flag; `ageBand` derived on-the-fly (no persisted field); NO DB bump (v25). Tests (+234 across 2 files): prime/undefined adds EXACTLY 0 (`toEqual(omitted)`, rawDelta 0, proposedRating byte-identical); young 18-21 → +0.8 up / proposedRating 51; old 36+ → −0.8 down / 49; speed/fielding/arm steeper than power/contact; gravity passes through cap+clamp+dampener; sweep test (38yo negative, 20yo positive, 28yo byte-identical, L10 neutral). **GATE (Opus ran it himself, serialized):** build 0; focused 54/54; FULL suite **1 failed / 8278 passed (519)** = `wpaRuntimeBoundary` (hard) only ⇒ ZERO NEW REDS; `expectedStatsEngine`/`iv_oracle` untouched. **OPEN-DECISION-for-JK:** the age-curve slope magnitudes + per-attribute steepness are §16 sim-tune placeholders (shape spec'd: prime neutral, tails pull, defensive tools steeper; numbers tunable). The §5 line-110 calibration constraint (old-age modifier must not fully cancel the curve decline) becomes verifiable once BOTH A2.5 + RA-5 are flag-live — JK to sim-check the net.
...
## 2026-06-26 (attended Hybrid `/kbl-captain`) — DT-FIX-1 DONE; DT-FIX-2 + RA-9a IN FLIGHT (two-lane)

Fresh hybrid captain session; baton claimed; two-lane Shape-A resumed (MAIN trait-fix wave + `codex/ratings-finish-c`). JK present, surfacing forks inline; the one apparent fork (Volatile salary/OVR) resolved on grounding as DEAD code → no JK decision needed.

**DT-FIX-1 — ✅ DONE (`22786153`, `codex/franchise-v1-next`, branch-only, ZERO-NEW-REDS) — JK DT-x rulings 1-3 in `traitCandidateBuilder.ts`.** (1) `WEB_GEM_PLAY_TYPES += 'Robbed HR'` (Over Shoulder/Wall Catch still excluded). (2) `addErrorSignals` peer cohort widened error-makers-only → **league-wide**: the emit loop now iterates `seasonFieldingByPlayer.keys() ∪ gamesByPlayer.keys()` (mirroring `addArmSignals`' union + `fielding?.games ?? gamesByPlayer ?? 0` resolution + localeCompare sort), defaulting clean fielders to a 0-rate Wild Thrower/Noodle Arm signal so the league-leader stands out in the percentile pool. (3) Wild Thrower numerator = **throwing-only** (`countAttribution` drops the `'fielding'` disjunct → a `'fielding'`-type attribution now credits NEITHER trait; Butter Fingers owns muffs via `FieldingEvent.playType==='error'`); Noodle Arm stays mental-only. **Audit (Opus, real diff):** source exactly the 3 changes; tests recomputed from the new logic — web-gem `1/3→2/3` (Diving✓+RobbedHR✓ over 3), main-fixture Wild Thrower `0.2→0.1`, `undefined→0-rate` for clean-fielders-with-games, `'fielding'`-undone→`'throwing'`-undone to keep the exclusion test on a counted type, `runnerError('fielding')→('throwing')` to preserve the multi-fielder credit intent — PLUS 2 NEW coverage tests (league-wide 0-rate across games-only + season-only; fielding credits neither). No trait/registry/oracle/pricing/scorer/tier touch; trackerDb v25. **GATE (Opus ran the authoritative full suite himself):** tsc 0; build 0; focused 143/143; FULL **2 failed / 8282 passed (519)** = `wpaRuntimeBoundary` (hard) + `franchiseManualSmokeFixture` (order-flake) ⇒ ZERO NEW REDS (AwardsWatchlist passed clean in the authoritative run, confirming it's the known flake).

**DT-FIX-2 — IN FLIGHT (`codex/franchise-v1-next`):** ruling 4 — move `'Volatile'` `NEGATIVE_IMAGE_TRAITS`→`POSITIVE_IMAGE_TRAITS` in `traitAcquisition.ts` (image/fame/narrative/morale axis) + reconcile the image-axis ripple in the same diff. **Captain grounding resolved the apparent salary/OVR fork:** the salary `SEVERE_NEGATIVE_TRAITS` consumer `calculateTraitModifier` is `@deprecated` (live salary = traitModifier=1.0) and the `ovrCalculator` Volatile buckets are UNCONSUMED → flipping them has ZERO live effect → left untouched per the handoff's "don't touch pricing" intent. **OPEN-DECISION-for-JK (cosmetic, non-blocking):** reconcile those dead lists' Volatile-negative labels only if the cosmetic consistency is wanted; no live impact either way.

**RA-9a — IN FLIGHT (`codex/ratings-finish-c`):** §8 trend-tilt ENGINE FOUNDATION. **RA-9 SPLIT 9a/9b** (Captain, this session) because the recent-window category-rate aggregation has no clean reuse (`getSeasonBattingStats/…` are season-cumulative; the window scan `resolveWindowActivePlayerIds` collects only player IDs) and needs a NEW windowed event aggregator — too meaty to bundle. **9a (now)** = the OPTIONAL recent-signal blend, default-identity/build-dark: `trendTiltWeight` (§16 default 0) + optional `recentSignal` blended `(1−w)·cum + w·recent` into the dev path's signal (before `computeRawRatingDelta`); optional `recentCategoryRates` on `CheckpointSignalMember` → a SECOND `expectedAndSignal` pass against the SAME cumulative-calibrated pool stats; return shape `{cumulative, recent}` with a minimal sweep read-change. No sweep aggregation, no DB bump, no oracle/trait touch. **9b (next)** = the sweep recent-window aggregation that supplies the recent rates. Grounded via `general-purpose` agent + Captain re-verification of every make-or-break anchor in the `kbl-ratings-c` worktree.

**Lane-C selection rationale (Captain, grounding agent + re-verify):** RA-9 chosen over the other ratings-finish candidates — re-grade-per-checkpoint COLLIDES with the active trait lane (it reads `franchiseTraitGrantCompute.ts`, a trait-engine file) + has an unresolved grade-display fork; RA-11 needs a cross-branch spec-pin with Branch-B B14 first (not self-contained); V8 (park→WAR) excluded (known spec conflict, not build-dark).

**DT-FIX-2 — ✅ DONE (`71727e5b`, `codex/franchise-v1-next`, branch-only, ZERO-NEW-REDS).** Moved `'Volatile'` `NEGATIVE_IMAGE_TRAITS`→`POSITIVE_IMAGE_TRAITS` in `traitAcquisition.ts` → `getImageValence('Volatile')==='positive'`; Consistent↔Volatile pair stays valence-neutral. Captain grounding confirmed the apparent salary/OVR "ripple" is DEAD code (`calculateTraitModifier` `@deprecated`/live=1.0; `ovrCalculator` buckets unconsumed) → left untouched, zero live effect; pricing already positive (untouched). New test asserts positive valence + no image driver + pair intact. **Audit (Opus, real diff):** exactly the set-move; only 2 files; protected (pricing/salary/ovr/scorer/interaction-matrix/candidateBuilder/trackerDb/oracle) all empty. **GATE (Opus authoritative full suite):** build 0; focused 120/120; FULL **2 failed / 8283 passed** = `wpaRuntimeBoundary` + `franchiseManualSmokeFixture` (order-flake) ⇒ ZERO NEW REDS. **OPEN-DECISION (cosmetic, non-blocking):** the dead salary/OVR Volatile-negative labels left as-is — reconcile only if cosmetic consistency wanted.

**RA-9a — ✅ DONE (`c56d1c8e`, `codex/ratings-finish-c`, branch-only, ZERO-NEW-REDS).** §8 trend-tilt ENGINE FOUNDATION (RA-9 split 9a engine / 9b sweep). `ratingsDevelopment`: `trendTiltWeight` (default 0) + optional `recentSignal`; `effectiveSignal=(1−w)·cum+w·recent` blended into the dev signal BEFORE `computeRawRatingDelta`, only when recent is finite AND w>0 (else identity); RA-5 ageGravity still applied after. `checkpointRatingSignal`: optional `recentCategoryRates`; `computeCheckpointRatingSignals`→`{cumulative, recent}` (recent = a 2nd `expectedAndSignal` pass against the SAME cumulative-calibrated pool stats + same flat-floor gate; only when supplied). Sweep: 4-line read-change to `.cumulative` (supplies no recent → byte-identical). **Audit (Opus, real diff):** make-or-break verified — default-identity byte-identical; pool stays cumulative-calibrated; recent is a convex SIGNAL tilt (not post-cap additive); tests prove exact blend math + clamp/cap + recent-vs-pool-mean + thin-recent floor-gating; protected (expectedStats/L10/oracle/trackerDb) empty. **GATE (Opus authoritative full suite, serialized after the MAIN suite):** build 0; focused 60/60; FULL **1 failed / 8284 passed** = `wpaRuntimeBoundary` only ⇒ ZERO NEW REDS.

**PARK→WAR (V8) — INVESTIGATED + JK-RULED (not a build this turn; queued).** Adversarially-verified investigation (`wf_2355914a-e2e`) found the "conflict" is a stale snapshot artifact (the audit listing V8 open was captured 54 min BEFORE the §9.D ruling). **JK 2026-06-26: §9.D STANDS — v1 WAR uses seed/geometry park factors; adaptive deferred.** Build = the small seed-park→WAR wiring (bWAR consumption branch already exists but dead; pWAR park hook net-new; homePA-scoped double-count guard; FIP-adjust-before-diff; no oracle/DB/store; Lane-C-disjoint from RA-9 + the trait lane). ⚠ Interim: bWAR runs on a `floor(pa/2)` 50/50 home/road approximation until the §9.D data-retention item lands (JK accepted); pWAR exact. Full detail = DECISIONS_LOG 2026-06-26. Queued as a Lane-C ratings ticket.

**DT-F1 — ✅ DONE (`1d74050c`, `codex/franchise-v1-next`, branch-only, ZERO-NEW-REDS).** Base Jogger + Wild Thing earnable (the 2 fully-grounded bespoke §0.6b row F traits; DT-F SPLIT — Metal Head + Workhorse → DT-F2). `addBaseJoggerSignals` = REVERSE of Base Rounder over the IDENTICAL opportunity set (separate accumulator, `toBase 'end'` excluded both ways, thrown-out = jogger success, `success = !reachedRealBase || toBaseOrdinal <= forcedMinOrdinal` — NOT 1−rate). `addWildThingSignals` = pitcher-keyed `(wild_pitch betweenPlayEvents + WP_K at-bats) / battersFaced` (passed-ball disjoint, undone excluded). Both → BUILDABLE; Wild Thing → NEGATIVE_IMAGE (no driver, §16); Base Jogger already negative. NO new data load / no registry/oracle/pricing/tier/scorer/DB touch. **Audit (Opus, real diff):** make-or-break verified — the held-runner test pins both traits' `sampleSize=2` (3 held excluded from BOTH denominators ⇒ shared opportunity set, not 1−BaseRounder), out-case asymmetry proven, Wild Thing 2-source/BF + passed-ball-disjoint + undone-exclusion + pitcher-role pinned. **GATE (Opus authoritative full suite, serialized):** build 0; focused 264/264; FULL **2 failed / 8291 passed** = `wpaRuntimeBoundary` + `franchiseManualSmokeFixture` (order-flake) ⇒ ZERO NEW REDS. **OPEN-DECISIONS (documented defaults):** Wild Thing counts ALL wild_pitch (not only runner-advancing); Wild Thing valence negative no-driver.

**RA-9b — ✅ DONE (`972da2a5`, `codex/ratings-finish-c`, branch-only, ZERO-NEW-REDS).** Wires the dark sweep to SUPPLY recent-window category rates into RA-9a's recent-signal seam. NEW pure `src/engines/checkpointWindowedCategoryRates.ts` builds window-only batting/pitching count rows from non-undone AtBatEvents, reuses `contactQualityAggregator` + `ubrAggregator` + `toExpectedStatsCategoryRates` (IDENTICAL category semantics as cumulative = make-or-break); hitter fielding cats left UNPOPULATED (no at-bat-event source → flat-floor gate drops them; no fabrication); read-only/pure. Sweep: aggregate recent over `(prevBoundary, current]` (reuse the scan via new `resolveWindowAtBatEvents`), set `member.recentCategoryRates` per role, read `computeCheckpointRatingSignals().recent`, add `recentSignalByRatingKey` to `CheckpointRosterEntry`, thread `recentSignal` into the dev call; checkpoint-1 → no recent → neutral; window-active scan now also skips undone (benign refinement). **Audit (Opus, real diff):** RA-9a engine seams (`ratingsDevelopment`/`checkpointRatingSignal`) UNTOUCHED; `toExpectedStatsCategoryRates` reuse confirmed; pure (no persistence/store); trendTiltWeight stays 0 → byte-identical; expectedStats/L10/oracle/trackerDb untouched. **GATE (Opus authoritative full suite, serialized):** build 0; focused 69/69 (incl 6 new aggregator tests); FULL **1 failed / 8293 passed** = `wpaRuntimeBoundary` only ⇒ ZERO NEW REDS. **🎉 RA-9 COMPLETE (9a `c56d1c8e` + 9b `972da2a5`).**

**DT-F2 — ✅ DONE (`ed9e003e`, `codex/franchise-v1-next`, branch-only, ZERO-NEW-REDS).** Workhorse earnable: `addWorkhorseSignals` = IP/game `(outsRecorded/3)/games` from a NEW `seasonPitchingByPlayer` load (threaded through `loadSeasonTraitData`; processCompletedGame partial-mock tests got test-only `getSeasonPitchingStats` stubs — the contracted transitive-mock fix). SP/RP split = the LOCAL `poolTraitKey` fold mirroring Two Way (`Workhorse|SP` gamesStarted>0 / `Workhorse|RP`). BUILDABLE += Workhorse; Workhorse → POSITIVE_IMAGE. **Audit (Opus, real diff):** make-or-break verified — test pins `peerPoolSize=3` for both an SP (6.0 IP/G) AND an RP (1.5 IP/G) each at percentile≈1 in their OWN pool (impossible if co-pooled). No oracle/pricing/tier/scorer/DB touch. **GATE (Opus authoritative full suite, serialized):** build 0; focused 269/269; FULL **2 failed / 8296 passed** = `wpaRuntimeBoundary` + `franchiseManualSmokeFixture` (order-flake) ⇒ ZERO NEW REDS.

**V8 (park→WAR) — ✅ DONE (`19615d80`, `codex/ratings-finish-c`, branch-only, ZERO-NEW-REDS).** Wires SEED/geometry park factors into WAR (§9.D). pWAR: new `options.parkFactor` → `applyPitcherParkFactor` adjusts FIP BEFORE the leagueFIP diff (default 1.0 → byte-identical). bWAR: `calculateBWARSimplified` threads `{parkFactors, stadiumName, batterHand}` into the existing homePA-scoped park branch; `floor(pa/2)` fallback when homePA absent (tested). Wiring across the WAR call sites (the Simplified season path `useSeasonStats` +116, `warOrchestrator`, `processCompletedGame`, `useWARCalculations` + UI opt-in via `franchiseId` on TeamHubContent/SeasonSummary/useFranchiseData): SEED-only (`source==='SEED'`/`buildFromPark`), 40%-season activation gate; adaptive deferred. **Audit (Opus, real diff — scope WIDER than the contract's named files but ALL legitimate WAR call sites):** no double-count (homePA-scope + FIP-before-diff + WAR reads counting stats only); park-less/early-season byte-identical (no WAR golden moved); no oracle/DB/store/ratings-dev/trait touch (RA-9 lane untouched). ⚠ **NOW LIVE (not flag-gated):** park reaches WAR for franchises with seed park factors past the 40% gate; the bWAR leg uses the `floor(pa/2)` home/road approximation until the per-game data-retention item lands (JK accepted). **GATE (Opus authoritative full suite, serialized):** build 0; focused 43/43 (+ 2 new test files); FULL **1 failed / 8302 passed** = `wpaRuntimeBoundary` only ⇒ ZERO NEW REDS. **→ BV-V8 (browser sign-off PENDING — user-visible live WAR change).**

**DT-F3 — ✅ DONE (`40cdf4c1`, `codex/franchise-v1-next`, branch-only, ZERO-NEW-REDS) ⇒ 🎉 DT-F COMPLETE ⇒ the DORMANT-TRAIT ENABLEMENT WAVE (DT-B/C/D/E/F) is DONE.** Metal Head earnable: `addMetalHeadSignals` = per-pitcher `(at-bats whose enrichment.modifiers ∋ KILLED_PITCHER|NUT_SHOT, counted ONCE/AB) / battersFaced`, keyed to `atBat.pitcherId` (victim, NOT batter). Reads the EXISTING `atBatEvents` input → NO new load (data path definitively traced via a dedicated agent — the first grounding's fame-event claim was WRONG; the modifier durably lands on `enrichment.modifiers` via the KP/NUT manual-prompt → `appendModifierToAtBatEvent`). BUILDABLE += Metal Head; → POSITIVE_IMAGE. **Audit (Opus, real diff):** test pins 3/5 (KP + NUT + KP&NUT-once over 5 BF, undone + non-KP/NUT excluded), batter→undefined (pitcher-keyed). No registry/oracle/pricing/tier/scorer/DB touch. **GATE (Opus authoritative full suite):** build 0; focused 274/274; FULL **2 failed / 8301 passed** = `wpaRuntimeBoundary` + `franchiseManualSmokeFixture` (order-flake) ⇒ ZERO NEW REDS.

**CAP-MISS — ✅ DONE (`41cb5a66`, `codex/franchise-v1-next`, branch-only, ZERO-NEW-REDS) — ⚠ LIVE capture fix, BV-REQUIRED.** Ticket 1 of the BF⇄MH work: persist a tagged missed Dive/Leap/Robbery as a MISS. `fieldingEventExtractor.ts`: `normalizeSavedBasesSpecialPlayType`→identity (stop the Missed→made relabel); `makeEvent` gains `overrideSuccess`; a tagged miss emits ONE `success:false` `'putout'`-shaped event (NOT `base_save` → no held-count inflation; box-score putouts come from `gameStats`, unaffected) carrying the missed specialPlayType, on saved-run hits + plain hits + outs; out-events on a miss play don't inherit the gem label; `events.length` sequencing keeps made-play output byte-identical. **`'Failed Robbery'` first-class** SpecialPlayType (both `game.ts` copies + the `fieldingPlayType` mapper; 3 compiler-flagged consumers `fwarCalculator`[0 run-value/not-star]/`difficultyFieldingAggregator`[missed-opp]/`useFameDetection` treat it like other misses) — JK ruled label-it. **Audit (Opus, real diff — careful, live change):** miss = `success:false` putout not base_save; made gems + non-spectacular byte-identical; box-score safe (gameStats-sourced putouts; held avoided; OF-arm = assists only); bug-pinning test flipped to the new-correct expectation + new coverage (missed-leap, out-branch, failed-robbery). NO DB bump (FieldingEvent `success`+`specialPlayType` already exist; `'Failed Robbery'` is a string value), NO trait-engine touch. **GATE (Opus authoritative full suite):** build 0; focused 174/174; FULL **2 failed / 8308 passed** = `wpaRuntimeBoundary` + `franchiseManualSmokeFixture` (order-flake) ⇒ ZERO NEW REDS. **→ BV-CAP-MISS PENDING (JK browser sign-off: tag a missed Dive/Leap/failed-Robbery on a hit / an out / a HR-robbery → confirm it saves as a MISS, doesn't inflate putouts/held, doesn't credit a gem; a MADE gem control unchanged).**

**BF-MH — ✅ DONE (`80586ce0`, `codex/franchise-v1-next`, branch-only, ZERO-NEW-REDS) — the Butter Fingers ⇄ Magic Hands opposed pair (JK ruling, built on CAP-MISS).** NEW `addFieldingHandsSignals` (replaces `addWebGemSignals` + `addButterFingersSignals`): one per-fielder bucket = made gems + missed gems + deduped fielding-errors. Magic Hands/Dive Wizard = `madeGems / (made+missed+fieldingErrors)` (rating-gated fielding<80 / arm>80); Butter Fingers = `(missedGems[Failed Robbery ×0.5] + fieldingErrors) / same denom` ⇒ complementary/diametrically-opposed. Fielding-errors = `ErrorAttribution.type==='fielding'` ONLY (NOT throwing/mental); deduped vs missed-gems per `(atBatEventId, fielder)` (missed-gem wins). `FAILED_ROBBERY_BUTTER_FINGERS_WEIGHT=0.5` exported §16 dial (JK: failed robbery ≈ pitcher's fault). never-both = the EXISTING `OPPOSITE_PAIRS` duel (traitAcquisition UNTOUCHED). **Audit (Opus, real diff):** shared denominator + dedup (missed-gem wins) + fielding-only + failed-rob-half-weight + denominator-narrowing all test-pinned (`(1+0.5+1)/4`; dedup→1/1; throwing/mental ignored; Magic Hands 2/4 with 2 failed-robs; the duel test); only `traitCandidateBuilder.ts` + its test changed. **GATE (Opus authoritative full suite):** build 0; focused 282/282; FULL **3 failed / 8315 passed** = `wpaRuntimeBoundary` + `franchiseManualSmokeFixture` + `AwardsWatchlist` (all 3 characterized/order-flakes; AwardsWatchlist solo-pass CONFIRMED 2/2) ⇒ ZERO NEW REDS. 🎉 **The fielding-hands opposed pair + the whole dormant-trait/trait-detection buildout is COMPLETE.**

## 2026-06-26 (UNATTENDED AUTH-4, fresh `/kbl-captain` session) — T-3 SPLIT (3a/3b); T-3a DONE

**T-3 SPLIT (Captain, 2 grounding agents from source):** T-3a = SP/RP cohort split (mechanically grounded, mirrors DT-F2 Workhorse fold) → build first; T-3b = trend (hot/cold recent-stretch) factor (design-heavier, needs a NEW recent-window aggregator + a `buildProposalBase` term; RA-9 code is NOT in this branch — clone-not-import). Mirrors the RA-9a/9b precedent.

**T-3a — ✅ DONE (`3ecbc6c0`, `codex/franchise-v1-next`, branch-only, ZERO-NEW-REDS).** SP/RP pitcher-trait cohort split + min-peer fallback (§4A:55-62, RULED 2026-06-22). Generalized `poolTraitKey` (`traitCandidateBuilder.ts`) so EVERY pitcher-only trait except the Two Way family-fold folds to `Trait|SP` (gamesStarted>0) / `Trait|RP` — `SP_RP_SPLIT_TRAITS = PITCHER_ONLY_TRAITS − TWO_WAY_VARIANTS` (exported `PITCHER_ONLY_TRAITS` from `traitRealityScorer.ts` for the derivation; Workhorse key byte-identical under the general fold). `buildPeerPools` now also accumulates a bare combined-pitcher pool per split trait; `computeSeasonTraitCandidates` falls back to that combined pool when the split cohort `length < TRAIT_REALITY_SCORER_TUNING.minPeerPool` (3) — matching the scorer's exact `peerValues.length < 3` enforcement. Two Way unchanged (whole-pitcher family pool, the Shohei case). **Audit (Opus, real diff — NOT the Codex paste):** make-or-break verified — IDENTICAL key expression at both call sites (build + score); generalized fold preserves Two Way + Workhorse; combined push guarded to split-traits-only (no double-count); fallback threshold = the scorer const. Tests PROVE the behavioral diff via `.not.toBeCloseTo()` (sp-mid 2/3 within the 3-starter cohort, NOT 5/6 of the combined 6) + the fallback (2-member SP cohort → full 6-pool, 5/6, NOT the thin 1/2) + a position-trait regression guard; percentile math hand-verified (`count≤value / n`). Tests purely additive (no existing test re-baselined — the min-peer fallback keeps small cohorts identical, bounding the blast radius). Only 3 files; oracle/trackerDb/traitAcquisition/pricing/tier all untouched. **GATE (Opus ran independently, authoritative full suite):** tsc 0; build 0; focused 285/285; FULL **2 failed / 8319 passed** = `wpaRuntimeBoundary` (hard) + `franchiseManualSmokeFixture` (order-flake; the timeout under full-suite load — solo-passes 4/4) ⇒ ZERO NEW REDS. NO DB bump (v25).

**T-3b further SPLIT (mirror RA-9a/9b): T-3b-1 = engine seam / T-3b-2 = sweep supply.**

**T-3b-1 — ✅ DONE (`0df09fda`, `codex/franchise-v1-next`, branch-only, ZERO-NEW-REDS).** The §4A trend-tilt ENGINE seam (default-identity/build-dark; RA-9a analogue). `traitAcquisition.ts`: `buildProposalBase` gains a multiplicative `trendTilt = 1 + clamp(recentPercentile − realityPercentile, -1, 1) × trendTiltWeight` folded into the SAME `clamp01` probability product as the other tilts; `trendTiltWeight?` (optional, default `0`) on `TraitAcquisitionTuning`/`TRAIT_ACQUISITION_TUNING`; optional `recentPercentile?` on `TraitCandidate` threaded (clamp01'd) through `computeTraitAcquisition`; `trendTilt` added to `TraitChangeProposal.factors`. Direction UNIFORM across valence (negative signals are badness-rates + the gain/loss comparison is valence-blind ⇒ recentP>aggregateP raises P = eases gain / resists loss for BOTH polarities; NO per-valence branch — the make-or-break, verified). **Audit (Opus, real diff — NOT the Codex paste):** `trendTilt === 1` exactly when `trendTiltWeight===0` (default) OR recent absent/non-finite ⇒ byte-dark; tests PIN byte-identity-at-default (recentPercentile set + default weight → probability + all other factors unchanged), the exact multiplicative tilt hot/cold (`1+(1−0.8)×0.5`, `1+(0.1−0.3)×0.5`, valence stays 'lose'), the DOUBLE clamp (clamp01 on recentPercentile + clamp[-1,1] on the delta → 1.5/0.5), absent/NaN→identity, and the exhaustive factors-keys test updated additively (not weakened). Only 2 files; no `franchiseTraitGrantCompute.ts`/oracle/pricing/tier/scorer/trackerDb touch. **GATE (Opus ran independently, authoritative full suite):** tsc 0; build 0; focused 290/290; FULL **2 failed / 8324 passed** = `wpaRuntimeBoundary` (hard) + `franchiseManualSmokeFixture` (order-flake, solo-pass 4/4) ⇒ ZERO NEW REDS. NO DB bump (v25).

**T-3b-2 — ✅ DONE (`e3b7e76d`, `codex/franchise-v1-next`, branch-only, ZERO-NEW-REDS) ⇒ 🎉 T-3b + T-3 COMPLETE (T-3a `3ecbc6c0` + T-3b-1 `0df09fda` + T-3b-2 `e3b7e76d`).** The §4A trend-factor DATA half (RA-9b analogue). `franchiseTraitGrantCompute.ts`: when `(TRAIT_ACQUISITION_TUNING.trendTiltWeight ?? 0) > 0` AND `prevBoundary > 0` (`resolvePreviousCheckpointGameNumber`), re-run `computeSeasonTraitCandidates` over events windowed to `(prevBoundary, current]` (each game→gameNumber via the already-imported `getScheduledGame`; `filterEventsByGameId`) using the SAME `candidateInput` spread (aggregate maps unchanged — cohort/min-sample full-season-stable), then `attachRecentPercentiles` pairs each player's recent `realityPercentile` per trait onto the candidates. Default-dark: at weight 0 OR checkpoint 1 the recent path is SKIPPED (the candidate builder runs ONCE) ⇒ byte-identical, no extra cost. **Audit (Opus, real diff — NOT the Codex paste):** the guard short-circuits the recent re-run when dark; the recent re-run spreads the identical input overriding ONLY the 3 windowed event arrays; window = `gn > prevBoundary && gn <= current`; only sufficient+non-null recent percentiles attached. Tests PIN builder-called-ONCE-at-default + `candidates` is the SAME aggregate reference + no `recentPercentile` field (the build-dark proof), builder-called-TWICE-when-tuned with the 2nd call's events = exactly the window game + the aggregate maps reference-equal + `recentPercentile=0.95` attached, and CP1-neutral (called once even at weight 0.5); the tuning mutation restored in `afterEach`. Only 2 files; no `traitAcquisition.ts`/`traitCandidateBuilder.ts`/`franchiseCheckpointSweepCompute.ts`/oracle/trackerDb touch. **GATE (Opus ran independently, authoritative full suite):** tsc 0; build 0; focused 131/131; FULL **2 failed / 8327 passed** = `wpaRuntimeBoundary` (hard) + `franchiseManualSmokeFixture` (order-flake) ⇒ ZERO NEW REDS (the builder's run also showed `franchiseOffseasonGuards.component`, a characterized order-flake solo-pass 24/24, which did NOT recur in the Captain's independent run). NO DB bump (v25). **OPEN-DECISION (documented, inert at weight 0):** the recent pass re-percentiles vs a recent-only peer pool (cohort drift vs RA-9's fixed-pool approach) — a sim-tune refinement, harmless while dark.

**T-6a — ✅ DONE (`31afa050`, `codex/franchise-v1-next`, branch-only, ZERO-NEW-REDS) — §8C position-mismatch protection, engine half.** NEW exported `POSITION_MISMATCH_UNREGAINABLE = {Cannon Arm: 1B/2B/3B/SS}` (§8C:133 — arm signal is OF-assists-only, an infielder holding Cannon Arm accrues an artificial ~0 → unfair strip) + `POSITION_MISMATCH_KEEP_BOOST=3` (§16). For a held trait in a mismatch combo: (a) the lose-gate SKIPS it (`skipped: position_mismatch_protected`) instead of proposing a loss; (b) `keepScore × 3` (much-harder displacement bar) — but a far-stronger gain whose `gainScore` clears the boosted bar STILL displaces it (§8C:134 "not impossible"). `primaryPosition?` threaded into `TraitAcquisitionInput` + `reconcileGainProposals` + the caller's `computeTraitAcquisition` call (`entry.primaryPosition` already existed). **Position format Captain-verified `'1B'/'2B'/'3B'/'SS'` (`Position` type / `INFIELD_SCOPE`) — matches the map keys.** **Audit (Opus, real diff):** ONLY `traitAcquisition.ts` + `franchiseTraitGrantCompute.ts`(1 line) + the test changed — **`traitCandidateBuilder.ts`/`addErrorSignals`/Noodle Arm UNTOUCHED** (the §8C:138 "delete Noodle Arm" line is SUPERSEDED by DT-D; Codex correctly ignored it). Non-mismatch byte-identical (`isPositionMismatchProtected` false for everything but a held Cannon-Arm-at-IF and when position absent). Tests pin protect-loss + OF/absent-position controls + non-Cannon-at-IF-not-protected + boost-blocks-ordinary-gain (explicit gainScore-between-unboosted-and-boosted math) + far-stronger-still-displaces (Big Hack > boosted keep) + OF byte-identical. No DB/oracle/scorer/pricing/tier touch. **GATE (Opus ran independently, authoritative full suite):** tsc 0; build 0; focused 136/136; FULL **2 failed / 8332 passed** = `wpaRuntimeBoundary` (hard) + `franchiseManualSmokeFixture` (order-flake) ⇒ ZERO NEW REDS. NO DB bump (v25). **T-6b DEFERRED:** the scout-draft-flag + Roster-Analyzer-flag consumers (§8C:136-137) are a UI/display surface (consume the now-exported map) → queued for a browser-verified pass with the other BV-pending UI items, not built unattended.

**T-7 — ✅ DONE (`7340ca18`, `codex/franchise-v1-next`, branch-only, ZERO-NEW-REDS) ⇒ 🎉 the MAIN trait tail (T-3 → T-6 → T-7) is COMPLETE.** Captain-authored (no-logic confirmation — Captain-verified ALREADY-SATISFIED, nothing to decorrelate). The end-of-season trait change is already "one more checkpoint": the final game (`gameNumber === totalGames`) is ALWAYS the last 20%-grid checkpoint boundary (`isCheckpointBoundary` — Captain-verified the math [final game → left `C-1` ≠ right `C`] AND test-pinned for T=10/32/162 in `franchiseCheckpointSweepCompute.test.ts`), so the season-end trait+ratings pass runs via `processCompletedGame` with the same thresholds. Trait Wheel Spin never existed as live code (`TraitLotteryWheel` deleted in the mode1-b merge, zero refs). T-7 = (a) a confirming COMMENT at the boundary gate in `franchiseTraitGrantCompute.ts` (no logic); (b) a SUPERSEDED-for-v1 banner on `EOS_RATINGS_ADJUSTMENT_SPEC.md §449` (the award-luck/Wheel-Spin model); (c) a v1 implementation note in `TRAIT_GAIN_LOSS §8` clarifying the "5+1" phrasing (the 100% grid boundary IS the season-end checkpoint, NOT a distinct 6th event) + an OPEN-DECISION flag for JK only if a SEPARATE post-final-game EOS pass is ever wanted. NO engine behavior change; `AwardsCeremonyFlow` placeholder-trait UI deliberately NOT touched (cosmetic, separate). **GATE (Opus ran independently):** tsc 0; FULL suite **2 failed / 8332 passed** = `wpaRuntimeBoundary` (hard) + `franchiseManualSmokeFixture` (order-flake) ⇒ ZERO NEW REDS (comment-only source change cannot shift behavior). NO DB bump (v25).

---

## 🏁 SESSION TALLY (2026-06-26, UNATTENDED AUTH-4 `/kbl-captain`) — the MAIN trait tail SHIPPED

**5 commits, all `codex/franchise-v1-next`, branch-only / Opus-audited-the-REAL-diff (builder≠auditor) / Opus-ran-the-INDEPENDENT-authoritative-full-suite gate / ZERO-NEW-REDS / NO DB bump (v25):**
- **T-3a `3ecbc6c0`** — SP/RP pitcher-trait cohort split + min-peer fallback (§4A); generalized `poolTraitKey` to all pitcher traits except Two Way + a combined-pool fallback under the scorer's minPeerPool valve.
- **T-3b-1 `0df09fda`** — trend-tilt ENGINE seam (`trendTilt` in `buildProposalBase`, default-identity/dark; RA-9a analogue; uniform across valence).
- **T-3b-2 `e3b7e76d`** — sweep SUPPLIES the since-last-checkpoint recent percentile (window re-run, guarded so weight-0 default = byte-identical) ⇒ 🎉 **T-3 COMPLETE**.
- **T-6a `31afa050`** — §8C position-mismatch protection (Cannon Arm at IF: suppress artificial self-loss + keepScore ×3 displacement bar; far-stronger gain still displaces). Noodle Arm UNTOUCHED (the §8C:138 delete is SUPERSEDED by DT-D).
- **T-7 `7340ca18`** — EOS = one more checkpoint (confirmed already-satisfied; superseded Trait Wheel Spin) ⇒ 🎉 **MAIN trait tail (T-3/T-6/T-7) COMPLETE.**

Every ticket grounded FROM SOURCE (4 grounding agents: T-3, T-3b, T-6, T-7), make-or-break Captain-RE-verified, gated on the Captain's OWN independent `npm test` (never the Codex paste). All build-dark behind the Phase-2 trait flag.

**DEFERRED / OPEN (for JK / a fresh session):**
- **T-6b** — scout-draft-flag + Roster-Analyzer-flag consumers of `POSITION_MISMATCH_UNREGAINABLE` (§8C:136-137); UI/display surface → queued for a browser-verified pass.
- **2 BV items pending JK:** BV-V8 (park→WAR live) · BV-CAP-MISS (missed-catch capture).
- **OPEN-DECISIONS taken (documented defaults, inert while dark — JK can adjust on review):** `trendTiltWeight=0` (§16, trend factor magnitude — Simulation Gate owns it); `POSITION_MISMATCH_KEEP_BOOST=3` (§16); T-3b recent pass re-percentiles vs a recent-only pool (cohort drift vs RA-9's fixed pool); the §8 "5+1" EOS-cadence framing (flagged for JK only if a separate post-final-game EOS pass is wanted).

**LANE-C MERGE — ✅ DONE (`0c2b4a04`, 2-parent merge `626e6095`+`19615d80`).** Folded the 5 build-dark ratings commits (A2.5 age-modifier · RA-5 age-curve gravity · RA-9a/9b trend-tilt + windowed aggregator · V8 park→WAR) into the integration line. CLEAN auto-merge (only `processCompletedGame.trueValue.test.ts` auto-merged, zero conflicts; merge-tree pre-assessed exit 0). **Gate (Opus ran on the STAGED merge BEFORE committing):** tsc 0 (NO duplicate-def collision between RA-9's ratings-dev trend-tilt and the trait trend-tilt I cloned into the trait sweep — confirmed separate), build 0, FULL **2 failed / 8366 passed** = `wpaRuntimeBoundary` + `franchiseManualSmokeFixture` ⇒ ZERO NEW REDS (+34 Lane-C ratings tests). trackerDb v25 (reconciled). `codex/ratings-finish-c` now fully merged (0 un-merged).

---

## ⚙️ JK SESSION RULINGS (2026-06-26, attended mid-run) + the BUTTON-UP BUILDOUT QUEUE

JK paused the **draft + draft-UX side branches** (`codex/draft-pipeline-fix` [12 commits, draft-pool-setup redesign, "done pending JK sign-off"] + `codex/auction-draft-ux-rehaul` [6 commits, auction UX prototype, "greenlight-gated"]) until his return — do NOT touch/merge those. He asked to button up everything ELSE that can be. **4 RULINGS:**
1. **BUILD-SCOPE = MAXIMUM** — build EVERYTHING build-dark-able with documented §16 defaults (all reversible; JK reviews on return).
2. **RE-GRADE DISPLAY = build-dark default** — re-grade evolves the player's rating each checkpoint; the updated rating surfaces only at Phase-2 flip-flip (consistent with the wave). JK noted no live seasons matter now → safe either way; mid-season-hidden-even-after-flip is a small later tweak if he wants it.
3. **TRADE-DEMAND = BUILD FULLY incl. the trackerDb bump** — JK EXPLICITLY AUTHORIZED the saved-shape/`TRACKER_DB_VERSION` bump for A1.3b because **no critical seasons are live**. ⚠ This LIFTS the standing "no unattended DB bump" rule FOR THIS ITEM ONLY — bump correctly (migration-safe; update the version-pin tests incl. `franchiseSeasonLedgerStorage.test.ts` which hard-pins TRACKER_DB_VERSION + the full store list per [[trackerdb-version-bump-test-pins]]). Take documented defaults on the 5 A1.3b forks.
4. **RA-11 = BUILD STANDALONE** — build the Branch-A pitcher-non-pitching §4A signal to a documented assumption now; reconcile with the paused Mode-1 B14 at the eventual merge.

**🔍 CONCURRENCY PARTITION (Captain-proved 2026-06-26 — the button-up queue CAN run parallel with JK's paused draft/UX branches):** the union of files touched by `codex/draft-pipeline-fix` + `codex/auction-draft-ux-rehaul` is ENTIRELY the league-builder/draft/auction surface (`LeagueBuilder*` pages, `useAuctionDraft`/`useFarmAuctionDraft`, `leagueBuilderAuctionPipeline`/`PoolBuilder`/`PoolRegistration`/`Storage`, `useLeagueBuilderData`, `draftRouting`, `AuctionStage`, `leagueConstruction`, `Builder.tsx`, `App.tsx`/`main.tsx` routing). **ALL the button-up hot files are CLEAN on both draft branches** — `trackerDb.ts` (so the A1.3b DB bump does NOT collide — draft uses the separate `leagueBuilderStorage` DB), `types/game.ts`/`franchise.ts`, `processCompletedGame`, the checkpoint/trait sweeps, the ratings engines, `masterMoraleMatrix`/`franchiseL10SweepCompute`, AND `FranchiseHome.tsx`. ⇒ the ENTIRE ratings + living-season + stadium-records buildout is FILE-DISJOINT from the draft/UX work → safe concurrent. **Only 2 watch-points:** (a) **T-6b's scout-DRAFT-flag consumer** reaches into the draft UI → build only the team-hub/Roster-Analyzer half concurrently, HOLD the scout-board sliver; (b) new on-screen views (stadium §8 display, L12-6 All-Star) may add a route line to `App.tsx`/`main.tsx` (which the draft branches also touch) → a trivial 1-line auto-resolving merge, not a real conflict.

**➡ BUTTON-UP QUEUE (all on `codex/franchise-v1-next`, MAIN; Lane C merged so the ratings files are free; all build-dark unless noted; ground EACH fresh from source before contracting; FULL-suite gate; commit branch-only BY PATH):**
- **Ratings tail (4A/4-finish):** §6A convex curve (A2.5 remainder — gamma gate + delta edge-compression + equilibrium shape; distinct from the age-modifier already in) · RA-7 per-game park-adjust in the ratings expected-stats path · RA-10 bench-IF/bench-OF pool split (greenfield) · **RA-11 standalone** (per ruling 4) · re-grade-per-checkpoint (per ruling 2 — build-dark, surfaces at flip).
- **Living-season tail:** A1.3a trade-propensity wiring (contained, no DB bump) → **A1.3b trade-demand FULL incl. trackerDb bump** (per ruling 3) · A1.4 L12-6 (race/award/All-Star UI + `allStarSelections` career counter) · A1.5 L4b (matrix-sourced season takes over the L4a bus) · A1.5d STADIUM RECORDS (own db `kbl-franchise-stadium-records`, new `isFranchisePhase2StadiumRecordsEnabled` flag — NOT a trackerDb bump).
- **⚠ USER-VISIBLE (browser-gated — build the data layer, FLAG the UI for JK's sign-off, do NOT consider closed):** A1.5b-2 (EnrichmentPanel SVG marker re-derivation — a live UI change + precondition for Power-native) · T-6b (position-mismatch scout/analyzer flag display, consumes `POSITION_MISMATCH_UNREGAINABLE`).
- **Still blocked / out of scope this pass:** the draft + draft-UX branches (JK-paused) · RA-11's eventual B14 reconcile (at the Mode-1 merge) · the ship gate-chain (L-SIM final → RB-16 → D12 → D13 → flag-flip → F-141 — after the buildout + the draft branches land). ⚠ Do NOT flip Phase-2 until JK reviews the whole wave. ⚠ BV backlog pending JK (BV-V8, BV-CAP-MISS, + the new user-visible items above).

**SESSION TALLY (2026-06-26 attended Hybrid): 10 features shipped — MAIN: DT-FIX-1 `22786153` · DT-FIX-2 `71727e5b` · DT-F1 `1d74050c` · DT-F2 `ed9e003e` · DT-F3 `40cdf4c1` (🎉 DT-F + dormant-trait wave complete) · CAP-MISS `41cb5a66` (live capture fix, BV-pending) · BF-MH `80586ce0` (Butter Fingers⇄Magic Hands opposed pair); Lane C: RA-9a `c56d1c8e` · RA-9b `972da2a5` (🎉 RA-9 complete) · V8 `19615d80` (park→WAR live, BV-pending). All branch-only / Opus-audited-real-diff / authoritative serialized gates / ZERO-NEW-REDS / no DB bump (v25). Plus: park→WAR (V8) investigated+ruled; Butter Fingers⇄Magic Hands counter-trait pair ruled + capture-prerequisite discovered+FIXED. **2 BV items pending JK: BV-V8 (park→WAR live) + BV-CAP-MISS (missed-catch capture).** Next trait ticket = the BF⇄MH opposed-pair redesign (rest on CAP-MISS BV).**

---

## 2026-06-26 (UNATTENDED AUTH-4, fresh `/kbl-captain`) — BUTTON-UP ratings tail: RA-6A DONE; RA-7 SET-ASIDE; RA-10 already-built

Fresh session; baton claimed (`HANDOFF_DONE_20260626T004014Z_captain-buttonup`); sole worker confirmed; caffeinate alive (PID 84474). Working the BUTTON-UP QUEUE (ratings tail) in order, all on `codex/franchise-v1-next`, build-dark.

**RA-6A (§6A convex-curve dev-shape) — ✅ DONE (`55f7db4a`, contract `7a95b893`, branch-only, ZERO-NEW-REDS).** The A2.5 REMAINDER — the §6A convex γ-gate + δ edge-compression on the ratings-development move, DISTINCT from the merged A2.5 age bar-shift (`expectedStatsEngine.ts`, untouched) and RA-5 age-gravity (additive, unchanged). `computeRawRatingDelta`: convex gate `absR<startBar?0:sign(r)·((absR−startBar)/(1−startBar))^γ` replacing the bare signal before the `baseDeltaScale×morale` multiply (morale-sign branch switched `performanceSignal>=0`→`r>=0`, equivalent). `computeCheckpointRatingDevelopment`: δ edge `rawDelta≥0?(99−R)/99:R/99` ^δ applied to the PERFORMANCE move only, `ageGravity` added after un-compressed (OPEN-DECISION default — matches RA-5 separate-term). New §16 tunables `startBar/convexGamma/edgeCompressionDelta` default to IDENTITY `0/1/0` ⇒ convex→r, edge→1 ⇒ byte-identical. §6A(c) far-out ~25pt equilibrium safety rail SCOPED OUT (sweep-level follow-on). **Audit (Opus, REAL diff — not the Codex paste):** make-or-break verified — both functions byte-identical at defaults (analytic + tests); morale-sign preserved; A2.5/RA-5/sweep untouched; only the 2 contracted files; no DB/oracle. Tests PIN default-identity (computeRawRatingDelta morale-coupled + computeCheckpoint legacy rawDelta 3/−1.5) + the `toEqual` shape pin updated (not weakened) + convex non-identity (dead-band r=0.2<0.25→0, `(0.25/0.75)²`, sign) + edge non-identity (rating-90 gain `3·9/99` < rating-58 `3·41/99`, floor-loss `R/99`, AND the age-boosted case proving edge compresses the perf move but ageGravity is added uncompressed — ratingKey 'power' steepness 1). **GATE (Opus ran the AUTHORITATIVE full suite himself, 2 independent runs):** tsc 0; build 0; focused 30/30; FULL **2-fail-8368-pass** = `wpaRuntimeBoundary` (hard, fails solo too) + `franchiseManualSmokeFixture` (order-flake, SOLO-PASS 4/4 confirmed); `AwardsWatchlist` flaked in run #1, passed run #2 (the known order-flake) ⇒ ZERO NEW REDS. The whole 8368-test franchise/ratings/L-SIM fixture corpus stayed stable = the strongest byte-identity proof. NO DB bump (v25).

**RA-7 (per-game park-adjust in the ratings signal path) — ⏸ SET-ASIDE-AND-CONTINUE (OPEN-DECISION for JK).** Grounded from source (`expectedStatsCategoryRates.ts` / `franchiseCheckpointSweepCompute.ts` / `parkFactorDeriver.ts` / `seasonStorage.ts`). The spec (§3B:63) RULES *per-game park-adjusted production* and explicitly FORBIDS a 50/50 home/road approximation. But: the **recent/trend leg** can be per-game park-adjusted cleanly (per-AB events carry `parkContext.parkFactors`) — yet it's unconsumed at the default `trendTiltWeight=0`; the **cumulative/primary signal** reads season-cumulative rows that are **park-BLIND** (`PlayerSeasonBatting/Fielding` carry no park/home-road split — verified), so honoring the spec on the primary leg needs a NET-NEW full-season event re-aggregation that **swaps the primary development signal's data source → NOT cleanly default-identity** (would move fixtures even at park=1.0). This is RA-7's analogue of V8's deferred per-game home/road data-retention gap, except the spec pre-forbids the V8 escape hatch. **Captain ruling:** building only the recent-leg slice = near-zero value (unconsumed) + risks an INCONSISTENT park-adjust design vs whatever JK picks for the cumulative leg → poor engineering. SET ASIDE the primary-signal data-source fork for JK. **OPEN-DECISION-for-JK:** RA-7 cumulative-leg approach — (a) full-season event re-tally [spec-faithful, net-new, not default-identity], (b) per-player weighted-average park factor on the season rows [default-identity when avg=1.0, an approximation but uses true exposure weighting], (c) recent-leg-only now + cumulative later [primary stays park-blind]. Needs JK's call before an unattended build touches the core development signal.

**RA-10 (bench-IF/bench-OF pool split) — ✅ ALREADY BUILT (spec supersede note, NO code).** Grounded + Captain-re-verified from source: the bench-IF/bench-OF split ALREADY ships on the **decoupled `RatingsPoolKey` type** (`expectedStatsPoolAggregator.ts:30-31` `benchIF`/`benchOF` + `classifyRatingsPoolKey` `checkpointRatingSignal.ts:132-133` + `CHECKPOINT_POOL_LADDER` fallback rungs `:95-96`), built during the RA-2b/RA-2c wave (DECISIONS_LOG 2026-06-24). It is intentionally NOT on `FranchiseTrueValuePoolKey`/`getPositionPeerPool` — those are the LIVE salary/TrueValue path, and §10's literal "contained edit on `FranchiseTrueValuePoolKey`/`getPositionPeerPool`" wording would have WRONGLY changed displayed salaries (the STOP-IF). Decoupling VERIFIED: no salary/IV/TrueValue file imports `RatingsPoolKey`/`checkpointRatingSignal`/`expectedStatsPoolAggregator`. Test-pinned (`checkpointRatingSignal.test.ts`). ⇒ §10:167 marked DONE/SUPERSEDED (RATINGS_ADJUSTMENT_SPEC.md), no build. *(Adjacent genuine item = the separate DECISIONS_LOG 2026-06-24 "position-pure base + same-position bench" ladder refinement — its own ticket, not RA-10.)*

**➡ NEXT: RA-11 (pitcher non-pitching §4A signal, STANDALONE per JK ruling 4) — grounded safeToAutoBuild:YES (genuinely UNBUILT). SPLIT: RA-11a (cumulative/primary — extend `toExpectedStatsCategoryRates` pitcher branch via the `addHitterRates` template + add power/contact/speed/fielding to the pitcher `baseRatings`; SP/RP pool keying already done) building now; RA-11b (windowed/trend mirror) deferred (doubly-dark, graceful cumulative fallback). Then re-grade-per-checkpoint (ruling 2), then the living-season tail (A1.3a→A1.3b w/ the JK-authorized trackerDb bump, A1.4 L12-6, A1.5 L4b, A1.5d stadium records).**

**RA-11a (pitcher non-pitching §4A signal, cumulative leg) — ✅ DONE (`03be3dd4`, contract `35cc8459`, branch-only, ZERO-NEW-REDS).** Pitchers now develop power/contact/speed/fielding (NO arm) at checkpoints, pooled vs other pitchers (the existing SP/RP cohort — pool keying was already done). STANDALONE per JK ruling 4; reconcile with the paused Mode-1 B14 (§5.7) at the eventual merge. `toExpectedStatsCategoryRates` pitcher branch now also calls `addHitterRates(batting, fielding)` (the hitter-identical per-category decomposition reused for the pitcher's bat/field). **The shared-function trap handled:** `addHitterRates` early-returns when batting AND fielding are both absent → the WINDOWED/recent pitcher call (passes neither) stays a no-op → pitcher recent rates remain pitching-only (RA-11b trend mirror DEFERRED — the dev engine's trend blend falls back to cumulative-only via the existing finite-gate; graceful). Pitcher `baseRatings` gains power/contact/speed/fielding (NOT arm — `armThrowingRate` is dormant/null + no arm baseRating ⇒ NO pitcher arm development, the §4A make-or-break). **Audit (Opus, REAL diff):** hitter path byte-identical (guard never fires for hitters — batting always present); `addPitcherRates` untouched; only ADD pitcher non-pitching; no key collision; no DB/oracle. Tests PIN the windowed pitcher no-op (recent `sampleSizeByCat` = the 4 pitching cats only; powerSlugging/fieldingFieldingPct undefined), the cumulative pitcher's exact non-pitching rates + DORMANT_CATEGORIES absent (no arm), the re-baselined "pitcher combines pitching+non-pitching" (intended), hitter ignores pitching. **GATE (Opus ran the AUTHORITATIVE full suite himself):** tsc 0; build 0; focused 56/56; FULL **8373 pass**, fails only the characterized set (`wpaRuntimeBoundary` hard + `franchiseManualSmokeFixture`/`AwardsWatchlist` order-flakes — 2-or-3 across runs, all confirmed flakes) ⇒ ZERO NEW REDS. NO DB bump (v25). **RA-11b (windowed/recent trend mirror) DEFERRED** — doubly-dark (flag off + trendTiltWeight 0) with graceful cumulative fallback; a pure trend-responsiveness enhancement, not a correctness requirement.

**➡ NEXT: re-grade-per-checkpoint (JK ruling 2 — build-dark) building now: recompute the canonical `scoreSmb4Player` grade from each checkpoint's EVOLVED ratings, store on the existing `franchiseTrueValueSnapshots` checkpoint store (REUSE → NO DB bump), never overwrite the live `overallGrade` (surfaces at flip). Then the LIVING-SEASON tail (A1.3a → A1.3b trade-demand w/ the JK-authorized trackerDb bump → A1.4 L12-6 → A1.5 L4b → A1.5d stadium records).**

**re-grade-per-checkpoint — ⏸ SET-ASIDE-AND-CONTINUE (OPEN-DECISION for JK: the grade-snapshot STORE).** Grounded + Captain-re-verified. The COMPUTE design is READY: in the dark checkpoint sweep, assemble each player's EVOLVED ratings (`entry.baseRatings` overlaid with `dev.proposedRating` for shifted keys), call the canonical `scoreSmb4Player` (`smb4GradeEmulator.ts:671`; the fitted model, NOT 3:3:2:1:1 gradeEngine) with the player's traits/handedness/positions threaded onto `CheckpointRosterEntry`, store numericScore+letter. Ruling DECISIONS_LOG:127 (re-grade EACH checkpoint off then-current ratings+traits); build-dark, surfaces at flip; must NEVER overwrite the live stored `overallGrade`. **The blocker = WHERE to store the per-(player,checkpoint) grade.** A dedicated grade-snapshot store = a `TRACKER_DB_VERSION` bump, which JK authorized for **A1.3b ONLY**. Reusing `franchiseTrueValueSnapshots` (the grounding's recommended DB-neutral default) is ENTANGLED: that store is actively written by the trueValue path (`processCompletedGame.ts:362` `saveFranchiseTrueValueSnapshotRows`, every game, full `store.put` replace) AND read by race standings (`franchiseRaceStandingsCompute.ts`) — a sweep grade-write (running later at `:1101`) risks field-clobber / writer-race / checkpoint-key misalignment. A clean reuse needs a read-modify-write + checkpoint-key-alignment design that is a real data-integrity decision, not a safe unattended default. **OPEN-DECISION-for-JK:** (a) authorize a dedicated `franchise-grade-snapshots` store (a small trackerDb bump, same as A1.3b), or (b) rule the reuse-coordination design (read-modify-write onto the existing trueValue snapshot rows at the aligned checkpoint key). Compute is mechanical once the store is ruled.

**➡ NEXT: A1.3a (trade-propensity wiring — contained, NO DB bump) building now — wire the orphaned `computeTradeRequestPropensity` as a post-roll hard `wouldRequest` gate on L10 `trade_demand` candidates + thread loyalty/teamId onto the candidate. Then A1.3b (trade-demand FULL incl. the JK-authorized trackerDb bump), A1.4 L12-6, A1.5 L4b, A1.5d stadium records.**

**A1.3a (trade-propensity wiring, contained) — ✅ DONE (`93d39584`, contract in PROMPT_CONTRACTS.md, branch-only, ZERO-NEW-REDS).** Wired the orphaned `computeTradeRequestPropensity` as a post-roll hard `wouldRequest` gate on L10 `trade_demand` candidates (`franchiseL10EventEngine.ts` `computeFranchiseL10Events`): after `buildEventCandidate`, a `trade_demand` event is DROPPED when `!wouldRequest` (loyalty + own morale + fan anger drive it, not a blind roll). `FranchiseL10Candidate` gains optional `loyalty?`/`teamId?`; the sweep (`franchiseL10SweepCompute.ts:143-151`) threads `player.hiddenPersonalityModifiers?.loyalty ?? 50` + `teamId`. Hard boolean gate — NO `Math.random`, NO new roll; null-safe defaults (COMPETITIVE/50/50/50). **Audit (Opus, REAL diff):** gate fires ONLY on `trade_demand` — mentorship/clubhouse_rift/all other families byte-identical (the test pins both via `expectRosterSubRoll` + a full-`toEqual` regression). Engine `computeTradeRequestPropensity` signature UNCHANGED (consumed as-is); NO trackerDb bump, NO A1.3b scope (no flashpoint tax / TRADE_DEMAND emitter / persisted demander), no oracle. Tests: content high-loyalty player → trade_demand dropped; angry low-morale low-loyalty → survives; missing personality/loyalty → defaults, no crash; mentorship/clubhouse_rift unchanged. **GATE (Opus authoritative full suite):** tsc 0; build 0; focused 35/35 (`tradeRequestGeneration` green); FULL **2-fail-8378-pass** = `wpaRuntimeBoundary` (hard) + `franchiseManualSmokeFixture` (order-flake) ⇒ ZERO NEW REDS. NO DB bump (v25). NOT default-identity on the trade_demand slice (fewer fire) but L10 is dark (default-OFF flag, pending/unapplied) ⇒ safe.

---

## 🏁 SESSION TALLY (2026-06-26, UNATTENDED AUTH-4 `/kbl-captain` — BUTTON-UP ratings + living-season head)

**3 features SHIPPED** (all `codex/franchise-v1-next`, branch-only / Codex-built / Opus-audited-the-REAL-diff / Opus-ran-the-INDEPENDENT-authoritative-full-suite / ZERO-NEW-REDS / NO DB bump v25 / build-dark):
- **RA-6A `55f7db4a`** — §6A convex-curve dev-shape (convex γ-gate + δ edge-compression, default-identity 0/1/0).
- **RA-11a `03be3dd4`** — pitcher non-pitching §4A signal (cumulative leg; pitchers develop power/contact/speed/fielding, no arm; windowed no-op via null-safe guard).
- **A1.3a `93d39584`** — trade-propensity gate on L10 `trade_demand` (orphan wired; trade_demand-only; no DB bump).

**3 tickets RESOLVED without a build:**
- **RA-10** — ALREADY BUILT (bench-IF/OF split on the decoupled `RatingsPoolKey`); §10 spec line was STALE (its literal wording would touch the live salary path) → spec marked SUPERSEDED.
- **RA-7 (per-game park-adjust)** — SET-ASIDE → **OPEN-DECISION for JK:** the primary/cumulative signal reads park-BLIND season rows; honoring "per-game park-adjust" needs a net-new event re-aggregation that is NOT default-identity on the core dev signal, and the spec forbids the 50/50 approx. Forks: full event re-tally vs weighted-avg-factor vs recent-leg-only.
- **re-grade-per-checkpoint** — SET-ASIDE → **OPEN-DECISION for JK:** compute ready (`scoreSmb4Player` from evolved ratings), but the per-(player,checkpoint) grade STORE is a fork — a dedicated store = a trackerDb bump (JK authorized only for A1.3b); reusing `franchiseTrueValueSnapshots` is entangled with the live trueValue writer (`processCompletedGame.ts:362`) + race-standings reader (clobber/coordination risk).

**DEFERRED:** RA-11b (windowed/recent trend mirror for pitcher non-pitching — doubly-dark, graceful cumulative fallback; pure enhancement).

**➡ NEXT (fresh session): A1.3b — trade-demand FULL incl. the JK-AUTHORIZED trackerDb bump** (the saved-shape bump JK explicitly OK'd because no critical seasons are live; bump migration-safe + update the version-pin tests, esp. `franchiseSeasonLedgerStorage.test.ts` per [[trackerdb-version-bump-test-pins]]). Take documented defaults on the 5 forks (demander source-of-truth · flashpoint-tax-vs-one-shot-morale-vs-both · albatross↔trade_demander row-key collision · resolution/counter-reset event · intensity source). Then A1.4 L12-6 (race/award/All-Star UI [user-visible → FLAG for JK] + `allStarSelections` career counter), A1.5 L4b (matrix-sourced season bus), A1.5d STADIUM RECORDS (own db + own flag). Then return to re-grade + RA-7 once JK rules their OPEN-DECISIONS.

---

## 2026-06-26 (UNATTENDED AUTH-4, fresh `/kbl-captain`) — A1.3b trade-demand FULL SHIPPED (the authorized DB bump)

Fresh session; baton claimed (`HANDOFF_DONE_20260626T023631Z_a13b-captain-resume`); sole worker confirmed (no `codex exec`, no other `claude` on this branch); caffeinate alive (PID 84474). Worked the BUTTON-UP living-season tail, build-dark on `codex/franchise-v1-next`.

**A1.3b (trade-demand FULL — the §13 tooth #2 mechanism) — ✅ DONE (`c87ff779`, contract `3f7deba8`, branch-only, ZERO-NEW-REDS).** After A1.3a only *gated* which `trade_demand` events fire; A1.3b makes a confirmed demander do something real downstream — exactly mirroring how a locked Albatross works (membership store + shared flashpoint tax + one-shot morale event). **Three coupled pieces, all triple-dark behind the existing default-OFF L10/flashpoint/morale Phase-2 flags:**
1. **Persisted demander store** (the JK-AUTHORIZED `TRACKER_DB_VERSION` v25→v26 bump). NEW `franchiseTradeDemandState` store (keyPath `[franchiseId,seasonId,statsScopeId,playerId]`, `by_scope` index) + NEW `franchiseTradeDemandStorage.ts` (clone of the flashpoint storage). ALL bump touchpoints registered: trackerDb onupgradeneeded (guarded/additive), `backupRestore.ts` `trackerStores` registry (`optional:true`) + `STATIC_DATABASE_SCHEMAS['kbl-tracker'].version`→26, `syncConfig.ts` `SYNC_REGISTRY`. The structural-alignment parity test (`backupRestore.franchiseParity.test.ts:320` asserts schema-registry == objectStoreNames) made the backup registration mandatory — verified green.
2. **Flashpoint seam fill** (`franchiseFlashpointDecayCompute.ts:95`): `resolveTurnedOnPlayers` now reads active demanders from the new store, filtered to the completed game's home/away teams, deduped with **albatross-precedence** (a player who is BOTH gets ONE decay row, kind=`albatross` — shared tax base, no magnitude lost). Demanders flow through the existing `computeFlashpointGameTax` exactly like albatross. Albatross byte-path unchanged (test-pinned).
3. **L10 write + one-shot morale**: `persistDarkL10ForCompletedGame` now upserts a confirmed demander (idempotent — never overwrites `confirmedAtGameNumber`, so the one-shot fires exactly once) when a `trade_demand` event fires; a NEW `persistDarkTradeDemandMoraleForCompletedGame` (in `processCompletedGame.ts`, called right after the L10 block, self-gated on the morale flag) emits the EXISTING `TRADE_DEMAND` matrix consequence (player self −3, fans, clubhouse −1) once at the confirmation game, idempotent via a `confirmedAtCheckpoint`-keyed sourceEventId, deterministic timestamp from the persisted `confirmedAtIso`.

**5 FORKS — DOCUMENTED DEFAULTS (Captain-ruled under AUTH-4):** (1) demander source-of-truth = a dedicated membership store (the albatross-designation analogue), NOT the generic L10 overlay log; (2) tax-vs-morale-vs-both = **BOTH** (mirrors albatross: `ALBATROSS_LOCKED` one-shot + ongoing tax; no double-count — initial event vs ongoing-unresolved bleed); (3) collision = albatross-precedence dedupe; (4) resolution = NO auto-resolution in v1 (no trade-execution UI) — a `status:'active'|'resolved'` field is the documented SEAM for a later resolution/trade ticket (sets `'resolved'` + resets the counter); (5) intensity = NOT recorded on the row (tax + matrix delta are intensity-independent; intensity governs only WHETHER L10 fires, handled upstream).

**Audit (Opus, REAL diff — not the Codex paste):** all 14 changed files within contract; DB-bump spine byte-correct; idempotent demander write (test: 2nd sweep at game 21 does NOT overwrite the game-20 confirmation); albatross-precedence + per-team filter + tax accrual all test-pinned; one-shot morale gated/deterministic; NO `Date.now`/`Math.random`/`new Date()` added; `franchisePhase2Flags.ts` has NO diff (no flag flipped); no oracle/engine/UI touch. Build-dark proven: the "flag-ON but seam empty STILL writes nothing" test is preserved. **GATE (Opus ran the AUTHORITATIVE full suite himself):** tsc 0; build 0 (vite); FULL **2 failed / 8386 passed** = `wpaRuntimeBoundary` (hard pre-existing, unrelated WPA subsystem) + `franchiseManualSmokeFixture` (order-flake — Captain re-ran it SOLO → 4/4 pass, confirmed NOT an A1.3b regression) ⇒ ZERO NEW REDS (+50 focused A1.3b tests). **trackerDb now v26.**

**OPEN-DECISIONS taken (documented defaults, inert while dark — JK can adjust on review):** all 5 forks above (the BOTH ruling on fork 2 is the most likely JK-review point — if he wants tax-only or one-shot-only, it's a small revert); the v1 no-auto-resolution leaves a demander taxing for the rest of the season until a future resolution ticket lands (the `status` seam is ready).

**➡ NEXT: A1.4 L12-6** — race/award/All-Star UI surfacing + `allStarSelections` career counter. ⚠ **USER-VISIBLE** → build the data layer build-dark, FLAG the UI for JK browser sign-off, do NOT mark closed. Then A1.5 L4b (matrix-sourced season bus), A1.5d STADIUM RECORDS (own db + own flag, NOT a trackerDb bump). Then return to re-grade + RA-7 once JK rules their OPEN-DECISIONS.

---

## 2026-06-26 (UNATTENDED AUTH-4) — A1.4 / L12-6 DATA-LAYER (allStarSelections career counter) SHIPPED

**A1.4 / L12-6 (data-layer half — the `allStarSelections` career-counter write-path) — ✅ DONE (`bcf12d0a`, contract `019185e9`, branch-only, ZERO-NEW-REDS).** L12-6 is "Almanac/UI surfacing + the career counter"; per JK's standing browser-sign-off rule, this ticket built ONLY the **DATA LAYER** (build-dark). The `allStarSelections` field existed on both `PlayerCareerBatting`/`PlayerCareerPitching` (`careerStorage.ts:72/:130`) with NO live writer (greenfield). Now: a NEW 4th sub-step `applyFranchiseAllStarCareerSelections` in `runFranchiseAllStarLockPayouts` (the L12-5 honor-ratchet orchestrator, `franchiseAllStarLockPayouts.ts`) increments each selected player's career `allStarSelections` by 1 when the dark All-Star roster LOCKS — **type-routed** (`position` SP/RP → career pitching, else [field positions + WILDCARD] → career batting), via the existing `getOrCreateCareer*`/`updateCareer*` exports. Added to `franchiseAllStarLockPayoutSeam` as `incrementCareerSelections` (mockable); return shape widened `{emit,reachFloor,snub,careerSelections}` (the caller `franchiseAllStarRosterCompute.ts:91` discards the return → safe). **Idempotency = the PARENT lock-once** (`franchiseAllStarRosterCompute` early-returns on `existing?.locked`; the payouts fire once per season at the 60% break) — NO new guard added. **No DB bump** (field + `playerCareerBatting`/`playerCareerPitching` stores already exist in trackerDb). Build-dark: the increment only runs inside the `isFranchisePhase2L12Enabled()`-gated, lock-once parent (default OFF → never fires → careerStorage byte-identical).

**DEFAULTS-TAKEN (documented in code):** (1) type-routing by selected `position`; (2) `playerId` as the getOrCreate `playerName` fallback (the career record already exists by the 60% lock since the player has season stats → name only matters on create, which won't happen); (3) reuse the existing `updateCareer*` path (it stamps `lastUpdated` via `Date.now()` — the established career-write behavior, build-dark-inert at the default flag, so the L-SIM default-flag baseline is unaffected). **NOTE (forward-looking):** the season-end HOF read of `allStarSelections` is currently param-threaded (`isHofCaliber`) and its only `playerCareerStats` map builder is the INACTIVE `OffseasonFlow.tsx:266` (passes `new Map()`) — so there is NO live store-coupled read yet; the counter is a forward deposit, no read-mismatch risk.

**Audit (Opus, REAL diff):** only the 2 contracted files touched; type-routing + +1-per-selection (starters AND reserves) + NO cross-store contamination all test-pinned (the new test pre-seeds a hitter's pitching record + a pitcher's batting record and proves they stay untouched); the careerSelections sub-step is LAST + try/catch-isolated (the existing emit/reachFloor/snub byte-paths unchanged); no UI/flag/oracle/careerStorage-source/DB touch. **GATE (Opus ran the AUTHORITATIVE full suite himself):** tsc 0; build 0; FULL **2 failed / 8387 passed** = `wpaRuntimeBoundary` (hard pre-existing) + `franchiseManualSmokeFixture` (order-flake — re-ran SOLO with the A1.4 focused tests → 3 files / 17 tests all green) ⇒ ZERO NEW REDS. trackerDb stays v26 (no bump).

**⚠ FLAGGED FOR JK (browser-pending, NOT closed):** the L12-6 UI-surfacing half — add the 4 new award categories + TV-family three to `AWARD_ORDER` (`AwardsWatchlist.tsx:17`), feed the flag-off All-Star UI (`FranchiseHome.tsx:1483-1722`) from the real builder, surface race standings/bands. Low-med risk, extends existing surfaces; needs JK's browser sign-off per the standing rule.

**➡ NEXT: A1.5 L4b** — matrix-sourced season takes over the L4a bus (deps L3 + L4a met). Then A1.5d STADIUM RECORDS (own db `kbl-franchise-stadium-records` + own `isFranchisePhase2StadiumRecordsEnabled` flag — NOT a trackerDb bump). Then return to re-grade + RA-7 once JK rules their OPEN-DECISIONS.

---

## 2026-06-26 (UNATTENDED AUTH-4) — A1.5 / L4b DETERMINISTIC CORE (matrix-sourced season-take adapter) SHIPPED

**A1.5 / L4b (the build-dark "matrix is the math" half) — ✅ DONE (`fee76d37`, contract `575e8ee6`, branch-only, ZERO-NEW-REDS).** L4b = "the reporter narrates L3 master-morale-matrix events as season takes over the L4a bus." Grounding (`Explore` agent + Captain re-verify) found: **greenfield** (no L4b code); the adapter half is PURE/deterministic but the EMISSION half requires a live LLM call (`generateSeasonNewsTake` → `callClaudeMessages`, temp 0.6) — inherently browser/LLM-pending, exactly like every existing adapter (L10/L11 are pure+dormant; L12/L13 have LLM-wired emission seams). Per the spec rule §M ("**the matrix is the math [deterministic]; the reporter is the words [LLM-generated]**"), this ticket built ONLY the deterministic math half.

NEW pure `src/src_figma/app/engines/reporter/franchiseL3MatrixNewsAdapter.ts` (+ test) — `buildFranchiseMatrixMoraleSeasonNewsEvent(input): SeasonNewsEvent | null`: a significant matrix consequence → a `SeasonNewsEvent`; **reuses the existing `'SEASON_SUMMARY'` NarrativeEventType** (natural fit for a season-level take; AVOIDS adding a new type → zero `hedgingModifier` exhaustive-Record touch). Significance gate: `null` when `isNeutral` OR `max(|totalPlayerMoraleDelta|, |teamFanMoraleDelta|) < threshold` (per-play morale noise is not a season take). `dramaticWeight = clamp(base + magnitudeScale·clamp(magnitude/denom,0,1),0,1)`. PURE: no LLM, no IO, no `Date.now`, no `Math.random`/`crypto`, fully synchronous, deeply-equal output for equal input. DORMANT (no production caller — mirrors L10/L11). Does NOT mint `id`/`createdAt` (downstream reporter's job).

**DEFAULTS-TAKEN (§16 sim-tune placeholders, documented in the file header):** reuse `'SEASON_SUMMARY'`; `seasonTakeSignificanceThreshold=5` (the line between noise and a take); `base=0.35`, `magnitudeScale=0.35`, `magnitudeDenominator=10` (conservative, mirroring L11's placeholder framing — the Sim Gate tunes later).

**Audit (Opus, REAL diff):** ONLY 2 NEW files; ZERO existing files modified (no `narrativeEngine.ts`/`seasonNewsGenerator.ts`/`processCompletedGame.ts`/flag/oracle touch); purity grep CLEAN; `'SEASON_SUMMARY'` reuse confirmed; test pins the exact event+facts+arithmetic weight, the two null cases (sub-threshold magnitude + neutral-at-high-magnitude), determinism, and weight-scaling-with-clamp (severe case clamps to the 0.70 ceiling) + asserts no id/createdAt. **GATE (Opus ran the AUTHORITATIVE full suite himself):** tsc 0; build 0; focused 5/5; FULL **2 failed / 8392 passed** = `wpaRuntimeBoundary` (hard) + `franchiseManualSmokeFixture` (flake — a brand-new isolated file imported by nothing but its test CANNOT cause it; confirmed solo-passing earlier this session) ⇒ ZERO NEW REDS (+5 L4b tests). No DB bump (v26).
...
## 2026-06-26 (UNATTENDED AUTH-4, same-session continue per JK "keep going") — A1.5d STADIUM RECORDS, hop-1a

JK said "keep going" after the L4b handoff → reclaimed the baton (`HANDOFF_DONE_…_a15d-same-session-continue`), continuing in-session. A1.5d is a 6-hop feature (DETECT → fame swap → fan morale → reporter → Almanac → rivalry edge) + §8 display; building HOP-BY-HOP per the spec ("seam + gate per hop"). The storage + foundation already exist (own db `kbl-franchise-stadium-records`, NO trackerDb bump); deps A1.5a/A1.5b verified MET.

**A1.5d hop-1a (DETECT foundation — the `changes[]` return) — ✅ DONE (`86ffea39`, contract `07482cd4`, branch-only, ZERO-NEW-REDS).** `upsertFranchiseStadiumRecordsFromFoundationReport` now returns `changes: FranchiseStadiumRecordChange[]` — the records that gained a NEW SOLE player holder this upsert. Rule (§5 hop-1): a change fires IFF the new record has a strict sole player holder AND it differs from the prior stored sole holder; `changeKind='set'` when no prior record (new identity), `'overtake'` when an existing record's sole holder changed; **SILENT** on same-holder-improves, tie/co-leadership, and team-only records (no player leaders). `recordFromCandidate` takes the candidate wholesale (the candidate = the recomputed season-to-date record; `existing` = prior game's stored record) → the existing-vs-new sole-holder pair IS the SET/OVERTAKE signal. PURE/ADDITIVE: what gets STORED is byte-unchanged; only the return gained `changes[]` (existing consumers ignore it). Deterministically sorted (stadiumId→recordType→recordKey).

**Audit (Opus, REAL diff):** only the 2 contracted files; the change interface + classification exactly per contract; storage byte-path preserved; no flag/processCompletedGame/foundation/UI/oracle touch; no new record types. Tests pin all 5 cases through the REAL foundation→candidate pipeline (SET; OVERTAKE across two upserts to one store; silent same-holder-improves; silent tie; silent team-only). **GATE (Opus ran the AUTHORITATIVE full suite himself):** tsc 0; build 0; focused 27/27 (storage + the `franchiseSaveSlotManifest` consumer — additive field didn't break it); FULL **2 failed / 8397 passed** = `wpaRuntimeBoundary` (hard) + `franchiseManualSmokeFixture` (flake) ⇒ ZERO NEW REDS (+5 hop-1a tests). No DB change.

**➡ NEXT: A1.5d hop-1b** — the per-game DETECT TAP: add `isFranchisePhase2StadiumRecordsEnabled` (default false; L14 is taken) to `franchisePhase2Flags.ts`; new `persistDarkStadiumRecordsForCompletedGame` in the dark-tap chain (`processCompletedGame.ts:1048-1095`) that assembles `BuildFranchiseStadiumFoundationInput` from the season loads (mirror `franchiseL10SweepCompute`'s `getSeasonGames`/`getGameEvents`), builds the foundation report, reads prior via `listFranchiseStadiumRecords`, and calls the now-`changes[]`-returning upsert — build-dark, changes captured but NOT yet consumed (hops 2-6 consume them). FULL-suite gate (processCompletedGame chain). Then hop-2 (fame swap, §6) → hop-3 (fan morale, §7 — EXHAUSTIVE-RECORD touch: new `PARK_RECORD_SET` MasterMoraleEventType) → hop-4 (reporter, EXHAUSTIVE-RECORD touch + LLM browser-pending: PURE adapter only per [[reporter-adapter-pure-emission-llm-split]]) → hop-5 (Almanac) → hop-6 (rivalry edge) → §8 display (USER-VISIBLE → flag for JK). The §4 fame-bearing catalog (HR-distance/handedness/WPA records) needs the foundation report extended (thread hrDistance/handedness/WPA onto the spray rows) — sequence with hop-2.

---

## 2026-06-26 (UNATTENDED AUTH-4, same-session) — A1.5d hop-1b (per-game DETECT tap + flag) — ⇒ the DETECT foundation is COMPLETE

**A1.5d hop-1b (per-game DETECT tap + the flag) — ✅ DONE (`8ae5e683`, contract `0d3eccfb`, branch-only, ZERO-NEW-REDS).** NEW default-OFF `isFranchisePhase2StadiumRecordsEnabled` flag (clones the L13/L14 pattern; L14 was taken) + NEW `franchiseStadiumRecordsTap.ts` (`persistDarkStadiumRecordsForCompletedGame` behind a mockable `stadiumRecordsTapSeam`): per completed game (flag ON) it builds the stadium foundation report from the season's `completedGames` (`getRecentGames(1000,{franchiseId,seasonId})`) + the franchise stadium snapshots (`getFranchiseConfig(franchiseId)?.stadiums`) — the LIGHT input proven by the de-orphan caller `franchiseSeasonSummaryStorage.ts:694` — and calls hop-1a's `changes[]`-returning upsert (changes captured, NOT yet consumed). Wired into the `processCompletedGame` dark-tap chain after the L12 block; **double-gated + build-dark** (flag OFF ⇒ zero-cost no-op, NO loads — test proves none of the 4 seam loaders are called). `PersistedTrueValueScope` exported (type-only import into the tap → no runtime cycle). Light input only; the at-bat/fielding event loads (for the spray + §4 fame-bearing records) are hop-1c. No DB change.

**⚠ The transitive-import-mock-break fired + was FIXED in the same commit (the full-suite gate earned its keep).** The new tap (transitively imported via `processCompletedGame`) reads `getRecentGames`/`getFranchiseConfig` at module-load through the eager seam object → 3 partial-mock test files (`processCompletedGame.trueValue`, `processCompletedGame.almanac`, `FranchiseHomeLaunch`) failed to COLLECT ("No getRecentGames export on the ../gameStorage mock") — the documented [[processcompletedgame-transitive-import-mock-break]] (deterministic, NOT a flake; a scoped run would've missed it). Fix = test-only mock stubs: `getRecentGames: vi.fn().mockResolvedValue([])` into the two `gameStorage` mocks + `getFranchiseConfig: vi.fn().mockResolvedValue(null)` into FranchiseHomeLaunch's `franchiseManager` mock (its gameStorage mock already had getRecentGames). Captain applied the stubs directly (mechanical test unblock; the full-suite re-run is the objective gate). **GATE (Opus, AUTHORITATIVE full suite, post-fix):** tsc 0; build 0; focused 3/3 (tap) + 35/35 (the 3 ex-broken files + tap re-collect green); FULL **2 failed / 8400 passed** = `wpaRuntimeBoundary` (hard) + `franchiseManualSmokeFixture` (flake) ⇒ ZERO NEW REDS (+3 tap tests). No DB change.

⇒ 🎯 **A1.5d hop-1 DETECT FOUNDATION COMPLETE** (hop-1a `changes[]` + hop-1b per-game tap+flag) — the per-game "a player set/took over a park record" detection is wired build-dark; nothing consumes the changes yet.

**➡ NEXT options for A1.5d:** **hop-1c** (extend the foundation input with at-bat/fielding event loads + thread `hrDistance`/`handedness`/WPA onto the spray rows → the §4 fame-bearing record catalog) OR **hop-2** (fame swap, §6 sign-law — the first CONSUMER of `changes[]`: polarity-signed heatDeltas concat onto `fameResult.playerHeatDeltas`). Then hop-3 fan morale (⚠ new `PARK_RECORD_SET` MasterMoraleEventType — exhaustive-Record touch) → hop-4 reporter (⚠ new `STADIUM_RECORD` NarrativeEventType + PURE adapter per [[reporter-adapter-pure-emission-llm-split]]) → hop-5 Almanac → hop-6 rivalry edge → §8 display (USER-VISIBLE → flag for JK). After A1.5d the queue is JK-DECISION-BLOCKED (re-grade store + RA-7).

---

## 2026-06-26 (UNATTENDED AUTH-4, fresh `/kbl-captain`) — A1.5d hop-1c (§4 fame-bearing record catalog) SHIPPED + an adversarial-audit fix-pass

Fresh session; baton claimed (`HANDOFF_DONE_20260626T043953Z_a15d-hop1c-captain`); sole worker confirmed (no `codex exec`, no other `claude` on this branch); caffeinate alive (PID 84474). Continued A1.5d hop-by-hop, build-dark on `codex/franchise-v1-next`.

**A1.5d hop-1c (§4 fame-bearing park-record catalog — DETECT) — ✅ DONE (`a97b50b8`, contract `f85acd63`, fix-contract `3fb3be6e`, branch-only, ZERO-NEW-REDS).** Adds the 10 §4 fame-bearing record types so the DETECT records become meaningful; they ride hop-1a's existing `changes[]` SET/OVERTAKE detection for free.

**⚙️ KEY GROUNDING REVISION (Captain-verified from source — REVISES the handoff's premise):** the handoff envisioned "extend the foundation input with `getGameEvents` loads + thread hrDistance/WPA onto `FranchiseSprayChartRow`." Verification found that UNNECESSARY and higher-risk: the §4 source data is ALREADY INLINE on the `CompletedGameRecord[]` the tap passes to the upsert — `AtBatEvent.wpa` (per-play, batter's-team perspective, `eventLog.ts:248`), `.enrichment.hrDistance` (`:436`), `.batterContext.handedness` (`:322`), and `CompletedGameRecord.playerWpaTotals` (`KblWpaPlayerTotal` with `totalWpa`/`pitchingWpa`, `gameStorage.ts:624`; both `atBatEvents` + `playerWpaTotals` are archived inline by `enrichArchivedAtBatEvents`/the completeGame writer and returned by `getRecentGames`). So the §4 records are computed DIRECTLY from `options.completedGames` in a new `fameBearingCandidates` builder — **touching ONE production file (`franchiseStadiumRecordsStorage.ts`) + its test**, NO `franchiseStadiumFoundation.ts` (read-only contract) change, NO tap/flag/`processCompletedGame`/DB change, and purely additive (the existing storage tests pass bare `completedGame()` fixtures with no inline atBat/WPA → zero new records → `toHaveLength(3)` stayed 3). Also lower-risk for the single-play-swing records, which need full play context (inning/outs/score) that doesn't belong on a spray row.

**The 10 new `FranchiseStadiumRecordType` + a `FRANCHISE_STADIUM_RECORD_TYPE_POLARITY` map** (the seam hop-2 reads; 8 existing types = 0/non-fame-bearing, 10 new = ±1 per §4): farthest-hr-rhb/lhb (+, max `hrDistance`, switch-hitters 'S'/null-handedness excluded from both, blank distance ineligible but still counts toward HR records), most-hr-here-season (+, per batter), most-hr-allowed-pitcher (−, per pitcher), highest/lowest-cumulative-wpa-position (±, `totalWpa−pitchingWpa`), highest/lowest-cumulative-wpa-pitcher (±, `pitchingWpa`), largest-positive/negative-wpa-swing (±, max/min `AtBatEvent.wpa`, batter-attributed, play context in `evidenceSummary`+`evidenceIds` for hop-4).

**🔍 ADVERSARIAL AUDIT (ultracode — a parallel skeptic subagent ran while the first gate ran) found TWO real DETECT-correctness defects the green tests didn't cover → Captain fixed both via a Codex fix-dispatch (fix-iter #1, builder≠auditor kept):**
1. **WPA eligibility:** the builder built BOTH a position aggregate (`total−pitching`) AND a pitcher aggregate (`pitching`) for EVERY player row → a pure pitcher sat at position-`0` and could win the `lowest-cumulative-wpa-position` record (a position record won by someone who never batted); symmetric for pure hitters at pitcher-`0`. **Fix:** restrict each pool to aggregates with a NON-ZERO contribution in that dimension (`positionEligible`/`pitcherEligible`) before max/min/leaders.
2. **Single-play-swing ties:** the swing records used `reduce` → exactly ONE leader, so an equal-`wpa` tie collapsed to a sole leader and fired an ILLEGAL `overtake` (§5/§12: ties stay silent — only a strict new SOLE holder swaps). **Fix:** collect ALL `wpa === maxWpa`/`=== minWpa` events as co-leaders (mirrors the already-correct farthest-HR/`leadersByValue` pattern) → a tie yields >1 leader → the `changes[]` detector correctly stays SILENT.

**DEFERRED (documented §16 refinement for hop-2, NOT fixed):** float-equality (`===`) on SUMMED cumulative WPA for co-leader detection — an exact float tie can falsely become a sole leader. Low-impact (the max-holder is always correctly identified; only true near-ties are affected) + matches the existing integer-record exact-equality pattern; adding epsilon is its own judgment call → leave for hop-2 when the consumer + magnitudes land.

**Audit (Opus, REAL diff — not the Codex paste):** read the full storage + test diff; the 10 types + polarity map exact per §4; `recordFromCandidate`/upsert loop/`changes[]` classification UNTOUCHED (new records ride them); only the 2 contracted files; no foundation/tap/flag/processCompletedGame/oracle/DB touch; no `Date.now`/`Math.random`. Tests pin: farthest-hr-rhb SET→OVERTAKE; switch-hitter+blank-distance excluded from farthest but counted in HR records; most-hr-allowed; position-vs-pitcher WPA split with a two-way player holding BOTH; lowest picks the min holder; swing batter-attribution + play context + all-positive→no-negative-record; the polarity map; the idempotency `toHaveLength(3)` unchanged; + the 2 fix tests (pure-pitcher excluded from lowest-position; equal-wpa tie → co-leaders + silent changes). **GATE (Opus ran the AUTHORITATIVE full suite himself, post-fix):** build 0; FULL **8406 passed / 3 failed** = `wpaRuntimeBoundary` (hard) + `franchiseManualSmokeFixture` + `franchiseOffseasonGuards` (both order-flakes — `franchiseOffseasonGuards` re-ran SOLO → 24/24 green) ⇒ ZERO NEW REDS (storage 20 ✓, tap 3 ✓). No DB change (v26).

⇒ 🎯 **A1.5d hop-1 DETECT is now MEANINGFUL** — the per-game detection records the full §4 fame-bearing catalog (HR distance/count, HR-allowed, cumulative + single-play WPA), still build-dark, changes[] still unconsumed.

**➡ NEXT: hop-2 FAME swap (§6).** (Grounding for the next session below.)

### §1 (fame-fluctuation prerequisite) — ✅ ALREADY DONE / RESOLVED-WITHOUT-BUILD (Captain-verified from source 2026-06-26)
The spec §1 says "build this first" to remove a per-game `reachFloor` ratchet so fame can fall (the §6 sign-law needs an overtaken player to LOSE fame). **Verification: the ratchet is already gone — §1's stated `franchiseFameCompute.ts:109 updateReachFloor(stored.reachFloor, heat)` does not exist; the actual `franchiseFameCompute.ts:108` is `const reachFloor = stored.reachFloor;` (pure pass-through), and `updateReachFloor` is called ONLY in `fameModel.test.ts`, never live.** Heat fluctuates freely via `applyHeatUpdate(stored.heat, breakdown.total)` (`:107`, applies the signed delta). The honor floor (§1 point 2) already pins to REGIONAL_STAR: `franchiseHonorReachFloor.ts:34` = `Math.max(row.reachFloor, FAME_TIER_RANK.REGIONAL_STAR)`. And `resolveFameTier(heat, reachFloor)` (`fameModel.ts:205`) floors the displayed tier by `Math.max(heatRank, reachFloor)` → a non-honored player (reachFloor 0) tracks heat and **CAN fall** = §1's intent. The L6 fame buildout already implemented §1; the spec was written against the older ratcheted code. **⇒ hop-2 is UNBLOCKED.** (Optional nicety deferred: an explicit L-SIM `soul.*` invariant that a non-honored tier can fall — verification-only, not a code blocker.)

### hop-2 FAME swap — architecture grounding (do this next; ground the remaining bits before contracting)
**Goal (§6 sign-law):** consume the stadium `changes[]` (now produced by the hop-1c-complete DETECT) → polarity-signed fame heat bumps. Per record polarity `s = FRANCHISE_STADIUM_RECORD_TYPE_POLARITY[recordType]` (the seam hop-1c exported) + base magnitude `B` (§16: `B_set 2.0`, `B_break 1.5`, `B_overtaken 1.0`; farthest-HR + the single-play-swing records weighted above most-HRs-here). SET → `newHolderΔ = +B_set·s`. OVERTAKE → `newHolderΔ = +B_break·s`, `oldHolderΔ(each priorLeaderPlayerId) = −B_overtaken·s`.
**The seam (verified file:lines):** the fame bridge `persistFameMoraleConsequencesAfterFame` (`processCompletedGame.ts:520`) iterates `playerHeatDeltas: {playerId, heatDelta}[]` (skip 0) and composes a `FAME_HEAT_CHANGED` morale consequence per entry (player + fan morale), dedup'd by `fameMoraleSourceEventId` (`fame:…:checkpoint:playerId:heat-delta`, `:503`). It is called at `:1168-1180` inside `if (isFranchisePhase2FameEnabled())`, right after `fameResult = persistDarkFameRecordsForCompletedGame(...)` produces `fameResult.playerHeatDeltas`. §5.2's "concat the polarity heatDeltas onto `fameResult.playerHeatDeltas`" rides this bridge.
**THE 4 THINGS TO RESOLVE BEFORE CONTRACTING (grounding not yet done):**
1. **Ordering / restructure:** the hop-1b stadium tap (`persistDarkStadiumRecordsForCompletedGame`) runs LATER (after the L12 block, ~`:1224+`), AFTER the fame block — and it currently RETURNS only `changes: <count>`, not the array. hop-2 needs the actual `FranchiseStadiumRecordChange[]` available BEFORE/at the fame block. Cleanest: surface the DETECT changes[] earlier (run the detect right after trueValueSnapshots, hold the array) and have hops 2–6 consume it; the tap's `PersistDarkStadiumRecordsResult` must return the `changes` ARRAY (not the count). Decide: restructure-to-run-detect-early vs compute-detect-inside-the-fame-block (double-compute, build-dark, no reorder). The restructure is the right long-term shape for hops 3–6 but touches the chokepoint ordering.
2. **HEAT vs MORALE split (re-read first):** the bridge moves MORALE only (`FAME_HEAT_CHANGED`). The stored fame HEAT is written by `persistDarkFameRecordsForCompletedGame` (`franchiseFameCompute.ts:107-126`, `heat = applyHeatUpdate(stored.heat, breakdown.total)`). §6 says the swap "moves both holders' HEAT and their player morale." Confirm whether concat'ing `playerHeatDeltas` alone moves the stored heat, or whether the stadium bump must also be added to `breakdown.total`/the heat write inside the fame compute. Ground `persistDarkFameRecordsForCompletedGame` + whether the bridge writes heat.
3. **Sentinel collision / idempotency:** the bridge dedups by `fame:…:playerId:heat-delta`. A naive concat of a stadium bump for the SAME player produces a duplicate sourceEventId → collision. §5.2 mandates a DISTINCT per-bump sentinel `park-record:{stadiumId}:{recordType}:{recordKey}:{checkpoint}:{holderId}`. So the stadium bumps need their own sentinel/source-event path (not the fame heat-delta one) — likely a separate compose/apply with the park-record sourceEventId, OR a merge of same-player deltas before the bridge. Decide + ground how composeMoraleConsequence/the apply path keys the sentinel.
4. **Double-count guard (§6):** the holder's PRIDE lives ONLY in the fame heat path (which moves self-morale via the bridge). hop-3's `PARK_RECORD_SET` morale row must carry ONLY fan + teammate deltas (self-delta 0) — mirror the §13 Albatross guard. (This is a hop-3 constraint to note now so the two don't double-count self-morale.)
**Gate:** `isFranchisePhase2FameEnabled` (+ the StadiumRecords flag for the detect). Build-dark at default flags → L-SIM baseline byte-identical (run smoke+season, standard 60g leg LAST). FULL suite (processCompletedGame chokepoint).

Then hop-3 fan morale (⚠ exhaustive-Record `PARK_RECORD_SET` MasterMoraleEventType) → hop-4 reporter (⚠ exhaustive-Record `STADIUM_RECORD` NarrativeEventType + PURE adapter per [[reporter-adapter-pure-emission-llm-split]]) → hop-5 Almanac → hop-6 rivalry edge → §8 display (USER-VISIBLE → flag for JK). After A1.5d the queue is JK-DECISION-BLOCKED (re-grade store + RA-7).

---

## 2026-06-26 (UNATTENDED AUTH-4 `/kbl-captain`, fresh session) — A1.5d hop-2 FAME SWAP SHIPPED + a PRE-EXISTING L-SIM season break discovered (NOT hop-2)

**A1.5d hop-2 (stadium-record fame swap, §6 polarity sign-law) — ✅ SHIPPED (`219bdb3a`, contract `1a61e6e6`, branch-only, build-dark).** The first CONSUMER of the DETECT `changes[]`. Per §6: each record's polarity sign `s` (+glory/−dubious) drives a fame HEAT swap — new sole holder gains (`+B_set`/`+B_break`), each dethroned prior holder loses (`−B_overtaken`), sign-preserving (negative records invert → an overtaken infamy-holder is RELIEVED and rises). **Captain design call (adversarial-review-upheld): FOLD the polarity bump into the single fame heat write** (`persistDarkFameRecordsForCompletedGame` via `applyHonorHeatBump`, additive/no-decay) so stored heat moves AND the existing fame→morale bridge moves morale from the combined per-player delta — **no bridge change, no new sentinel.** The spec's literal §5.2 ("concat onto playerHeatDeltas" + a distinct `park-record:` sentinel, with the bridge untouched) is **internally unimplementable** (the bridge derives its OWN per-player `fame:` sentinel and never reads a per-entry field); folding is the faithful resolution (summing into one per-player delta keeps the `fame:` sentinel unique → no collision; idempotency comes from the DETECT emitting an empty `changes[]` on checkpoint re-run). Files: NEW pure `franchiseStadiumRecordFame.ts` (`buildStadiumRecordFameHeatBumps` — "the matrix is the math", unit-tested incl. the negative-record-relief + net-aggregation cases) · `franchiseFameCompute.ts` (optional `stadiumChanges` param, default `[]` → byte-identical; folds bumps + writes dethroned non-playing holders) · `processCompletedGame.ts` (detect runs EARLY → feeds the fame writer; late duplicate detect removed) · `franchiseStadiumRecordsTap.ts` (returns `changeList[]`) + 3 test files.

**Adversarial design review (ultracode, 4 skeptic lenses before the build):** UPHELD the fold (sign-law correct incl. the inverted negative-record case; `applyHonorHeatBump` is the right no-decay primitive; the deviation is spec-intent-faithful + reversible). It also CORRECTLY flagged that the L-SIM baselines would change under flags-ON (fame/morale are dumped) — which led to the season run that surfaced the finding below.

**GATE (Captain ran everything himself):** build 0 · **FULL suite ZERO-NEW-REDS (8413 pass / 3 characterized fails)** · L-SIM smoke 24g all-CRITICAL-pass (heatTransitions up160/down338 → fame genuinely fluctuates DOWN, the §1/§6 intent; infamy tiers populate) · **all fame/morale/idempotency/double-count soul invariants PASS through game 36** · the **determinism-seed leg runs a full clean 60g WITH hop-2 and is same-seed byte-identical** (`sameSeedByteIdentical: true`). No DB change (v26).

**🔧 A pre-existing L-SIM test-harness leak — FIXED (`19b75104`).** The stadium-records own-DB (`kbl-franchise-stadium-records`) was absent from the L-SIM sandbox reset list (`DB_NAMES_TO_DELETE`), so its state leaked across the determinism legs (leg 2 inheriting leg 1's records). Invisible to the dumped digest until hop-2 wires records→fame; without the fix, hop-2's same-seed determinism leg FAILS (the leak yields different `changes[]` → different fame). Added the DB to the reset list → determinism restored. (Pristine HEAD without hop-2 is unaffected: `sameSeed` is already true there.)

### ⚠ OPEN-DECISION FOR JK (CRITICAL — PRE-EXISTING, NOT hop-2): the L-SIM season is RED at game 36 and the committed baselines are STALE
**Symptom:** the L-SIM **season** (`seasonRunner.scenario.ts`, baseline seed `lsim-h2-baseline`, standard cadence) **stops early at game 36** with 1 CRITICAL finding: **`soul.trait-two-slot-no-offset-hysteresis`** — 5 trait **GAIN** overlays at `trait-grant-36` (Wild Thrower ×3, Noodle Arm, Tough Out on team-03/06) are persisted **pending with stored probability 0.717–0.746, BELOW the gain threshold 0.75**. The invariant flags `gain && probability < gainThreshold`.
**Proven PRE-EXISTING (not hop-2, not the harness fix):** ran the season on **pristine HEAD** (hop-2 fully reverted, harness fix reverted) → it trips at game 36 **identically** (same digest `5763890:53248585`, `sameSeed: true`). So current `codex/franchise-v1-next` HEAD (≤ hop-1c `a97b50b8`) already trips this. The committed `lsim-h2-baseline-checkpoint-040/050/060.json` are dated **Jun 23** → STALE; the season hasn't completed 60g since some post-Jun-23 soul-layer commit introduced the trip. The **smoke (24g) never reaches game 36**, so smoke-based verification (the prior cadence) missed it; the scenario test asserts `games>0` (not zero findings / not `stoppedEarly`), so it "passes" regardless.
**Likely root (for the trait investigation):** the trait-grant GATE (`traitAcquisition.ts:420`, `proposalBase.probability >= gainThreshold`) and the STORED `proposal.probability` **diverge** — a gain gated at ≥0.75 is stored at <0.75. Most likely the trend-tilt recent-window recompute (T-3b, `trendTilt` factor in `buildProposalBase`) or the `reconcileGainProposals` duel adjusts probability AFTER the gate. This is a latent trait-grant consistency bug, OR `soul.trait-two-slot-no-offset-hysteresis`'s gain-threshold assertion is checking a post-trend probability the grant intentionally stores — a trait-system call.
**Impact:** the canonical L-SIM **season** regression anchor is broken + stale → no clean 60g re-bake is possible for ANY soul-layer work (hops 3-6 + beyond) until this is resolved. hop-2 landed on the established cadence (full-suite + smoke + determinism-seed 60g), but did NOT touch the stale committed season baselines.
...
## 2026-06-26 (UNATTENDED AUTH-4 `/kbl-captain`, JK "keep rolling") — ✅ A1.5d hop-3 PARK_RECORD_SET fan-morale buzz SHIPPED

**A1.5d hop-3 (home-crowd fan buzz, §5.3+§6+§7-first-half) — ✅ DONE (`6805ab74`, contract `ed32eaae`, branch-only, build-dark).** Second consumer of hop-2's `stadiumChanges`. When a stadium record gains a new SOLE holder on the stadium's HOME team, the home fans get a morale buzz via a new `PARK_RECORD_SET` MasterMoraleEventType (clone of PLAYER_MILESTONE: fan +4, teammate +1, **self 0**). New dark writer `persistDarkParkRecordMoraleForCompletedGame` (mirrors the trade-demand sibling): gate `isFranchisePhase2MoraleEnabled` → per change, sole-holder-only (`newLeaderPlayerIds.length===1`) → resolve home team via `getFranchiseConfig().stadiums.find(stadiumId).teamId`, holder team via `sourceTeamIdForFameMorale` → fire ONLY if holder is on the home team (visitor/tie → no buzz) → `composeMoraleConsequence({ type: 'PARK_RECORD_SET' })` → `applyFranchiseMoraleMatrixConsequence` with a checkpoint-embedded `park-record-set:…` sourceEventId. Wired after the fame block, before flashpoint. 2 prod files (masterMoraleMatrix exhaustive row + processCompletedGame writer/wiring) + 1 new test.

**§6 DOUBLE-COUNT — adversarially confirmed a NON-ISSUE (the contract's grounding NOTE was wrong, the build is right):** the worry was that hop-2's fame→morale bridge also moves the holder's TEAM-FAN morale, double-counting with hop-3's fan buzz. **FALSE, verified at source:** `resolveFameTap` (`masterMoraleMatrix.ts:439`) forces `teamFanMoraleDelta: 0` (`// never push §20.6 fan morale`), and `franchiseMoraleState.ts:544` gates the team-fan write on `!== 0` → the fame bridge writes SELF/player morale ONLY. So PARK_RECORD_SET is the SOLE path to home-fan morale for a record; self-delta-0 handles the only real overlap (self/pride). hop-3 correctly KEEPS its fan=+4 (zeroing would delete the §7 buzz). (Correct the stale "bridge moves both" note wherever it appears.)

**GATE (Captain ran everything):** build 0 · **FULL suite ZERO-NEW-REDS (8419 pass / 2 characterized fails** = `wpaRuntimeBoundary` hard + `franchiseManualSmokeFixture` flake; `franchiseOffseasonGuards` passed this run) — transitive-mock-break did NOT fire (existing mocks already satisfy the new `getFranchiseConfig` import) · L-SIM smoke 24g all-CRITICAL-pass · **falsification 44/44** · **season clean 60g (findings 0, criticalFindings 0, sameSeedByteIdentical true** — `soul.morale-bounds` held; the +4 buzz stays in [0,99]). Re-baked the season anchor (digest `9425360:10944d37`; the flags-on home-fan buzz is additive on home-team sole-leader records). No DB change (v26).

**Adversarial design review (ultracode, 3 lenses) UPHELD the design** (no fan double-count [source-verified], exhaustiveness single-touch-point, idempotent, build-dark). NIT (documented, non-blocking): stadium-name/id collision in `getFranchiseConfig().stadiums.find` is first-match — theoretical for v1 (one league, unique park ids).

**➡ NEXT: A1.5d hop-4 (reporter — a `STADIUM_RECORD` NarrativeEventType + a PURE news adapter; the LLM emission seam is browser/JK-pending per [[reporter-adapter-pure-emission-llm-split]]).** hop-4 IS the narrativeEngine touch (the exhaustive `NarrativeEventType` hedging/highStakes maps — distinct from hop-3's MasterMoraleEventType). Then hop-5 Almanac → hop-6 rivalry edge (+ the §7 home-park-RIVALRY 2× deferred slice) → §8 display (USER-VISIBLE → flag for JK). The L-SIM season anchor is HEALTHY; build hops 4-6 against it.

---

## 2026-06-26 (UNATTENDED AUTH-4 `/kbl-captain`, JK "keep rolling") — ✅ A1.5d hop-4 STADIUM_RECORD reporter PURE adapter SHIPPED

**A1.5d hop-4 (reporter deterministic half, §5.4) — ✅ DONE (`20732a1c`, contract committed, branch-only, build-dark/DORMANT).** A new `STADIUM_RECORD` NarrativeEventType + a PURE adapter `buildFranchiseStadiumRecordSeasonNewsEvent` (`src/src_figma/app/engines/reporter/franchiseStadiumRecordNewsAdapter.ts`) that maps a `FranchiseStadiumRecordChange` → a `SeasonNewsEvent` (facts = recordType/recordKey/stadiumId/stadiumName/changeKind/newValue/oldValue/new+overtaken holders/batterHand[HR records]/playContext[null]/triggerPhase; `dramaticWeight = clamp(set 0.5|overtake 0.6 + 0.25·magnitude)`; subjectIds deduped new-first; NO id/createdAt). PURE — no LLM/IO/`Date.now`/`Math.random`/`crypto`. DORMANT (no production caller — mirrors `franchiseL3MatrixNewsAdapter`).

**Per the reporter split [[reporter-adapter-pure-emission-llm-split]]:** BUILT the deterministic adapter ("the matrix is the math"); **FLAGGED for JK (NOT built — the "reporter words" half):** the LLM emission seam (`generateSeasonNewsTake`→`callClaudeMessages`), the `processCompletedGame` wiring, the emission flag + dedup-on-emit. (Clone `franchiseRelationshipFlareEmission`.)

**FORK RULED (Captain, from grounding):** ADD a new `STADIUM_RECORD` NarrativeEventType rather than reuse `SEASON_SUMMARY`. Exhaustive touch = exactly 2 trivial compile-enforced edits (`narrativeEngine.ts` union + `hedgingModifier: 1.0` [factual, like MILESTONE/AWARD_RESULT]) — confirmed the ONLY `Record<NarrativeEventType>` requiring an entry (`requiresRetraction`/`highStakesEvents` is a plain array — records aren't retraction-prone, omitted; `reporter.ts perEventRate` is `Partial`, optional). Reuse would collide with `franchiseL3MatrixNewsAdapter`'s SEASON_SUMMARY + muddy the Almanac-scoped event identity — not worth saving 2 edits.

**DEFERRED (documented v1, §4 best-effort):** `playContext: null` for the single-play-swing records — the exact play lives in `CompletedGameRecord.atBatEvents`, off the change/record; threading it would break the pure change→event contract. A later schema ticket threads it. (Plain-language flagged to JK; reversible.)

**GATE (Captain ran everything):** build 0 · **FULL suite ZERO-NEW-REDS (8425 pass / 3 characterized fails** = `wpaRuntimeBoundary` + `franchiseManualSmokeFixture` + `franchiseOffseasonGuards`) · adapter unit test 7/7 (incl. a verbatim-facts `toEqual` + dramaticWeight arithmetic + batterHand + subjectIds dedup + no-id/createdAt) · **L-SIM season BYTE-IDENTICAL** (digest `9425360:10944d37` unchanged, git diff empty — the new type is DORMANT in the sim; no caller emits it, `hedgingModifier` only read during LLM emission which the sim doesn't run). No DB change (v26).

**➡ NEXT: A1.5d hop-5 (Almanac — the stadium-record change maps 1:1 to a new `'park-record'` AlmanacNarrativeArchiveEntry, same scope keys; + the standing record embeds in the persisted season summary so the Almanac houses it independent of the LLM take, §5.5). De-orphan at season-finalize + the per-game tap.** Then hop-6 rivalry edge (+ the §7 home-park-rivalry 2× deferred slice) → §8 display (USER-VISIBLE → flag for JK). **HANDED OFF here** (clean seam, nothing in flight) — this session shipped hop-2 + the L-SIM harness fix + the pre-existing trait-trip fix + hop-3 + hop-4; handing hop-5 to fresh context. The L-SIM season anchor is HEALTHY (clean 60g, reproducible byte-identical).

---

## 2026-06-26 (UNATTENDED AUTH-4 `/kbl-captain`, fresh session) — ✅ A1.5d hop-5 ALMANAC SHIPPED (`84175be3`)

**A1.5d hop-5 (Almanac housing, §5.5 + §9 de-orphan) — ✅ DONE (`84175be3`, contract `38890a97` + fix `A1.5d-hop5-fix`, branch-only, build-dark).** Houses each standing stadium record in the franchise-season Almanac INDEPENDENT of the LLM reporter take, in two flag-gated (`isFranchisePhase2StadiumRecordsEnabled`) deliverables.

**⚙️ MAKE-OR-BREAK GROUNDING REVISION (Captain-verified from source — REVISED the spec premise):** §5.5 says "the change maps 1:1 to a new `'park-record'` AlmanacNarrativeArchiveEntry," but `AlmanacNarrativeArchiveEntry` is **NOT a persisted store with a writer — it is a DERIVED VIEW** (`listAlmanacNarrativeArchive` materializes entries on-the-fly from gameStories/commentaryFeed/transaction-log; no `putAlmanac…`). So the faithful, lowest-risk delivery = teach that derivation to ALSO materialize `'park-record'` entries from the already-persisted `FranchiseStadiumRecord` rows (mirroring `transactionToArchiveEntry`). Read-side, no new write/store, L-SIM byte-identical.

- **A (Almanac derivation, `almanacNarrativeArchive.ts`):** `+ "park-record"` on `AlmanacNarrativeKind`; new pure `stadiumRecordToArchiveEntry` (id=`park-record:{identityKey}`, kind, gameMode=franchise, finite-guarded `Date.parse(updatedAt)` ts, scope + leaders + headline/body); flag-gated branch in `listAlmanacNarrativeArchive` (after the transaction branch) that derives `seasonNumber` from a matching completedGame and reads `listFranchiseStadiumRecords`. Flag-off ⇒ no park-record entries (byte-identical).
- **B (season-summary embed, `franchiseSeasonSummaryStorage.ts`):** new `FranchiseSeasonSummaryStadiumRecord` trimmed-snapshot type + `FranchiseSeasonSummaryStadiumRecords` union; optional `stadiumRecords?` field on `FranchiseSeasonSummary`; `toStadiumRecordSnapshot` (drops policies/storageVersion, adds polarity from the map); flag-gated populate in `buildFranchiseSeasonSummary`, **OMIT-when-off** (zero blast radius — existing flag-off summaries byte-identical).

**🔍 ADVERSARIAL REVIEW (ultracode, 4 skeptic lenses) + a source-trace fix-pass (fix-iter #1):** lsim-dormancy CLEAN (both deliverables provably never invoked in the sim → no re-bake), spec-faithfulness CLEAN, blast-radius CLEAN (the Almanac UI mislabel is the known JK-pending polish, not a regression). The scope-derivation lens raised a MAJOR: deliverable A originally read the records' `statsScopeId` from a completedGame's `statsScopeId` field, which *could* diverge from the records' write-scope → silent zero results. **Source-trace VERDICT: the franchise invariant `statsScopeId === seasonId === getFranchiseSeasonId` is GUARANTEED** (`processCompletedGame.ts:275-278` WAR scope `{seasonId, statsScopeId: seasonId}`; `franchisePersistenceContract.ts:79-85` canonical handoff; the completed game's stored scope coincides; the L-SIM's own `lsimIds` shows `seasonId === statsScopeId`). So the finding can't fire in the only flow with franchise records — but **HARDENED anyway** (read `statsScopeId: filters.statsScopeId ?? filters.seasonId`, dropping the completedGame-`statsScopeId` dependency, aligning A with B) + a NEW divergent-`statsScopeId` regression test that pins the robustness (would fail pre-hardening) + invariant comments in both files.

**GATE (Captain ran everything):** build 0 · **FULL suite ZERO-NEW-REDS (8431 pass / 2 characterized fails** = `wpaRuntimeBoundary` HARD + `franchiseManualSmokeFixture` order-flake [solo-passed 28/28 this session]; `franchiseOffseasonGuards` passed clean) · +8 hop-5 tests green · **L-SIM season clean 60g BYTE-IDENTICAL** (`stoppedEarly:false, findings:0, criticalFindings:0, sameSeedByteIdentical:true, finalDigest 9425360:10944d37` = the committed anchor; baseline JSONs git-diff empty → no re-bake; no stray own-DB files). No DB change (v26).

**⚠ FLAGGED FOR JK (browser-pending, NOT built — USER-VISIBLE):** the Almanac UI polish (`AlmanacNarratives.tsx`: a "PARK RECORDS" `KIND_FILTERS` chip + a `kindLabel`/`kindIcon` case — a new kind currently renders gracefully under the default "POST-GAME SUMMARY" label). Plus §8 per-stadium stat-display (separate later ticket).

**➡ NEXT: A1.5d hop-6 (player RIVALRY edge, §5.6) — an OVERTAKE stamps a persisted `RelationshipEdgeRow` (old↔new holder) via `putFranchiseRelationshipEdge` (event-spark writer; RIVALRY or unused HISTORY slot), gated `isFranchisePhase2L13Enabled`.** Then the §7 home-park-rivalry 2× deferred slice → §8 display (USER-VISIBLE → flag for JK). After A1.5d the queue is JK-DECISION-BLOCKED (re-grade store · RA-7 park-adjust). ⚠ hop-6 writes a relationship edge — GROUND whether the edge store is DUMPED (→ L-SIM re-bake) or its own db, and whether L13 is already flag-defined.

---

## 2026-06-26 (ATTENDED — JK directing) — HOME-PARK TEAM-RIVALRY feature, STEP 1 (HPR-A "rival brain") SHIPPED

**Context:** JK redirected (attended) from the queued hop-6 (player↔player edge) to a NEW **home-park TEAM rivalry** (a refinement/expansion of spec §7). JK's rulings (2026-06-26): a team's home-park rival = the opposing team that OWNS its home park this season; **ranked by wins at the park** (player-style park records held break a wins-TIE ONLY); **holding a record is NOT required** to qualify (JK corrected this mid-build — "if a team has won three games at your park and no one else has won that many, they are your rival regardless of whether they hold a park record"); **sticky incumbency** (a challenger replaces the incumbent only by STRICT overtake — more wins, or equal-wins+more-records; a pure tie never dethrones); **directional/asymmetric** (per home team — you may not be theirs); **per-season**. Plus (for step 3): rival games (home AND away — widens §7's home-only) amplify FAN morale AND slightly the team CAPTAIN's morale, win or lose, "a bit more" (tunable). Decomposed into 3 steps: **(1) rival brain [this], (2) the RED first-person-hub coloring [USER-VISIBLE], (3) the morale amplifier [DUMPED store → re-bake].**

**HPR-A (step 1 — rival engine + store + tap) — ✅ DONE (`64dd9ab1`, contract `6858a788` + correction, branch-only, build-dark).** A grounding workflow (5 areas) mapped the terrain: 5 disconnected pre-existing "rivalry" concepts (none is this), the orphaned `rivalryScores` store, the live `vsRival` = symmetric DIVISION membership driving a flat ±1 (NOT a multiplier — the spec §7 "freebie/2× dial-flip" framing is INACCURATE; the amp is net-new), and the hub has NO name-color mechanism + NO lens-team threaded. Built: **(a)** PURE `franchiseHomeParkRivalCompute.ts` (`computeHomeParkRival` — the full rule, unit-tested 8 cases incl. wins-leader-with-zero-records-crowned, records-tiebreak-only-on-equal-wins, more-records-fewer-wins-does-NOT-overtake, pure-tie-retains, strict-overtake); **(b)** own DB `franchiseHomeParkRivalStorage.ts` (`kbl-franchise-home-park-rivals` v1, store `homeParkRivals`, scope-keyed, `onversionchange` guard — NOT in L-SIM `DUMP_DATABASES` → byte-identical; ADDED to sandbox `DB_NAMES_TO_DELETE`); **(c)** `franchiseHomeParkRivalTap.ts` (per-game, gated `isFranchisePhase2StadiumRecordsEnabled` [reused — rival is a downstream consumer of the records], runs EVERY franchise game: home team's CONFIGURED park, opponent wins = away>home, distinct-visitor incl. 0-win, records-held = player-attributable [`leaderPlayerIds.length>0`] count, idempotent persist); wired after the stadium-records tap.

**ADVERSARIAL REVIEW (1 skeptic) + Captain trace:** all rule clauses CLEAN; the one flagged path (synthetic incumbent when the stored rival isn't in candidates) is UNREACHABLE from the tap (the incumbent is crowned with ≥1 win and within-season wins only accumulate under the 2000-game load cap, so it's always in candidates; the synthetic fallback only fires on data corruption, where re-crowning a real eligible team over the ghost is the DESIRABLE recovery, not a violation). **GATE (Captain ran everything):** build 0 · FULL ZERO-NEW-REDS (**8446 pass / 2 characterized fails** = `wpaRuntimeBoundary` HARD + `franchiseManualSmokeFixture` order-flake [solo-passed]; +15 new tests) · L-SIM season clean 60g (`stoppedEarly:false, findings:0, criticalFindings:0, sameSeedByteIdentical:true`, digest `9425360:10944d37` = anchor) **BYTE-IDENTICAL** (own DB not dumped; no re-bake; no stray files). No trackerDb change.

**➡ NEXT: step 2 — THE RED (first-person hub).** Thread the lens team (controlled team) + its rival (`getHomeParkRival(scope, lensTeamId)`) into `FranchiseDataContext`; color the rival's name RED in standings (`FranchiseHome.tsx:3193`) + the award races (`AwardsWatchlist.tsx:216/255` — needs a player→team resolver, candidates carry only playerId); all-star board is flag-dark (`MODE_2_V1_ALL_STAR_UI_ENABLED=false`) → defer. New `--franchise-rival` token (or reuse a red). USER-VISIBLE → JK browser sign-off. Then step 3 — the morale amplifier (fan + captain, home+away; the live `vsRival` flat-±1 path needs net-new amp wiring; touches the DUMPED `kbl-franchise-morale` → L-SIM re-bake expected).

### 🎉 HOME-PARK RIVALRY FEATURE COMPLETE (HPR-A → B1 → B2 → C), all build-dark
- **HPR-A `64dd9ab1`** — the rival brain (pure `computeHomeParkRival` + own DB `kbl-franchise-home-park-rivals` [not L-SIM-dumped] + per-game tap; rule: most wins at your park, records break a wins-TIE only [no record required to qualify — JK corrected], sticky incumbency, directional/per-season). L-SIM byte-identical.
- **HPR-B1 `5370747e`** — standings rival-red (lens=`controlledTeams[0]`, `getHomeParkRival` threaded through `useFranchiseData`, `--franchise-rival #E5484D`, `StandingEntry.teamId` added). Dark-safe.
- **HPR-B2 `76cb4b29`** — races rival-red (additive `FranchiseAwardCandidate.teamId` + `FranchiseAwardRow.winnerTeamId` from `currentTeamId`; `AwardsWatchlist` colors via a `rivalTeamId` prop). L-SIM checkpoint-060 re-baked (digest-only). ⚠ introduced a NEW non-deterministic `AwardsWatchlist.test.tsx` cross-file isolation order-flake (green solo/in-dir/re-run; same class as `franchiseManualSmokeFixture`) — characterized, not a logic defect.
- **HPR-C `bd60f945`** — the grudge (rival games home/away → additive FAN ±2 + CAPTAIN ±1 morale, signed by result; new `RIVAL_GAME_WIN/LOSS` matrix rows; reads the PRE-game rival; per-team asymmetric; null captain → fan-only; Channel A untouched). L-SIM re-baked checkpoints 030-060 — delta = digest + the intended morale→development coupling (+2 ratings overlays from the captain morale change; deterministic, findings 0, morale in bounds).

**Process:** attended JK session redirecting from the queued hop-6 player-edge to this NEW §7-derived team-rivalry. Each step: Captain grounding workflow → contract → Codex build → Captain audit-the-real-diff → build + FULL ZERO-NEW-REDS + L-SIM (byte-identical where own-DB/dormant, sane re-bake where dumped) → commit branch-only by path. JK iterated the rule mid-build (dropped the record-eligibility gate → records are the wins-tie tiebreaker only; widened the morale to home+away + added the captain). All four commits branch-only, nothing pushed.

**➡ NEXT (JK to steer):** JK browser sign-off (enable `isFranchisePhase2StadiumRecordsEnabled`, play games → red + grudge). DEFERRED/flagged: all-star-board rival-red (flag-dark UI); multi-controlled-team lens selector (v1 = `controlledTeams[0]`); §16 magnitude tuning (fan 2 / captain 1); the `AwardsWatchlist` order-flake. Then the queue returns to JK-DECISION-BLOCKED (re-grade store · RA-7) + A1.5d hop-6 (player-rivalry edge).

---

## 2026-06-27 (UNATTENDED AUTH-4 `/kbl-captain`, fresh session) — ⏳ PLAYOFF-DRIVER-2 DISPATCHED (in flight)

**Baton claimed** (`HANDOFF_DONE_20260627T212025Z_playoff-driver2-captain`); sole worker confirmed; trunk `experiment/manager-wpa-window`. Lane = `/Users/johnkruse/Projects/kbl-playoff` (`claude/v1-playoff-driver`, HEAD `a74ae54f` = PLAYOFF-DRIVER-1).

**PLAYOFF-DRIVER-2 — Captain grounded the whole seam FROM SOURCE before dispatch** (contract appended to the LANE's PROMPT_CONTRACTS.md, markers `CONTRACT: PLAYOFF-DRIVER-2`). Make-or-break GREEN: the seed-path helpers (`buildFranchisePlayoffSeedingReview` pure/exported; `buildGamesPerRoundFromSetup`/`buildPlayoffTeamsFromConfirmedReview`/`getStoredFranchisePlayoffSetup` module-level) are headless-callable; `calculateStandings` reads completed-game records (works in L-SIM; the schedule store is NOT populated → SKIP the schedule-completion gate); `resolveLeague` fallback splits the 4 qualifiers top-2→Eastern/bottom-2→Western → `generateBracket` yields a coherent 2-round bracket (gamesPerRound `[3,5]`, rounds 2).

**TWO determinism landmines found + designed around:** (1) `createPlayoff`/`createSeries` ids embed module counters that increment across the 3 legs → `kbl-playoffs` is NOT added to `storeDump` DUMP_DATABASES (would break `sameSeedByteIdentical`); determinism proven instead by an id-normalized `postseasonProof` (champion + bracket, team-ids only) compared across the two determinism legs; `kbl-playoffs` IS added to sandbox `DB_NAMES_TO_DELETE`. (2) the driver's per-game seed embeds the non-deterministic series id → the L-SIM `playGame` builds its OWN seed from stable fields (team ids+round+gameNumberInSeries).

**Grounded design (vs the handoff's "generate+process"):** NO `processCompletedGame` for playoff games — the season is frozen post-finalize and `generateLsimSyntheticCompletedGame` requires schedule rows + stamps the regular-season scope (would pollute frozen stats). Instead a deterministic seeded-score `playGame` writing only to `kbl-playoffs` → existing baselines stay BYTE-IDENTICAL (zero re-bake), postseason proven via the new proof + CRITICAL clean-end invariants (champion recorded, bracket integrity = winner wins===ceil(bestOf/2), no offseason, determinism). **MVP DEFERRED** (OPEN-DECISION for JK — real playoff player stats need the forbidden process path; per JK "defer unless easy").

**Dispatched** Codex (high effort, bg, sandbox-disabled) at 21:20Z. Gate-on-completion: build 0 · focused `playoffPostseason.test.ts` green · FULL suite ZERO-NEW-REDS · Captain runs the L-SIM season (read SUMMARY JSON; expect `criticalFindings:0`, `sameSeedByteIdentical:true`, baselines git-diff EMPTY). **If this session dies mid-flight:** check the lane tree for partial uncommitted work (re-dispatch to resume, don't rebuild).

**✅ PLAYOFF-DRIVER-2 SHIPPED — `303c799a`** (lane `claude/v1-playoff-driver`, branch-only, build-dark). The L-SIM now drives a REAL franchise postseason to a champion after `runSeasonFinalize`, with clean-end invariants. Built by Codex, audited by Captain on the REAL diff + a 5-lens adversarial workflow.

- **What landed:** `test-utils/lsim/playoffPostseason.ts` (`runSeasonPostseason` — seeds the bracket from real L-SIM standings via the PRODUCTION pure seeding math, drives `drivePlayoffToChampion`, returns an id-normalized `LsimPostseasonProof`); seasonRunner runs it after BOTH `runSeasonFinalize` sites + pushes CRITICAL findings on clean-end / same-seed-determinism failure; sandbox `DB_NAMES_TO_DELETE += kbl-playoffs`; usePlayoffData exports 3 seeding helpers (export-only).
- **Two determinism landmines (Captain-grounded, both handled):** (1) playoff/series ids embed module counters that differ across legs → `kbl-playoffs` deliberately NOT in `storeDump` DUMP_DATABASES; determinism proven by the id-normalized proof (champion + bracket, team-ids only). (2) the driver's per-game seed embeds the non-deterministic series id → the L-SIM `playGame` builds its own STABLE-field seed. Grounded design vs the handoff's "generate+process": NO `processCompletedGame` for playoff games (would need absent schedule rows + would pollute the frozen season) → a deterministic seeded-score resolver → existing baselines BYTE-IDENTICAL (zero re-bake).
- **GATE (Captain ran all):** build 0 · FULL suite ZERO-NEW-REDS (playoffPostseason green in-suite @19.4s under load, 60s timeout; remaining fails = characterized `wpaRuntimeBoundary` HARD + `historicalArchetypes` flake + order-flake `franchiseManualSmokeFixture`) · **L-SIM season: gamesSimulated 60, stoppedEarly false, findings 0, criticalFindings 0, sameSeedByteIdentical TRUE, postseason {champion lsim-team-03, rounds 2, series 3, games 10, cleanEndOk TRUE, sameSeedChampionMatch TRUE}, baselines git-diff EMPTY (zero re-bake)**.
- **Adversarial 5-lens workflow:** 4 CLEAN (determinism / frozen-state-pollution / spec-faithfulness / blast-radius) + 1 MINOR (clean-end invariant vacuity) + 0 confirmed-real. The MINOR was FOLDED IN → added a non-vacuous `bracketComplete` invariant (series == teamsQualifying-1, all COMPLETED, rounds match) + re-gated green.
- **OPEN-DECISIONS for JK (logged, not blocking):** (1) **Champion-MVP DEFERRED** — the ADDENDUM said "reuse MVP voting, easy"; grounding showed the L-SIM postseason produces NO per-player playoff stats (no `processCompletedGame` for playoff games, by design), so a playoff-stats MVP is NOT easy here. Options when revisited: compute MVP from frozen REGULAR-season stats scoped to the champion roster (no playoff stats needed), or add a postseason stats-fidelity ticket. (2) Same prior open items (winner-honors rarity metric, V8 park factors intended-verify, the flip/freeze/tuning sequence).

**➡ NEXT (LANE A): #18 winner-honors resolver** (extend `franchiseSeasonEndHonors.ts` to honor ALL award winners, rarity-scaled per JK 2026-06-27; build mechanism + placeholder prestige, flag §16 magnitude) → #20 honor-news path → Tier-6 hardening (#34/#38/#35/#37). P3 (#23 champion-MVP) DEFERRED (OPEN-DECISION above). NOTE: #1 archetype→capIdentity is DONE on **Lane B** (`2e53a0a6`, a SEPARATE concurrent Captain — do NOT touch its baton/surface; see [[lane-b-draft-ui-progress]]).

---

## 2026-06-27 (UNATTENDED AUTH-4 `/kbl-captain`, fresh session) — ✅ WINNER-HONORS-18 SHIPPED (`c7c0982d`); then DRAFT re-prioritized (Lane B owns it)

**Baton claimed** (`HANDOFF_DONE_20260627T222924Z_18-winner-honors-captain`); sole Lane-A worker; trunk `experiment/manager-wpa-window`.

**WINNER-HONORS-18 (#18 — all award winners honored, prestige-scaled + additive) — ✅ SHIPPED (`c7c0982d`, contract `e001c2c4`, branch-only, build-dark/L12-gated).** Built by Codex (high), audited by Captain on the REAL diff.
- **What landed (5 scoped files):** `fameModel.ts` honorHeatBump extended with 6 §16-tunable placeholder prestige keys (silverSlugger 8 / goldGlove 7 / rookie 6 / reliever 5 / benchPlayer 3 / boogerGlove 2; mvp 12 / cyYoung 10 / all-star unchanged); `franchiseHonorReachFloor.ts` `FranchiseHonorTier` +6 tiers and the reach-floor now **groups honorees by playerId and SUMS the bumps → one write per player (ADDITIVE)**; `franchiseSeasonEndHonors.ts` unified all 8 player WAR award categories into ONE config, honors winners of ALL of them via a single aggregated reach-floor call, **preserves MVP/CY news + MVP/CY/ROY/Reliever snub + finalize-rerun idempotency (sentinel pre-filter)**, keeps the no-team skip; +2 test files (additive aggregation + dual-award stacking + extended prestige ladder).
- **Make-or-break (additive vs the sentinel):** the old per-category sentinel guard gave a dual winner only ONE bump; #18 aggregates per-winner and bumps by the SUM while staying idempotent on re-run. Resolved by collecting one honoree entry per unsentinel'd win and summing inside the reach-floor.
- **JK ruling 2026-06-27#2 honored:** rarity = per-award prestige; multi-award winners stack additively; tunable placeholder magnitudes flagged for §16. Dubious-award (boogerGlove) fame direction flagged as a §16/JK open-decision (placeholder = small positive notoriety).
- **GATE (Captain ran all):** build 0 · focused tests 20/20 · **FULL suite ZERO-NEW-REDS** (failed files all characterized: `wpaRuntimeBoundary` HARD + `historicalArchetypes` + `poolFeasibility` flake + order-flakes; #18's 2 test files PASS) · **L-SIM season findings 0, criticalFindings 0, sameSeedByteIdentical TRUE**, exercising the triple-award additive case (`lsim-team-01-mlb-01-C` swept MVP+ROY+SilverSlugger). **Clean re-bake**: only `lsim-h2-baseline-checkpoint-060.json` changed, **row counts identical** (`franchiseFameRecords:60`), digest-suffix-only delta (`bd68198c`→`be3352a4`) — the expected additive-fame signature.

**➡ RE-PRIORITIZATION (JK, this session): DRAFT-FIRST before Tuesday.** JK needs a playable/save-able draft ASAP. **The DRAFT is OWNED + FINISHED by the concurrent Lane B captain + Codex 5.5** (draft lane `claude/v1-draft-ui`, worktree `kbl-draftlane`, HEAD `ff024385`): Draft Room setup screen live, flow routed through it, E2E **saves + launches into franchise mode-2 with rosters** verified (Lane B re-ran the gate; `STREAMB_FINISH_REPORT.md` = YES). Lane B found+fixed a real chain break (FranchiseSetup was clearing ownership on league-select). **Lane A does NOT touch the draft surface** (two-workers-one-surface collision = the #1 failure). JK can run drafts TONIGHT from the `kbl-draftlane` worktree — **no merge needed**.

**➡ NEXT (LANE A, Tuesday — see HANDOFF_NEEDED):** the headline is **ASSEMBLY/MERGE** (nothing is on trunk yet: lineups-fenway hub 64 ahead, draft lanes, playoff-driver 2 ahead). DEFERRED living-season: #20 honor-news for the new non-MVP winners, #19 TV-award family, #34/#35/#37/#38 Tier-6 hardening, #23 champion-MVP (deferred). All NON-blocking for playing the draft.

**ADDENDUM (same session) — Lane-A drove one DRAFT-LANE feature for JK's first draft.** JK needed a playable draft (incl. scout guide + staff-hire) BEFORE Tuesday. Lane B had wound down (its Codex finisher completed), so as sole worker I dispatched Codex on the draft lane (`kbl-draftlane`, contract `DRAFT-STAFF-GUIDE-WIRE`): scout-hire screen BEFORE the auction + in-auction scout guide (price band / 20-80 grade / affordability) + manager+beat-reporter staff-hire ceremony after the farm auction, all persisted (scoutProfiles / managerIdentityStorage / reporterStorage). Captain-audited the real diff (additive only; bid/award/freeze untouched); build 0; `draftPipeline.integration` 4/4 incl. a new scout/staff persistence test. Committed branch-only on the draft lane (`adc757e8`; contract `2466d4c8`). JK drafts tonight from the worktree on **port 5173** (per-origin IndexedDB; data carries to main Tuesday — all branches trackerDb v26). Tuesday's assembly merges draft-lane HEAD `adc757e8`.

---

## 2026-06-30 (UNATTENDED AUTH-4 `/kbl-captain`, fresh session) — ✅ HONOR-NEWS-20 SHIPPED (`8ced1958`); then JK asked for an independent v1 PLAN

**Baton claimed** (`HANDOFF_DONE_20260630T*_lane-a-captain`); sole Lane-A worker; trunk `experiment/manager-wpa-window`.

**SEQUENCING OPEN-DECISION (logged, AUTH-4 default taken):** the Tuesday HANDOFF headline was **ASSEMBLY/MERGE**, which JK paused "until his return" (JK-confirm-gated). I did NOT touch the merge. Instead I picked up the next INDEPENDENT, additive, non-gated ticket — **#20 honor-news**, the explicit ledger NEXT after #18 — to keep the loop rolling without crossing the JK gate. #20's file surface (honors/reporter, Mode-2) is disjoint from every pending merge branch (draft = Mode-1, hub = lineups), so it does not complicate the assembly. The HANDOFF's "do after assembly" was a conflict-avoidance preference; with the merge paused indefinitely and zero file overlap, doing it now is safe.

**HONOR-NEWS-20 (#20 — all season-end award winners emit honor news) — ✅ SHIPPED (`8ced1958`, contract in `8ced1958`, branch-only, build-dark/L12-gated).** Built by Codex (high), audited by Captain on the REAL diff (builder≠auditor).
- **What landed (4 scoped files):** widened the news `FranchiseHonorKind` (+6 season-end categories: ROOKIE/RELIEVER/GOLD_GLOVE/SILVER_SLUGGER/BENCH_PLAYER/BOOGER_GLOVE) + 6 §16-tunable placeholder dramatic weights (0.53→0.13, monotonic by prestige, Booger smallest); split the season-end config's dual-use `honorKind` into `newsKind` (ALL 8 emit news) + `snubKind` (the UNCHANGED 5-kind snub set MVP/CY/ROY/RELIEVER); +2 test files (the Rookie "without news"→"with news" flip, the dual MVP+SilverSlugger emit-twice case, a new Gold-Glove no-snub news case, adapter cases for the new kinds).
- **Make-or-break HELD:** the config field did double duty (news emit + snub) and the two diverge after #20 (GG/SS/Bench/Booger emit news but have no snub). Splitting `newsKind`/`snubKind` kept the snub set BYTE-IDENTICAL while news now fires for all 8. Fame/reach-floor logic untouched.
- **GATE (Captain ran all, independent):** build 0 · focused reporter tests **24/24** · **FULL suite ZERO-NEW-REDS** (5 failed files all characterized [`wpaRuntimeBoundary` HARD + `historicalArchetypes`/`archetypeBalanceSimulator` batch-flakes + `franchiseManualSmokeFixture` order-flake] OR dirty-WIP-attributable [`prospectChemistryRebalance` — fails SOLO on a prospect-generation golden shifted by the UNCOMMITTED Mode-1 auction WIP in the tree, NOT by #20]; #20's honor files PASS) · **L-SIM season findings 0, criticalFindings 0, sameSeedByteIdentical TRUE, baselines git-diff EMPTY (zero re-bake)** — proves #20 dormant headless (news is LLM-gated → persists nothing; fame unchanged), even though the run crowned Gold-Glove/Silver-Slugger/Rookie winners.

**⚠ TREE NOTE for next worker:** there is UNCOMMITTED Mode-1 auction WIP in the main tree working dir (auctionStateMachine/cpuTeamRoles/useAuctionDraft/farmAuctionPool/prospectScoutingDraftEngine + tests, +695/-173). It currently breaks the `prospectChemistryRebalance` golden. NOT mine — left untouched; commit it to its own branch before any assembly merge (per the v1 plan step 1).

**➡ THEN: JK asked for an INDEPENDENT v1 PLAN** (Codex sent a sync on the draft-lane work; JK wants my ideas BEFORE his). Ran a 15-agent ground→design→judge→synthesize→critique workflow → wrote **`spec-docs/V1_PLAN_2026-06-30.md`** (companion to V1_STATUS_AND_ASSEMBLY_PLAN, git-verified). Key independent corrections vs the prior framing: (1) merge order = draft-ui → playoff-driver → hub-LAST with CONTAINMENT (draft-setup-ui ⊂ v1-draft-ui; auction-ux-rehaul ⊂ lineups-fenway-hub) — don't merge children separately; size with a real merge-tree dry-run, NOT "64 commits/~10 files" (unverified). (2) The "deep three-plan scout" is NOT greenfield — the live auction (`capIdentity`/`CAP_MODIFICATION_FRACTIONS`) and the optimizer (`HISTORICAL_ARCHETYPES`/`buildBestRoster`, half-built-dark) use DIFFERENT archetype vocabularies → reconciliation is a prerequisite to ANY draft-scout work. (3) L-SIM hardening + the freeze→real-frozen-franchise bridge = the schedule-driving landmine (XL). (4) draft-guide spec is faithful but NOT buildable as-is (needs a delta-map + pinned math + a plan-distinctness test). Plain-language plan + Decisions A–E sent to JK; **awaiting his ideas + rulings (esp. Decision A: draft-depth bar for v1).**

**PARKED WIP (2026-06-30):** the loose uncommitted Mode-1 auction/draft/prospect work that was sitting in the trunk working tree (18 files: auctionStateMachine/cpuTeamRoles/useAuctionDraft/useFarmAuctionDraft/farmAuctionPool/prospectScoutingDraftEngine + tests + 2 analysis docs) was parked onto branch **`wip/mode1-auction-draft-2026-06-30`** (commit `12723301`) per JK, so an assembly merge can't wipe it. Trunk working tree is now clean (only the HANDOFF baton deletion remains). It shifts the `prospectChemistryRebalance` golden — re-baseline/finish before integrating. Discoverable via task chip `task_6e11dbd7`. (Recovery note: the parking initially hit a zsh word-split bug that partially popped pre-existing `stash@{0}`; reverted the stray package.json/supabase changes, all 3 pre-existing stashes intact, nothing lost.)

**JK CAP/DEAD-CAP CORRECTIONS (2026-06-30):** folded into `IN_SEASON_CAP_DEADCAP_ANALYSIS_2026-06-30.md` §9 — (C-2) no release / no FA pool (removal = send-down or trade; trade = clean salary swap; the offseason `releaseFranchisePhase11Player` path is a STRAY to remove/gate); (C-3) demotion-to-save IS intended (rebuilding) — dropped the "demotion never reduces cap" floor, brake is morale not cap; (C-3/C-5) fan morale is player-value-driven per move (fame × net-TV on send-down AND call-up; "Fan Hopeful" rookie call-up → morale up) PLUS the aggregate dead-money-% drag; (C-4) dead money should charge on archetype TRUE cost not face. All feed the scouting-intelligence interrogation as the in-season half of the ONE true-cost economy.

---
**AUTH-4 RUN — 2026-07-11 (Fable captain, living-season program, JK away-window)**
- Authority: OBSERVER_GROUNDWORK_BRIEF §5 rulings + LIVING_SEASON_V1_EXECUTION_MAP. Builder=Codex 5.6 Sol; auditor=Fable (JK ruling: no opus this window); ship=branch PRs (merge rights withheld by classifier — stacked PRs, JK merges on return).
- **WAVE 1 (attended, complete):** four lanes built+audited+shipped — PR #80 MIRROR-1 (console-mirror service; 1 fix-pass: stale-boundary quarantine), PR #81 HUNTFIX-TRACKER-1 (4 GameTracker truth seams; 1 fix-pass: immaculate re-baseline), PR #82 HUNTFIX-ENGINE-1 (fWAR season-invariance + 7 engine defects + R-A floor ratchet; E4 STOPPED→moved to KERNEL2 K7), PR #83 KERNEL-TRUTH-1 (archive-early outcome ledger + 8 repairs; 2 fix-passes: TV-scope FAILED receipts, franchise-only ledger). Every lane needed exactly one real captain-caught correction its own green tests missed.
- JK rulings this session: R-A faded-infamy (positive-only floor), R-B no decay-morale, R-C age gravity=tuning dial, R-D beloved-firing backfire, fame-cleanup greenlight (kill on-screen fame numbers, single ledger), three-mode truth (exhibition/elimination/franchise — franchise-less season completions = fail closed).
- **WAVE 2 (in flight):** KERNEL2 dispatched (worktree kbl-kernel2, stacked on codex/living-kernel; K1-K7 incl. innings-aware CG, L11 execution identity, honor-bump idempotency, R-B/R-D, confidence denominator, WPA conservation). MODE-TRUTH sweep dispatched (Sol read-only: classify every completion entry point against three-mode truth; kill list for zombie fallback chains).
- **QUEUE:** FAME-CLEANUP (needs KERNEL+HFT integration branch; scope: kill fame-value UI flashes, reconcile dual fame ledgers, retire player.fame numeric + FameLeaderboardCard placeholder) → MIRRORUI-1 (takeover wiring + Lens truth batch, stacked on MIRROR+KERNEL) → FIDELITY-1 (L-SIM bridge: neutral+slump→recovery, 7-step closure proof) → TUNE-0 (targets+baseline+sensitivity vs LIVING_SEASON_KNOBS registry) → MIRROR-1b (sweep-time CAS stamping).
- OPEN-DECISIONS for JK: none blocking; merge queue #80-83 (file-disjoint, any order) + wave-2 PRs stacked on them.
- **WAVE 2 (complete):** KERNEL2 shipped PR #84 (K1-K7: innings-aware CG + true walk-offs, honor-bump replay guard, L11 execution-identity CAS, decay-morale split per JK R-B, beloved-firing backlash per R-D, gamesPerTeam confidence fix, WPA orphan re-attribution with unattributedDefensiveWpa) — audited APPROVED (commit `190948eb`, gates: build 0, full vitest 9,664 passed/0 persistent reds, 93 focused proofs, L-SIM smoke 24/24 clean). MODE-KILL-1 shipped PR #87 (three-mode fail-closed; mode-aware startup recovery closes the live leak; quarantine for unclassifiable archives; no season-1 invention; launch identity blocks; `archiveBatchGameResult` deleted) — audited APPROVED (one characterized pool-lock flake; L-SIM smoke all-critical-invariants green). MIRRORUI-1 shipped PR #86 (takeover wired to the mirror service with true ordinals + confirm/adjust/reject + development log; nine Lens truth fixes) — audited APPROVED after one service-gap STOP (apply-failed retryability; ruled + "fixed in PR #80 first" per the commit's own words — **verified this pass:** the fix commit `c274a708` was pushed to the `codex/console-mirror` branch AFTER PR #80 had already merged, so it is NOT actually on `main` via #80; it reaches `main` when PR #86 merges, since #86's branch carries it forward). FAME-CLEANUP-1 shipped PR #85 (one fame ledger, no on-screen fame arithmetic, narrator calls removed, net -178 lines) — audited APPROVED. FIDELITY-1 committed on `integration/living-season-full` @ `0be5ea2e` (7-step development-loop closure proof PASSED, byte-identical replay, falsification suite 49/49; proves closure not feel) — audited APPROVED. Union gate: `integration/living-season-full` = all lanes merged, full vitest reported 9,746 passed / 1 characterized Draft-Setup batch flake (solo-green twice) — **figure as reported by the captain/lane record; this scribe pass did not independently re-run the suite.** MODE-TRUTH sweep banked (`spec-docs/MODE_TRUTH_SWEEP_2026-07-11.md` — present on local `main` @ `dd06469c` at booking time, carried onto this docs branch by cherry-pick since it had not yet reached `origin/main`). TUNE-0 dispatched (in flight, Codex on the `kbl-integration` worktree).
- **Merge-queue status verified this pass (`gh pr list`/`gh pr view`, 2026-07-11):** #80, #81, #82 are **ALREADY MERGED** to `main` (not merely queued). What remains OPEN: #83 → #84 → #87 stacked chain; #86 stacked on (already-merged) #80; #85 needs #83 first (its #81 dependency is already satisfied by the merge). One-click alternative unchanged: fast-forward/merge the certified `integration/living-season-full` branch, which already contains every lane.
- **Contract-file accuracy note (verified via `git log --all` across every branch):** only five lane contracts actually exist as committed files in `spec-docs/contracts/` — `CONTRACT_HUNTFIX_ENGINE_2026-07-11.md`, `CONTRACT_HUNTFIX_TRACKER_2026-07-11.md`, `CONTRACT_KERNEL2_2026-07-11.md`, `CONTRACT_KERNEL_TRUTH_2026-07-11.md`, `CONTRACT_MIRROR_1_2026-07-11.md`. The MIRRORUI-1, FAME-CLEANUP-1, and FIDELITY-1 commit messages each cite a `CONTRACT_MIRRORUI_2026-07-11.md` / `CONTRACT_FAME_CLEANUP_2026-07-11.md` / `CONTRACT_FIDELITY_2026-07-11.md` file, but none of the three was ever actually committed anywhere in git history — their rulings/gates live only in the commit message and PR body text. **RESOLVED same day:** captain committed all five missing wave-2 contract files (MIRRORUI, FAME_CLEANUP, FIDELITY, MODE_KILL, TUNE0) on this docs branch at `6bb42ac8`; the dispatched TUNE-0 contract was diff-verified identical to the committed copy.
- **TUNE-0 shipped (`5dac70d2` on `integration/living-season-full`) — audited APPROVED (captain).** Deliverable: `spec-docs/TUNE0_BASELINE_2026-07-11.md` + 31 artifacts under `test-utils/lsim/results/tune0/` + harness (`tune0*.ts`, additive `feedbackClosure.ts` metrics seam — default digest behavior preserved). T1 baselines: closure 30g/5cp (410 proposals/781 abs pts, 203/1 traits, 59 L10, 1 L11, 45 formations), smoke 24/24 zero findings. T2 one-factor sweep, same seed, 8 knobs. T3 ranking: fame decay 0.287 (**UNSTABLE at 2× — clamp saturation, Immortals 8→38**), relationship threshold 0.077 (**UNSTABLE at 0.5× — runtime runaway, archived without fabricated numbers**), cadence 0.060, morale spread 0.014, age-gravity 0.005, fan dampener 0.004; **INERT:** K5 backlash (coverage caveat: no beloved-firing exercised) + sweep-side performanceSignalScale (vestigial — real caller consumes pre-normalized signals); **NOT-SWEEPABLE:** wpaToHeatScale (module-private). Draft target bands labeled FOR JK/CAPTAIN RATIFICATION. Audit spot-checks: doc numbers traced to raw JSONs exactly (fame-high p75=50/Immortal=38/min=−30; ranking impacts; CP1 rows); `src/**` untouched; canonical lsim-h2 baselines diff-empty. Sol's sandbox couldn't reach git worktree metadata — captain committed post-audit. **Ship plan:** FIDELITY (`0be5ea2e`) + TUNE-0 (`5dac70d2`) are test-utils+docs-only commits sitting on the integration branch; after the lane PRs merge, cherry-pick both onto main as one final PR (they exercise KERNEL-shape code, so they gate on #83/#84 landing first).
- **WAVE 3 (evening, JK back + all wave-1/2 PRs merged, tokens cleared):** JK challenged two TUNE-0 readings and both became rulings — **R-E bold-dynamics** (no magnitude caps as targets; the only brakes = meaningfulness + non-chaos; season = multiple seasons in one, likely ends playoffs+championship, no rollover) and **R-F organic relationships** (JK smelled the cadence→relationship coupling; code truth confirmed formation was checkpoint-batch-gated). Both logged in `DECISIONS_LOG.md`; the TUNE-0 target table reframed by addendum (the +3..6 age-gravity band WITHDRAWN). Shipped via PR #89 (merged) together with the FIDELITY + TUNE-0 cherry-picks onto main.
- **RELORGANIC-1 shipped PR #93** — formation goes organic per-game (deterministic seeded hazard; strong pairs ~3 games, marginal ~2%/game, sub-threshold never; active edges immutable; potential upgrades; replay no-op; cadence-independence proven byte-identical). One captain-audit catch: the contract's own sub-threshold→potential mapping crossed axes with the codebase's cross-team `potential` semantics and would have minted active edges BELOW threshold ~4× faster than above it — Amendment 1 made the live path monotone (fail-before 0.0899→0 proof). L-SIM invariant rewritten (`soul.l13-relationship-formation-organic`: integrity + liveness + anti-batch-spike + R-E density ceiling 8/team); falsification 47/47.
- **⚠ MAIN IS RED finding (pre-existing, heals on #93 merge):** pure main's season leg STOPS at game 24 — CRITICAL `soul.l13-relationship-formation-checkpoint-write`, `missingCurrentBoundary=true` (game 24 = boundary 2 of 5). Root cause: the old invariant demands every boundary-with-edges write a NEW edge; FIDELITY's ratings-aware generator (merged via #89) shifted event-driven edge timing so the game-24 batch had nothing new to write. Main can no longer complete its own season leg → its canonical baselines are stale against its own harness. #93 carries the audited re-bake (attribution run on pure main separated FIDELITY's effect — milestones/fame/stadium/fielding — from RELORGANIC's own footprint — relationships/morale cascade). **Merge #93 promptly.**
- **MIRROR-1b shipped PR #92** — sweep-time CAS stamping (tier-1): ratings rows stamp the live base value; trait rows carry both slots and derive stamps by REUSING `applyTraitDisplacement` (stamp = apply path by construction). One correct builder STOP (slot identity discarded by the sweep roster) → Amendment 1 widened the fence. Drift-conflict fail-before/pass-after proven on both kinds. Full suite: 1 fail = poolLock flake, proven red on untouched main under identical load (a different case each run — load-rotating).
- **Gate discipline note:** the machine ran 3+ concurrent heavy lanes (two other captain threads) all evening; batch flakes rotated (poolLock, AwardsWatchlist) — every red was solo-verified AND base-verified before attribution. Heavy suites serialized between lanes.
- **QUEUE next session:** SWITCH-3A (per-league activation; contract still unwritten, the standing stretch slice) → TUNE-1 re-sweep of relationship knobs against the hazard model (TUNE-0's relationship rows describe the retired batch model) → ratify TUNE-0 targets (post R-E reframe) → flip gate (JK browser walk).
- **WAVE 4 (late evening):** JK merged the wave-3 PRs (#92/#93/#94) + #95 as they landed. Shipped: **TUNE-1 (PR #95, merged)** — relationship dials re-swept against the organic model; cadence independence proven season-level (impact exactly 0); thresholds = the dominant-but-guarded dial (unsafe both extremes), activeSlopePerPoint = the everyday dial; targets in R-E shape terms, DRAFT. **SWITCH-3A (PR #100)** — per-franchise living-season activation chosen at creation, immutable for the season (captain ruling dissolves the migration choice); resolution chain test→console-flag→console-global→franchise-family-switch→compiled-default; creation toggle default OFF; tuning-profile provenance stamped; two captain-audit fix-iters (Lens rival-card gate honors the switch; activation test isolation). **Stamp re-bake (PR #99)** — canonical baselines refreshed for #92's proposal stamps; field-level proof: exactly one changed field per checkpoint (the digest); plus the definitive post-merge health check: merged main season leg 60/60, findings 0, byte-identical determinism.
- **Contention post-mortem (no regression):** three season-leg attempts hit the 900s scenario timeout at ~16s/game with a near-idle worker; a smoke bisect across #90/#91/#92/#93 first suggested a code regression, then a clean re-measure at tip ran 58.7s at full CPU — the whole slowdown was MACHINE CONTENTION from concurrent lanes across three captain threads (incl. this captain's own overlapping dispatches — self-caught scheduling error; heavy legs now run strictly solo). Recorded so no future session chases this ghost.
- **Cross-thread finding (chipped, task_3f6a3621):** the snake merges (#90/#91) left main with three base-red tests (snake-POC compiled-default mismatch + two Draft-Setup snake UI tests). NOT this program's surface. Honesty note: Sol's base-red claim was CORRECT and the captain's first counter-check was wrong (ran on a stale pre-#90/#91 base) — corrected in Amendment 2 and verified on true clean base.
- **Program state at close:** every living-season v1 slice from the execution map is BUILT and shipped. Open PRs: #99 (re-bake), #100 (SWITCH-3A). Remaining before flip: ratify TUNE-0/TUNE-1 draft target tables (JK+captain sit-down) → set knob values → 60g leg green → **JK's browser walk on a fresh switch-ON franchise = the sole acceptance gate.**
