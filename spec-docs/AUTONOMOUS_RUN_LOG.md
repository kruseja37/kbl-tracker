...
**➡ RESUME HERE = DT-C (diving/chase — NEW rating-gate mechanism)** — the next wave ticket (`V1_BUILD_QUEUE A-W3.5`). ⚠ **NOT a pure T-9a/b clone:** DT-C introduces a NEW rating-gate mechanism (per the queue), so ground it fresh from `TRAIT_MEASUREMENT_SPEC §0.6b` + `DECISIONS_LOG 2026-06-25` before contracting — do NOT assume the aggregator/BUILDABLE/image-valence template covers it. Then DT-D (errors incl. **Noodle-Arm-re-added-as-mental-errors**) · DT-E (mojo Volatile/Consistent) · DT-F (bespoke incl. **Metal-Head = PITCHER protective**). Then T-3 (trend + SP/RP split) / T-6 (position-mismatch protection-only) → T-7 (EOS).
