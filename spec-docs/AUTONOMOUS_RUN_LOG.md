...
**➡ NEXT: MAIN = T-6 (position-mismatch protect-only — GROUNDED + Captain-verified: NEW `POSITION_MISMATCH_UNREGAINABLE` map `{Cannon Arm: 1B/2B/3B/SS}` per §8C:133; thread player `primaryPosition` into `TraitAcquisitionInput` + `reconcileGainProposals` [not carried today; `entry.primaryPosition` already in the caller]; SUPPRESS self-loss at the lose-gate + BOOST `keepScore` [much-harder-not-impossible displacement bar] for a protected held trait; NON-mismatch cases byte-identical; do NOT touch `addErrorSignals`/Noodle Arm — DT-D owns it; SPLIT T-6a engine [build] / T-6b scout+analyzer flag [defer]; FULL-suite gate) → T-7 (EOS — ALREADY-SATISFIED in engine: final game already fires a checkpoint, test-pinned; doc-reconciliation + 1-line confirm comment only — Captain can author directly). ⚠ 2 BV items pending JK (BV-V8 park→WAR, BV-CAP-MISS). Do NOT flip Phase-2 traits until the wave is JK-reviewed.**
