...
**➡ NEXT: MAIN = T-3 (trend + SP/RP split) / T-6 (position-mismatch protection-only) → T-7 (EOS) — the remaining ratings-engine/EOS trait tails; Lane C = near end of clean-independent ratings work (A2.5+RA-5+RA-9+V8 done; re-grade COLLIDES with the trait lane, RA-11 needs Branch-B B14 cross-pin, §6A convex/RA-7 are the BLOCKING-before-flag-flip pieces handled separately). ⚠ 2 BV items pending JK (BV-V8 park→WAR, BV-CAP-MISS missed-catch capture) — surface before the next pick. Do NOT flip Phase-2 traits until BF-MH + the wave are JK-reviewed.**
