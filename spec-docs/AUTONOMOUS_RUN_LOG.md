...
**➡ NEXT: MAIN = T-3b-2 (sweep SUPPLY — GROUNDED: in `franchiseTraitGrantCompute`, when `(prevBoundary>0)` AND `trendTiltWeight>0`, map each `seasonData.games` → gameNumber via the already-imported `getScheduledGame`, window the events to `(prevBoundary, current]` via `resolvePreviousCheckpointGameNumber`, RE-RUN `computeSeasonTraitCandidates` on the windowed events with the SAME aggregate maps, pair `recentPercentile` per player+trait onto the candidates; CP1 prevBoundary===0 → skip → neutral; guarded so weight-0 default skips the work entirely = byte-identical; FULL-suite gate, extend the getScheduledGame mock to map window gameNumbers; OPEN-DECISION: recent re-percentiles vs a recent-only pool [cohort drift, inert at weight 0]) → T-6 (position-mismatch protect-only, Cannon Arm at IF; do NOT delete Noodle Arm — GROUNDED) → T-7 (EOS — ALREADY-SATISFIED in engine: final game already fires a checkpoint, test-pinned; doc-reconciliation + 1-line confirm comment only). ⚠ 2 BV items pending JK (BV-V8 park→WAR, BV-CAP-MISS). Do NOT flip Phase-2 traits until the wave is JK-reviewed.**
