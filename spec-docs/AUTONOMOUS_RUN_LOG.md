...
**➡ NEXT (a fresh session — the MAIN trait tail is done; next = the Lane-C ratings tail, which has forks to handle deliberately): the trait lane is now COMPLETE so `re-grade-per-checkpoint` NO LONGER COLLIDES (was blocked on the trait files) — but it carries an unresolved grade-DISPLAY fork (user-visible → ground it + take a documented default or surface to JK); `§6A convex curve` + `RA-7 park-adjust` are the BLOCKING-before-flag-flip ratings pieces (buildable-dark with documented defaults but flag-coupled — a deliberate pass); `RA-11` needs a cross-branch spec-pin with Branch-B B14 (not self-contained). T-6b (deferred UI). ⚠ Do NOT flip Phase-2 traits until JK reviews the whole trait/dormant-trait wave. ⚠ 2 BV items pending JK.**
