...
**➡ NEXT = PRIORITY-2 (TRAITS — the biggest remaining chunk; mostly unstarted, engine still the original simple version):** T-1 value/scarcity traitWeight · T-2 4-pos/3-neg tiers · T-4 weighted generation (genWeight=1−traitWeight, NEG 0.27) · T-6 position-mismatch map + protection (catcher arm re-earnable; RA-8 fields exist) · T-7 EOS threshold engine (replaces the Trait Wheel Spin) · T-3 trend factor + SP/RP cohort split · T-5 reconcileGainProposals value tiers + incumbency β=1.25 + seeded firing · L9b-2 R1/R2/R3 of the 16→~50 rebuild (remove Noodle Arm from BUILDABLE_TRAITS). Soul-layer → ground hard + surface genuine measurement forks to JK (Hybrid) before baking. Specs: TRAIT_GAIN_LOSS_THRESHOLD_SPEC · TRAIT_INTEGRATION_SPEC · TRAIT_MEASUREMENT_SPEC · TRAIT_SIGNAL_CERTIFICATION · smb4_traits_reference.
