...
**➡ RESUME HERE = DT-B (pitch-LOCATION traits)** — the next wave ticket (`V1_BUILD_QUEUE A-W3.5`, 4D-W2): the 4 hitter traits **High / Low / Inside / Outside Pitch** earn from net-outcome on `enrichment.pitchLocation`-tagged ABs (low/high/inside/outside; the field also has `'outOfZone'` — `eventLog.ts:438`), REUSING the T-9a HITTER net-outcome scorer (big hits/Ks/BBs/RBIs incl. SF + RBI-GO per §0.6b). The 4 traits are an OPPOSITE-PAIR set already (`High↔Low`, `Inside↔Outside` — see `PROSPECT_TRAIT_CONFLICT_PAIRS` + `OPPOSITE_PAIRS`), so NO new group-exclusion (the existing opposite-pair duels handle it). Verify the 4 are canonical + `POSITION_ONLY` (they're in `traitRealityScorer.ts:106`). Pattern = T-9a/b (aggregator + BUILDABLE + image valence; FULL suite). Then DT-C (diving/chase — NEW rating-gate mechanism) · DT-D (errors incl. Noodle-Arm-re-added) · DT-E (mojo) · DT-F (bespoke incl. Metal Head).
