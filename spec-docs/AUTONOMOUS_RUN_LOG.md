...
**➡ NEXT (traits, all forks ruled) — RESUME HERE:** **T-4b** (the NEGATIVE-polarity pass — §5 pass 1: per trait slot roll `randomUnit(`${seed}:traitN:polarity`) < 0.27` → draw from a NEW negative pool [hitter 10 / pitcher 8, all priced+eligible per the grounding] weighted by genWeight, else the positive pool; **no-2-negatives** [if slot1 negative, force slot2 positive] + slot2 excludes trait1/conflicts/**opposite(trait1)** so no contradictory pair like Tough Out+Whiffer; re-baseline the 2 generation goldens again; **§13 grade-dist is the make-or-break — flaws LOWER grades, HIGHER risk the anchor can't absorb → STOP-IF if §13 drifts >1.72pp**, do NOT re-baseline the §3.2 target). Then **T-3** (trend tilt + SP/RP cohort split, §4A — FORK-C ruled: split, fold SP-RP/CP into relievers, fallback to full pool) / **T-6** (position-mismatch protection-only map for arm@1B/2B/3B/SS, §8C — FORK-D ruled protection-only, catcher re-earnable DEFERRED; FULL suite) → **T-7** (EOS = one more checkpoint of the same engine, FORK-E ruled same thresholds; deprecate the Trait Wheel Spin; FULL suite). T-1 exports `computeTraitWeight`/`assignTier`/the §6 constants. Then Priority-3 (ratings-engine finish: A2.5 §6A · RA-5/7 · RA-11 · re-grade-per-checkpoint · RA-9 trend · V8 park→WAR), Priority-4 (L4b reporter takes · trade-request wiring), Priority-5 (Mode-1/launch tails).
