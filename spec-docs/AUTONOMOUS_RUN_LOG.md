...
**➡ NEXT (traits, all forks ruled):** **T-2** (widen the flat 0.75/0.35 gain/lose to the 7-tier lookup from `assignTier` — the seam is built per §7: `traitAcquisition.ts:91-92,285-295`, swap the 2 comparisons + the negative `P_bad` inversion) → **T-4** (rarity-weighted generation, genWeight=1−traitWeight, NEG 0.27) ∥ → **T-5** (reconcile value tiers + incumbency β=1.25 + seeded firing, §8B) → **T-3** (trend tilt + SP/RP cohort split, §4A) / **T-6** (position-mismatch protection-only, §8C; catcher re-earnable deferred per FORK-D) → **T-7** (EOS = one more checkpoint, same thresholds per FORK-E). T-6/T-7 = FULL-suite (checkpoint-sweep wiring); rest pure/dark. Then Priority-3 (ratings-engine finish: A2.5 §6A · RA-5/7 · RA-11 · re-grade-per-checkpoint · RA-9 trend · V8 park→WAR).
