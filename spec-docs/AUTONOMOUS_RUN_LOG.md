...
**CAT-MIGRATE — DISPATCHED (in flight):** drop the 4 worksheet-retired box-score categories (`powerIso`/`contactAverage`/`contactOnBase`/`pitchingFipPrevention`), KEEP `powerHomeRunRate` (JK 4-drop ruling). Survivor check verified from source (power 2 / contact 2 / accuracy 1; all others untouched). Lockstep across `EXPECTED_STATS_CATEGORY_META` + `ONE_BY_CATEGORY` + `SMB4_EXPECTED_STATS_BASELINES` + `CHECKPOINT_SAMPLE_FLOORS` + the emitters + `calculateFIP` import; 3 test files repointed (emitter-specific assertions deleted; generic-sample `contactAverage` → `contactAvoidStrikeoutRate`). FULL-suite gate + repo-wide retired-literal grep.

**➡ NEXT:** CAT-MIGRATE audit/gate/commit → NATIVE-WIRE (Fielding only — wire `difficultyFieldingAggregator`, additive season field no DB bump, repoint `fieldingRangeRate`; Speed already wired RA-2c-3; **Power BLOCKED on A1.5b-2**). Then Priority-2 (traits).
