...
**➡ NEXT: hop-2 FAME swap (§6) — BUT §1 (the fame-fluctuation prerequisite) gates it.** §1 = remove the per-game `reachFloor` ratchet (`franchiseFameCompute.ts:109`) so heat fluctuates up/down, and pin the season-end honor floor to `REGIONAL_STAR` — required because the §6 sign-law needs losing fame when overtaken on a positive record (impossible while fame is upward-only). Ground §1 first (is the ratchet flag-gated/build-dark? how is reachFloor consumed? which characterized fame tests + L-SIM `soul.*` fame invariants assert it?), then hop-2 (polarity-signed `heatDeltas` from `FRANCHISE_STADIUM_RECORD_TYPE_POLARITY` concat onto `fameResult.playerHeatDeltas` → `resolveFameTap`, gated `isFranchisePhase2FameEnabled` + per-bump sentinel). Then hop-3 fan morale (⚠ exhaustive-Record `PARK_RECORD_SET`) → hop-4 reporter (⚠ exhaustive-Record `STADIUM_RECORD` + PURE adapter) → hop-5 Almanac → hop-6 rivalry edge → §8 display (USER-VISIBLE → flag for JK). After A1.5d the queue is JK-DECISION-BLOCKED (re-grade store + RA-7).
