...
**➡ RESUME HERE = T-9c** (the LAST T-9 sub-ticket): engine-side **max-1 Elite-pitch mutual-exclusion** = a NEW group-exclusion pass in `reconcileGainProposals` (`traitAcquisition.ts`, AFTER the opposite-pair gain duel ~`:537`, BEFORE the 2-cap loop — can't reuse opposite-pairs [elites have no opposites] nor the 2-cap [per-player, not per-group]; among held+gained Elite pitches keep only the highest `gainScore`, skip the rest with a clear reason). + a shared canonical `ELITE_PITCH_TRAITS` const (cleanest home `src/data/traitTierConfig.ts` — both the generator `prospectScoutingDraftEngine.ts` + `traitAcquisition.ts` already import `traitTierConfig`, acyclic), refactor T-4c's `PROSPECT_ELITE_PITCH_TRAITS` to import it (and T-9a's local `ELITE_PITCH_BY_CODE`/`ELITE_PITCH_CODES` can optionally consume it later). FULL suite (imports into the processCompletedGame chain). ⚑ T-4c GENERATION already enforces max-1; T-9c adds the SAME rule to the in-season GAIN engine (a pitcher could earn a 2nd elite pitch).
