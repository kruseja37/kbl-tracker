...
**RA-12b — DISPATCHED (in flight at handoff of this entry):** retire the Model-C aging random-walk (`calculateRatingChange`/`processEndOfSeasonAging`/`PHASE_CONFIGS` + the orphaned mutation hook `src/src_figma/app/hooks/useAgingData.ts` + the dead `processSeasonEnd` in the LIVE `src/hooks/useAgingData.ts` + SpringTrainingFlow's projection → thin placeholder + barrel/integration re-exports + 2 characterized tests), keep ALL age/career-phase/retirement DISPLAY live. **Grounding CORRECTED the recon twice:** (1) the live `src/hooks/useAgingData.ts` (consumed by `useOffseasonData:281`) has a dead `processSeasonEnd` that DOES call the random-walk → must strip while keeping `getPlayerAgingInfo`/`getRetirementCandidates`; (2) `SpringTrainingFlow` is the only rendered consumer of `calculateRatingChange` but PERSISTS NOTHING and is offseason-flag-dark. FULL-suite gate (core-engine + barrel ripple). **BV-RA-12b** (spring-training placeholder, post-flag).

**➡ NEXT:** RA-12b audit/gate/commit → CAT-MIGRATE (4-drop, keep HR-rate, FULL suite + repo-wide retired-literal grep) → NATIVE-WIRE (Fielding only — wire `difficultyFieldingAggregator`, additive season field no DB bump, repoint `fieldingRangeRate`; Speed already wired RA-2c-3; **Power BLOCKED on A1.5b-2**). Then Priority-2 (traits).
