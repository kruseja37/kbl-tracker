...
**NATIVE-WIRE — DISPATCHED (in flight):** wire the SMB4-native difficulty-fielding signal — accrue `aggregateDifficultyFielding` output into two additive `PlayerSeasonFielding` fields (no DB bump, RA-8 pattern; reuse the events at `seasonAggregator.ts:392`, field-leak-safe by stamped playerId) + repoint `fieldingRangeRate` from `(PO+A)/games` to the difficulty-weighted rate. Speed = ALREADY wired (RA-2c-3 — worksheet §SPEED line corrected this commit, no code). **Power = BLOCKED on A1.5b-2** (carry converter untrustworthy until the EnrichmentPanel SVG re-derivation + browser sign-off) — explicitly NOT wired. FULL-suite gate + version-pin stays green.

**➡ NEXT:** NATIVE-WIRE audit/gate/commit ⇒ 🎉 **Priority-1 ratings cleanups COMPLETE** → Priority-2 (TRAITS — the biggest remaining chunk: T-1..T-7 threshold rebuild + L9b-2 16→~50 expansion).
