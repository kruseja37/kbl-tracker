...
**➡ NEXT (traits, all forks ruled) — RESUME HERE:** **T-5b** (the SEEDED firing roll — §8B step 2 / the §107 "Likelihood" para + Appendix-A example 4: clearing a threshold makes a trait merely ELIGIBLE, then a **seeded probability scaling with the margin-past-threshold** [monotonic, higher tiers slightly harder; exact curve = §16 sim-tune placeholder] decides if it fires; loss symmetric. **SEED SOURCE GROUNDED (Captain-verified):** thread an optional `input.seed`; the caller `franchiseTraitGrantCompute.ts:241` passes ``${scope.franchiseId}:${scope.seasonId}:${scope.statsScopeId}:${entry.playerId}:${sourceEventId}`` [`sourceEventId = `trait-grant-${gameNumber}`` :232 = the checkpoint marker; stable within a checkpoint, distinct across, zero new plumbing]; the engine appends `:${traitName}:${valence}` per roll → a private FNV-1a→[0,1) helper mirroring `scoutValueRange.ts:51-61`/`relationshipIntensity.ts:74-84` [NO Math.random/Date]; fire if draw < firingProbability. **OPT-IN via seed presence** — absent-seed skips the roll [preserves the 72 T-5a tests]. `franchiseTraitGrantCompute.test.ts` STUBS the seam ⇒ caller wiring is test-safe; flag OFF everywhere ⇒ build-dark. FULL suite [processCompletedGame mock-load chain].). Then **T-4** (rarity-weighted generation, genWeight=1−traitWeight, NEG 0.27, exclude Sign Stealer/Stimulated, §5 — a SEPARATE generation path, not the gain/loss engine) → **T-3** (trend tilt + SP/RP cohort split, §4A — FORK-C ruled: split, fold SP-RP/CP into relievers, fallback to full pool) / **T-6** (position-mismatch protection-only map for arm@1B/2B/3B/SS, §8C — FORK-D ruled protection-only, catcher re-earnable DEFERRED; FULL suite) → **T-7** (EOS = one more checkpoint of the same engine, FORK-E ruled same thresholds; deprecate the Trait Wheel Spin; FULL suite). T-1 exports `computeTraitWeight`/`assignTier`/the §6 constants. Then Priority-3 (ratings-engine finish: A2.5 §6A · RA-5/7 · RA-11 · re-grade-per-checkpoint · RA-9 trend · V8 park→WAR), Priority-4 (L4b reporter takes · trade-request wiring), Priority-5 (Mode-1/launch tails).
