...
**➡ NEXT (traits, all forks ruled) — RESUME HERE:** **T-5** (the §8B reconcile/slot-duel — extend `reconcileGainProposals` `traitAcquisition.ts:411-473`: `gainScore=P×traitWeight`, `keepScore=P_held×traitWeight×β` β=1.25, rank firing gains by gainScore desc, **recompute weakest after each displacement** [fixes the double-displacement collision], + the SEEDED firing roll [margin-past-threshold, monotonic, seeded-deterministic — **GROUND THE SEED SOURCE FIRST**: computeTraitAcquisition has no seed input today; thread a deterministic per-player-per-checkpoint seed from the sweep], + 2 new tuning constants `maxTraits=2`/`incumbencyBeta=1.25`, + reuse `computeTraitWeight` from traitTierConfig). Keep the determinism source-grep green [no Math.random/Date.now]). Then **T-4** (rarity-weighted generation, genWeight=1−traitWeight, NEG 0.27, exclude Sign Stealer/Stimulated, §5 — a SEPARATE generation path, not the gain/loss engine) → **T-3** (trend tilt + SP/RP cohort split, §4A — FORK-C ruled: split, fold SP-RP/CP into relievers, fallback to full pool) / **T-6** (position-mismatch protection-only map for arm@1B/2B/3B/SS, §8C — FORK-D ruled protection-only, catcher re-earnable DEFERRED; FULL suite) → **T-7** (EOS = one more checkpoint of the same engine, FORK-E ruled same thresholds; deprecate the Trait Wheel Spin; FULL suite). T-1 exports `computeTraitWeight`/`assignTier`/the §6 constants. Then Priority-3 (ratings-engine finish: A2.5 §6A · RA-5/7 · RA-11 · re-grade-per-checkpoint · RA-9 trend · V8 park→WAR), Priority-4 (L4b reporter takes · trade-request wiring), Priority-5 (Mode-1/launch tails).
