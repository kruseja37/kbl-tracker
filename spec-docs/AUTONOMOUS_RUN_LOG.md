...
**➡ NEXT (traits, all forks ruled) — RESUME HERE:** **T-4c** (JK req 2026-06-25: **a player may hold at most ONE Elite-pitch trait** — the 8 `Elite {2F,4F,CB,CF,CH,FK,SB,SL}`; no Elite 4F + Elite SB together. **GENERATION side:** a `PROSPECT_ELITE_PITCH_TRAITS` set + extend `prospectTraitsConflict` so two Elite-pitch traits conflict [slot2 excludes any Elite pitch when trait1 is one]; build on T-4b's `pickSecondProspectTrait`. **Existing-data check DONE — ZERO offenders** [no stock/seeded player holds 2; 59 hold exactly 1], no scrub needed. ⚠ The engine-side max-1 [a pitcher EARNING a 2nd] now belongs to **T-9** since elite pitches become earnable — see below.) Then **T-9** (NEW — JK ruled 2026-06-25 elite pitches EARNABLE opt-in via pitch tags, reversing the §0.6 dormant-park; DECISIONS_LOG 2026-06-25): a per-pitch-type strikeout aggregator from `enrichment.pitchType` [`eventLog.ts:437`, set via EnrichmentPanel] → wire the 8 elite pitches into `BUILDABLE_TRAITS` + a per-pitch reality signal in `traitCandidateBuilder` [today all pitcher signals are whole-arsenal, `:600-611`] gated by a **min-sample valve** [dormant until enough pitch-tagged Ks → never fires on noise, zero burden if untagged] + the engine-side **max-1 Elite-pitch mutual-exclusion** in `reconcileGainProposals` [the elite-pitch group as a conflict set, like opposite-pairs]; reuse the `ELITE_TRAIT_TO_PITCH` map `prospectScoutingDraftEngine.ts:401-410`. Build-dark. Grounding `wf_5b2d4ae8-aa1`. **Likely SPLIT** (aggregator / signal-wire+BUILDABLE / engine-max-1). Then **T-3** (trend tilt + SP/RP cohort split, §4A — FORK-C ruled: split, fold SP-RP/CP into relievers, fallback to full pool) / **T-6** (position-mismatch protection-only map for arm@1B/2B/3B/SS, §8C — FORK-D ruled protection-only, catcher re-earnable DEFERRED; FULL suite) → **T-7** (EOS = one more checkpoint of the same engine, FORK-E ruled same thresholds; deprecate the Trait Wheel Spin; FULL suite). T-1 exports `computeTraitWeight`/`assignTier`/the §6 constants. Then Priority-3 (ratings-engine finish: A2.5 §6A · RA-5/7 · RA-11 · re-grade-per-checkpoint · RA-9 trend · V8 park→WAR), Priority-4 (L4b reporter takes · trade-request wiring), Priority-5 (Mode-1/launch tails).
