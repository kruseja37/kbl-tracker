...
| Stadium spray missing in real smoke | Stadium spray did not appear in Team Hub during real-app smoke | Stadium analytics spec | Backend archive/stadium evidence path exists, but real-app Team Hub visibility still needs smoke confirmation | Team Hub Stadium should show archive-backed spray evidence or clear blocked/empty reason | P1 Stadium/League Builder Flow | Stadium Spray Evidence Visibility In Real Team Hub if smoke still fails | Wrong-scope/score-only evidence blocked | Adaptive factors/story automation blocked |
...
- If implementation remains necessary, the next priority is **Stadium Spray Evidence Visibility In Real Team Hub**. Manager WPA lineup delta is now a smoke-confirmation item and should only become an implementation target again if a newly completed archive with stored manager records still fails to render the evidence.
...
Playable v1 should still not be declared approved. The right next gate is a real-app user smoke rerun. If that rerun still shows missing stadium spray data in Team Hub, make **Stadium Spray Evidence Visibility In Real Team Hub** the next implementation slice. Manager WPA lineup delta should now be checked as archive-backed Game Detail visibility: newly completed archives with stored manager records should render evidence, while older archives without committed manager records are not backfilled. If the blocker set is cleared, ask for the playable-v1 approval decision rather than starting a speculative new feature. The advanced Mode 2 foundation remains useful, but it should stay read-only, preview-only, confirmation-gated, or blocked until the user approves the playable loop.
