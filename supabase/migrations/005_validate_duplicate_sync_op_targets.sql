-- KBL Tracker duplicate sync op hardening
--
-- A duplicate op_id is idempotent only when it matches the original target
-- metadata. This keeps corrupted durable queues or improbable op-id collisions
-- from being cleared as "already applied" for a different row/key.

DO $kbl_duplicate_op_target_rpc$
BEGIN
...
BEGIN
  ...
  LOOP
    ...
    IF v_op_id IS NOT NULL THEN
      SELECT target_table, target_key, changed_at
      INTO v_existing_op
      ...
      IF FOUND THEN
        IF
          ...
          v_existing_op.changed_at = v_changed_at
        THEN
          status := 'duplicate';
        ELSE
          status := 'skipped';
