-- KBL Tracker legacy duplicate sync-op replay compatibility
--
-- Migration 006 started recording deleted state and payload fingerprints for
-- accepted sync op IDs. Rows accepted before that migration have NULL metadata.
-- Those legacy rows should still be idempotent for crash-after-accept durable
-- queue replays when the original table/key/timestamp match exactly.

DO $kbl_legacy_duplicate_op_rpc$
BEGIN
    ...
    IF v_op_id IS NOT NULL THEN
      ...
      IF FOUND THEN
        IF
          ...
          (
            (
              v_existing_op.deleted IS NULL AND
              v_existing_op.payload_fingerprint IS NULL
            ) OR (
